# L25.33 Microsoft Edge controlled runtime archive manifest package-run broad checkpoint

L25.33 follows accepted L25.32b and acts as the broad checkpoint over the controlled synthetic package-run ladder.

Accepted package-run ladder:

- L25.31 package-run authorization gate.
- L25.32 failed before acceptance because the synthetic ZIP used the wrong bundle shape.
- L25.32a failed before acceptance because a brittle upstream source-chain `ok:true` assertion blocked the proof.
- L25.32b accepted by proving the supported synthetic bundle shape directly before one bounded run-package call.

Allowed in L25.33:

- read back the accepted L25.32b proof payload from the validator;
- confirm the supported bundle shape was present;
- confirm the explicit token-gated package-run proof executed one bounded PatchOps run-package call;
- confirm PatchOps run-package exited `0`, did not time out, and was not rejected by bundle review;
- confirm the controlled package launcher executed;
- confirm the package-run scope remains synthetic launcher only;
- confirm browser, pasteback, real-artifact, localhost, extension, commit, and push permissions remain closed.
