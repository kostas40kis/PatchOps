...
That boundary remains one of the most important rules in the repo and should not be weakened by future work. îˆ€fileciteîˆ‚turn11file13îˆ
...
Manifests remain the explicit instruction surface for what PatchOps should do right now. Project packets are being added as a maintained guidance surface, but they are not a replacement for profiles or manifests. îˆ€fileciteîˆ‚turn11file9îˆ‚turn11file1îˆ

### 3.2 Thin PowerShell, reusable Python

PowerShell remains a thin operator-facing surface.
Reusable workflow logic should continue to live in Python-owned surfaces rather than being duplicated in launcher scripts. This remains a hard architectural rule for the project. îˆ€fileciteîˆ‚turn11file9îˆ‚turn11file14îˆ

### 3.3 One canonical report per run

PatchOps should continue producing one canonical evidence artifact per run.
The report contract should stay explicit and should not be weakened for documentation-only work, maintenance work, or future onboarding-related features. îˆ€fileciteîˆ‚turn11file9îˆ‚turn11file14îˆ
...
If the task is target-specific, the relevant project packet should then be read as an additional surface rather than as a replacement for handoff. îˆ€fileciteîˆ‚turn11file5îˆ
...
That practical flow remains a core usage story for the wrapper. îˆ€fileciteîˆ‚turn11file7îˆ
...
That handoff direction was introduced so a future LLM can resume from concrete repo-generated state instead of rebuilding context from scattered documents. The current project-packet work does not replace that. It extends the onboarding story for brand-new target projects. îˆ€fileciteîˆ‚turn11file15îˆ
...
That separation should remain explicit in all future docs and code. îˆ€fileciteîˆ‚turn11file1îˆ‚turn11file5îˆ
...
- and the target project being developed with PatchOps. îˆ€fileciteîˆ‚turn11file9îˆ

The project-packet direction is meant to make new target-project startup faster without redesigning PatchOps into a different kind of product. The plan explicitly says this should be an additive feature, not a rewrite. îˆ€fileciteîˆ‚turn11file5îˆ‚turn11file9îˆ
...
10. continue patch by patch. îˆ€fileciteîˆ‚turn11file3îˆ‚turn11file12îˆ

This means the project-packet layer is meant to reduce blank-page startup work while preserving the existing profile, manifest, report, and handoff model. îˆ€fileciteîˆ‚turn11file5îˆ
...
7. only then add generator/refresh CLI support. îˆ€fileciteîˆ‚turn11file5îˆ
...
with explicit target root, wrapper root, runtime, selected profile, boundary rules, recommended manifests, phased guidance, and current status. îˆ€fileciteîˆ‚turn11file0îˆ
...
These are meant to prevent the new workflow from drifting into vague documentation. îˆ€fileciteîˆ‚turn11file0îˆ‚turn11file12îˆ
...
This support is intended to scaffold starter packets and refresh mutable sections conservatively from known state such as handoff files and report metadata. îˆ€fileciteîˆ‚turn11file0îˆ‚turn11file12îˆ
...
That work belongs to the later phase of the plan and should not be described as present before those patches exist. îˆ€fileciteîˆ‚turn11file3îˆ‚turn11file12îˆ
...
1. Do not move target-project business logic into PatchOps. îˆ€fileciteîˆ‚turn11file13îˆ
2. Do not let project packets replace profiles, manifests, reports, or handoff files. îˆ€fileciteîˆ‚turn11file1îˆ‚turn11file9îˆ
3. Do not let PowerShell become the home of reusable wrapper logic. îˆ€fileciteîˆ‚turn11file9îˆ‚turn11file14îˆ
4. Do not weaken the one-report evidence model just because the work is documentation-heavy. îˆ€fileciteîˆ‚turn11file9îˆ‚turn11file14îˆ
5. Prefer additive change over broad redesign. îˆ€fileciteîˆ‚turn11file9îˆ‚turn11file15îˆ
...
12. `docs/project_packet_workflow.md` îˆ€fileciteîˆ‚turn11file9îˆ‚turn11file13îˆ

For takeover of an already-running PatchOps effort, read the handoff bundle first, then read the relevant project packet if the task is target-specific. îˆ€fileciteîˆ‚turn11file5îˆ
...
That sequence keeps the rollout controlled and aligned with the planâ€™s recommended first implementation order. îˆ€fileciteîˆ‚turn11file5îˆ
...
That is the correct status interpretation to preserve until the later project-packet phases are actually implemented. îˆ€fileciteîˆ‚turn11file5îˆ‚turn11file9îˆ
...
- latest successful patch before this gate: Patch 129K Ã¢â‚¬â€ `patch_129k_new_target_onboarding_current_contract_repair`
...
## Patch 134B Ã¢â‚¬â€ summary-integrity authoring unblocker
...
## Patch 134C Ã¢â‚¬â€ summary-integrity derivation repair
...
## Patch 134H Ã¢â‚¬â€ apply-flow report-directory creation repair
...
## Patch 134I Ã¢â‚¬â€ report-dir proof repair
...
## Patch 134J Ã¢â‚¬â€ report-dir proof pytest repair
...
## Summary-integrity repair stream status Ã¢â‚¬â€ March 30, 2026
...
Pythonization self-hosted proof is now exercised through PatchOps itself.
This proof uses check, inspect, plan, and apply against PatchOps itself while keeping the repo changes narrow and maintenance-facing.

## Pythonization Stream Status
The Pythonization micro-patch stream is now closed through MP51.

- MP42Ã¢â‚¬â€œMP47 landed the suspicious-run helper, detector tests, structured artifact model, opt-in emission, compact report mention, and maintenance guidance.
...
## Pythonization stream status
The Pythonization micro-patch stream is now complete through MP51.
...
<!-- PATCHOPS_PHASE3A_REPAIR_APPENDIX:START -->
## Maintenance-mode clarification appendix

This additive appendix exists to clarify the current maintenance posture without replacing the passing project-status contract surface above.

### Current maintained identity reminder

PatchOps should be treated as a maintained utility rather than an open-ended architecture buildout.
The core wrapper, handoff-first continuation, and project-packet onboarding layers already exist in the repo today.

### Safe reading posture

- Read the current state snapshot first.
- Treat handoff-first continuation as the default for already-running PatchOps work.
- Treat older plans and superseded continuation plans as historical anchors rather than the main current truth.
- Prefer narrow maintenance, refinement, or target-specific expansion over broad redesign.

### Why this appendix is additive

This appendix is intentionally additive-only.
It keeps the current passing `docs/project_status.md` contract intact and layers clearer maintenance-mode wording around it instead of replacing the whole file.
<!-- PATCHOPS_PHASE3A_REPAIR_APPENDIX:END -->
