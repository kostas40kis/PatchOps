...
It records the verified pre-gate posture after Patch 129K, appends an explicit F6 gate status block to `docs/project_status.md`, and adds `tests/test_final_release_maintenance_gate.py` so the maintained docs, handoff bundle files, examples, key profiles, and operator launchers have to exist together before the repo can be described as maintenance-ready.
...
<!-- PATCHOPS_PATCH134A_LEDGER:START -->
## Patch 134A

Patch 134A resets the summary-integrity repair stream into one maintained repo-visible story.

It does three narrow things:
- creates a current-state repair note for the stream,
- records the exact observed chain `133 -> 133A -> attempted 134`,
- explicitly separates wrapper/content-path failure, the confirmed summary-integrity product bug, and the malformed-manifest authoring failure.

This is a documentation-first maintenance patch.
It does not repair the reporting engine yet.
Its purpose is to make the next repair steps obvious without depending on chat history.
<!-- PATCHOPS_PATCH134A_LEDGER:END -->

<!-- PATCHOPS_PATCH134B_LEDGER:START -->
## Patch 134B â€” summary-integrity authoring unblocker

Patch 134B is the second small patch in the six-patch summary-integrity repair circle.

Purpose:
- repair the self-hosted authoring mechanics for the current repair stream without widening into manifest-system redesign.

What it locks:
- self-hosted manual-repair manifests should be emitted as valid JSON from structured data,
- self-hosted `content_path` values should remain manifest-local and relative,
- the current stream can now prove `check`, `inspect`, `plan`, and `apply` against a temporary self-hosted patch root through a narrow regression test.

This patch is an unblocker only.
The confirmed product bug remains the summary-integrity contradiction proven by Patch 133A.
<!-- PATCHOPS_PATCH134B_LEDGER:END -->

<!-- PATCHOPS_PATCH134C_LEDGER:START -->
## Patch 134C â€” summary-integrity derivation repair

Patch 134C is the real product-bug repair in the six-patch summary-integrity stream.

Purpose:
- make the rendered summary truthful when required command evidence already proves failure.

What changed:
- `patchops/reporting/renderer.py` now derives summary exit/result from required validation and smoke command evidence before rendering the summary block.
- a current regression test locks:
  - required validation failure -> rendered summary `FAIL`,
  - earlier required failure stays sticky,
  - tolerated non-zero exits listed in `allowed_exit_codes` still render `PASS`.

This patch stays narrow and does not redesign workflow orchestration.
The next hardening wave can make the broader workflow state fail closed as well.
<!-- PATCHOPS_PATCH134C_LEDGER:END -->
...
<!-- PATCHOPS_PATCH134G_LEDGER:START -->
## Patch 134G
...
## Patch 134Z â€” final proof / handoff refresh after report-path fallback repair
...
- MP51 â€” final status refresh for the Pythonization stream completed after the MP50 self-hosted proof patch.
