...
CONFIRM_CHROME_TAB_ENTER_PICKER_SELF_REPORT_UPLOAD_NO_SEND = "PATCHOPS_CONFIRM_CHROME_TAB_ENTER_PICKER_SELF_REPORT_UPLOAD_NO_SEND"
PASS_CHROME_SELF_REPORT_TAB_ENTER_PICKER_ATTACHED_NO_SEND = "PASS_CHROME_SELF_REPORT_TAB_ENTER_PICKER_ATTACHED_NO_SEND"
BLOCKED_CHROME_TAB_ENTER_CONFIRMATION_REQUIRED = "BLOCKED_CHROME_TAB_ENTER_CONFIRMATION_REQUIRED"
BLOCKED_CHROME_TAB_ENTER_CONFIRMATION_MISMATCH = "BLOCKED_CHROME_TAB_ENTER_CONFIRMATION_MISMATCH"
BLOCKED_CHROME_TAB_ENTER_LIVE_BROWSER_REQUIRED = "BLOCKED_CHROME_TAB_ENTER_LIVE_BROWSER_REQUIRED"
BLOCKED_CHROME_TAB_ENTER_CONFIG_INVALID = "BLOCKED_CHROME_TAB_ENTER_CONFIG_INVALID"
BLOCKED_CHROME_TAB_ENTER_REPORT_MISSING = "BLOCKED_CHROME_TAB_ENTER_REPORT_MISSING"
BLOCKED_CHROME_TAB_ENTER_REPORT_HASH_MISMATCH = "BLOCKED_CHROME_TAB_ENTER_REPORT_HASH_MISMATCH"
BLOCKED_CHROME_TAB_ENTER_TARGET_NOT_READY = "BLOCKED_CHROME_TAB_ENTER_TARGET_NOT_READY"
BLOCKED_CHROME_TAB_ENTER_PICKER_NOT_OPENED = "BLOCKED_CHROME_TAB_ENTER_PICKER_NOT_OPENED"
BLOCKED_CHROME_TAB_ENTER_PICKER_AMBIGUOUS = "BLOCKED_CHROME_TAB_ENTER_PICKER_AMBIGUOUS"
BLOCKED_CHROME_TAB_ENTER_PATH_WRITE_FAILED = "BLOCKED_CHROME_TAB_ENTER_PATH_WRITE_FAILED"
BLOCKED_CHROME_TAB_ENTER_OPEN_CLICK_FAILED = "BLOCKED_CHROME_TAB_ENTER_OPEN_CLICK_FAILED"
BLOCKED_CHROME_TAB_ENTER_ATTACHMENT_NOT_VERIFIED = "BLOCKED_CHROME_TAB_ENTER_ATTACHMENT_NOT_VERIFIED"
BLOCKED_SEND_RISK = "BLOCKED_SEND_RISK"

DEFAULT_CONFIG_PATH = Path("data/config/uploader_status_target_config.json")
DEFAULT_JSON_OUTPUT_PATH = Path("data/runtime/copilot_handoff/latest_pseudo_self_report_tab_enter_picker_upload_no_send.json")
DEFAULT_TXT_OUTPUT_PATH = Path("data/runtime/copilot_handoff/latest_pseudo_self_report_tab_enter_picker_upload_no_send.txt")
...
SAFE_SHORTCUTS_USED = ("TAB", "ENTER_FOR_PICKER_TRIGGER_ONLY", "SECOND_ENTER_FOR_PICKER_TRIGGER_ONLY_IF_NEEDED")
...
class TabEnterPickerResult:
    ...
    safety: TabEnterPickerSafety = field(default_factory=TabEnterPickerSafety)
...
class TabEnterPickerAdapter(Protocol):
    provider_name: str

    def attach_via_tab_enter_picker(self, report_path: Path) -> TabEnterPickerProbe:
        ...


class FakeTabEnterPickerAdapter:
    ...
    def attach_via_tab_enter_picker(self, report_path: Path) -> TabEnterPickerProbe:
        ...
        if self.mode == "no-target":
            return TabEnterPickerProbe(False, False, False, False, False, False, 0, False, False, False, False, 0, 0, None, ("fake target missing",))
        if self.mode == "no-picker":
            return TabEnterPickerProbe(True, True, True, True, True, False, 0, False, False, False, False, 0, 0, title, ("fake picker missing",))
        if self.mode == "ambiguous-picker":
            return TabEnterPickerProbe(True, True, True, True, True, True, 2, False, False, False, False, 0, 0, title, ("fake ambiguous picker",))
        if self.mode == "path-failed":
            return TabEnterPickerProbe(True, True, True, True, True, True, 1, False, False, False, False, 0, 0, title, ("fake path write failed",))
        if self.mode == "open-failed":
            return TabEnterPickerProbe(True, True, True, True, True, True, 1, True, False, False, False, 0, 0, title, ("fake open click failed",))
        if self.mode == "not-verified":
            return TabEnterPickerProbe(True, True, True, True, True, True, 1, True, True, True, False, 0, 0, title, ("fake attachment not verified",))
        return TabEnterPickerProbe(True, True, True, True, True, True, 1, True, True, True, True, 0, 1, title, ())


class PywinautoTabEnterPickerAdapter:
    ...
    def _desktop(self) -> Any:
        try:
            ...
            raise RuntimeError(f"pywinauto is required for live Chrome tab-enter picker upload: {exc}") from exc
    ...
    def _keyboard(self) -> Any:
        try:
            ...
            raise RuntimeError(f"pywinauto keyboard is required for live Chrome tab-enter picker upload: {exc}") from exc
        return keyboard
    ...
    def attach_via_tab_enter_picker(self, report_path: Path) -> TabEnterPickerProbe:
        ...
        if window is None:
            return TabEnterPickerProbe(False, False, False, False, False, False, 0, False, False, False, False, 0, 0, None, issues)
        title = _safe_window_title(window)
        try:
            window.set_focus()
            time.sleep(self.settle_seconds)
        except Exception as exc:
            return TabEnterPickerProbe(True, False, False, False, False, False, 0, False, False, False, False, 0, 0, title, (f"Chrome focus failed: {exc}",))
        safe_clicked = self._safe_focus_click(window)
        if not safe_clicked:
            return TabEnterPickerProbe(True, True, False, False, False, False, 0, False, False, False, False, 0, 0, title, ("safe focus click failed",))
        ...
        try:
            ...
            return TabEnterPickerProbe(True, True, True, True, False, False, 0, False, False, False, False, len(before), len(before), title, (f"tab/enter picker keyboard trigger failed: {exc}",))
        ...
        if len(pickers) == 0:
            try:
                ...
                return TabEnterPickerProbe(True, True, True, True, True, False, 0, False, False, False, False, len(before), len(before), title, (f"second Enter picker trigger failed: {exc}",))
            pickers = self._wait_for_picker()        if len(pickers) == 0:
            return TabEnterPickerProbe(True, True, True, True, True, False, 0, False, False, False, False, len(before), len(before), title, ("native file picker did not open after safe-click Tab/Enter trigger",))
        if len(pickers) != 1:
            return TabEnterPickerProbe(True, True, True, True, True, True, len(pickers), False, False, False, False, len(before), len(before), title, (f"expected exactly one native file picker; found {len(pickers)}",))

        picker = pickers[0]
        edit = self._path_edit(picker)
        if edit is None:
            return TabEnterPickerProbe(True, True, True, True, True, True, 1, False, False, False, False, len(before), len(before), title, ("native file picker path edit was not found",))
        try:
            edit.set_edit_text(str(report_path.resolve(strict=True)))
            time.sleep(self.settle_seconds)
        except Exception as exc:
            return TabEnterPickerProbe(True, True, True, True, True, True, 1, False, False, False, False, len(before), len(before), title, (f"file picker path write failed: {exc}",))

        button = self._open_button(picker)
        if button is None:
            return TabEnterPickerProbe(True, True, True, True, True, True, 1, True, False, False, False, len(before), len(before), title, ("native file picker Open button was not found",))
        try:
            button.click_input()
            time.sleep(self.settle_seconds)
        except Exception as exc:
            return TabEnterPickerProbe(True, True, True, True, True, True, 1, True, False, False, False, len(before), len(before), title, (f"native file picker Open click failed: {exc}",))
        ...
        if not verified:
            return TabEnterPickerProbe(True, True, True, True, True, True, 1, True, True, closed, False, before_count, after_count, title, (f"new visible attachment was not verified after picker Open; before_matches={before_count}; after_matches={after_count}",))
        return TabEnterPickerProbe(True, True, True, True, True, True, 1, True, True, closed, True, before_count, after_count, title, ())
...
def build_adapter(provider: str) -> TabEnterPickerAdapter:
    if provider.startswith("fake-"):
        return FakeTabEnterPickerAdapter(provider.removeprefix("fake-"))
    if provider == "pywinauto":
        return PywinautoTabEnterPickerAdapter()
...
def make_result(
    *,
    ...
    probe: TabEnterPickerProbe | None,
    provider: str,
    issues: Sequence[str],
) -> TabEnterPickerResult:
    title = probe.selected_window_title if probe else None
    safety = TabEnterPickerSafety(
        browser_action_performed=bool(probe and (probe.chrome_target_focused or probe.tab_key_pressed or probe.open_button_clicked)),
        safe_focus_click_performed=bool(probe and probe.safe_focus_click_performed),
        tab_key_pressed=bool(probe and probe.tab_key_pressed),
        picker_trigger_enter_pressed=bool(probe and probe.picker_trigger_enter_pressed),
    ...
    return TabEnterPickerResult(
        ...
        keyboard_shortcuts_used=SAFE_SHORTCUTS_USED if probe and probe.tab_key_pressed else (),
        forbidden_keyboard_shortcuts_used=FORBIDDEN_SHORTCUTS_USED,
        safe_focus_click_performed=safety.safe_focus_click_performed,
        tab_key_pressed=safety.tab_key_pressed,
        picker_trigger_enter_pressed=safety.picker_trigger_enter_pressed,
        enter_key_pressed_in_chat_composer=False,
        enter_key_pressed_in_picker=False,
...
def run_tab_enter_picker_upload_no_send(
    *,
    ...
    if expected_self_report_sha256 and expected_self_report_sha256 != computed_hash:
        return make_result(ok=False, result_label=BLOCKED_CHROME_TAB_ENTER_REPORT_HASH_MISMATCH, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, report_path=report_path, report_sha256=expected_self_report_sha256, computed_report_sha256=computed_hash, probe=empty_probe, provider=provider, issues=("self report hash mismatch",))

    try:
        adapter = build_adapter(provider)
        probe = adapter.attach_via_tab_enter_picker(report_path)
    except Exception as exc:
        return make_result(ok=False, result_label=BLOCKED_CHROME_TAB_ENTER_TARGET_NOT_READY, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, report_path=report_path, report_sha256=computed_hash, computed_report_sha256=computed_hash, probe=empty_probe, provider=provider, issues=(str(exc),))

    if not probe.chrome_target_ready or not probe.chrome_target_focused:
        label = BLOCKED_CHROME_TAB_ENTER_TARGET_NOT_READY
    elif not probe.picker_opened:
        label = BLOCKED_CHROME_TAB_ENTER_PICKER_NOT_OPENED
    elif probe.picker_count != 1:
        label = BLOCKED_CHROME_TAB_ENTER_PICKER_AMBIGUOUS
    elif not probe.path_written:
        label = BLOCKED_CHROME_TAB_ENTER_PATH_WRITE_FAILED
    elif not probe.open_button_clicked:
        label = BLOCKED_CHROME_TAB_ENTER_OPEN_CLICK_FAILED
    elif not probe.attachment_verified:
        label = BLOCKED_CHROME_TAB_ENTER_ATTACHMENT_NOT_VERIFIED
    else:
        label = PASS_CHROME_SELF_REPORT_TAB_ENTER_PICKER_ATTACHED_NO_SEND

    return make_result(ok=label == PASS_CHROME_SELF_REPORT_TAB_ENTER_PICKER_ATTACHED_NO_SEND, result_label=label, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, report_path=report_path, report_sha256=computed_hash, computed_report_sha256=computed_hash, probe=probe, provider=adapter.provider_name, issues=probe.issues)


def render_text(result: TabEnterPickerResult) -> str:
    lines = [
        ...
        f"tab_key_pressed: {str(result.tab_key_pressed).lower()}",
        f"picker_trigger_enter_pressed: {str(result.picker_trigger_enter_pressed).lower()}",
...
def write_evidence(result: TabEnterPickerResult, *, json_output_path: Path = DEFAULT_JSON_OUTPUT_PATH, txt_output_path: Path = DEFAULT_TXT_OUTPUT_PATH) -> tuple[Path, Path]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Attach a self-report file in Chrome via tab-enter picker path without Send")
