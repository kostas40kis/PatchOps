...
PATCH_NAME = "pseudo_self_report_upload_no_send_repair_10_desktop_filename_enter"
CONFIRM_CHROME_DESKTOP_FILENAME_SELF_REPORT_UPLOAD_NO_SEND = "PATCHOPS_CONFIRM_CHROME_DESKTOP_FILENAME_SELF_REPORT_UPLOAD_NO_SEND"
PASS_CHROME_SELF_REPORT_DESKTOP_FILENAME_ATTACHED_NO_SEND = "PASS_CHROME_SELF_REPORT_DESKTOP_FILENAME_ATTACHED_NO_SEND"
BLOCKED_CHROME_DESKTOP_FILENAME_CONFIRMATION_REQUIRED = "BLOCKED_CHROME_DESKTOP_FILENAME_CONFIRMATION_REQUIRED"
BLOCKED_CHROME_DESKTOP_FILENAME_CONFIRMATION_MISMATCH = "BLOCKED_CHROME_DESKTOP_FILENAME_CONFIRMATION_MISMATCH"
BLOCKED_CHROME_DESKTOP_FILENAME_LIVE_BROWSER_REQUIRED = "BLOCKED_CHROME_DESKTOP_FILENAME_LIVE_BROWSER_REQUIRED"
BLOCKED_CHROME_DESKTOP_FILENAME_CONFIG_INVALID = "BLOCKED_CHROME_DESKTOP_FILENAME_CONFIG_INVALID"
BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_MISSING = "BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_MISSING"
BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_NOT_ON_DESKTOP = "BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_NOT_ON_DESKTOP"
BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_HASH_MISMATCH = "BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_HASH_MISMATCH"
BLOCKED_CHROME_DESKTOP_FILENAME_TARGET_NOT_READY = "BLOCKED_CHROME_DESKTOP_FILENAME_TARGET_NOT_READY"
BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_NOT_OPENED = "BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_NOT_OPENED"
BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_AMBIGUOUS = "BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_AMBIGUOUS"
BLOCKED_CHROME_DESKTOP_FILENAME_WRITE_FAILED = "BLOCKED_CHROME_DESKTOP_FILENAME_WRITE_FAILED"
BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_ENTER_FAILED = "BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_ENTER_FAILED"
BLOCKED_CHROME_DESKTOP_FILENAME_ATTACHMENT_NOT_VERIFIED = "BLOCKED_CHROME_DESKTOP_FILENAME_ATTACHMENT_NOT_VERIFIED"

DEFAULT_CONFIG_PATH = Path("data/config/uploader_status_target_config.json")
DEFAULT_JSON_OUTPUT_PATH = Path("data/runtime/copilot_handoff/latest_pseudo_self_report_desktop_filename_upload_no_send.json")
DEFAULT_TXT_OUTPUT_PATH = Path("data/runtime/copilot_handoff/latest_pseudo_self_report_desktop_filename_upload_no_send.txt")
...
SAFE_SHORTCUTS_USED = ("/", "ENTER_FOR_UPLOAD_COMMAND_ONLY", "ENTER_IN_NATIVE_FILE_PICKER_ONLY")
...
class DesktopFilenameSafety:
    ...
    desktop_filename_written: bool = False
    full_path_written_to_picker: bool = False
...
class DesktopFilenameResult:
    ...
    safety: DesktopFilenameSafety = field(default_factory=DesktopFilenameSafety)
...
class DesktopFilenameAdapter(Protocol):
    provider_name: str

    def attach_via_desktop_filename(self, report_path: Path, desktop_dir: Path) -> DesktopFilenameProbe:
        ...


class FakeDesktopFilenameAdapter:
    ...
    def attach_via_desktop_filename(self, report_path: Path, desktop_dir: Path) -> DesktopFilenameProbe:
        ...
        if self.mode == "no-target":
            return DesktopFilenameProbe(False, False, False, False, False, False, 0, str(desktop_dir), report_path.name, False, False, False, False, 0, 0, None, ("fake target missing",))
        if self.mode == "no-picker":
            return DesktopFilenameProbe(True, True, True, True, True, False, 0, str(desktop_dir), report_path.name, False, False, False, False, 0, 0, title, ("fake picker missing",))
        if self.mode == "ambiguous-picker":
            return DesktopFilenameProbe(True, True, True, True, True, True, 2, str(desktop_dir), report_path.name, False, False, False, False, 0, 0, title, ("fake ambiguous picker",))
        if self.mode == "write-failed":
            return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, False, False, False, False, 0, 0, title, ("fake filename write failed",))
        if self.mode == "enter-failed":
            return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, True, False, False, False, 0, 0, title, ("fake picker enter failed",))
        if self.mode == "not-verified":
            return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, True, True, True, False, 0, 0, title, ("fake attachment not verified",))
        return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, True, True, True, True, 0, 1, title, ())


class PywinautoDesktopFilenameAdapter:
    provider_name = "pywinauto"

    def __init__(self, *, settle_seconds: float = 0.65, picker_timeout_seconds: float = 10.0, attach_timeout_seconds: float = 25.0) -> None:
    ...
    def _desktop(self) -> Any:
        try:
            ...
            raise RuntimeError(f"pywinauto is required for live Chrome Desktop filename upload: {exc}") from exc
    ...
    def _keyboard(self) -> Any:
        try:
            ...
            raise RuntimeError(f"pywinauto keyboard is required for live Chrome Desktop filename upload: {exc}") from exc
        return keyboard
    ...
    def _picker_windows_once(self) -> list[Any]:
        ...
        for window in windows:
            ...
            if "#32770" in cls or any(token in title for token in PICKER_TITLE_TOKENS):
                if self._path_edit(window) is not None:
    ...
    def attach_via_desktop_filename(self, report_path: Path, desktop_dir: Path) -> DesktopFilenameProbe:
        ...
        if window is None:
            return DesktopFilenameProbe(False, False, False, False, False, False, 0, str(desktop_dir), report_path.name, False, False, False, False, 0, 0, None, issues)
        title = _safe_window_title(window)
        try:
            window.set_focus()
            time.sleep(self.settle_seconds)
        except Exception as exc:
            return DesktopFilenameProbe(True, False, False, False, False, False, 0, str(desktop_dir), report_path.name, False, False, False, False, 0, 0, title, (f"Chrome focus failed: {exc}",))
        safe_clicked = self._safe_focus_click(window)
        if not safe_clicked:
            return DesktopFilenameProbe(True, True, False, False, False, False, 0, str(desktop_dir), report_path.name, False, False, False, False, 0, 0, title, ("safe focus click failed",))
        ...
        try:
            ...
            return DesktopFilenameProbe(True, True, True, False, False, False, 0, str(desktop_dir), report_path.name, False, False, False, False, len(before), len(before), title, (f"slash + Enter picker trigger failed: {exc}",))

        pickers = self._wait_for_picker()
        if len(pickers) == 0:
            return DesktopFilenameProbe(True, True, True, True, False, False, 0, str(desktop_dir), report_path.name, False, False, False, False, len(before), len(before), title, ("native file picker did not open after slash + Enter",))
        if len(pickers) != 1:
            return DesktopFilenameProbe(True, True, True, True, True, True, len(pickers), str(desktop_dir), report_path.name, False, False, False, False, len(before), len(before), title, (f"expected exactly one native file picker; found {len(pickers)}",))

        picker = pickers[0]
        edit = self._path_edit(picker)
        if edit is None:
            return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, False, False, False, False, len(before), len(before), title, ("native file picker filename edit was not found",))
        try:
            edit.set_focus()
            time.sleep(0.1)
            ...
            return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, False, False, False, False, len(before), len(before), title, (f"Desktop filename write failed: {exc}",))

        try:
            self._keyboard().send_keys("{ENTER}", pause=0.05)
            time.sleep(self.settle_seconds)
        except Exception as exc:
            return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, True, False, False, False, len(before), len(before), title, (f"native picker Enter failed after Desktop filename write: {exc}",))
        ...
        if not verified:
            return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, True, True, closed, False, before_count, after_count, title, (f"new visible attachment was not verified after typing Desktop filename and pressing Enter in picker; before_matches={before_count}; after_matches={after_count}",))
        return DesktopFilenameProbe(True, True, True, True, True, True, 1, str(desktop_dir), report_path.name, True, True, closed, True, before_count, after_count, title, ())
...
def build_adapter(provider: str) -> DesktopFilenameAdapter:
    if provider.startswith("fake-"):
        return FakeDesktopFilenameAdapter(provider.removeprefix("fake-"))
    if provider == "pywinauto":
        return PywinautoDesktopFilenameAdapter()
...
def make_result(
    *,
    ...
    probe: DesktopFilenameProbe | None,
    provider: str,
    issues: Sequence[str],
) -> DesktopFilenameResult:
    title = probe.selected_window_title if probe else None
    safety = DesktopFilenameSafety(
        ...
        desktop_filename_written=bool(probe and probe.filename_written),
    ...
    return DesktopFilenameResult(
        ok=ok,
        result_label=result_label,
        patch_name=PATCH_NAME,
        selected_action="upload_self_report_desktop_filename_enter_no_send",
        browser_lane="chrome",
        ...
        enter_key_pressed_in_chat_composer=False,
        enter_key_pressed_in_picker=safety.enter_key_pressed_in_picker,
        picker_opened=bool(probe and probe.picker_opened),
        picker_count=probe.picker_count if probe else 0,
        filename_written=safety.desktop_filename_written,
        full_path_written_to_picker=False,
        open_button_clicked=False,
        picker_enter_pressed=safety.picker_enter_pressed,
...
def run_desktop_filename_upload_no_send(
    *,
    ...
    confirmation_matched = confirmation_text == CONFIRM_CHROME_DESKTOP_FILENAME_SELF_REPORT_UPLOAD_NO_SEND
    desktop_path = Path(desktop_dir) if desktop_dir else DEFAULT_DESKTOP_DIR
    empty_probe = DesktopFilenameProbe(False, False, False, False, False, False, 0, str(desktop_path), Path(self_report_path).name if self_report_path else None, False, False, False, False, 0, 0, None, ())
    if not config_ok:
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_CONFIG_INVALID, config_path=config_path, status_chat_configured=False, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=live_browser, confirmation_text=confirmation_text, confirmation_matched=confirmation_matched, desktop_dir=desktop_path, report_path=Path(self_report_path) if self_report_path else None, report_sha256=expected_self_report_sha256, computed_report_sha256=None, probe=empty_probe, provider=provider, issues=config_issues)
    if confirmation_text is None:
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_CONFIRMATION_REQUIRED, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=live_browser, confirmation_text=None, confirmation_matched=False, desktop_dir=desktop_path, report_path=Path(self_report_path) if self_report_path else None, report_sha256=expected_self_report_sha256, computed_report_sha256=None, probe=empty_probe, provider=provider, issues=("live Desktop filename confirmation is required",))
    if not confirmation_matched:
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_CONFIRMATION_MISMATCH, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=live_browser, confirmation_text=confirmation_text, confirmation_matched=False, desktop_dir=desktop_path, report_path=Path(self_report_path) if self_report_path else None, report_sha256=expected_self_report_sha256, computed_report_sha256=None, probe=empty_probe, provider=provider, issues=(f"confirmation must exactly match {CONFIRM_CHROME_DESKTOP_FILENAME_SELF_REPORT_UPLOAD_NO_SEND}",))
    if not live_browser:
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_LIVE_BROWSER_REQUIRED, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=False, confirmation_text=confirmation_text, confirmation_matched=True, desktop_dir=desktop_path, report_path=Path(self_report_path) if self_report_path else None, report_sha256=expected_self_report_sha256, computed_report_sha256=None, probe=empty_probe, provider=provider, issues=("--live-browser is required",))
    if not self_report_path:
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_MISSING, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, desktop_dir=desktop_path, report_path=None, report_sha256=expected_self_report_sha256, computed_report_sha256=None, probe=empty_probe, provider=provider, issues=("self report path is required",))
    report_path = Path(self_report_path)
    if not report_path.exists() or not report_path.is_file():
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_MISSING, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, desktop_dir=desktop_path, report_path=report_path, report_sha256=expected_self_report_sha256, computed_report_sha256=None, probe=empty_probe, provider=provider, issues=(f"self report not found: {report_path}",))
    if not is_child_of(report_path, desktop_path):
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_NOT_ON_DESKTOP, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, desktop_dir=desktop_path, report_path=report_path, report_sha256=expected_self_report_sha256, computed_report_sha256=None, probe=empty_probe, provider=provider, issues=(f"self report must be on Desktop: {desktop_path}",))
    computed_hash = sha256_file(report_path)
    if expected_self_report_sha256 and expected_self_report_sha256 != computed_hash:
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_REPORT_HASH_MISMATCH, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, desktop_dir=desktop_path, report_path=report_path, report_sha256=expected_self_report_sha256, computed_report_sha256=computed_hash, probe=empty_probe, provider=provider, issues=("self report hash mismatch",))

    try:
        adapter = build_adapter(provider)
        probe = adapter.attach_via_desktop_filename(report_path, desktop_path)
    except Exception as exc:
        return make_result(ok=False, result_label=BLOCKED_CHROME_DESKTOP_FILENAME_TARGET_NOT_READY, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, desktop_dir=desktop_path, report_path=report_path, report_sha256=computed_hash, computed_report_sha256=computed_hash, probe=empty_probe, provider=provider, issues=(str(exc),))

    if not probe.chrome_target_ready or not probe.chrome_target_focused:
        label = BLOCKED_CHROME_DESKTOP_FILENAME_TARGET_NOT_READY
    elif not probe.picker_opened:
        label = BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_NOT_OPENED
    elif probe.picker_count != 1:
        label = BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_AMBIGUOUS
    elif not probe.filename_written:
        label = BLOCKED_CHROME_DESKTOP_FILENAME_WRITE_FAILED
    elif not probe.picker_enter_pressed:
        label = BLOCKED_CHROME_DESKTOP_FILENAME_PICKER_ENTER_FAILED
    elif not probe.attachment_verified:
        label = BLOCKED_CHROME_DESKTOP_FILENAME_ATTACHMENT_NOT_VERIFIED
    else:
        label = PASS_CHROME_SELF_REPORT_DESKTOP_FILENAME_ATTACHED_NO_SEND

    return make_result(ok=label == PASS_CHROME_SELF_REPORT_DESKTOP_FILENAME_ATTACHED_NO_SEND, result_label=label, config_path=config_path, status_chat_configured=True, status_chat_url_hash_or_redacted=url_hash, target_url_sha256=target_hash, live_browser=True, confirmation_text=confirmation_text, confirmation_matched=True, desktop_dir=desktop_path, report_path=report_path, report_sha256=computed_hash, computed_report_sha256=computed_hash, probe=probe, provider=adapter.provider_name, issues=probe.issues)


def render_text(result: DesktopFilenameResult) -> str:
    lines = [
...
def write_evidence(result: DesktopFilenameResult, *, json_output_path: Path = DEFAULT_JSON_OUTPUT_PATH, txt_output_path: Path = DEFAULT_TXT_OUTPUT_PATH) -> tuple[Path, Path]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Attach a Desktop self-report file in Chrome by typing filename in native picker and pressing Enter, without Send")
    parser.add_argument("--config-path", default=str(DEFAULT_CONFIG_PATH))
    parser.add_argument("--provider", choices=("fake-ready", "fake-no-target", "fake-no-picker", "fake-ambiguous-picker", "fake-write-failed", "fake-enter-failed", "fake-not-verified", "pywinauto"), default="fake-ready")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_desktop_filename_upload_no_send(
