...
PATCH_NAME = "u3_c31_chrome_fail_report_upload_no_send"

ACTION_UPLOAD_OPERATOR_REPORT = "upload_operator_report"
CONFIRM_CHROME_FAIL_REPORT_UPLOAD_NO_SEND = "PATCHOPS_CONFIRM_CHROME_FAIL_REPORT_UPLOAD_NO_SEND"

PASS_CHROME_FAIL_REPORT_ATTACHMENT_READY_NO_SEND = "PASS_CHROME_FAIL_REPORT_ATTACHMENT_READY_NO_SEND"
BLOCKED_CHROME_REPORT_TARGET_NOT_READY = "BLOCKED_CHROME_REPORT_TARGET_NOT_READY"
BLOCKED_CHROME_REPORT_UPLOAD_CONFIRMATION_REQUIRED = "BLOCKED_CHROME_REPORT_UPLOAD_CONFIRMATION_REQUIRED"
BLOCKED_CHROME_REPORT_UPLOAD_CONFIRMATION_MISMATCH = "BLOCKED_CHROME_REPORT_UPLOAD_CONFIRMATION_MISMATCH"
...
BLOCKED_CHROME_REPORT_UPLOAD_NOT_VERIFIED = "BLOCKED_CHROME_REPORT_UPLOAD_NOT_VERIFIED"
BLOCKED_CHROME_REPORT_UPLOAD_MESSAGE_MISMATCH = "BLOCKED_CHROME_REPORT_UPLOAD_MESSAGE_MISMATCH"
...
DEFAULT_JSON_OUTPUT_PATH = Path("data/runtime/copilot_handoff/latest_chrome_fail_report_upload_no_send.json")
DEFAULT_TXT_OUTPUT_PATH = Path("data/runtime/copilot_handoff/latest_chrome_fail_report_upload_no_send.txt")

CHATGPT_TITLE_TOKENS = ("chatgpt", "openai")
ATTACH_BUTTON_NAME_TOKENS = (
    "attach",
    "upload",
    "add files",
    "add photos",
...
FILE_DIALOG_TITLE_TOKENS = (
    "open",
    "upload",
    "choose file",
    "file upload",
)
ATTACHMENT_READY_TOKENS = (
    "uploaded",
    "attached",
    "file",
)
...
class ChromeReportUploaderSafety:
...
@dataclass(frozen=True)
class ChromeFailReportUploadNoSendResult:
    ok: bool
    result_label: str
    patch_name: str | None
    ...
    safety: ChromeReportUploaderSafety = field(default_factory=ChromeReportUploaderSafety)
...
class ChromeReportUploadAdapter(Protocol):
    provider_name: str

    def focus_target(self) -> TargetProbe:
        ...

    def upload_report_no_send(self, report_path: Path) -> UploadAttempt:
        ...
...
def read_report_values_from_gate(path: Path) -> tuple[str | None, str | None, str | None, str | None, str | None, str | None]:
    if not path.exists():
        return None, None, ACTION_UPLOAD_OPERATOR_REPORT, "chrome", None, None
    return report_values_from_armed_gate_payload(load_json_object(path))


class FakeChromeReportUploadAdapter:
    ...
    def upload_report_no_send(self, report_path: Path) -> UploadAttempt:
        if self.mode == "send-risk":
            return UploadAttempt(False, False, False, False, False, True, ("fake send risk detected",))
        if self.mode == "upload-not-verified":
            self.uploaded_path = str(report_path)
            return UploadAttempt(True, True, True, True, False, False, ("fake upload not verified",))
        if self.mode == "picker-fails":
            return UploadAttempt(True, True, False, False, False, False, ("fake picker path write failed",))
        self.uploaded_path = str(report_path)
        return UploadAttempt(True, True, True, True, True, False, ())


class PywinautoChromeReportUploadAdapter:
    ...
    def _desktop(self) -> Any:
        try:
            ...
            raise RuntimeError(f"pywinauto is required for live Chrome report upload probing: {exc}") from exc
    ...
    def focus_target(self) -> TargetProbe:
        ...
        return TargetProbe(True, True, len(windows), 1, _safe_window_title(selected), len(candidates), ())
    ...
    def _find_attachment_buttons(self, window: Any) -> list[Any]:
        ...
        for element in descendants:
            try:
                ...
                continue
            if not enabled or not visible or control_type != "button":
                continue
            if any(token in lowered for token in ATTACH_BUTTON_NAME_TOKENS):
    ...
    def upload_report_no_send(self, report_path: Path) -> UploadAttempt:
        if self._selected_window is None:
            return UploadAttempt(False, False, False, False, False, False, ("Chrome target was not focused first",))

        buttons = self._find_attachment_buttons(self._selected_window)
        if not buttons:
            return UploadAttempt(False, False, False, False, False, False, ("no safe Chrome attachment button candidate",))
        if len(buttons) > 1:
            return UploadAttempt(False, False, False, False, False, True, ("multiple attachment buttons; refusing to choose",))

        try:
            buttons[0].click_input()
            time.sleep(self.settle_seconds)
        except Exception as exc:
            return UploadAttempt(True, False, False, False, False, False, (f"attachment button click failed: {exc}",))

        dialog = self._find_file_dialog()
        if dialog is None:
            return UploadAttempt(True, False, False, False, False, False, ("file picker did not appear",))

        edit = self._find_file_name_edit(dialog)
        if edit is None:
            return UploadAttempt(True, True, False, False, False, False, ("file picker filename edit not found",))
        ...
        try:
            ...
            return UploadAttempt(True, True, False, False, False, False, (f"file picker path write failed: {exc}",))

        ready = self._attachment_ready(report_path.name)
        if not ready:
            return UploadAttempt(True, True, True, False, False, False, ("attachment readiness was not verified",))
        return UploadAttempt(True, True, True, True, True, False, ())
...
def build_adapter(provider: str) -> ChromeReportUploadAdapter:
    if provider.startswith("fake-"):
        return FakeChromeReportUploadAdapter(provider.removeprefix("fake-"))
    if provider == "pywinauto":
        return PywinautoChromeReportUploadAdapter()
...
def make_result(
    *,
    ...
    attachment_candidate_count: int = 0,
    issues: Sequence[str] = (),
) -> ChromeFailReportUploadNoSendResult:
    safety = ChromeReportUploaderSafety(
        browser_action_performed=bool(chrome_target_focused or upload_trigger_attempted or file_picker_used or file_path_written),
        chatgpt_submit_performed=False,
        operator_report_uploaded=bool(upload_verified),
    ...
    return ChromeFailReportUploadNoSendResult(
        ok=ok,
...
def run_chrome_fail_report_upload_no_send(
    *,
    patch_name: str | None,
    patch_result: str | None,
    selected_action: str | None,
    ...
    confirmation_matched = confirmation_text == CONFIRM_CHROME_FAIL_REPORT_UPLOAD_NO_SEND
    if not live_browser:
        hard_gate_issues.append("--live-browser is required for C31")
    if confirmation_text is None:
        return make_result(
            ok=False,
            result_label=BLOCKED_CHROME_REPORT_UPLOAD_CONFIRMATION_REQUIRED,
            patch_name=patch_name,
            patch_result=patch_result,
            selected_action=selected_action,
            browser_lane=browser_lane,
    ...
    if not confirmation_matched:
        return make_result(
            ok=False,
            result_label=BLOCKED_CHROME_REPORT_UPLOAD_CONFIRMATION_MISMATCH,
            patch_name=patch_name,
            patch_result=patch_result,
            selected_action=selected_action,
            browser_lane=browser_lane,
            ...
            send_risk_detected=True,
            provider=provider,
            issues=(f"upload confirmation must exactly match {CONFIRM_CHROME_FAIL_REPORT_UPLOAD_NO_SEND}",),
        )
    if not stop_before_send:
        hard_gate_issues.append("--stop-before-send is required for C31")
    if hard_gate_issues:
        return make_result(
            ok=False,
    ...
    if selected_action != ACTION_UPLOAD_OPERATOR_REPORT:
        message_issues.append(f"selected_action must be {ACTION_UPLOAD_OPERATOR_REPORT}")
    if browser_lane != "chrome":
        message_issues.append("browser_lane must be chrome")
    if message_issues:
        return make_result(
            ok=False,
            result_label=BLOCKED_CHROME_REPORT_UPLOAD_MESSAGE_MISMATCH,
            patch_name=patch_name,
            patch_result=patch_result,
            selected_action=selected_action,
            browser_lane=browser_lane,
    ...
    try:
        ...
        return make_result(
            ok=False,
            result_label=BLOCKED_CHROME_REPORT_TARGET_NOT_READY,
            patch_name=patch_name,
            patch_result=patch_result,
            selected_action=selected_action,
            browser_lane=browser_lane,
    ...
    upload = adapter.upload_report_no_send(report_path)
    combined_issues = tuple(target.issues + upload.issues)
    if upload.send_risk_detected:
        label = BLOCKED_SEND_RISK
    elif not upload.upload_verified:
        label = BLOCKED_CHROME_REPORT_UPLOAD_NOT_VERIFIED
    else:
        label = PASS_CHROME_FAIL_REPORT_ATTACHMENT_READY_NO_SEND

    return make_result(
        ok=upload.upload_verified,
        result_label=label,
        patch_name=patch_name,
        patch_result=patch_result,
        selected_action=selected_action,
        ...
        upload_verified=upload.upload_verified,
...
def render_text(result: ChromeFailReportUploadNoSendResult) -> str:
    lines = [
        ...
        f"upload_verified: {str(result.upload_verified).lower()}",
...
def write_upload_evidence(
    result: ChromeFailReportUploadNoSendResult,
    *,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Chrome FAIL/BLOCKED operator-report upload no-send probe")
    ...
    parser.add_argument("--provider", choices=("fake-ready", "fake-no-target", "fake-ambiguous-target", "fake-upload-not-verified", "fake-picker-fails", "fake-send-risk", "pywinauto"), default="fake-ready")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_chrome_fail_report_upload_no_send(
        ...
        browser_lane=(normalize_optional_text(args.browser_lane) or gate_browser_lane or "chrome").lower(),
