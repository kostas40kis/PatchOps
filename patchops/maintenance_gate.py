...
# PATCHOPS_D3C_MAINTENANCE_GATE_RUNTIME_SAFE_OVERRIDE_20260423
from dataclasses import asdict as _patchops_d3c_mg_asdict
from dataclasses import dataclass as _patchops_d3c_mg_dataclass
from subprocess import CompletedProcess as _patchops_d3c_CompletedProcess, run as _patchops_d3c_run
import sys as _patchops_d3c_sys
from typing import Callable as _patchops_d3c_Callable, Iterable as _patchops_d3c_Iterable, Sequence as _patchops_d3c_Sequence

@_patchops_d3c_mg_dataclass(frozen=True)
class MaintenanceGateCommandResult:
    name: str
    ok: bool
    exit_code: int
...
def run_pytest_paths(
    *,
    wrapper_project_root: str | Path,
    test_paths: _patchops_d3c_Sequence[str],
    name: str,
    python_executable: str | None = None,
) -> MaintenanceGateCommandResult:
    root = Path(wrapper_project_root).resolve()
    executable = python_executable or _patchops_d3c_sys.executable
    command = (str(executable), "-m", "pytest", "-q", *tuple(str(item) for item in test_paths))
    completed: _patchops_d3c_CompletedProcess[str] = _patchops_d3c_run(
        command,
        cwd=root,
        text=True,
        capture_output=True,
...
def build_maintenance_gate_snapshot(
    ...
    *,
    ...
    core_tests_state: str = "unknown",
    python_executable: str | None = None,
    command_runner: _patchops_d3c_Callable[..., MaintenanceGateCommandResult] = run_pytest_paths,
) -> MaintenanceGateSnapshot:
    normalized_core_tests_state = _normalize_core_tests_state(core_tests_state)
    wrapper_root = Path(wrapper_project_root).resolve()
    ...
    if not regression_gate.ok:
        issues.append("bundle manifest regression gate failed")
    if not smoke_gate.ok:
        issues.append("bundle post-build smoke gate failed")
    issues.extend(release_readiness.issues)

    if issues:
        status = "not_ready"
    elif normalized_core_tests_state == "green":
        status = "green"
    else:
        status = "review_required"
...
def maintenance_gate_as_dict(snapshot: MaintenanceGateSnapshot) -> dict[str, object]:
    payload = _patchops_d3c_mg_asdict(snapshot)
    payload["scope_lines"] = list(render_maintenance_gate_scope_lines(snapshot))
    payload["release_readiness_scope_lines"] = list(
        render_release_readiness_scope_lines(snapshot.release_readiness)
    )
...
def _items_or_none(items: _patchops_d3c_Sequence[str]) -> tuple[str, ...]:
    return tuple(items) if items else ("(none)",)
...
# PATCHOPS_D3D_MAINTENANCE_GATE_BEHAVIOR_OVERRIDE_20260423
def build_maintenance_gate_snapshot(
    wrapper_project_root: str | Path,
    *,
    available_profiles: _patchops_d3c_Iterable[str],
    ...
    if (not regression_gate.ok) or (not smoke_gate.ok) or release_readiness.status == "not_ready":
        status = "not_ready"
    elif normalized_core_tests_state == "green":
        status = "green"
    else:
