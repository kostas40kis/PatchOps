...
PATCH_NAME = "l26_12z_short_live_root_canonical_report"


@dataclass(frozen=True)
class ShortLiveRootCanonicalReportResult:
    patch_name: str = PATCH_NAME
    python_executable: str = sys.executable
    short_live_root_used: bool = False
    short_live_root_path: str = ""
    short_live_root_path_length: int = 0
...
def _write_json(path: Path, result: ShortLiveRootCanonicalReportResult) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(result.to_payload(), indent=2, sort_keys=True), encoding="utf-8")


def _write_live_report(path: Path, result: ShortLiveRootCanonicalReportResult) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(f"{k}: {v}" for k, v in result.to_payload().items()) + "\n", encoding="utf-8")


def _build_canonical_report(canonical_report_path: Path, inner_patchops_report_path: Path, live_payload: dict[str, object]) -> tuple[bool, bool, bool, int]:
...
def run_short_live_root_canonical_report(
    ...
    observe_seconds: int,
    probe_seconds: int,
) -> ShortLiveRootCanonicalReportResult:
    short_live_root.mkdir(parents=True, exist_ok=True)
    json_path = short_live_root / "l26_12z_short_live_root_canonical_report_result.json"
    live_report_path = short_live_root / "l26_12z_short_live_root_canonical_report_live_report.txt"
    state: dict[str, object] = {"short_live_root_used": True, "short_live_root_path": str(short_live_root), "short_live_root_path_length": len(str(short_live_root))}
    try:
        if len(str(short_live_root)) >= 120:
            raise RuntimeError(f"Short live root is too long for this proof: {short_live_root}")
        # Critical repair versus L26.12Y: run the accepted generated-candidate diff from a short child dir.
        # This prevents the nested L26.12U live-report path from exceeding Windows-friendly path limits.
        diff_root = short_live_root / "g"
        ...
        state.update({"result": "PASS" if ok else "FAIL", "failure_layer": "" if ok else "short_live_root_canonical_report", "error": "" if ok else f"Canonical proof failed; upstream_result={up.get('result')}; upstream_error={up.get('error')}"})
    except Exception as exc:
        state.update({"result": "FAIL", "failure_layer": "short_live_root_canonical_report", "error": f"{type(exc).__name__}: {exc}"})
    result = ShortLiveRootCanonicalReportResult(**state)
    _write_json(json_path, result)
    _write_live_report(live_report_path, result)
    return result


def assert_acceptance(result: ShortLiveRootCanonicalReportResult) -> None:
    p = result.to_payload()
    true_keys = [
        "short_live_root_used", "canonical_report_created", "canonical_contains_patchops_apply_evidence",
        "canonical_contains_browser_live_evidence", "canonical_contains_upload_submit_probe_evidence",
        "uploaded_safe_copy_hash_matches_inner_report", "chatgpt_submit_performed", "ready_for_next_probe", "post_scan_completed",
    ]
    false_keys = [
    ...
    if int(p.get("short_live_root_path_length") or 999) >= 120:
        missing.append("short_live_root_path_length_under_120")
    ...
    if missing or unexpected:
        raise AssertionError(f"L26.12Z acceptance failed; missing={missing}; unexpected={unexpected}; layer={result.failure_layer}; error={result.error}")
