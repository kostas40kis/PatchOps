...
PATCH_NAME = "l26_12b_direct_requested_chat_report_upload_staging_repair"
...
@dataclass(frozen=True)
class DirectReportUploadStagingResult:
...
def _find_plus_control(max_depth: int = 16, max_controls: int = 1600) -> tuple[UploadControlCandidate, object]:
    window = _edge_window()
...
def _find_upload_menu_item(timeout_seconds: float = 10.0, max_controls: int = 2600) -> tuple[UploadControlCandidate, object]:
    import pywinauto  # type: ignore
        ...
        for root in roots:
            ...
            while queue and scanned < max_controls:
                ...
                if any(word in blob for word in _UPLOAD_WORDS) and control_type in {"Button", "MenuItem", "Text", "ListItem", "Custom", "Hyperlink"}:
                    score = 170 + (30 if control_type in {"Button", "MenuItem"} else 0)
...
def _write_report(path: Path, result: DirectReportUploadStagingResult) -> None:
...
def _write_json(path: Path, result: DirectReportUploadStagingResult) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(result.to_payload(), indent=2, sort_keys=True), encoding="utf-8")


def run_l26_12b_direct_report_upload_staging_gate(*, output_dir: str | Path, target_url: str, start_if_missing: bool = True, settle_seconds: float = 10.0, allow_report_upload: bool = False) -> DirectReportUploadStagingResult:
    out_dir = Path(output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / "normal_edge_l26_12b_direct_report_upload_staging_result.json"
    report_path = out_dir / "normal_edge_l26_12b_direct_report_upload_staging_report.txt"
    ...
    try:
        if not allow_report_upload:
            raise RuntimeError("L26.12B requires explicit --allow-report-upload to attach the report file.")
        if not target_ok:
        ...
        if not (nav_observed and composer_found and focus_verified):
            raise RuntimeError("Requested chat navigation/composer focus was not proven directly.")
        ...
        result = DirectReportUploadStagingResult(
            ...
            result="FAIL",
            failure_layer="direct_report_upload_staging_gate",
...
def assert_l26_12b_acceptance(result: DirectReportUploadStagingResult) -> None:
    ...
    if missing_true or unexpected_true:
        raise AssertionError(f"L26.12B acceptance failed; missing_true={missing_true}; unexpected_true={unexpected_true}; failure_layer={result.failure_layer}; error={result.error}")
