"""L20.6 broad checkpoint for downloaded-file filesystem validation.

This broad checkpoint consolidates L20.1 through L20.5. It proves the passive
preflight, CLI/readback checkpoint, passive plan, controlled authorization gate,
and first controlled existence-only proof remain accepted. The only filesystem
operation allowed by this stream so far is the L20.5 existence-only check against
the synthetic PatchOps runtime fixture. It does not start Edge, inspect pages,
click, download, stat/hash files, read bytes, open/list/extract archives, read
manifests, paste, send, run packages, start localhost services, use browser
extensions, commit, or push.
"""
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L20.6 Microsoft Edge downloaded-file filesystem validation broad checkpoint"
COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-filesystem-validation-broad-checkpoint"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-filesystem-validation-existence-proof"
SOURCE_PATCH = "L20.5"
NEXT_PATCH = "L20.7 Microsoft Edge downloaded-file filesystem validation final acceptance marker"
...
def _missing_doc_phrases(root: Path) -> list[str]:
    text = _read_text(root / "docs/llm_browser_live_adapter_edge_downloaded_file_filesystem_validation_broad_checkpoint.md")
...
def _compact_source_summary(payload: Mapping[str, Any]) -> dict[str, Any]:
    result = payload.get("existence_validation_result") or {}
    candidate = payload.get("candidate_safety") or {}
    return {
        "ok": payload.get("ok"),
        "patch": payload.get("patch"),
        "l20_5_complete": payload.get("l20_5_complete"),
        "source_l20_4_controlled_authorization_gate_accepted": payload.get("source_l20_4_controlled_authorization_gate_accepted"),
        "filesystem_existence_proof_requested": payload.get("filesystem_existence_proof_requested"),
        "filesystem_existence_proof_authorized": payload.get("filesystem_existence_proof_authorized"),
        "candidate_safe_for_l20_5_existence_only_proof": candidate.get("candidate_safe_for_l20_5_existence_only_proof"),
...
def build_edge_downloaded_file_filesystem_validation_broad_checkpoint(
    repo_root: str | Path | None = None,
    *,
    target_url: str | None = None,
) -> dict[str, Any]:
    """Build the L20.6 broad checkpoint over L20.1 through L20.5."""
    ...
    authorized_payload = l20_05.build_edge_downloaded_file_filesystem_validation_existence_proof(
        ...
        authorization_token=REQUIRED_FILESYSTEM_EXISTENCE_PROOF_AUTHORIZATION_TOKEN,
        candidate_path_metadata=DEFAULT_CANDIDATE_RELATIVE_PATH,
        target_url=target,
    )
    unsafe_payload = l20_05.build_edge_downloaded_file_filesystem_validation_existence_proof(
        root,
        allow_filesystem_existence_proof=True,
        authorization_token=REQUIRED_FILESYSTEM_EXISTENCE_PROOF_AUTHORIZATION_TOKEN,
    ...
    default_ok = (
        ...
        and default_payload.get("l20_5_complete") is True
        and default_payload.get("source_l20_4_controlled_authorization_gate_accepted") is True
        and default_payload.get("filesystem_existence_proof_authorized") is False
        and default_payload.get("filesystem_validation_performed") is False
    ...
    authorized_ok = (
        ...
        and authorized_payload.get("l20_5_complete") is True
        and authorized_payload.get("source_l20_4_controlled_authorization_gate_accepted") is True
        and authorized_payload.get("filesystem_existence_proof_authorized") is True
        and authorized_payload.get("filesystem_validation_scope") == "existence_only_synthetic_patchops_runtime_fixture"
        and authorized_payload.get("candidate_safety", {}).get("candidate_safe_for_l20_5_existence_only_proof") is True
        and authorized_payload.get("existence_validation_result", {}).get("existence_only_validation_performed") is True
        and authorized_payload.get("existence_validation_result", {}).get("candidate_exists") is True
    ...
    unsafe_ok = (
        ...
        and unsafe_payload.get("filesystem_existence_proof_authorized") is False
        and unsafe_payload.get("candidate_safety", {}).get("candidate_safe_for_l20_5_existence_only_proof") is False
        and unsafe_payload.get("existence_validation_result", {}).get("existence_only_validation_performed") is False
        and unsafe_payload.get("real_file_exists_check_performed") is False
        and unsafe_payload.get("downloaded_file_exists_check_performed") is False
        and _always_false_ok(unsafe_payload)
    )
    l20_chain_ok = default_ok and authorized_ok and unsafe_ok

    checks = [
        _check("l20_1_through_l20_5_remain_accepted", l20_chain_ok),
        _check("default_existence_proof_readback_remains_passive", default_ok, _compact_source_summary(default_payload)),
        _check("authorized_existence_proof_readback_remains_existence_only", authorized_ok, _compact_source_summary(authorized_payload)),
        _check("unsafe_candidate_rejected_without_filesystem_access", unsafe_ok, _compact_source_summary(unsafe_payload)),
        _check("only_synthetic_fixture_existence_check_is_allowed", authorized_ok),
        _check("stat_hash_byte_archive_manifest_remain_blocked", _always_false_ok(authorized_payload), _always_false_summary(authorized_payload)),
        ...
        _check("docs_contain_l20_6_safety_contract", not missing_doc_phrases, {"missing_phrases": missing_doc_phrases}),
    ...
    return {
        ...
        "broad_filesystem_validation_checkpoint": True,
        "existence_only_stream_checkpoint": True,
        "synthetic_patchops_runtime_fixture_only": True,
        "microsoft_edge_first": True,
        "opera_second": True,
        "opera_active_implementation_target": False,
        "l20_1_through_l20_5_remain_accepted": l20_chain_ok,
        "existence_only_filesystem_validation_proof_remains_accepted": authorized_ok,
        "default_existence_proof_readback_remains_passive": default_ok,
        "authorized_existence_proof_readback_remains_existence_only": authorized_ok,
        "unsafe_candidate_remains_rejected_without_filesystem_access": unsafe_ok,
        ...
        "controlled_filesystem_validation_execution_allowed": bool(authorized_payload.get("controlled_filesystem_validation_execution_allowed")),
        "filesystem_validation_execution_allowed": bool(authorized_payload.get("filesystem_validation_execution_allowed")),
        "filesystem_validation_active": False,
        "filesystem_validation_performed": bool(authorized_payload.get("filesystem_validation_performed")),
        ...
        "real_file_exists_check_performed": bool(authorized_payload.get("real_file_exists_check_performed")),
        "downloaded_file_exists_check_performed": bool(authorized_payload.get("downloaded_file_exists_check_performed")),
        ...
        "l20_6_complete": ok,
        "remaining_l20_6_patches": [] if ok else [PATCH],
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L20.1-L20.5 Accepted          : {payload.get('l20_1_through_l20_5_remain_accepted')}",
        f"Existence Proof Accepted      : {payload.get('existence_only_filesystem_validation_proof_remains_accepted')}",
        f"Filesystem Validation Scope   : {payload.get('filesystem_validation_scope')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_downloaded_file_filesystem_validation_broad_checkpoint(args.repo_root, target_url=args.target_url)
    if args.json:
