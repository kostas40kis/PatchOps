"""L19.1 passive preflight gate for downloaded-file validation.

This starts the downloaded-file validation stream after the accepted L18.7
Microsoft Edge download workflow final marker. L19.1 is readback-only: it does
not start Edge, inspect real pages, click, download, create staging directories,
check real downloaded files, read file bytes, read artifact content, paste, send,
run packages, start localhost services, use a browser extension, commit, or push.
"""
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L19.1 Microsoft Edge downloaded-file validation passive preflight gate"
COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-validation-passive-preflight-gate"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-download-workflow-final-acceptance-marker"
SOURCE_PATCH = "L18.7"
NEXT_PATCH = "L19.2 Microsoft Edge downloaded-file validation CLI/readback checkpoint"
...
def _missing_doc_phrases(root: Path) -> list[str]:
    text = _read_text(root / "docs/llm_browser_live_adapter_edge_downloaded_file_validation_passive_preflight_gate.md")
...
def build_file_validation_preflight_authorization_readback(
    *,
    allow_file_validation_preflight: bool = False,
    authorization_token: str | None = None,
) -> dict[str, Any]:
    requested = bool(allow_file_validation_preflight)
    token_present = authorization_token == REQUIRED_FILE_VALIDATION_PREFLIGHT_AUTHORIZATION_TOKEN
    authorized = requested and token_present
    return {
        "downloaded_file_validation_preflight_requested": requested,
        "downloaded_file_validation_preflight_authorization_token_required": True,
        "downloaded_file_validation_preflight_authorization_token_present": token_present,
        "downloaded_file_validation_preflight_authorized": authorized,
        ...
        "required_authorization_token": REQUIRED_FILE_VALIDATION_PREFLIGHT_AUTHORIZATION_TOKEN,
    }


def _source_l18_7_ok(root: Path, target_url: str) -> tuple[bool, dict[str, Any]]:
    source = l18_07.build_edge_download_workflow_final_acceptance_marker(root, target_url=target_url)
    ok = (
        source.get("ok") is True
        and source.get("patch") == SOURCE_PATCH
        and source.get("l18_7_complete") is True
        and source.get("l18_download_workflow_stream_complete") is True
        and source.get("downloaded_file_validation_is_next_separate_stream") is True
...
def build_edge_downloaded_file_validation_passive_preflight_gate(
    ...
    *,
    ...
    """Build the L19.1 passive downloaded-file validation preflight payload."""
    root = _repo_root(repo_root)
    before_modules = set(sys.modules)
    target = target_url or DEFAULT_TARGET_URL
    source_ok, source = _source_l18_7_ok(root, target)
    auth = build_file_validation_preflight_authorization_readback(
        allow_file_validation_preflight=allow_file_validation_preflight,
    ...
    safety_ok = (
        auth["downloaded_file_validation_execution_allowed"] is False
    ...
    checks = [
        _check("source_l18_7_download_workflow_final_marker_accepted", source_ok),
        _check("downloaded_file_validation_preflight_authorization_surface_present", True),
        _check("downloaded_file_validation_preflight_authorization_is_readback_only", auth["downloaded_file_validation_preflight_authorization_is_readback_only_in_l19_1"] is True),
        _check("downloaded_file_validation_execution_blocked", safety_ok),
        _check("downloaded_file_byte_reading_blocked", True),
        _check("downloaded_archive_opening_and_manifest_reading_blocked", True),
        ...
        _check("docs_contain_l19_1_safety_contract", not missing_doc_phrases, {"missing_phrases": missing_doc_phrases}),
    ...
    return {
        ...
        "downloaded_file_validation_passive_preflight_gate": True,
        "microsoft_edge_first": True,
        "opera_second": True,
        "opera_active_implementation_target": False,
        "source_l18_7_download_workflow_final_marker_accepted": source_ok,
        "l18_download_workflow_stream_complete": bool(source.get("l18_download_workflow_stream_complete")),
        "downloaded_file_validation_preflight_readback": auth,
        "downloaded_file_validation_preflight_requested": auth["downloaded_file_validation_preflight_requested"],
        "downloaded_file_validation_preflight_authorized": auth["downloaded_file_validation_preflight_authorized"],
        "downloaded_file_validation_preflight_authorization_is_readback_only_in_l19_1": True,
        "downloaded_file_validation_execution_allowed": False,
        "downloaded_file_validation_active": False,
        "downloaded_file_validation_performed": False,
        ...
        "source_l18_7_summary": {
            "ok": source.get("ok"),
            "patch": source.get("patch"),
            "l18_7_complete": source.get("l18_7_complete"),
            "l18_download_workflow_stream_complete": source.get("l18_download_workflow_stream_complete"),
            "downloaded_file_validation_is_next_separate_stream": source.get("downloaded_file_validation_is_next_separate_stream"),
        ...
        "l19_1_complete": ok,
        "remaining_l19_1_patches": [] if ok else [PATCH],
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L18.7 Accepted                       : {payload.get('source_l18_7_download_workflow_final_marker_accepted')}",
        f"File Validation Preflight Authorized : {payload.get('downloaded_file_validation_preflight_authorized')}",
        f"File Validation Execution Allowed    : {payload.get('downloaded_file_validation_execution_allowed')}",
        f"Downloaded File Bytes Read           : {payload.get('downloaded_file_bytes_read')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--allow-file-validation-preflight", action="store_true")
    parser.add_argument("--authorization-token", default=None)
    parser.add_argument("--json", action="store_true")
    parser.add_argument("--compact", action="store_true")
    args = parser.parse_args(argv)

    payload = build_edge_downloaded_file_validation_passive_preflight_gate(
        args.repo_root,
        allow_file_validation_preflight=args.allow_file_validation_preflight,
