"""L15.5 passive handoff marker after the Edge live-start proof stream.

L15.5 locks the accepted L15.1-L15.4 state before real-page detection work.
It is deliberately passive: it does not start Microsoft Edge, import Selenium,
interact with ChatGPT, inspect a page, detect artifacts, download, paste, send,
run packages from the browser, commit, or push.
"""
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L15.5 Microsoft Edge live-start handoff marker before real-page detection"
COMMAND_NAME = "browser-start-supervised-launch-edge-live-start-handoff-marker-before-real-page-detection"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-first-controlled-open-proof-broad-checkpoint"
...
NEXT_PATCH = "L16.1 Microsoft Edge real-page detection passive preflight gate"
...
REQUIRED_REPO_PATHS = (
    ...
    "scripts/patch_l15_05_brief_validate.py",
    ...
    "tests/test_l15_05_edge_live_start_handoff_marker_current.py",
)
...
def _missing_doc_phrases(root: Path) -> list[str]:
    text = _read_text(root / "docs/llm_browser_live_adapter_edge_live_start_handoff_marker.md")
...
def _no_chat_or_page_side_effects(payload: Mapping[str, Any]) -> bool:
    return (
        payload.get("selenium_required") is False
        and payload.get("selenium_imported_by_readback") is False
        and payload.get("chatgpt_url_opened") is False
...
def build_edge_live_start_handoff_marker(repo_root: str | Path | None = None) -> dict[str, Any]:
    """Build the passive L15.5 handoff marker before real-page detection."""
    ...
    l15_4_dry = l15_04.build_edge_first_controlled_open_proof_broad_checkpoint(root)
    l15_4_cli_dry = _invoke_cli(
        root,
        [SOURCE_COMMAND_NAME, "--repo-root", str(root), "--json", "--compact"],
        timeout=150,
    )
    l15_4_cli_payload = l15_4_cli_dry.get("payload", {})

    names = _command_names()
    missing_commands = [
        name
        for name in (L15_1_COMMAND_NAME, L15_2_COMMAND_NAME, L15_3_COMMAND_NAME, SOURCE_COMMAND_NAME, COMMAND_NAME)
    ...
    l15_1_ok = (
        ...
        and l15_1_authorized.get("authorization_present_is_readback_only_in_l15_1") is True
    ...
    l15_3_dry_ok = (
        ...
        and _no_chat_or_page_side_effects(l15_3_dry)
    )
    l15_4_dry_ok = (
        l15_4_dry.get("ok") is True
        and l15_4_dry.get("l15_1_authorization_gate_still_green") is True
        and l15_4_dry.get("l15_2_cli_readback_still_green") is True
        and l15_4_dry.get("l15_3_dry_readback_still_green") is True
        and l15_4_dry.get("launch_execution_allowed") is False
        and l15_4_dry.get("browser_started") is False
        and l15_4_dry.get("edge_process_started") is False
        and _no_chat_or_page_side_effects(l15_4_dry)
    )
    l15_4_cli_dry_ok = (
        l15_4_cli_dry.get("ok") is True
        and l15_4_cli_payload.get("patch") == "L15.4"
        and l15_4_cli_payload.get("l15_4_complete") is True
        and l15_4_cli_payload.get("launch_execution_allowed") is False
        and l15_4_cli_payload.get("browser_started") is False
        and l15_4_cli_payload.get("edge_process_started") is False
        and _no_chat_or_page_side_effects(l15_4_cli_payload)
    )

    checks = [
        _check("l15_1_authorization_gate_accepted", l15_1_ok),
        _check("l15_2_cli_readback_accepted", l15_2_ok),
        _check("l15_3_first_controlled_open_proof_dry_readback_accepted", l15_3_dry_ok),
        _check("l15_4_broad_checkpoint_dry_readback_accepted", l15_4_dry_ok),
        _check("l15_4_broad_checkpoint_cli_dry_readback_accepted", l15_4_cli_dry_ok, {"returncode": l15_4_cli_dry.get("returncode")}),
        _check("microsoft_edge_first", True),
        _check("opera_second_not_active", True, {"opera_active": False}),
        _check("handoff_marker_is_passive", True),
        _check("real_page_detection_not_active_yet", True),
        ...
        _check("docs_contain_l15_5_safety_contract", not missing_doc_phrases, {"missing_phrases": missing_doc_phrases}),
    ...
    return {
        ...
        "passive_handoff_marker": True,
        "l15_live_start_stream_complete": ok,
        "l15_1_through_l15_4_accepted": l15_1_ok and l15_2_ok and l15_3_dry_ok and l15_4_dry_ok and l15_4_cli_dry_ok,
        ...
        "l15_5_complete": ok,
        "remaining_l15_5_patches": [] if ok else [PATCH],
        "l15_1_summary": {
        ...
        "l15_4_summary": {
            "ok": l15_4_dry.get("ok"),
            "l15_4_complete": l15_4_dry.get("l15_4_complete"),
            "launch_execution_allowed": l15_4_dry.get("launch_execution_allowed"),
            "browser_started": l15_4_dry.get("browser_started"),
            "edge_process_started": l15_4_dry.get("edge_process_started"),
            "next_patch": l15_4_dry.get("next_patch"),
        },
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L15.1-L15.4 Accepted     : {payload.get('l15_1_through_l15_4_accepted')}",
        f"Real Page Detection      : {payload.get('real_page_detection_active')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_live_start_handoff_marker(args.repo_root)
    if args.json:
