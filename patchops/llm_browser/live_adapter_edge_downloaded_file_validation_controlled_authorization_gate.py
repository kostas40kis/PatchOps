"""L19.4 controlled authorization gate for downloaded-file validation.

This checkpoint adds an explicit future authorization token/readback surface after
L19.3. It remains passive/readback-only: no Edge start, no real page inspection,
no click, no download, no real downloaded-file check, no stat/hash, no archive
open/list/extract, no manifest read, no file-byte read, no paste, no send, no
package run, no localhost server, no browser extension, no commit, and no push.
"""
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L19.4 Microsoft Edge downloaded-file validation controlled authorization gate"
COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-validation-controlled-authorization-gate"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-validation-passive-plan-checkpoint"
SOURCE_PATCH = "L19.3"
NEXT_PATCH = "L19.5 Microsoft Edge first controlled downloaded-file metadata validation proof"
DEFAULT_TARGET_URL = "https://chatgpt.com/"
REQUIRED_FILE_VALIDATION_AUTHORIZATION_TOKEN = "PATCHOPS_L19_EDGE_DOWNLOADED_FILE_VALIDATION_CONTROLLED_AUTHORIZED_READBACK_ONLY"
...
SAFETY_PHRASES = (
    ...
    COMMAND_NAME,
    SOURCE_COMMAND_NAME,
    REQUIRED_FILE_VALIDATION_AUTHORIZATION_TOKEN,
...
SOURCE_L19_3_PASSIVE_FALSE_FIELDS = tuple(
    field for field in PASSIVE_FALSE_FIELDS if field != "controlled_downloaded_file_validation_execution_allowed"
)
...
def _missing_doc_phrases(root: Path) -> list[str]:
    text = _read_text(root / "docs/llm_browser_live_adapter_edge_downloaded_file_validation_controlled_authorization_gate.md")
...
def _source_passive_false_summary(payload: Mapping[str, Any]) -> dict[str, bool]:
    return {field: payload.get(field) is False for field in SOURCE_L19_3_PASSIVE_FALSE_FIELDS}
...
def build_downloaded_file_validation_authorization_readback(
    *,
    allow_downloaded_file_validation_authorization: bool = False,
    authorization_token: str | None = None,
) -> dict[str, Any]:
    requested = bool(allow_downloaded_file_validation_authorization)
    token_present = authorization_token == REQUIRED_FILE_VALIDATION_AUTHORIZATION_TOKEN
    authorized = requested and token_present
    return {
        "downloaded_file_validation_authorization_requested": requested,
        "downloaded_file_validation_authorization_token_required": True,
        "downloaded_file_validation_authorization_token_present": token_present,
        "downloaded_file_validation_authorized_for_future_phase": authorized,
        ...
        "required_authorization_token": REQUIRED_FILE_VALIDATION_AUTHORIZATION_TOKEN,
        "operator_review_required_before_downloaded_file_validation": True,
...
def future_downloaded_file_validation_authorization_gate_plan() -> list[dict[str, Any]]:
    return [
        {
            "step": 1,
            "name": "confirm_l19_3_passive_plan_checkpoint_accepted",
            "status": "planned_not_executed",
            "side_effect": False,
        },
        {
            "step": 2,
            "name": "require_explicit_downloaded_file_validation_authorization_flag",
            "status": "planned_not_executed",
            "side_effect": False,
        },
        {
            "step": 3,
            "name": "require_exact_downloaded_file_validation_authorization_token",
            "status": "planned_not_executed",
            "required_authorization_token": REQUIRED_FILE_VALIDATION_AUTHORIZATION_TOKEN,
            "side_effect": False,
        },
        {
            "step": 4,
            "name": "future_l19_5_or_later_may_validate_path_metadata_only",
            "status": "planned_not_executed",
            "allowed_only_in_future_l19_5_or_later": True,
            "side_effect": False,
        ...
        {
            "step": 5,
            "name": "future_real_file_existence_stat_hash_require_separate_explicit_gate",
            "status": "planned_not_executed",
            "allowed_only_in_later_stream": True,
            "side_effect": True,
            "file_existence_check_in_l19_4": False,
            "file_stat_in_l19_4": False,
            "file_hash_in_l19_4": False,
            "file_byte_read_in_l19_4": False,
        },
        {
            "step": 6,
            "name": "l19_4_readback_does_not_execute_file_validation",
            "status": "planned_not_executed",
            "side_effect": False,
            "downloaded_file_validation_execution_allowed": False,
            "downloaded_file_exists_check_performed": False,
...
def _plan_blocks_file_validation_file_read_archive_package_run(plan: Sequence[Mapping[str, Any]]) -> bool:
    metadata_step = plan[3]
    real_file_step = plan[4]
    final = plan[-1]
    return (
        metadata_step.get("read_file_bytes") is False
        and metadata_step.get("archive_open") is False
        and metadata_step.get("package_run") is False
        and real_file_step.get("file_existence_check_in_l19_4") is False
        and real_file_step.get("file_stat_in_l19_4") is False
        and real_file_step.get("file_hash_in_l19_4") is False
        and real_file_step.get("file_byte_read_in_l19_4") is False
        and final.get("downloaded_file_validation_execution_allowed") is False
...
def build_edge_downloaded_file_validation_controlled_authorization_gate(
    ...
    *,
    ...
    """Build the passive L19.4 controlled downloaded-file validation authorization gate payload."""
    root = _repo_root(repo_root)
    before_modules = set(sys.modules)
    target = target_url or DEFAULT_TARGET_URL
    source = l19_03.build_edge_downloaded_file_validation_passive_plan_checkpoint(root, target_url=target)
    auth = build_downloaded_file_validation_authorization_readback(
        allow_downloaded_file_validation_authorization=allow_downloaded_file_validation_authorization,
        authorization_token=authorization_token,
    )
    plan = future_downloaded_file_validation_authorization_gate_plan()
    ...
    source_ok = (
        ...
        and source.get("l19_3_complete") is True
        and source.get("source_l19_2_cli_readback_checkpoint_accepted") is True
        and source.get("future_downloaded_file_validation_plan_blocks_file_checks_file_read_archive_package_run") is True
    ...
    authorization_surface_ok = (
        auth["downloaded_file_validation_authorization_token_required"] is True
        and auth["downloaded_file_validation_authorization_is_readback_only_in_l19_4"] is True
    )
    execution_blocked = (
        auth["controlled_downloaded_file_validation_execution_allowed"] is False
        and auth["downloaded_file_validation_execution_allowed"] is False
        and auth["downloaded_file_validation_active"] is False
        and auth["downloaded_file_validation_performed"] is False
    ...
    checks = [
        _check("source_l19_3_passive_plan_checkpoint_accepted", source_ok),
        _check("source_l19_3_readback_remains_passive", source_passive, _source_passive_false_summary(source)),
        ...
        _check("downloaded_file_validation_authorization_surface_present", authorization_surface_ok),
        _check("downloaded_file_validation_authorization_is_readback_only", True),
        _check("downloaded_file_validation_execution_blocked", execution_blocked),
        _check("future_authorization_gate_plan_is_planned_not_executed", plan_is_planned_not_executed),
        _check("future_authorization_gate_plan_blocks_file_validation_file_read_archive_package_run", plan_blocks),
        _check("real_file_checks_archive_reading_pasteback_and_package_run_still_blocked", True),
        ...
        _check("docs_contain_l19_4_safety_contract", not missing_doc_phrases, {"missing_phrases": missing_doc_phrases}),
    ...
    return {
        ...
        "passive_readback_only_in_l19_4": True,
        "microsoft_edge_first": True,
        "opera_second": True,
        "opera_active_implementation_target": False,
        "source_l19_3_passive_plan_checkpoint_accepted": source_ok,
        "l19_3_complete": bool(source.get("l19_3_complete")),
        "source_l19_2_cli_readback_checkpoint_accepted": bool(source.get("source_l19_2_cli_readback_checkpoint_accepted")),
        "downloaded_file_validation_authorization_readback": auth,
        "downloaded_file_validation_authorization_requested": auth["downloaded_file_validation_authorization_requested"],
        "downloaded_file_validation_authorization_token_present": auth["downloaded_file_validation_authorization_token_present"],
        "downloaded_file_validation_authorized_for_future_phase": auth["downloaded_file_validation_authorized_for_future_phase"],
        "downloaded_file_validation_authorization_is_readback_only_in_l19_4": True,
        "controlled_downloaded_file_validation_execution_allowed": False,
        "downloaded_file_validation_execution_allowed": False,
        "downloaded_file_validation_active": False,
        ...
        "pasteback_remains_inactive": True,
        "package_run_from_browser_remains_inactive": True,
        "future_authorization_gate_plan": plan,
        "future_authorization_gate_plan_is_planned_not_executed": plan_is_planned_not_executed,
        "future_authorization_gate_plan_blocks_file_validation_file_read_archive_package_run": plan_blocks,
        "source_l19_3_summary": {
            "ok": source.get("ok"),
            "patch": source.get("patch"),
            "l19_3_complete": source.get("l19_3_complete"),
            "source_l19_2_cli_readback_checkpoint_accepted": source.get("source_l19_2_cli_readback_checkpoint_accepted"),
        ...
        "l19_4_complete": ok,
        "remaining_l19_4_patches": [] if ok else [PATCH],
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L19.3 Accepted                    : {payload.get('source_l19_3_passive_plan_checkpoint_accepted')}",
        f"Authorization Requested           : {payload.get('downloaded_file_validation_authorization_requested')}",
        f"Future Authorization Accepted     : {payload.get('downloaded_file_validation_authorized_for_future_phase')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--allow-downloaded-file-validation-authorization", action="store_true")
    parser.add_argument("--authorization-token", default=None)
    parser.add_argument("--json", action="store_true")
    parser.add_argument("--compact", action="store_true")
    args = parser.parse_args(argv)

    payload = build_edge_downloaded_file_validation_controlled_authorization_gate(
        args.repo_root,
        allow_downloaded_file_validation_authorization=args.allow_downloaded_file_validation_authorization,
