"""Passive L5.24 Microsoft Edge live-start preflight CLI/readback.

This module reads back the accepted L5.23 live-start preflight contract through
an explicit CLI/readback layer. It remains passive: no Selenium import, no Edge
process start, no browser session, no driver, no profile directory creation, no
click/download, no paste/send, no package run, and no git operation.
"""
...
from . import live_adapter_edge_supervised_launch_live_start_preflight_contract as l5_23_contract

PATCH = "L5.24"
PHASE = "L5"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L5.24 Microsoft Edge Supervised Launch Live Start Preflight CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-live-start-preflight-readback"
SOURCE_COMMAND_NAME = l5_23_contract.COMMAND_NAME
NEXT_PATCH = "L5.25 Live adapter Microsoft Edge supervised launch explicit authorization argument gate"
SIDE_EFFECT_BOUNDARY = "edge-supervised-launch-live-start-preflight-cli-readback-only"
BROWSER_PRIORITY = ("edge", "opera")

REQUIRED_COMMANDS: tuple[str, ...] = tuple(dict.fromkeys((*l5_23_contract.REQUIRED_COMMANDS, COMMAND_NAME)))

REQUIRED_REPO_PATHS: tuple[str, ...] = tuple(
    dict.fromkeys(
        (
            *l5_23_contract.REQUIRED_REPO_PATHS,
            "patchops/llm_browser/live_adapter_edge_supervised_launch_live_start_preflight_cli_readback.py",
            "docs/llm_browser_live_adapter_edge_supervised_launch_live_start_preflight_cli_readback.md",
            "tests/test_l5_24_edge_supervised_launch_live_start_preflight_cli_readback_current.py",
        )
    )
)
...
COMMAND_PLAN: tuple[str, ...] = (
    "python -m compileall patchops/llm_browser tests scripts/patch_l5_24_wire_edge_live_start_preflight_readback.py",
    "python -m pytest -q tests/test_l5_23_edge_supervised_launch_live_start_preflight_contract_current.py tests/test_l5_24_edge_supervised_launch_live_start_preflight_cli_readback_current.py",
    "python -m patchops.llm_browser.live_adapter_edge_supervised_launch_live_start_preflight_cli_readback --repo-root C:\\dev\\patchops --json --compact",
...
def _command_plan_state() -> dict[str, Any]:
    ...
    expected_fragments = [
        ...
        "pytest -q",
        "test_l5_23_edge_supervised_launch_live_start_preflight_contract_current.py",
        "test_l5_24_edge_supervised_launch_live_start_preflight_cli_readback_current.py",
        "live_adapter_edge_supervised_launch_live_start_preflight_cli_readback",
        COMMAND_NAME,
        "git status --short --branch",
    ]
...
def _l5_23_summary(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "explicit_operator_authorization_required": contract.get("explicit_operator_authorization_required") is True,
        "authorization_flag_required": contract.get("authorization_flag_required"),
        "profile_dir_argument_required": contract.get("profile_dir_argument_required"),
        "dedicated_profile_required": contract.get("dedicated_profile_required") is True,
        "default_profile_forbidden": contract.get("default_profile_forbidden") is True,
        "manual_user_login_required": contract.get("manual_user_login_required") is True,
        "silent_auto_submit_default": contract.get("silent_auto_submit_default"),
        "localhost_patchops_server_required": contract.get("localhost_patchops_server_required"),
        "browser_extension_required": contract.get("browser_extension_required"),
...
def _l5_23_passive_ok(summary: Mapping[str, Any]) -> bool:
    return (
        summary.get("ok") is True
        and summary.get("status") == STATUS_PASS
        and summary.get("patch") == "L5.23"
        ...
        and summary.get("authorization_flag_required") == "--allow-live-start"
        and summary.get("profile_dir_argument_required") == "--profile-dir"
...
def build_edge_supervised_launch_live_start_preflight_cli_readback(repo_root: str | Path | None = None) -> dict[str, Any]:
    root = _repo_root(repo_root)
    before_modules = set(sys.modules)

    l5_23_payload = l5_23_contract.build_edge_supervised_launch_live_start_preflight_contract(root)
    l5_23_summary = _l5_23_summary(l5_23_payload)
    source_command_static = _command_static_presence(root, SOURCE_COMMAND_NAME, "PATCHOPS L5.23 START")
    readback_command_static = _command_static_presence(root, COMMAND_NAME, "PATCHOPS L5.24 START")
    ...
    checks = [
        _check("l5_23_edge_live_start_preflight_contract_still_passes", l5_23_summary.get("ok") is True and l5_23_summary.get("status") == STATUS_PASS, l5_23_summary),
        _check("l5_23_edge_live_start_preflight_contract_remains_passive", _l5_23_passive_ok(l5_23_summary), l5_23_summary),
        _check("l5_23_source_command_still_registered", source_command_static.get("ok") is True, source_command_static),
        _check("l5_24_live_start_preflight_readback_command_registered", readback_command_static.get("ok") is True, readback_command_static),
        ...
        _check("edge_l5_docs_contain_cli_readback_boundary", doc_state.get("ok") is True, doc_state),
        _check("l5_24_command_plan_is_readback_only", command_plan_state.get("ok") is True, command_plan_state),
        _check("adapter_logic_executes_no_validation_commands", command_plan_state.get("executed_by_adapter_logic") == [], command_plan_state),
        _check("edge_live_start_authorization_gate_still_required", l5_23_summary.get("explicit_operator_authorization_required") is True, l5_23_summary),
        _check("edge_live_start_dedicated_profile_gate_still_required", l5_23_summary.get("dedicated_profile_required") is True and l5_23_summary.get("default_profile_forbidden") is True, l5_23_summary),
        _check("edge_live_start_manual_login_and_no_auto_submit_still_required", l5_23_summary.get("manual_user_login_required") is True and l5_23_summary.get("silent_auto_submit_default") is False, l5_23_summary),
    ...
    payload: dict[str, Any] = {
        ...
        "l5_23_edge_live_start_preflight_contract": l5_23_payload,
        "l5_23_summary": l5_23_summary,
...
def render_text(payload: Mapping[str, Any]) -> str:
    ...
    if not isinstance(summary, Mapping):
        summary = {}
    lines = [
        "PatchOps L5.24 Microsoft Edge supervised-launch live-start preflight CLI/readback",
        ...
        f"L5.23 Status    : {summary.get('patch')} / {summary.get('status')}",
        f"Authorization   : required={summary.get('explicit_operator_authorization_required')} flag={summary.get('authorization_flag_required')}",
        f"Profile         : dedicated={summary.get('dedicated_profile_required')} default_forbidden={summary.get('default_profile_forbidden')}",
        f"Manual Login    : {summary.get('manual_user_login_required')}",
        f"Silent Submit   : {summary.get('silent_auto_submit_default')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_supervised_launch_live_start_preflight_cli_readback(args.repo_root)
    if args.json:
        if args.compact:
