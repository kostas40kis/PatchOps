"""L25.39 broad checkpoint for manifest validation.

This follows accepted L25.38. It validates the accepted L25.38 source payload
and summarizes the real-download-artifact readback and manifest-validation
ladder. It is source-payload-only: it does not open artifact paths, read bytes,
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L25.39 Microsoft Edge controlled runtime real-download artifact manifest validation broad checkpoint"
SOURCE_PATCH = "L25.38"
NEXT_PATCH = "L25.40 Microsoft Edge controlled runtime real-download artifact manifest validation final acceptance marker"
DEFAULT_TARGET_URL = "https://chatgpt.com/"
REQUIRED_BROAD_CHECKPOINT_TOKEN = "PATCHOPS_L25_EDGE_REAL_DOWNLOAD_ARTIFACT_MANIFEST_VALIDATION_BROAD_AUTHORIZED"
...
FALSE_FIELDS = (
    "artifact_path_opened_by_l25_39",
    "artifact_bytes_read_by_l25_39",
    "zip_member_opened_by_l25_39",
...
def _normalize_l25_38_source(source_payload: Mapping[str, Any] | None) -> dict[str, Any]:
    source = dict(source_payload or {})
    return {
        "ok": source.get("ok"),
        "patch": source.get("patch"),
        "manifest_validation_checkpoint": source.get("manifest_validation_checkpoint"),
        "manifest_validation_from_source_payload_only": source.get("manifest_validation_from_source_payload_only"),
        "manifest_validation_allowed": source.get("manifest_validation_allowed"),
        ...
        "malformed_manifest_source_rejected": source.get("malformed_manifest_source_rejected"),
        "artifact_path_opened_by_l25_38": source.get("artifact_path_opened_by_l25_38", False),
        "artifact_bytes_read_by_l25_38": source.get("artifact_bytes_read_by_l25_38", False),
        "zip_member_opened_by_l25_38": source.get("zip_member_opened_by_l25_38", False),
...
def build_real_download_artifact_manifest_validation_broad_checkpoint(
    repo_root: str | Path | None = None,
    *,
    source_payload: Mapping[str, Any] | None = None,
    allow_broad_checkpoint: bool = False,
    authorization_token: str | None = None,
    target_url: str = DEFAULT_TARGET_URL,
) -> dict[str, Any]:
    Path(repo_root or ".").resolve()
    source = _normalize_l25_38_source(source_payload)
    token_present = authorization_token is not None
    token_valid = authorization_token == REQUIRED_BROAD_CHECKPOINT_TOKEN
    source_ok = bool(
        source.get("ok") is True
        and source.get("patch") == "L25.38"
        and source.get("manifest_validation_checkpoint") is True
        and source.get("manifest_validation_from_source_payload_only") is True
        and source.get("manifest_validation_allowed") is True
        ...
        and source.get("malformed_manifest_source_rejected") is True
        and source.get("artifact_path_opened_by_l25_38") is False
        and source.get("artifact_bytes_read_by_l25_38") is False
        and source.get("zip_member_opened_by_l25_38") is False
    ...
    broad_allowed = bool(source_ok and allow_broad_checkpoint and token_valid)
    checks: list[dict[str, Any]] = [
        {"name": "source_l25_38_payload_ok", "ok": source_ok},
        {"name": "broad_checkpoint_token_present", "ok": token_present},
        {"name": "broad_checkpoint_token_valid", "ok": token_valid},
        {"name": "broad_checkpoint_explicitly_allowed", "ok": allow_broad_checkpoint is True},
        {"name": "manifest_validation_broad_checkpoint_allowed", "ok": broad_allowed},
    ]
    if broad_allowed:
        checks.extend([
            {"name": "manifest_member_name_stable", "ok": source.get("accepted_manifest_member_name") == EXPECTED_MANIFEST_MEMBER_NAME},
            {"name": "manifest_patch_name_stable", "ok": source.get("accepted_manifest_patch_name") == EXPECTED_MANIFEST_PATCH_NAME},
            ...
            {"name": "malformed_manifest_source_rejected", "ok": source.get("malformed_manifest_source_rejected") is True},
            {"name": "l25_38_source_payload_only_boundary", "ok": source.get("artifact_path_opened_by_l25_38") is False and source.get("artifact_bytes_read_by_l25_38") is False and source.get("zip_member_opened_by_l25_38") is False},
            {"name": "no_execution_or_extraction", "ok": source.get("package_execution_allowed") is False and source.get("package_run") is False and source.get("patchops_cli_run_package_invoked_for_real_artifact") is False and source.get("real_archive_candidate_extracted") is False},
        ])
    ...
    payload: dict[str, Any] = {
        ...
        "manifest_validation_broad_checkpoint_requested": bool(allow_broad_checkpoint or token_present),
        "manifest_validation_broad_checkpoint_token_present": token_present,
        "manifest_validation_broad_checkpoint_token_valid": token_valid,
        "manifest_validation_broad_checkpoint_allowed": broad_allowed,
        "source_l25_38_summary": source,
        "real_artifact_readback_ladder_summary": [
        ...
        "accepted_malformed_manifest_source_rejected": source.get("malformed_manifest_source_rejected") is True,
        ...
        "no_browser_permission_added_by_l25_39": True,
        "no_download_permission_added_by_l25_39": True,
        "no_artifact_or_zip_read_permission_added_by_l25_39": True,
        "no_archive_extraction_permission_added_by_l25_39": True,
        "no_package_execution_permission_added_by_l25_39": True,
        "no_pasteback_permission_added_by_l25_39": True,
        "no_git_permission_added_by_l25_39": True,
...
def render_text(payload: Mapping[str, Any]) -> str:
    return "\n".join([
        ...
        f"Broad Checkpoint              : {payload.get('manifest_validation_broad_checkpoint_allowed')}",
        ...
        f"Artifact Opened By L25.39     : {payload.get('artifact_path_opened_by_l25_39')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--allow-broad-checkpoint", action="store_true")
    parser.add_argument("--authorization-token", default=None)
    parser.add_argument("--json", action="store_true")
    parser.add_argument("--compact", action="store_true")
    args = parser.parse_args(argv)
    source_payload = json.loads(args.source_payload_json) if args.source_payload_json else None
    payload = build_real_download_artifact_manifest_validation_broad_checkpoint(
        args.repo_root,
        source_payload=source_payload,
        allow_broad_checkpoint=args.allow_broad_checkpoint,
