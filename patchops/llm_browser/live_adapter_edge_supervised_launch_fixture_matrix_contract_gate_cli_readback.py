"""Passive L5.17 Microsoft Edge fixture contract-gate CLI/readback.

This module exposes a dedicated readback/checkpoint around the accepted L5.16
Microsoft Edge supervised-launch fixture matrix contract gate. It remains fully
...
from . import live_adapter_edge_supervised_launch_fixture_matrix_contract_gate as contract_gate

PATCH = "L5.17"
PHASE = "L5"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L5.17 Microsoft Edge Supervised Launch Fixture Matrix Contract Gate CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-fixtures-contract-gate-readback"
SOURCE_COMMAND_NAME = contract_gate.COMMAND_NAME
NEXT_PATCH = "L5.18 Live adapter Microsoft Edge supervised launch L5 aggregate readiness gate"
SIDE_EFFECT_BOUNDARY = "edge-supervised-launch-fixture-matrix-contract-gate-cli-readback-only"
BROWSER_PRIORITY = ("edge", "opera")

REQUIRED_CASE_IDS: tuple[str, ...] = contract_gate.REQUIRED_CASE_IDS
...
def _gate_contract_summary(gate_payload: Mapping[str, Any]) -> dict[str, Any]:
    gate = gate_payload.get("edge_fixture_contract_gate", {}) if isinstance(gate_payload, Mapping) else {}
    ...
    return {
        ...
        "required_case_ids": list(gate.get("required_case_ids", [])),
        "actual_case_ids": list(gate.get("actual_case_ids", [])),
...
def build_edge_supervised_launch_fixture_matrix_contract_gate_cli_readback(repo_root: str | Path | None = None) -> dict[str, Any]:
    root = _repo_root(repo_root)
    before_modules = set(sys.modules)

    l5_16 = contract_gate.build_edge_supervised_launch_fixture_matrix_contract_gate(root)
    source_command_static = _command_static_presence(root, SOURCE_COMMAND_NAME, "PATCHOPS L5.16 START")
    readback_command_static = _command_static_presence(root, COMMAND_NAME, "PATCHOPS L5.17 START")
    missing_repo_paths = _missing_paths(root, REQUIRED_REPO_PATHS)
    gate_summary = _gate_contract_summary(l5_16)
    ...
    gate_still_blocks_side_effects = (
        l5_16.get("ok") is True
        and gate_summary["ok"] is True
        and gate_summary["status"] == STATUS_PASS
        and gate_summary["case_count"] == 6
        and gate_summary["required_case_ids"] == list(REQUIRED_CASE_IDS)
        and gate_summary["missing_case_ids"] == []
        and gate_summary["unexpected_case_ids"] == []
        and gate_summary["violations"] == []
        and gate_summary["edge_first_required"] is True
        and gate_summary["dedicated_profile_required"] is True
        and gate_summary["manual_user_login_required"] is True
        and gate_summary["silent_auto_submit_must_remain_false"] is True
        and gate_summary["no_extension_required"] is True
        and gate_summary["no_localhost_server_required"] is True
        and gate_summary["current_patch_must_remain_passive"] is True
    )

    command_plan = [
        "python -m compileall patchops/llm_browser tests scripts/patch_l5_17_wire_edge_fixture_matrix_contract_gate_readback.py",
        "python -m pytest -q tests/test_l5_16_edge_supervised_launch_fixture_matrix_contract_gate_current.py tests/test_l5_17_edge_supervised_launch_fixture_matrix_contract_gate_cli_readback_current.py",
        "python -m patchops.llm_browser.live_adapter_edge_supervised_launch_fixture_matrix_contract_gate_cli_readback --repo-root C:\\dev\\patchops --json --compact",
    ...
    checks = [
        _check("l5_16_edge_fixture_matrix_contract_gate_still_passes", l5_16.get("ok") is True and l5_16.get("status") == STATUS_PASS, {"patch": l5_16.get("patch")}),
        _check("l5_16_source_command_still_registered", source_command_static.get("ok") is True, source_command_static),
        _check("l5_17_edge_fixture_matrix_contract_gate_readback_command_registered", readback_command_static.get("ok") is True, readback_command_static),
        _check("edge_fixture_matrix_contract_gate_still_passes", gate_summary["ok"] is True and gate_summary["status"] == STATUS_PASS, gate_summary),
        _check("edge_fixture_matrix_contract_gate_still_blocks_live_side_effects", gate_still_blocks_side_effects, gate_summary),
        _check("edge_remains_first_supported_live_browser", BROWSER_PRIORITY[0] == "edge", {"browser_priority": list(BROWSER_PRIORITY)}),
        _check("l5_17_artifacts_present", not missing_repo_paths, {"missing": missing_repo_paths}),
    ...
    payload: dict[str, Any] = {
        ...
        "edge_fixture_contract_gate_summary": gate_summary,
...
def render_text(payload: Mapping[str, Any]) -> str:
    gate = payload.get("edge_fixture_contract_gate_summary", {})
    lines = [
        "PatchOps L5.17 Microsoft Edge supervised-launch fixture matrix contract gate CLI/readback",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_supervised_launch_fixture_matrix_contract_gate_cli_readback(args.repo_root)
    if args.json:
        if args.compact:
