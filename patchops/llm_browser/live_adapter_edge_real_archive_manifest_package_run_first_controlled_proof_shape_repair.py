"""L25.32a repaired controlled package-run proof.

This repairs the failed L25.32 bundle shape. The controlled synthetic package ZIP
now places the supported launcher and required bundle-review files under
`bundle/`: `bundle/run_with_patchops.ps1`, `bundle/bundle_meta.json`,
`bundle/README.txt`, `bundle/manifest.json`, and `bundle/content/.keep`.

The proof still invokes exactly one bounded PatchOps run-package command against
that controlled synthetic package only. It does not start a browser, inspect
pages, paste, send, use Selenium/CDP/DOM scraping, use a real downloaded
artifact, or run a package produced by ChatGPT.
"""
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L25.32a Microsoft Edge controlled runtime package-run bundle shape repair proof"
SOURCE_PATCH = "L25.31"
...
DEFAULT_CONTROLLED_CANDIDATE_RELATIVE_PATH = "data/runtime/browser_downloads/l25_32a_package_run/future_real_package_run_bundle.zip"
...
def _run_patchops_run_package(repo_root: Path, package_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "review_rejected": "review rejection" in stderr_lower or "missing_launcher" in stderr_lower or "bundle review issue" in stderr_lower,
        }
...
def _default_payload(root: Path, target_url: str) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "subprocess_patchops_run_package_allowed_by_l25_32a": True,
        "no_browser_permission_added_by_l25_32a": True,
        "no_pasteback_or_real_artifact_permission_added_by_l25_32a": True,
...
def build_archive_manifest_package_run_first_controlled_proof_shape_repair(
    ...
    *,
    ...
    checks: list[dict[str, Any]] = [
        ...
        {"name": "candidate_path_is_isolated_run_package_repair_fixture", "ok": "l25_32a_package_run" in candidate_text},
    ...
    payload: dict[str, Any] = {
        ...
        "bundle_review_expected_launcher_path": "bundle/run_with_patchops.ps1",
        "bundle_review_expected_metadata_path": "bundle/bundle_meta.json",
        "bundle_review_expected_readme_path": "bundle/README.txt",
        "bundle_review_expected_content_path": "bundle/content/.keep",
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"Bundle Shape Repaired         : {payload.get('bundle_shape_repaired')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_archive_manifest_package_run_first_controlled_proof_shape_repair(
        args.repo_root,
