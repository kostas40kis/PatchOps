"""Passive L4 final acceptance marker for browser-start dry-run handoff.

This module closes the L4 dry-run handoff stack by checking that the accepted
L4 broad-validation checkpoint remains green, its CLI/readback wrapper remains
present, and the passive safety boundary is still intact.

It does not import Selenium or optional browser dependencies, start a browser,
create a browser session, create profile directories, perform adapter
filesystem writes, click/download/paste/send, run downloaded packages, or
commit/push.
"""
...
from . import live_adapter_browser_start_dry_run_handoff_l4_broad_validation_checkpoint as broad_checkpoint
from . import commands as llm_browser_commands

NAME = "L4.12 Browser Start Dry-Run Handoff L4 Final Acceptance Marker"
PHASE = "L4"
PATCH = "L4.12"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NEXT_PATCH = "L5.1 Live adapter browser-start supervised launch handoff contract"
SIDE_EFFECT_BOUNDARY = "passive-l4-final-acceptance-marker-only"
...
REQUIRED_L4_FINAL_SOURCE_PATHS: tuple[str, ...] = (
...
REQUIRED_L4_FINAL_DOC_PATHS: tuple[str, ...] = (
    ...
    "docs/llm_browser_live_adapter_browser_start_dry_run_handoff_l4_broad_validation_cli_readback.md",
    "docs/llm_browser_live_adapter_browser_start_dry_run_handoff_l4_final_acceptance_marker.md",
)

REQUIRED_L4_FINAL_TEST_PATHS: tuple[str, ...] = (
    "tests/test_l4_10_browser_start_dry_run_handoff_l4_broad_validation_checkpoint_current.py",
    "tests/test_l4_11_browser_start_dry_run_handoff_l4_broad_validation_cli_readback_current.py",
    "tests/test_l4_12_browser_start_dry_run_handoff_l4_final_acceptance_marker_current.py",
)
...
def _safe_broad_payload(repo_root: Path) -> tuple[Mapping[str, Any], str | None]:
    try:
        payload = broad_checkpoint.build_l4_browser_start_dry_run_handoff_broad_validation_checkpoint(repo_root)
...
def build_l4_browser_start_dry_run_handoff_final_acceptance_marker(repo_root: str | Path | None = None) -> dict[str, Any]:
    ...
    missing_sources = _missing_paths(root, REQUIRED_L4_FINAL_SOURCE_PATHS)
    missing_docs = _missing_paths(root, REQUIRED_L4_FINAL_DOC_PATHS)
    missing_tests = _missing_paths(root, REQUIRED_L4_FINAL_TEST_PATHS)
    doc_text = _read_text(root, "docs/llm_browser_live_adapter_browser_start_dry_run_handoff_l4_final_acceptance_marker.md")
    ...
    checks: list[dict[str, Any]] = [
        _check("l4_broad_validation_checkpoint_still_passes", broad_ok, {"patch": broad_payload.get("patch"), "status": broad_payload.get("status"), "error": broad_error}),
        _check("l4_broad_validation_checkpoint_remains_passive", broad_no_effects, {"broad_no_effects": broad_no_effects}),
        _check("l4_cli_surfaces_remain_present", commands_present, {"missing_commands": missing_commands, "required_commands": list(REQUIRED_COMMANDS)}),
        _check("l4_final_acceptance_artifacts_present", missing_sources == [] and missing_docs == [] and missing_tests == [], {"missing_sources": missing_sources, "missing_docs": missing_docs, "missing_tests": missing_tests}),
        _check("l4_final_acceptance_doc_contains_passive_boundary", missing_doc_phrases == [], {"missing_phrases": missing_doc_phrases}),
        _check("l4_final_acceptance_command_plan_is_readback_only", command_plan_passive, {"forbidden_fragments": forbidden_command_fragments, "planned_commands": list(FINAL_ACCEPTANCE_COMMANDS)}),
        _check("l4_final_acceptance_does_not_execute_planned_commands", executed_validation_commands == [], {"executed_validation_commands": executed_validation_commands}),
        _check("l4_final_acceptance_no_new_optional_browser_dependency_imports", newly_loaded_forbidden == [], {"loaded_forbidden_modules": loaded_forbidden, "newly_loaded_forbidden_modules": newly_loaded_forbidden}),
        _check("l4_final_acceptance_selenium_not_imported", "selenium" not in loaded_forbidden and "selenium" not in newly_loaded_forbidden, {"loaded_forbidden_modules": loaded_forbidden}),
    ]
    ...
    checks.append(_check("l4_final_acceptance_payload_json_safe", _json_safe(preview), preview))
    ...
    return {
        ...
        "missing_commands": missing_commands,
        "required_source_paths": list(REQUIRED_L4_FINAL_SOURCE_PATHS),
        "required_doc_paths": list(REQUIRED_L4_FINAL_DOC_PATHS),
        "required_test_paths": list(REQUIRED_L4_FINAL_TEST_PATHS),
...
def build_final_acceptance_marker(repo_root: str | Path | None = None) -> dict[str, Any]:
    return build_l4_browser_start_dry_run_handoff_final_acceptance_marker(repo_root)


def render_text(payload: Mapping[str, Any]) -> str:
    checks = _as_list(payload.get("checks"))
    lines = [
        "PatchOps LLM browser browser-start dry-run handoff L4 final acceptance marker",
        "PatchOps LLM browser live adapter browser-start dry-run handoff L4 final acceptance marker",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Passive L4 final acceptance marker for browser-start dry-run handoff.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_l4_browser_start_dry_run_handoff_final_acceptance_marker(args.repo_root)
    if args.json:
        if args.compact:
