"""L21.2 passive CLI/readback checkpoint for downloaded-archive validation.

This checkpoint proves the accepted L21.1 default and authorized preflight
readbacks without starting Edge, inspecting pages, clicking, downloading,
stating or hashing files, opening/listing/extracting archives, reading manifests,
reading bytes, reading artifact content, pasting, sending, running packages,
starting localhost services, using browser extensions, committing, or pushing.
"""
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L21.2 Microsoft Edge downloaded-archive validation CLI/readback checkpoint"
COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-archive-validation-cli-readback-checkpoint"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-archive-validation-passive-preflight-gate"
SOURCE_PATCH = "L21.1"
NEXT_PATCH = "L21.3 Microsoft Edge downloaded-archive validation passive plan checkpoint"
...
def _missing_doc_phrases(root: Path) -> list[str]:
    text = _read_text(root / "docs/llm_browser_live_adapter_edge_downloaded_archive_validation_cli_readback_checkpoint.md")
...
def _compact_source_summary(payload: Mapping[str, Any]) -> dict[str, Any]:
    readback = payload.get("archive_validation_preflight_readback") or {}
    return {
        "ok": payload.get("ok"),
        "patch": payload.get("patch"),
        "source_l20_7_final_marker_accepted": payload.get("source_l20_7_final_marker_accepted"),
        ...
        "archive_validation_preflight_requested": payload.get("archive_validation_preflight_requested"),
        "archive_validation_preflight_authorized": payload.get("archive_validation_preflight_authorized"),
        "archive_validation_preflight_authorization_token_present": readback.get("archive_validation_preflight_authorization_token_present"),
        "archive_validation_preflight_authorization_is_readback_only_in_l21_1": payload.get("archive_validation_preflight_authorization_is_readback_only_in_l21_1"),
...
def build_edge_downloaded_archive_validation_cli_readback_checkpoint(
    repo_root: str | Path | None = None,
    *,
    target_url: str | None = None,
) -> dict[str, Any]:
    """Build the passive L21.2 CLI/readback checkpoint payload."""
    root = _repo_root(repo_root)
    before_modules = set(sys.modules)
    target = target_url or DEFAULT_TARGET_URL

    default_payload = l21_01.build_edge_downloaded_archive_validation_passive_preflight_gate(
        root,
        allow_archive_validation_preflight=False,
        authorization_token=None,
        target_url=target,
    )
    authorized_payload = l21_01.build_edge_downloaded_archive_validation_passive_preflight_gate(
        root,
        allow_archive_validation_preflight=True,
        authorization_token=REQUIRED_ARCHIVE_PREFLIGHT_AUTHORIZATION_TOKEN,
        target_url=target,
    )
    ...
    default_passive = _archive_payload_is_passive(default_payload)
    authorized_passive = _archive_payload_is_passive(authorized_payload)
    default_ok = (
        default_payload.get("ok") is True
        and default_payload.get("patch") == SOURCE_PATCH
        and default_payload.get("l21_1_complete") is True
        and default_payload.get("source_l20_7_final_marker_accepted") is True
        and default_payload.get("l20_downloaded_file_filesystem_validation_stream_complete") is True
        and default_payload.get("archive_validation_preflight_authorized") is False
        and default_payload.get("archive_validation_preflight_authorization_is_readback_only_in_l21_1") is True
        and default_payload.get("archive_validation_execution_allowed") is False
        and default_payload.get("archive_validation_active") is False
        ...
        and default_payload.get("downloaded_archive_extracted") is False
        and default_payload.get("downloaded_manifest_read") is False
        and default_payload.get("downloaded_file_bytes_read") is False
        and default_payload.get("downloaded_file_stat_performed") is False
        and default_payload.get("downloaded_file_hash_performed") is False
        and default_payload.get("browser_started") is False
        and default_payload.get("edge_process_started") is False
        and default_payload.get("package_run_performed_by_adapter") is False
        and default_passive
    )
    authorized_ok = (
        authorized_payload.get("ok") is True
        and authorized_payload.get("patch") == SOURCE_PATCH
        and authorized_payload.get("l21_1_complete") is True
        and authorized_payload.get("source_l20_7_final_marker_accepted") is True
        and authorized_payload.get("l20_downloaded_file_filesystem_validation_stream_complete") is True
        and authorized_payload.get("archive_validation_preflight_authorized") is True
        and authorized_payload.get("archive_validation_preflight_authorization_is_readback_only_in_l21_1") is True
        and authorized_payload.get("archive_validation_execution_allowed") is False
        and authorized_payload.get("archive_validation_active") is False
        and authorized_payload.get("archive_validation_performed") is False
    ...
    checks = [
        _check("l21_1_default_archive_preflight_readback_ok", default_ok),
        _check("l21_1_authorized_archive_preflight_readback_ok", authorized_ok),
        _check("default_archive_preflight_readback_remains_passive", default_passive, _archive_false_summary(default_payload)),
        _check("authorized_archive_preflight_readback_remains_passive", authorized_passive, _archive_false_summary(authorized_payload)),
        _check("archive_validation_execution_still_blocked", True),
        ...
        _check("avoid_nested_cli_validation_cascades", True),
        ...
        _check("docs_contain_l21_2_safety_contract", not missing_doc_phrases, {"missing_phrases": missing_doc_phrases}),
    ...
    return {
        ...
        "downloaded_archive_validation_cli_readback_checkpoint": True,
        "passive_cli_readback_checkpoint": True,
        "microsoft_edge_first": True,
        "opera_second": True,
        "opera_active_implementation_target": False,
        "l21_1_passive_preflight_gate_accepted": default_ok and authorized_ok,
        "l21_1_default_archive_preflight_readback_ok": default_ok,
        "l21_1_authorized_archive_preflight_readback_ok": authorized_ok,
        "default_archive_validation_preflight_authorized": default_payload.get("archive_validation_preflight_authorized"),
        "authorized_archive_validation_preflight_authorized": authorized_payload.get("archive_validation_preflight_authorized"),
        ...
        "l21_2_complete": ok,
        "remaining_l21_2_patches": [] if ok else [PATCH],
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L21.1 Default Readback        : {payload.get('l21_1_default_archive_preflight_readback_ok')}",
        f"L21.1 Authorized Readback     : {payload.get('l21_1_authorized_archive_preflight_readback_ok')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_downloaded_archive_validation_cli_readback_checkpoint(args.repo_root, target_url=args.target_url)
    if args.json:
