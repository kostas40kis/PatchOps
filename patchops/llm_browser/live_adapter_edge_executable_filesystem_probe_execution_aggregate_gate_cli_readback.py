"""L13.6 passive CLI/readback over the Microsoft Edge executable filesystem probe execution aggregate gate.

This module wraps the accepted L13.5 aggregate gate with a compact readback
surface. It keeps the gate passive and truthful: the read-only filesystem probe
may run only after the accepted L12 execution-preflight gates are ready, and the
selected path, if any, must be an existing reported Microsoft Edge executable
candidate. It does not launch Edge, import Selenium, create sessions or drivers,
create profiles, click/download/paste/send, run packages, commit, or push.
"""
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_aggregate_gate as l13_05

PATCH = "L13.6"
PHASE = "L13"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L13.6 Microsoft Edge executable filesystem probe execution aggregate gate CLI/readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-aggregate-gate-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-aggregate-gate"
NEXT_PATCH = "L13.7 Live adapter Microsoft Edge executable filesystem probe execution broad validation checkpoint"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-execution-aggregate-gate-cli-readback-only"
...
REQUIRED_REPO_PATHS = (
    ...
    "docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_aggregate_gate.md",
...
def _source_l13_05_safe(source: Mapping[str, Any]) -> bool:
    return (
        source.get("ok") is True
        and source.get("status") == STATUS_PASS
        and source.get("patch") == "L13.5"
        and source.get("source_patch") == "L13.4"
        and source.get("command_name") == SOURCE_COMMAND_NAME
        and source.get("source_l13_04_fixture_matrix_readback_remains_accepted") is True
        and source.get("execution_aggregate_gate_enforced") is True
...
def build_edge_executable_filesystem_probe_execution_aggregate_gate_readback(
    ...
    *,
    ...
    source = l13_05.build_edge_executable_filesystem_probe_execution_aggregate_gate(
        root,
    ...
    source_safe = _source_l13_05_safe(payload)
    ...
    checks = [
        _check("source_l13_05_aggregate_gate_remains_accepted", source_safe),
        _check("execution_aggregate_gate_cli_readback_enforced", True),
        _check("execution_aggregate_gate_enforced", payload.get("execution_aggregate_gate_enforced") is True),
        ...
        _check("required_l13_aggregate_readback_paths_exist", bool(required_paths["ok"]), {"missing": required_paths["missing"]}),
    ...
    payload.update(
        {
            ...
            "source_l13_05_aggregate_gate_remains_accepted": source_safe,
            "l13_05_aggregate_gate_remains_accepted": source_safe,
            "l13_04_fixture_matrix_readback_remains_accepted": source.get("l13_04_fixture_matrix_readback_remains_accepted") is True or source.get("source_l13_04_fixture_matrix_readback_remains_accepted") is True,
            "l13_03_execution_fixture_matrix_remains_accepted": source.get("l13_03_execution_fixture_matrix_remains_accepted") is True,
            "execution_aggregate_gate_cli_readback_enforced": True,
            ...
            "no_executable_launch_attempted_by_aggregate_readback": no_launch,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_execution_aggregate_gate_readback(
        args.repo_root,
