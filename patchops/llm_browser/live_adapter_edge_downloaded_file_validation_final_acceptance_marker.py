"""L19.7 final acceptance marker for downloaded-file validation.

This marker closes the L19 Microsoft Edge downloaded-file validation stream after
the accepted L19.6 broad checkpoint. Completion means metadata/readback-only
validation readiness is accepted. It does not mean PatchOps can check real files,
stat/hash files, open/list/extract archives, read manifests, read bytes, run a
package, paste back, or send/submit from a browser.
"""
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L19.7 Microsoft Edge downloaded-file validation final acceptance marker"
COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-validation-final-acceptance-marker"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-validation-broad-checkpoint"
SOURCE_PATCH = "L19.6"
NEXT_PATCH = "L20.1 Microsoft Edge downloaded-file filesystem validation passive preflight gate"
...
SOURCE_L19_6_PASSIVE_FALSE_FIELDS = tuple(
...
def _missing_doc_phrases(root: Path) -> list[str]:
    text = _read_text(root / "docs/llm_browser_live_adapter_edge_downloaded_file_validation_final_acceptance_marker.md")
...
def _source_passive_false_summary(payload: Mapping[str, Any]) -> dict[str, bool]:
    return {field: payload.get(field) is False for field in SOURCE_L19_6_PASSIVE_FALSE_FIELDS}
...
def _source_l19_6_ok(root: Path, target_url: str) -> tuple[bool, dict[str, Any]]:
    source = l19_06.build_edge_downloaded_file_validation_broad_checkpoint(root, target_url=target_url)
    ok = (
        source.get("ok") is True
        and source.get("patch") == SOURCE_PATCH
        and source.get("l19_6_complete") is True
        and source.get("l19_1_through_l19_5_remain_accepted") is True
        and source.get("metadata_only_downloaded_file_validation_proof_remains_accepted") is True
        and source.get("default_metadata_proof_readback_remains_passive") is True
        and source.get("authorized_positive_metadata_proof_readback_remains_passive") is True
        and source.get("negative_metadata_fixture_remains_rejected_without_side_effects") is True
        and source.get("forbidden_metadata_keys_remain_rejected_without_side_effects") is True
        and source.get("downloaded_file_metadata_validation_ready_from_positive_metadata") is True
...
def build_edge_downloaded_file_validation_final_acceptance_marker(
    repo_root: str | Path | None = None,
    *,
    target_url: str | None = None,
) -> dict[str, Any]:
    """Build the L19.7 final acceptance marker payload."""
    root = _repo_root(repo_root)
    before_modules = set(sys.modules)
    target = target_url or DEFAULT_TARGET_URL
    source_ok, source = _source_l19_6_ok(root, target)
    ...
    checks = [
        _check("source_l19_6_broad_checkpoint_accepted", source_ok),
        _check("l19_1_through_l19_6_remain_accepted", source_ok),
        _check("l19_downloaded_file_validation_stream_complete_metadata_readback_only", source_ok),
        _check("final_marker_has_no_browser_filesystem_archive_manifest_byte_read_pasteback_or_package_side_effects", final_safety_ok),
        _check("real_filesystem_validation_remains_separate_next_stream", True),
        _check("archive_validation_remains_separate_future_stream", True),
        _check("pasteback_and_package_run_remain_separate_future_streams", True),
        ...
        _check("docs_contain_l19_7_safety_contract", not missing_doc_phrases, {"missing_phrases": missing_doc_phrases}),
    ...
    return {
        ...
        "downloaded_file_validation_final_acceptance_marker": True,
        "microsoft_edge_first": True,
        "opera_second": True,
        "opera_active_implementation_target": False,
        "source_l19_6_broad_checkpoint_accepted": source_ok,
        "l19_1_through_l19_6_remain_accepted": source_ok,
        "l19_downloaded_file_validation_stream_complete": ok,
        "downloaded_file_validation_stream_completion_is_metadata_readback_only": True,
        "metadata_only_downloaded_file_validation_proof_accepted": bool(source.get("metadata_only_downloaded_file_validation_proof_remains_accepted")),
        "downloaded_file_metadata_validation_ready_from_positive_metadata": bool(source.get("downloaded_file_metadata_validation_ready_from_positive_metadata")),
        ...
        "source_l19_6_summary": {
            "ok": source.get("ok"),
            "patch": source.get("patch"),
            "l19_6_complete": source.get("l19_6_complete"),
            "l19_1_through_l19_5_remain_accepted": source.get("l19_1_through_l19_5_remain_accepted"),
            "metadata_only_downloaded_file_validation_proof_remains_accepted": source.get("metadata_only_downloaded_file_validation_proof_remains_accepted"),
            "default_metadata_proof_readback_remains_passive": source.get("default_metadata_proof_readback_remains_passive"),
            "authorized_positive_metadata_proof_readback_remains_passive": source.get("authorized_positive_metadata_proof_readback_remains_passive"),
            "negative_metadata_fixture_remains_rejected_without_side_effects": source.get("negative_metadata_fixture_remains_rejected_without_side_effects"),
            "forbidden_metadata_keys_remain_rejected_without_side_effects": source.get("forbidden_metadata_keys_remain_rejected_without_side_effects"),
            "downloaded_file_metadata_validation_ready_from_positive_metadata": source.get("downloaded_file_metadata_validation_ready_from_positive_metadata"),
        ...
        "l19_7_complete": ok,
        "remaining_l19_patches": [] if ok else [PATCH],
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L19.1-L19.6 Accepted               : {payload.get('l19_1_through_l19_6_remain_accepted')}",
        f"L19 Stream Complete                : {payload.get('l19_downloaded_file_validation_stream_complete')}",
        f"Completion Metadata/Readback Only  : {payload.get('downloaded_file_validation_stream_completion_is_metadata_readback_only')}",
        f"Filesystem Validation Active       : {payload.get('real_filesystem_validation_active')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_downloaded_file_validation_final_acceptance_marker(args.repo_root, target_url=args.target_url)
    if args.json:
