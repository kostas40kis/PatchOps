"""L14.7 passive CLI/readback over the Microsoft Edge dedicated profile lifecycle aggregate gate.

This layer wraps the accepted L14.6 dedicated profile lifecycle aggregate gate
with a compact CLI/readback surface. It confirms that the aggregate profile
lifecycle state remains truthful, rooted under the allowed runtime profile area,
and passive. It still does not create, delete, clean, or mutate profile
directories, launch Edge, import Selenium, create sessions, drivers, downloads,
paste/send actions, package runs, commits, pushes, localhost services, or browser
extensions.
"""
...
from patchops.llm_browser import live_adapter_edge_dedicated_profile_lifecycle_aggregate_gate as l14_06

PATCH = "L14.7"
PHASE = "L14"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L14.7 Microsoft Edge dedicated profile lifecycle aggregate gate CLI/readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-dedicated-profile-lifecycle-aggregate-gate-cli-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-dedicated-profile-lifecycle-aggregate-gate"
NEXT_PATCH = "L14.8 Microsoft Edge dedicated profile lifecycle broad validation checkpoint, still no launch"
...
def _source_l14_06_safe(source: Mapping[str, Any]) -> bool:
    return (
        source.get("ok") is True
        and source.get("status") == STATUS_PASS
        and source.get("patch") == "L14.6"
...
def build_edge_dedicated_profile_lifecycle_aggregate_gate_cli_readback(
    repo_root: str | Path | None = None,
    *,
    profile_relative_path: str | None = None,
) -> dict[str, Any]:
    root = Path.cwd().resolve() if repo_root is None else Path(repo_root).resolve()
    source = l14_06.build_edge_dedicated_profile_lifecycle_aggregate_gate(root, profile_relative_path=profile_relative_path)
    required = _required_paths_status(root)
    source_safe = _source_l14_06_safe(source)
    aggregate_readback_truthful = (
        source.get("profile_aggregate_truthful") is True
        and source.get("profile_readback_truthful") is True
        and source.get("profile_candidate_under_allowed_runtime_root") is True
        and isinstance(source.get("profile_candidate_path"), str)
    )
    aggregate_readback_passive = (
        source.get("profile_aggregate_gate_is_passive") is True
        and source.get("profile_directory_created") is False
        and source.get("profile_directory_mutated") is False
        and source.get("profile_directory_create_allowed") is False
        and source.get("profile_directory_cleanup_allowed") is False
    ...
    checks = [
        _check("l14_06_profile_lifecycle_aggregate_gate_remains_accepted", source_safe),
        _check("dedicated_profile_lifecycle_aggregate_gate_cli_readback_enforced", True),
        _check("profile_aggregate_cli_readback_truthful", aggregate_readback_truthful),
        _check("profile_aggregate_cli_readback_is_passive", aggregate_readback_passive),
        ...
        _check("required_l14_profile_aggregate_readback_paths_exist", bool(required["ok"]), {"missing": required["missing"]}),
    ...
    return {
        ...
        "dedicated_profile_lifecycle_aggregate_gate_cli_readback_enforced": True,
        ...
        "profile_aggregate_cli_readback_truthful": aggregate_readback_truthful,
        "profile_aggregate_cli_readback_is_passive": aggregate_readback_passive,
        ...
        "source_l14_06_status": {
            "ok": source.get("ok"),
            "status": source.get("status"),
            "patch": source.get("patch"),
            "profile_aggregate_truthful": source.get("profile_aggregate_truthful"),
            "profile_aggregate_passive": source.get("profile_aggregate_gate_is_passive"),
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"CLI Readback Truthful : {payload.get('profile_aggregate_cli_readback_truthful')}",
        f"CLI Readback Passive  : {payload.get('profile_aggregate_cli_readback_is_passive')}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = build_edge_dedicated_profile_lifecycle_aggregate_gate_cli_readback(args.repo_root, profile_relative_path=args.profile_relative_path)
    if args.json:
