"""L20.2 passive CLI/readback checkpoint for filesystem validation.

This checkpoint proves the accepted L20.1 default and authorized compact
readbacks without starting Edge, inspecting pages, clicking, downloading,
checking real files, stating or hashing files, opening/listing/extracting
archives, reading manifests, reading bytes, pasting, sending, running packages,
...
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L20.2 Microsoft Edge downloaded-file filesystem validation CLI/readback checkpoint"
COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-filesystem-validation-cli-readback-checkpoint"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-downloaded-file-filesystem-validation-passive-preflight-gate"
SOURCE_PATCH = "L20.1"
NEXT_PATCH = "L20.3 Microsoft Edge downloaded-file filesystem validation passive plan checkpoint"
...
def _missing_doc_phrases(root: Path) -> list[str]:
    text = _read_text(root / "docs/llm_browser_live_adapter_edge_downloaded_file_filesystem_validation_cli_readback_checkpoint.md")
...
def _compact_source_summary(payload: Mapping[str, Any]) -> dict[str, Any]:
    readback = payload.get("filesystem_validation_preflight_readback") or {}
    return {
        "ok": payload.get("ok"),
        "patch": payload.get("patch"),
        "l20_1_complete": payload.get("l20_1_complete"),
        "source_l19_7_final_acceptance_marker_accepted": payload.get("source_l19_7_final_acceptance_marker_accepted"),
        "filesystem_validation_preflight_requested": payload.get("filesystem_validation_preflight_requested"),
        "filesystem_validation_preflight_authorized": payload.get("filesystem_validation_preflight_authorized"),
        "filesystem_validation_preflight_authorization_token_present": readback.get("filesystem_validation_preflight_authorization_token_present"),
        "filesystem_validation_preflight_authorization_is_readback_only_in_l20_1": payload.get("filesystem_validation_preflight_authorization_is_readback_only_in_l20_1"),
...
def build_edge_downloaded_file_filesystem_validation_cli_readback_checkpoint(
    repo_root: str | Path | None = None,
    *,
    target_url: str | None = None,
) -> dict[str, Any]:
    """Build the passive L20.2 CLI/readback checkpoint payload."""
    ...
    default_payload = l20_01.build_edge_downloaded_file_filesystem_validation_passive_preflight_gate(
        root,
        allow_filesystem_validation_preflight=False,
    ...
    authorized_payload = l20_01.build_edge_downloaded_file_filesystem_validation_passive_preflight_gate(
        ...
        authorization_token=REQUIRED_FILESYSTEM_VALIDATION_PREFLIGHT_AUTHORIZATION_TOKEN,
        target_url=target,
    )
    ...
    default_ok = (
        ...
        and default_payload.get("l20_1_complete") is True
        and default_payload.get("source_l19_7_final_acceptance_marker_accepted") is True
        and default_payload.get("l19_downloaded_file_validation_stream_complete") is True
        and default_payload.get("filesystem_validation_preflight_authorized") is False
        and default_payload.get("filesystem_validation_preflight_authorization_is_readback_only_in_l20_1") is True
        and default_payload.get("filesystem_validation_execution_allowed") is False
        and default_payload.get("filesystem_validation_active") is False
        and default_payload.get("filesystem_validation_performed") is False
        and default_payload.get("real_filesystem_validation_active") is False
        and default_payload.get("real_file_exists_check_performed") is False
        and default_payload.get("real_file_stat_performed") is False
        and default_payload.get("real_file_hash_performed") is False
    ...
    authorized_ok = (
        ...
        and authorized_payload.get("l20_1_complete") is True
        and authorized_payload.get("source_l19_7_final_acceptance_marker_accepted") is True
        and authorized_payload.get("l19_downloaded_file_validation_stream_complete") is True
    ...
    checks = [
        _check("l20_1_default_compact_readback_ok", default_ok),
        _check("l20_1_authorized_compact_readback_ok", authorized_ok),
        ...
        _check("real_file_existence_stat_hash_byte_archive_manifest_package_run_still_blocked", True),
        ...
        _check("docs_contain_l20_2_safety_contract", not missing_doc_phrases, {"missing_phrases": missing_doc_phrases}),
    ...
    return {
        ...
        "filesystem_validation_cli_readback_checkpoint": True,
        "microsoft_edge_first": True,
        "opera_second": True,
        "opera_active_implementation_target": False,
        "l20_1_passive_preflight_gate_accepted": default_ok and authorized_ok,
        "l20_1_default_compact_readback_ok": default_ok,
        "l20_1_authorized_compact_readback_ok": authorized_ok,
        ...
        "l20_2_complete": ok,
        "remaining_l20_2_patches": [] if ok else [PATCH],
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L20.1 Default Readback        : {payload.get('l20_1_default_compact_readback_ok')}",
        f"L20.1 Authorized Readback     : {payload.get('l20_1_authorized_compact_readback_ok')}",
        f"Filesystem Validation Allowed : {payload.get('filesystem_validation_execution_allowed')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_downloaded_file_filesystem_validation_cli_readback_checkpoint(args.repo_root, target_url=args.target_url)
    if args.json:
