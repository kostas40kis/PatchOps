"""L13.5 passive aggregate gate for Microsoft Edge executable filesystem probe execution.

The aggregate gate sits over the accepted L13.4 fixture-matrix CLI/readback
surface. It proves the accepted execution contract remains passive and truthful:
filesystem probing can happen only after all L12 execution-preflight gates are
ready, selected executable paths are existing reported candidates, and no
browser/Selenium/profile/download/paste/send/package/git side effects occur.
"""
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_fixture_matrix_cli_readback as l13_04

PATCH = "L13.5"
PHASE = "L13"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L13.5 Microsoft Edge executable filesystem probe execution aggregate gate"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-aggregate-gate"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-fixture-matrix-readback"
NEXT_PATCH = "L13.6 Live adapter Microsoft Edge executable filesystem probe execution aggregate gate CLI/readback"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-execution-aggregate-gate-only"
...
REQUIRED_REPO_PATHS = (
    ...
    "docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_fixture_matrix_cli_readback.md",
...
def _source_l13_04_safe(source: Mapping[str, Any]) -> bool:
    return (
        source.get("ok") is True
        and source.get("status") == STATUS_PASS
        and source.get("patch") == "L13.4"
        and source.get("source_patch") == "L13.3"
        and source.get("command_name") == SOURCE_COMMAND_NAME
        and source.get("l13_03_execution_fixture_matrix_remains_accepted") is True
        and source.get("execution_fixture_matrix_cli_readback_enforced") is True
        and _selected_candidate_ok(source)
        and _no_launch_side_effects(source)
    )


def build_edge_executable_filesystem_probe_execution_aggregate_gate(
    ...
    *,
    ...
    source = l13_04.build_edge_executable_filesystem_probe_execution_fixture_matrix_readback(
        root,
    ...
    source_safe = _source_l13_04_safe(payload)
    ...
    checks = [
        _check("source_l13_04_fixture_matrix_readback_remains_accepted", source_safe),
        _check("execution_aggregate_gate_enforced", True),
        _check("execution_fixture_matrix_cli_readback_enforced", payload.get("execution_fixture_matrix_cli_readback_enforced") is True),
        ...
        _check("required_l13_aggregate_paths_exist", bool(required_paths["ok"]), {"missing": required_paths["missing"]}),
    ...
    payload.update(
        {
            ...
            "source_l13_04_fixture_matrix_readback_remains_accepted": source_safe,
            "l13_04_fixture_matrix_readback_remains_accepted": source_safe,
            "l13_03_execution_fixture_matrix_remains_accepted": source.get("l13_03_execution_fixture_matrix_remains_accepted") is True,
            "execution_aggregate_gate_enforced": True,
            ...
            "no_executable_launch_attempted_by_aggregate_gate": no_launch,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_execution_aggregate_gate(
        args.repo_root,
