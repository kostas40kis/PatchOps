...
SENTINEL_START = "# PATCHOPS L5.23 START"
...
def main() -> int:
    ...
    if SENTINEL_START in text:
        print("L5.23 Edge live-start preflight contract command wrapper already present")
        return 0
    if not text.endswith("\n"):
        text += "\n"
    COMMANDS_PATH.write_text(text + "\n" + PATCH_BLOCK.strip() + "\n", encoding="utf-8")
    print("L5.23 Edge live-start preflight contract command wrapper appended to patchops/llm_browser/commands.py")
    return 0
