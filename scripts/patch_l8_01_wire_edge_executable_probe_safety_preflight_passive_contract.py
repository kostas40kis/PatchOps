...
MODULE_CONTENT = r'''"""L8.1 passive Microsoft Edge executable-probe safety preflight contract.
...
DOC_CONTENT = r'''# L8.1 Microsoft Edge executable probe safety preflight passive contract
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-contract"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_01_candidate"
...
def main() -> int:
    no_auth = contract.build_edge_executable_probe_safety_preflight_passive_contract(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    ...
    for name, live, profile, probe_auth in cases:
        ...
        print(
            "PASS {0}: patch={1} preflight_passed={2} probe_execution_allowed={3} exe_probe={4}".format(
                name,
                payload["patch"],
    ...
    assert payload["ok"] is True
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; safety preflight stayed passive")
    print("PASS L8.1 brief validation: safety preflight contract accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L8.1 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L8.1 command wrapper appended")
    else:
        print("L8.1 command wrapper already present")


def _append_l7_08_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_authorization_final_acceptance_marker.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L8.1 Live adapter Microsoft Edge executable probe safety preflight passive contract"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe authorization slice:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L7.8 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l7_08_doc_pointer()
    print("L8.1 files written; validation is intentionally brief and compact")
    return 0
