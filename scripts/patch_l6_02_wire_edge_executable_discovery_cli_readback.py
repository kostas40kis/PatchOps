...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_discovery_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_discovery_cli_readback.md")
TEST_PATH = Path("tests/test_l6_02_edge_executable_discovery_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l6_02_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L6.2 passive Microsoft Edge executable-discovery CLI/readback.

This module wraps the accepted L6.1 executable-discovery passive contract in a
CLI/readback checkpoint. It remains passive: no filesystem probing for msedge,
no executable selection, no Selenium import, no browser start, no Edge process,
no driver/session creation, no profile creation, no click/download, no
...
from patchops.llm_browser import live_adapter_edge_executable_discovery_passive_contract as l6_01

PATCH = "L6.2"
PHASE = "L6"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L6.2 Microsoft Edge Executable Discovery CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-discovery-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-discovery-contract"
NEXT_PATCH = "L6.3 Live adapter Microsoft Edge executable discovery fixture matrix"
SIDE_EFFECT_BOUNDARY = "edge-executable-discovery-cli-readback-only"
...
def _source_l6_01_safe(summary: Mapping[str, Any]) -> bool:
    return (
        summary.get("patch") == "L6.1"
        and summary.get("ok") is True
        and summary.get("status") == STATUS_PASS
        and summary.get("command_name") == SOURCE_COMMAND_NAME
        and summary.get("l5_33_profile_parent_preflight_final_marker_remains_accepted") is True
        and summary.get("edge_executable_discovery_contract_enforced") is True
...
def build_edge_executable_discovery_cli_readback(
    ...
    *,
    ...
    source = l6_01.build_edge_executable_discovery_passive_contract(
        root,
    ...
    checks = [
        _check("l6_01_executable_discovery_contract_still_passes", _source_l6_01_safe(source), {"patch": source.get("patch"), "ok": source.get("ok"), "status": source.get("status")}),
        _check("l6_01_executable_discovery_contract_remains_passive", _source_l6_01_safe(source), {"exe_probe": source.get("edge_executable_filesystem_probe_performed"), "browser_started": source.get("browser_started")}),
        _check("l6_02_executable_discovery_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l6_01_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l6_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l6_docs_contain_executable_discovery_readback_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("edge_executable_candidates_remain_modeled", source.get("edge_executable_candidate_paths_modeled") is True and source.get("edge_executable_candidate_count", 0) >= 3, {"candidate_count": source.get("edge_executable_candidate_count")}),
        ...
        _check("edge_executable_launch_stays_unattempted", source.get("edge_executable_launch_attempted") is False and source.get("edge_process_started") is False, {}),
        ...
        _check("no_browser_profile_or_adapter_side_effects", _source_l6_01_safe(source), {}),
    ]
    ...
    return {
        ...
        "l6_01_executable_discovery_passive_contract_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l6_01_summary": source,
        "source_l5_33_summary": source.get("source_l5_33_summary", {}),
        "edge_executable_discovery_readback_enforced": True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_discovery_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L6.2 Microsoft Edge executable discovery CLI/readback
...
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-discovery-contract"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l6_02_candidate"
...
def test_l6_02_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l6_02_edge_executable_cli_readback_dedicated_profile_passes_passively() -> None:
    ...
    assert payload["edge_executable_discovery_readback_enforced"] is True
    ...
    assert payload["next_patch"] == "L6.3 Live adapter Microsoft Edge executable discovery fixture matrix"
    _assert_passive(payload)


def test_l6_02_default_and_missing_profile_cases_are_passive() -> None:
    default_payload = readback.build_edge_executable_discovery_cli_readback(
        PROJECT_ROOT,
        allow_live_start=True,
        profile_dir=DEFAULT_PROFILE,
    )
    missing_payload = readback.build_edge_executable_discovery_cli_readback(
        PROJECT_ROOT,
        allow_live_start=True,
        profile_dir=None,
    )
    assert default_payload["ok"] is True
    assert default_payload["source_l6_01_summary"]["source_l5_33_summary"]["source_l5_32_summary"]["source_l5_31_summary"]["source_l5_30_summary"]["default_profile_candidate_detected"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l6_01_summary"]["source_l5_33_summary"]["source_l5_32_summary"]["source_l5_31_summary"]["source_l5_30_summary"]["profile_parent_path_derived"] is False
    _assert_passive(missing_payload)


def test_l6_02_patchops_cli_json_readback_is_parseable_and_passive() -> None:
...
def test_l6_02_doc_mentions_executable_discovery_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_discovery_cli_readback.md"
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L6.2 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L6.2 command wrapper appended")
    else:
        print("L6.2 command wrapper already present")


def _append_l6_01_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_discovery_passive_contract.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L6.2 Live adapter Microsoft Edge executable discovery CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable discovery passive contract:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L6.1 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l6_01_doc_pointer()
    print("L6.2 files written; validation is intentionally brief")
    return 0
