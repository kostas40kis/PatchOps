...
from patchops.llm_browser import live_adapter_edge_real_download_artifact_manifest_validation_checkpoint as checkpoint
...
def _source_l25_37_payload() -> dict[str, Any]:
    return {
        "ok": True,
        "patch": "L25.37",
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    _assert(payload.get("patch") == "L25.38", "wrong patch marker", payload)
    _assert(payload.get("manifest_validation_checkpoint") is True, "checkpoint marker missing", payload)
    _assert(payload.get("manifest_validation_from_source_payload_only") is True, "source-payload-only marker missing", payload)
    _assert(payload.get("manifest_validation_allowed") is True, "manifest validation not allowed", payload)
    ...
    _assert(payload.get("artifact_path_opened_by_l25_38") is False, "L25.38 must not open artifact path", payload)
    _assert(payload.get("artifact_bytes_read_by_l25_38") is False, "L25.38 must not read artifact bytes", payload)
    _assert(payload.get("zip_member_opened_by_l25_38") is False, "L25.38 must not open zip members", payload)
...
def build_validation_payload(repo_root: str | Path = REPO_ROOT) -> dict[str, Any]:
    ...
    source = _source_l25_37_payload()

    default_payload = checkpoint.build_real_download_artifact_manifest_validation_checkpoint(root, source_payload=source)
    _assert(default_payload.get("ok") is False, "default should not pass without token", default_payload)
    _assert(default_payload.get("manifest_validation_allowed") is False, "default must not validate", default_payload)

    authorized_payload = checkpoint.build_real_download_artifact_manifest_validation_checkpoint(
        root,
        source_payload=source,
        allow_manifest_validation=True,
        authorization_token=checkpoint.REQUIRED_MANIFEST_VALIDATION_TOKEN,
    )
    _validate_payload(authorized_payload)

    malformed_source = dict(source)
    malformed_source["manifest_files_to_write_count"] = 1
    malformed_payload = checkpoint.build_real_download_artifact_manifest_validation_checkpoint(
        root,
        source_payload=malformed_source,
        allow_manifest_validation=True,
        authorization_token=checkpoint.REQUIRED_MANIFEST_VALIDATION_TOKEN,
    )
    _assert(malformed_payload.get("ok") is False, "malformed manifest source must fail", malformed_payload)

    module_path = root / "patchops" / "llm_browser" / "live_adapter_edge_real_download_artifact_manifest_validation_checkpoint.py"
    doc_path = root / "docs" / "llm_browser_live_adapter_edge_real_download_artifact_manifest_validation_checkpoint.md"
    module_text = module_path.read_text(encoding="utf-8")
    doc_text = doc_path.read_text(encoding="utf-8")
    _assert(not _forbidden_import_hits(module_text), "forbidden imports in L25.38 module", authorized_payload)
    _assert(not _forbidden_call_hits(module_text), "forbidden artifact/zip/execution calls in L25.38 module", authorized_payload)
    ...
    return {
        "ok": True,
        "patch": "L25.38",
        "manifest_validation_checkpoint": authorized_payload.get("manifest_validation_checkpoint"),
        "manifest_validation_from_source_payload_only": authorized_payload.get("manifest_validation_from_source_payload_only"),
        "manifest_validation_allowed": authorized_payload.get("manifest_validation_allowed"),
        ...
        "malformed_manifest_source_rejected": malformed_payload.get("ok") is False,
        "artifact_path_opened_by_l25_38": authorized_payload.get("artifact_path_opened_by_l25_38"),
        "artifact_bytes_read_by_l25_38": authorized_payload.get("artifact_bytes_read_by_l25_38"),
        "zip_member_opened_by_l25_38": authorized_payload.get("zip_member_opened_by_l25_38"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="L25.38 brief validator")
