...
DEFAULT_SELF_REPORT_FILENAME = "pseudo_self_report_upload_no_send_repair_10_desktop_filename_enter_self_report.txt"
...
def write_self_report(path: Path) -> str:
    ...
    lines = [
        ...
        "purpose: pseudo patch generated self-report on Desktop for Chrome native picker filename+Enter upload smoke",
        f"created_at: {datetime.now(timezone.utc).isoformat()}",
        "selected_action: upload_self_report_desktop_filename_enter_no_send",
        "browser_lane: chrome",
        "desktop_filename_mode: true",
        "picker_trigger: slash_then_enter_once",
        "picker_filename_entry: filename_only",
        "picker_submit: enter_in_native_file_picker_only",
        "expected_result_label: PASS_CHROME_SELF_REPORT_DESKTOP_FILENAME_ATTACHED_NO_SEND",
        ...
        "note: This file lives on Desktop. The helper types only the filename into the native Windows picker and presses Enter there. The smoke stops before Send.",
        "",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PatchOps apply smoke: Desktop filename + picker Enter attach in Chrome without Send")
    ...
    parser.add_argument("--confirm-live-browser-text", default=CONFIRM_CHROME_DESKTOP_FILENAME_SELF_REPORT_UPLOAD_NO_SEND)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return 0 if result.result_label == PASS_CHROME_SELF_REPORT_DESKTOP_FILENAME_ATTACHED_NO_SEND else 2
