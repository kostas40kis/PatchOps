...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_explicit_activation_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_cli_readback.md")
TEST_PATH = Path("tests/test_l10_02_edge_executable_filesystem_probe_explicit_activation_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l10_02_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L10.2 passive Microsoft Edge executable filesystem-probe activation CLI/readback.
...
DOC_CONTENT = r'''# L10.2 Microsoft Edge executable filesystem probe explicit activation CLI/readback
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l10_02_candidate"
...
def main() -> int:
    no_activation = readback.build_edge_executable_filesystem_probe_explicit_activation_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False)
    assert_passive(no_activation)
    assert no_activation["filesystem_probe_activation_ready"] is False
    print("PASS no_activation: patch=L10.2 l10_01_accepted=True activation_requested=False activation_ready=False activation_exec=False fs_probe=False path_selected=False")

    activation_only = readback.build_edge_executable_filesystem_probe_explicit_activation_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=True)
    ...
    for name, profile in cases:
        ...
        assert payload["filesystem_probe_activation_ready"] is True
        print(
            "PASS {0}: patch={1} source=L10.1 all_three_gates={2} activation_ready={3} activation_exec={4} fs_probe={5} path_selected={6}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; activation CLI/readback stayed passive")
    print("PASS L10.2 brief validation: explicit activation CLI/readback accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L10.2 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L10.2 command wrapper appended")
    else:
        print("L10.2 command wrapper already present")


def _append_l10_01_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_contract.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L10.2 Live adapter Microsoft Edge executable filesystem probe explicit activation CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe activation contract:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L10.1 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l10_01_doc_pointer()
    print("L10.2 files written; validation is intentionally brief and compact")
    return 0
