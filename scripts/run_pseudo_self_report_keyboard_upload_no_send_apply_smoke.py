...
DEFAULT_SELF_REPORT_PATH = Path("data/runtime/copilot_handoff/pseudo_self_report_keyboard_upload_no_send_self_report.txt")
...
def write_self_report(path: Path) -> str:
    ...
    lines = [
        ...
        "purpose: pseudo patch generated self-report for Chrome keyboard paste attach no-send smoke",
        f"created_at: {datetime.now(timezone.utc).isoformat()}",
        "selected_action: upload_self_report_keyboard_no_send",
        "browser_lane: chrome",
        "keyboard_shortcuts_used: CTRL+V",
        "forbidden_keyboard_shortcuts_used: none",
        "expected_result_label: PASS_CHROME_SELF_REPORT_KEYBOARD_ATTACHED_NO_SEND",
        "file_clipboard_written: expected_true_after_live_smoke",
        "ctrl_v_pressed: expected_true_after_live_smoke",
        ...
        "note: This file is attached to the Chrome composer by CF_HDROP clipboard + Ctrl+V only. The smoke test stops before Enter/Send.",
        "",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PatchOps apply smoke: keyboard-paste self-report file into Chrome without Enter/Send")
    ...
    parser.add_argument("--confirm-live-browser-text", default=CONFIRM_CHROME_KEYBOARD_SELF_REPORT_UPLOAD_NO_SEND)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return 0 if result.result_label == PASS_CHROME_SELF_REPORT_KEYBOARD_ATTACHED_NO_SEND else 2
