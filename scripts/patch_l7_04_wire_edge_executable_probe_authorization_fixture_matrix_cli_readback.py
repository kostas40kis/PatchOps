...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_authorization_fixture_matrix_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_authorization_fixture_matrix_cli_readback.md")
TEST_PATH = Path("tests/test_l7_04_edge_executable_probe_authorization_fixture_matrix_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l7_04_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L7.4 passive Edge executable-probe authorization fixture-matrix CLI/readback.
...
DOC_CONTENT = r'''# L7.4 Microsoft Edge executable probe authorization fixture matrix CLI/readback
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-authorization-fixture-matrix-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l7_04_candidate"
...
def main() -> int:
    ...
    for name, live, profile, probe_auth in cases:
        ...
        print(
            "PASS {0}: patch={1} source=L7.3 fixtures={2} probe_authorized={3} probe_permitted={4} exe_probe={5}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; fixture matrix CLI/readback stayed passive")
    print("PASS L7.4 brief validation: fixture matrix CLI/readback accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L7.4 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L7.4 command wrapper appended")
    else:
        print("L7.4 command wrapper already present")


def _append_l7_03_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_authorization_fixture_matrix.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L7.4 Live adapter Microsoft Edge executable probe authorization fixture matrix CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe authorization fixture matrix:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L7.3 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l7_03_doc_pointer()
    print("L7.4 files written; validation is intentionally brief and compact")
    return 0
