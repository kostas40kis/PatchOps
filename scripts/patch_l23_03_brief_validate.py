...
from patchops.llm_browser import live_adapter_edge_real_archive_manifest_validation_synthetic_archive_listing_authorization_gate as gate
...
def build_validation_payload(repo_root: str | Path = REPO_ROOT) -> dict[str, Any]:
    ...
    default_payload = gate.build_synthetic_archive_listing_authorization_gate(root)
    authorized_payload = gate.build_synthetic_archive_listing_authorization_gate(
        root,
        allow_synthetic_archive_listing_authorization=True,
        authorization_token=gate.REQUIRED_SYNTHETIC_ARCHIVE_LISTING_AUTHORIZATION_TOKEN,
    )

    module_path = root / "patchops" / "llm_browser" / "live_adapter_edge_real_archive_manifest_validation_synthetic_archive_listing_authorization_gate.py"
    doc_path = root / "docs" / "llm_browser_live_adapter_edge_real_archive_manifest_validation_synthetic_archive_listing_authorization_gate.md"
    ...
    _assert(authorized_payload.get("patch") == "L23.3", "wrong patch marker", authorized_payload)
    _assert(authorized_payload.get("source_l23_02_summary", {}).get("ok") is True, "L23.2 source summary not ok", authorized_payload)
    _assert(authorized_payload.get("source_l23_02_summary", {}).get("preflight_allowed") is True, "L23.2 preflight not allowed", authorized_payload)
    _assert(authorized_payload.get("source_l23_02_summary", {}).get("synthetic_archive_path_stat_performed") is True, "L23.2 synthetic path stat missing", authorized_payload)
    _assert(default_payload.get("synthetic_archive_listing_authorization_granted_for_future_patch") is False, "default should not grant future listing", default_payload)
    _assert(authorized_payload.get("synthetic_archive_listing_authorization_granted_for_future_patch") is True, "authorized readback should grant future listing", authorized_payload)
    _assert(authorized_payload.get("synthetic_archive_listing_execution_allowed") is False, "listing execution must remain false", authorized_payload)
    ...
    _assert(authorized_payload.get("no_new_archive_listing_permission_added_by_l23_3") is True, "L23.3 should add no listing execution permission", authorized_payload)
    ...
    _assert("It does not list anything yet" in doc_text, "doc missing no-listing-yet boundary", authorized_payload)
    _assert("Synthetic archive listing execution allowed remains false" in doc_text, "doc missing listing false boundary", authorized_payload)
    ...
    _assert("No ZIP archive module loading" in doc_text, "doc missing no ZIP module boundary", authorized_payload)
    ...
    return {
        "ok": True,
        "patch": "L23.3",
        "source_l23_02_ok": authorized_payload.get("source_l23_02_summary", {}).get("ok"),
        "source_l23_02_preflight_allowed": authorized_payload.get("source_l23_02_summary", {}).get("preflight_allowed"),
        "source_l23_02_path_stat_performed": authorized_payload.get("source_l23_02_summary", {}).get("synthetic_archive_path_stat_performed"),
        "default_future_authorized": default_payload.get("synthetic_archive_listing_authorization_granted_for_future_patch"),
        "authorized_readback": authorized_payload.get("synthetic_archive_listing_authorization_granted_for_future_patch"),
        "listing_execution_allowed": authorized_payload.get("synthetic_archive_listing_execution_allowed"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="L23.3 brief validator")
