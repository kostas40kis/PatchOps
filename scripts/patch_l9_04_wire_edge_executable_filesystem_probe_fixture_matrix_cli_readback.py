...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_fixture_matrix_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_fixture_matrix_cli_readback.md")
TEST_PATH = Path("tests/test_l9_04_edge_executable_filesystem_probe_fixture_matrix_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l9_04_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L9.4 passive Microsoft Edge executable filesystem-probe fixture-matrix CLI/readback.

Wraps the accepted L9.3 executable filesystem-probe fixture matrix in a compact
CLI/readback layer. The readback keeps the fixture matrix modeled only: no
authorization keeps contract_ready=false, authorized cases can report
contract_ready=true, and filesystem probing remains blocked by phase.
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_fixture_matrix as l9_03

PATCH = "L9.4"
PHASE = "L9"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L9.4 Microsoft Edge Executable Filesystem Probe Fixture Matrix CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-fixture-matrix-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-fixture-matrix"
NEXT_PATCH = "L9.5 Live adapter Microsoft Edge executable filesystem probe aggregate gate"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-fixture-matrix-cli-readback-only"
...
def _source_l9_03_safe(summary: Mapping[str, Any], *, expect_contract_ready: bool, expect_preflight_passed: bool) -> bool:
...
def build_edge_executable_filesystem_probe_fixture_matrix_cli_readback(
    ...
    *,
    ...
    source = l9_03.build_edge_executable_filesystem_probe_fixture_matrix(
        root,
    ...
    source_safe = _source_l9_03_safe(source, expect_contract_ready=expected_contract_ready, expect_preflight_passed=expected_preflight_passed)
    ...
    checks = [
        _check("l9_03_filesystem_probe_fixture_matrix_still_passes", source_safe, _compact_source_status(source)),
        _check("l9_03_filesystem_probe_fixture_matrix_remains_passive", source_safe, {"contract_ready": source.get("edge_executable_filesystem_probe_contract_ready"), "fs_probe": source.get("edge_executable_filesystem_probe_performed"), "path_selected": source.get("edge_executable_path_selected")}),
        _check("l9_04_filesystem_probe_fixture_matrix_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l9_03_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l9_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l9_docs_contain_filesystem_probe_fixture_matrix_readback_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("filesystem_probe_fixture_matrix_cli_readback_is_enforced", source.get("edge_executable_filesystem_probe_fixture_matrix_enforced") is True, {}),
    ...
    return {
        ...
        "l9_03_executable_filesystem_probe_fixture_matrix_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l9_03_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_fixture_matrix_cli_readback_enforced": True,
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L9.3 Accepted   : {payload.get('l9_03_executable_filesystem_probe_fixture_matrix_remains_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_fixture_matrix_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L9.4 Microsoft Edge executable filesystem probe fixture matrix CLI/readback
...
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-fixture-matrix"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l9_04_candidate"
...
def test_l9_04_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l9_04_no_auth_fixture_matrix_readback_contract_not_ready_and_no_probe() -> None:
...
def test_l9_04_authorized_fixture_matrix_readback_contract_ready_but_filesystem_probe_stays_blocked() -> None:
    payload = readback.build_edge_executable_filesystem_probe_fixture_matrix_cli_readback(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L9.5 Live adapter Microsoft Edge executable filesystem probe aggregate gate"
    _assert_passive(payload)


def test_l9_04_default_and_missing_profile_cases_are_passive_when_authorized() -> None:
    default_payload = readback.build_edge_executable_filesystem_probe_fixture_matrix_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = readback.build_edge_executable_filesystem_probe_fixture_matrix_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l9_03_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l9_03_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l9_04_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l9_04_doc_mentions_filesystem_probe_fixture_matrix_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_fixture_matrix_cli_readback.md"
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L9.4 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L9.4 command wrapper appended")
    else:
        print("L9.4 command wrapper already present")


def _append_l9_03_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_fixture_matrix.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L9.4 Live adapter Microsoft Edge executable filesystem probe fixture matrix CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe fixture matrix:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L9.3 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l9_03_doc_pointer()
    print("L9.4 files written; validation is intentionally brief and compact")
    return 0
