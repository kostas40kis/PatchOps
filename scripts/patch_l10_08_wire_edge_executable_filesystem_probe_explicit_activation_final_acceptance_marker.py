...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker.md")
TEST_PATH = Path("tests/test_l10_08_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker_current.py")
VALIDATE_PATH = Path("scripts/patch_l10_08_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L10.8 passive Microsoft Edge executable filesystem-probe activation final acceptance marker.
...
DOC_CONTENT = r'''# L10.8 Microsoft Edge executable filesystem probe explicit activation final acceptance marker
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker as marker

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-final-acceptance-marker"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-broad-validation-checkpoint"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l10_08_candidate"
...
def test_l10_08_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l10_08_final_marker_not_ready_without_all_gates() -> None:
    for kwargs in [
        ...
        payload = marker.build_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker(PROJECT_ROOT, **kwargs)
        assert payload["ok"] is True
        assert payload["patch"] == "L10.8"
        assert payload["l10_explicit_activation_stack_accepted"] is True
        assert payload["l10_07_explicit_activation_broad_validation_checkpoint_remains_accepted"] is True
...
def test_l10_08_all_gates_ready_but_execution_still_blocked() -> None:
    payload = marker.build_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker(
        PROJECT_ROOT,
    ...
    assert payload["filesystem_probe_activation_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L11.1 Live adapter Microsoft Edge executable filesystem probe real-probe preflight contract"
    _assert_passive(payload)


def test_l10_08_default_and_missing_profile_cases_are_passive_when_all_gates_present() -> None:
    default_payload = marker.build_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True)
    missing_payload = marker.build_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l10_07_status"]["ok"] is True
    ...
    assert missing_payload["source_l10_07_status"]["ok"] is True
    assert missing_payload["filesystem_probe_activation_ready"] is True
    _assert_passive(missing_payload)


def test_l10_08_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l10_08_doc_mentions_activation_final_marker_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-final-acceptance-marker"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l10_08_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l10_explicit_activation_stack_accepted"] is True
    assert payload["edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker_enforced"] is True
    assert payload["filesystem_probe_activation_execution_allowed"] is False
    assert payload["fixture_activation_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
...
def main() -> int:
    no_activation = marker.build_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False)
    assert_passive(no_activation)
    assert no_activation["filesystem_probe_activation_ready"] is False
    print("PASS no_activation: patch=L10.8 source=L10.7 stack=True final=True activation_ready=False activation_exec=False fs_probe=False path_selected=False")

    activation_only = marker.build_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=True)
    assert_passive(activation_only)
    assert activation_only["filesystem_probe_activation_ready"] is False
    print("PASS activation_only: stack=True final=True activation_ready=False activation_exec=False fs_probe=False path_selected=False")

    partial = marker.build_edge_executable_filesystem_probe_explicit_activation_final_acceptance_marker(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=False)
    assert_passive(partial)
    assert partial["filesystem_probe_activation_ready"] is False
    print("PASS live_and_probe_auth_without_activation: stack=True final=True activation_ready=False activation_exec=False fs_probe=False path_selected=False")
    ...
    for name, profile in [("all_gates_dedicated", DEDICATED), ("all_gates_default", DEFAULT), ("all_gates_missing_profile", None)]:
        ...
        assert payload["filesystem_probe_activation_ready"] is True
        print("PASS {0}: patch=L10.8 source=L10.7 fixtures=6 activation_ready=True activation_exec=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; activation final marker stayed passive")
    print("PASS L10.8 brief validation: activation final acceptance marker accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L10.8 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L10.8 command wrapper appended")
    else:
        print("L10.8 command wrapper already present")


def _append_l10_07_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_broad_validation_checkpoint.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L10.8 Live adapter Microsoft Edge executable filesystem probe explicit activation final acceptance marker"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe activation broad validation checkpoint:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L10.7 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l10_07_doc_pointer()
    print("L10.8 files written; validation is intentionally brief and compact")
    return 0
