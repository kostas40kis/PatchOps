...
DEFAULT_SELF_REPORT_FILENAME = "pseudo_self_report_upload_no_send_repair_13_existing_chrome_slash_immediate_path_enter_self_report.txt"
...
def write_self_report(path: Path) -> str:
    ...
    lines = [
        ...
        "purpose: use already-loaded Chrome target, no new tab, no clicks, slash+Enter, paste full Desktop path into ready Windows file picker, Enter, no Send",
        f"created_at: {datetime.now(timezone.utc).isoformat()}",
        "selected_action: upload_self_report_existing_chrome_slash_immediate_full_path_enter_no_send",
        "browser_lane: chrome",
        "manual_sequence: existing_chrome_loaded__slash__enter__paste_full_path__enter",
        "expected_result_label: PASS_CHROME_SELF_REPORT_EXISTING_SLASH_IMMEDIATE_PATH_ENTER_ATTACHED_NO_SEND",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PatchOps apply smoke: existing Chrome target, no clicks, slash+Enter, immediate full path paste, picker Enter, no Send")
    ...
    parser.add_argument("--confirm-live-browser-text", default=CONFIRM_CHROME_EXISTING_SLASH_IMMEDIATE_PATH_ENTER_NO_SEND)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return 0 if result.result_label == PASS_CHROME_SELF_REPORT_EXISTING_SLASH_IMMEDIATE_PATH_ENTER_ATTACHED_NO_SEND else 2
