...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_authorization_passive_contract.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_authorization_passive_contract.md")
TEST_PATH = Path("tests/test_l7_01_edge_executable_probe_authorization_passive_contract_current.py")
VALIDATE_PATH = Path("scripts/patch_l7_01_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L7.1 passive Microsoft Edge executable-probe authorization contract.
...
DOC_CONTENT = r'''# L7.1 Microsoft Edge executable probe authorization passive contract
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-authorization-contract"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l7_01_candidate"
...
def main() -> int:
    ...
    for name, live, profile, probe_auth in cases:
        ...
        print(
            "PASS {0}: patch={1} probe_authorized={2} probe_permitted={3} exe_probe={4}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; executable probe authorization stayed passive")
    print("PASS L7.1 brief validation: probe authorization modeled with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L7.1 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L7.1 command wrapper appended")
    else:
        print("L7.1 command wrapper already present")


def _append_l6_08_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_discovery_final_acceptance_marker.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L7.1 Live adapter Microsoft Edge executable probe authorization passive contract"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable discovery slice:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L6.8 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l6_08_doc_pointer()
    print("L7.1 files written; validation is intentionally brief and compact")
    return 0
