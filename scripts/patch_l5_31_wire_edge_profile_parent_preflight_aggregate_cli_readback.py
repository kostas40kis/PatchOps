...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_supervised_launch_profile_parent_preflight_aggregate_gate_cli_readback.py")
...
TEST_PATH = Path("tests/test_l5_31_edge_supervised_launch_profile_parent_preflight_aggregate_gate_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l5_31_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L5.31 passive Edge profile-parent preflight aggregate gate CLI/readback.
...
DOC_CONTENT = r'''# L5.31 Microsoft Edge supervised launch profile parent preflight aggregate gate CLI/readback
...
def main() -> int:
    ...
    for name, allow, profile in cases:
        payload = readback.build_edge_supervised_launch_profile_parent_preflight_aggregate_gate_cli_readback(ROOT, allow_live_start=allow, profile_dir=profile)
    ...
    payload = json.loads(completed.stdout)
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed, output intentionally not echoed")
    print("PASS L5.31 brief validation: no Selenium/browser/profile/click/download/paste/send/package-run side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L5.31 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L5.31 command wrapper appended")
    else:
        print("L5.31 command wrapper already present")


def _append_l5_30_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_supervised_launch_profile_parent_preflight_aggregate_gate.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L5.31 Live adapter Microsoft Edge supervised launch profile parent preflight aggregate gate CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the profile-parent preflight aggregate gate:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L5.30 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l5_30_doc_pointer()
    print("L5.31 files written; validation is intentionally brief")
    return 0
