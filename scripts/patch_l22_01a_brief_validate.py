...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Brief L22.1a validation")
    parser.add_argument("--repo-root", default=str(_REPO_ROOT))
    parser.add_argument("--target-url", default="https://chatgpt.com/")
    args = parser.parse_args(argv)

    root = Path(args.repo_root).resolve()
    default_payload = l22_01a.build_edge_downloaded_archive_manifest_validation_passive_preflight_gate_repair(root, target_url=args.target_url)
    authorized_payload = l22_01a.build_edge_downloaded_archive_manifest_validation_passive_preflight_gate_repair(
        root,
        allow_manifest_validation_preflight=True,
        authorization_token=l22_01a.REQUIRED_MANIFEST_PREFLIGHT_AUTHORIZATION_TOKEN,
    ...
    ok = (
        ...
        and authorized_payload.get("manifest_validation_execution_allowed") is False
        and authorized_payload.get("manifest_validation_active") is False
        and authorized_payload.get("manifest_validation_performed") is False
    ...
    summary = {
        ...
        "manifest_validation_preflight_authorization_is_readback_only_in_l22_1a": authorized_payload.get("manifest_validation_preflight_authorization_is_readback_only_in_l22_1a"),
