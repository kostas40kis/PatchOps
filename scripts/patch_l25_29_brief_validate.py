...
from patchops.llm_browser import live_adapter_edge_real_archive_manifest_patchops_apply_first_controlled_proof as proof
...
APPLY_MANIFEST_OBJECT = {
    ...
    "failure_policy": {},
    "report_preferences": {"write_to_desktop": False},
    "tags": ["synthetic", "l25_29", "apply_only", "zero_write", "zero_validation", "isolated_fixture"],
    "notes": "Synthetic manifest for L25.29 PatchOps apply-only proof. It has no writes or validations and is never run-package executed.",
}
...
def _refresh_apply_zip_fixture(root: Path) -> Path:
    return _write_zip(root / proof.DEFAULT_CONTROLLED_CANDIDATE_RELATIVE_PATH, APPLY_MANIFEST_PAYLOAD, b"# isolated L25.29 PatchOps apply proof fixture\n")
...
def build_validation_payload(repo_root: str | Path = REPO_ROOT) -> dict[str, Any]:
    ...
    default_payload = proof.build_archive_manifest_patchops_apply_first_controlled_proof(root)
    authorized_payload = proof.build_archive_manifest_patchops_apply_first_controlled_proof(
        root,
        allow_archive_manifest_patchops_apply_proof=True,
        authorization_token=proof.REQUIRED_ARCHIVE_MANIFEST_PATCHOPS_APPLY_PROOF_TOKEN,
        candidate_archive_path=str(apply_candidate),
        member_name=MANIFEST_MEMBER,
    )

    module_path = root / "patchops" / "llm_browser" / "live_adapter_edge_real_archive_manifest_patchops_apply_first_controlled_proof.py"
    doc_path = root / "docs" / "llm_browser_live_adapter_edge_real_archive_manifest_patchops_apply_first_controlled_proof.md"
    ...
    _assert(apply_candidate.exists(), "isolated apply proof zip fixture missing", authorized_payload)
    _assert(default_payload.get("ok") is True, "default payload is not ok", default_payload)
    _assert(default_payload.get("patchops_manifest_apply_performed") is False, "default should not run apply", default_payload)
    _assert(default_payload.get("apply_fixture_isolated_from_preflight_inspect_and_plan_fixtures") is True, "default should report isolated apply fixture", default_payload)
    _assert(authorized_payload.get("ok") is True, "authorized payload is not ok", authorized_payload)
    _assert(authorized_payload.get("patch") == "L25.29", "wrong patch marker", authorized_payload)
    _assert(authorized_payload.get("source_l25_28_summary", {}).get("ok") is True, "L25.28 source not ok", authorized_payload)
    _assert(authorized_payload.get("source_l25_28_summary", {}).get("future_patchops_apply_authorized") is True, "L25.28 future apply authorization missing", authorized_payload)
    _assert(authorized_payload.get("source_l25_28_summary", {}).get("patchops_manifest_apply_performed") is False, "L25.28 must not have run apply", authorized_payload)
    _assert(authorized_payload.get("source_l25_28_summary", {}).get("source_l25_27_plan_invoked") is True, "L25.27 source plan proof missing", authorized_payload)
    _assert(authorized_payload.get("archive_manifest_patchops_apply_allowed") is True, "PatchOps apply should be allowed", authorized_payload)
    _assert(authorized_payload.get("manifest_payload_read") is True, "manifest payload should be read", authorized_payload)
    _assert(authorized_payload.get("manifest_payload_json_parsed") is True, "manifest payload should be parsed for guard", authorized_payload)
    _assert(authorized_payload.get("manifest_zero_write_zero_validation") is True, "manifest must be zero-write and zero-validation", authorized_payload)
    _assert(authorized_payload.get("manifest_files_to_write_count") == 0, "manifest files_to_write count should be zero", authorized_payload)
    _assert(authorized_payload.get("manifest_validation_commands_count") == 0, "manifest validation_commands count should be zero", authorized_payload)
    _assert(authorized_payload.get("manifest_temp_file_written_for_apply") is True, "temp manifest should be written for apply", authorized_payload)
    _assert(authorized_payload.get("patchops_manifest_apply_performed") is True, "PatchOps apply should be performed", authorized_payload)
    _assert(authorized_payload.get("patchops_manifest_validation_performed") is False, "PatchOps validation should not be marked by apply-only proof", authorized_payload)
    _assert(authorized_payload.get("patchops_cli_apply_invoked_for_archive_manifest") is True, "PatchOps apply should be invoked", authorized_payload)
    _assert(authorized_payload.get("patchops_cli_apply_exit_code") == 0, "PatchOps apply exit mismatch", authorized_payload)
    _assert(authorized_payload.get("patchops_cli_apply_timed_out") is False, "PatchOps apply timed out", authorized_payload)
    _assert(authorized_payload.get("patchops_cli_apply_stdout_result_pass") is True, "PatchOps apply PASS marker missing", authorized_payload)
    _assert(authorized_payload.get("patchops_cli_apply_stdout_exit_zero") is True, "PatchOps apply stdout exit-zero marker missing", authorized_payload)
    _assert(authorized_payload.get("patchops_cli_apply_patch_name") == APPLY_MANIFEST_OBJECT["patch_name"], "PatchOps apply patch name mismatch", authorized_payload)
    ...
    _assert(authorized_payload.get("manifest_payload_byte_count") == len(APPLY_MANIFEST_PAYLOAD), "manifest payload byte count mismatch", authorized_payload)
    _assert(authorized_payload.get("patchops_apply_scope") == proof.PATCHOPS_APPLY_SCOPE, "wrong PatchOps apply scope", authorized_payload)
    _assert(authorized_payload.get("apply_fixture_isolated_from_preflight_inspect_and_plan_fixtures") is True, "apply fixture isolation marker missing", authorized_payload)
    ...
    _assert(authorized_payload.get("package_manifest_used_for_execution") is False, "manifest must not be used for package execution", authorized_payload)
    ...
    _assert(authorized_payload.get("no_patchops_run_package_added_by_l25_29") is True, "L25.29 should add no run-package", authorized_payload)
    _assert(authorized_payload.get("no_package_execution_added_by_l25_29") is True, "L25.29 should add no package execution", authorized_payload)
    _assert(authorized_payload.get("no_archive_extraction_added_by_l25_29") is True, "L25.29 should add no extraction", authorized_payload)
    _assert(authorized_payload.get("no_browser_permission_added_by_l25_29") is True, "L25.29 should add no browser permission", authorized_payload)
    ...
    _assert("PatchOps apply is the only new PatchOps CLI command invoked against the archive manifest copy" in doc_text, "doc missing apply-only boundary", authorized_payload)
    ...
    _assert("No click/download/archive-extract/package-run/paste/send side effect" in doc_text, "doc missing side-effect boundary", authorized_payload)
    ...
    return {
        ...
        "default_patchops_manifest_apply_performed": default_payload.get("patchops_manifest_apply_performed"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="L25.29 brief validator")
