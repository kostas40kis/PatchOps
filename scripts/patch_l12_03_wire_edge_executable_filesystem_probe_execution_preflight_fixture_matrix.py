...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_execution_preflight_fixture_matrix.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_fixture_matrix.md")
TEST_PATH = Path("tests/test_l12_03_edge_executable_filesystem_probe_execution_preflight_fixture_matrix_current.py")
VALIDATE_PATH = Path("scripts/patch_l12_03_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L12.3 passive Microsoft Edge executable filesystem-probe execution preflight fixture matrix.
...
DOC_CONTENT = r'''# L12.3 Microsoft Edge executable filesystem probe execution preflight fixture matrix
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_preflight_fixture_matrix as matrix

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-fixture-matrix"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-readback"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l12_03_candidate"
...
def test_l12_03_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l12_03_fixture_matrix_not_ready_without_all_required_gates() -> None:
    for kwargs in [
        ...
        payload = matrix.build_edge_executable_filesystem_probe_execution_preflight_fixture_matrix(PROJECT_ROOT, **kwargs)
        assert payload["ok"] is True
        assert payload["patch"] == "L12.3"
...
def test_l12_03_execution_preflight_ready_but_execution_still_blocked() -> None:
    payload = matrix.build_edge_executable_filesystem_probe_execution_preflight_fixture_matrix(
        PROJECT_ROOT,
    ...
    assert payload["execution_preflight_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L12.4 Live adapter Microsoft Edge executable filesystem probe execution preflight fixture matrix CLI/readback"
    _assert_passive(payload)


def test_l12_03_default_and_missing_profile_cases_are_passive_when_execution_preflight_ready() -> None:
    default_payload = matrix.build_edge_executable_filesystem_probe_execution_preflight_fixture_matrix(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=True)
    missing_payload = matrix.build_edge_executable_filesystem_probe_execution_preflight_fixture_matrix(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l12_02_status"]["ok"] is True
    assert default_payload["execution_preflight_ready"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l12_02_status"]["ok"] is True
    assert missing_payload["execution_preflight_ready"] is True
    _assert_passive(missing_payload)


def test_l12_03_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l12_03_doc_mentions_execution_preflight_fixture_matrix_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_fixture_matrix.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-fixture-matrix"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l12_03_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l12_02_execution_preflight_cli_readback_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_execution_preflight_fixture_matrix_enforced"] is True
    assert payload["execution_preflight_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_gates = matrix.build_edge_executable_filesystem_probe_execution_preflight_fixture_matrix(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False, allow_executable_filesystem_probe_execution=False)
    assert_passive(no_gates)
    assert no_gates["execution_preflight_ready"] is False
    print("PASS no_gates: patch=L12.3 fixtures=6 exec_ready=False exec_allowed=False fs_probe=False path_selected=False")

    execution_flag_only = matrix.build_edge_executable_filesystem_probe_execution_preflight_fixture_matrix(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False, allow_executable_filesystem_probe_execution=True)
    ...
    real_ready_without_execution = matrix.build_edge_executable_filesystem_probe_execution_preflight_fixture_matrix(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=False)
    ...
    for name, profile in [("execution_preflight_ready_dedicated", DEDICATED), ("execution_preflight_ready_default", DEFAULT), ("execution_preflight_ready_missing_profile", None)]:
        ...
        assert payload["execution_preflight_ready"] is True
        print("PASS {0}: patch=L12.3 source=L12.2 fixtures=6 exec_ready=True exec_allowed=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; execution-preflight fixture matrix stayed passive")
    print("PASS L12.3 brief validation: execution-preflight fixture matrix accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L12.3 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L12.3 command wrapper appended")
    else:
        print("L12.3 command wrapper already present")


def _append_l12_02_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L12.3 Live adapter Microsoft Edge executable filesystem probe execution preflight fixture matrix"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the execution preflight CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L12.2 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l12_02_doc_pointer()
    print("L12.3 files written; validation is intentionally brief and compact")
    return 0
