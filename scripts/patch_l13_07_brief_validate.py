...
def main() -> int:
    ...
    profile = root / "data" / "runtime" / "browser_profiles" / "edge_l13_07_brief"
    existing = root / "data" / "runtime" / "edge_probe_fixture" / "msedge_l13_07_brief.exe"
    missing = root / "data" / "runtime" / "edge_probe_fixture" / "missing-msedge-l13-07-brief.exe"
    ...
    commands.append(run_cmd([sys.executable, "-m", "compileall", "patchops/llm_browser", *existing_tests, "scripts/patch_l13_07_brief_validate.py"], root))
    ...
    ok = ok and source_payload.get("patch") == "L13.6"
    ok = ok and source_payload.get("source_patch") == "L13.5"
    ...
    ok = ok and checkpoint_payload.get("patch") == "L13.7"
    ok = ok and checkpoint_payload.get("source_patch") == "L13.6"
    ok = ok and checkpoint_payload.get("source_l13_06_aggregate_gate_readback_remains_accepted") is True
    ok = ok and checkpoint_payload.get("edge_executable_filesystem_probe_performed") is True
    ok = ok and checkpoint_payload.get("truthful_selected_path_existing_reported_candidate") is True
    ok = ok and checkpoint_payload.get("filesystem_probe_only_when_l12_execution_preflight_ready") is True
    ...
    ok = ok and checkpoint_payload.get("executed_validation_commands") == []
    ok = ok and _candidate_truthful(checkpoint_payload)
    ok = ok and _no_launch(checkpoint_payload)
    ...
    summary = {
        ...
        "checkpoint_probe_performed": checkpoint_payload.get("edge_executable_filesystem_probe_performed"),
        "checkpoint_truthful_selection": checkpoint_payload.get("truthful_selected_path_existing_reported_candidate"),
        "checkpoint_plan_passive": checkpoint_payload.get("planned_broad_validation_commands_are_passive"),
        "checkpoint_plan_executed": checkpoint_payload.get("broad_validation_commands_executed_by_checkpoint"),
        "checkpoint_no_launch": _no_launch(checkpoint_payload) if checkpoint_payload else False,
