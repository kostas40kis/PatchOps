...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_final_acceptance_marker.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_final_acceptance_marker.md")
TEST_PATH = Path("tests/test_l9_08_edge_executable_filesystem_probe_final_acceptance_marker_current.py")
VALIDATE_PATH = Path("scripts/patch_l9_08_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L9.8 passive Microsoft Edge executable filesystem-probe final acceptance marker.
...
DOC_CONTENT = r'''# L9.8 Microsoft Edge executable filesystem probe final acceptance marker

L9.8 is the passive final acceptance marker for the accepted Microsoft Edge executable filesystem-probe slice.
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_final_acceptance_marker as marker

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-final-acceptance-marker"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-broad-validation-checkpoint"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l9_08_candidate"
...
def test_l9_08_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l9_08_no_auth_final_marker_contract_not_ready_and_no_probe() -> None:
    payload = marker.build_edge_executable_filesystem_probe_final_acceptance_marker(
        PROJECT_ROOT,
    ...
    assert payload["l9_07_executable_filesystem_probe_broad_validation_checkpoint_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_final_acceptance_marker_enforced"] is True
...
def test_l9_08_authorized_final_marker_contract_ready_but_filesystem_probe_stays_blocked() -> None:
    payload = marker.build_edge_executable_filesystem_probe_final_acceptance_marker(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L10.1 Live adapter Microsoft Edge executable filesystem probe explicit activation contract"
    _assert_passive(payload)


def test_l9_08_default_and_missing_profile_cases_are_passive_when_authorized() -> None:
    default_payload = marker.build_edge_executable_filesystem_probe_final_acceptance_marker(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = marker.build_edge_executable_filesystem_probe_final_acceptance_marker(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l9_07_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l9_07_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l9_08_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l9_08_doc_mentions_filesystem_probe_final_marker_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_final_acceptance_marker.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-final-acceptance-marker"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l9_08_candidate"
...
def main() -> int:
    no_auth = marker.build_edge_executable_filesystem_probe_final_acceptance_marker(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    ...
    print("PASS no_probe_auth: patch=L9.8 final_marker_ok=True slice=True preflight_passed=False contract_ready=False fs_probe=False path_selected=False")
    ...
    for name, live, profile, probe_auth in cases:
        ...
        assert payload["edge_executable_filesystem_probe_contract_ready"] is True
        print(
            "PASS {0}: patch={1} source=L9.7 slice={2} fixtures={3} preflight_passed={4} contract_ready={5} fs_probe={6} path_selected={7}".format(
                name,
                payload["patch"],
                payload["l9_executable_filesystem_probe_passive_slice_accepted"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; filesystem probe final marker stayed passive")
    print("PASS L9.8 brief validation: executable filesystem probe final marker accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L9.8 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L9.8 command wrapper appended")
    else:
        print("L9.8 command wrapper already present")


def _append_l9_07_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_broad_validation_checkpoint.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L9.8 Live adapter Microsoft Edge executable filesystem probe final acceptance marker"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe broad checkpoint:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L9.7 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l9_07_doc_pointer()
    print("L9.8 files written; validation is intentionally brief and compact")
    return 0
