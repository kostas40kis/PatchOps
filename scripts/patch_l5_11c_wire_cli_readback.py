...
SENTINEL_START = "# PATCHOPS L5.11C START"
...
def main() -> int:
    ...
    if SENTINEL_START in text:
        print("L5.11c command wrapper already present")
        return 0
    if COMMAND not in text:
        print("L5.11 command was not present; L5.11c will add it")
    if not text.endswith("\n"):
        text += "\n"
    COMMANDS_PATH.write_text(text + "\n" + PATCH_BLOCK.strip() + "\n", encoding="utf-8")
    print("L5.11c command wrapper appended to patchops/llm_browser/commands.py")
    return 0
