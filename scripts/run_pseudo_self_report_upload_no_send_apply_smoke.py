...
PATCH_NAME = "pseudo_self_report_upload_no_send_repair_01_live_smoke"
DEFAULT_SELF_REPORT_PATH = Path("data/runtime/copilot_handoff/pseudo_self_report_upload_no_send_apply_self_report.txt")
DEFAULT_LIVE_JSON_PATH = Path("data/runtime/copilot_handoff/latest_pseudo_self_report_upload_no_send_apply_smoke.json")
DEFAULT_LIVE_TXT_PATH = Path("data/runtime/copilot_handoff/latest_pseudo_self_report_upload_no_send_apply_smoke.txt")
...
def write_self_report(path: Path) -> str:
    ...
    lines = [
        ...
        "purpose: pseudo patch generated self-report for Chrome attach no-send smoke",
        f"created_at: {datetime.now(timezone.utc).isoformat()}",
        "selected_action: upload_self_report_no_send",
        "browser_lane: chrome",
        "expected_result_label: PASS_CHROME_SELF_REPORT_ATTACHED_NO_SEND",
        ...
        "note: This file is attached to the Chrome composer only. The smoke test stops before Send.",
        "",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PatchOps apply smoke: generate and attach self-report in Chrome without Enter/Send")
    ...
    parser.add_argument("--confirm-live-browser-text", default=CONFIRM_CHROME_SELF_REPORT_UPLOAD_NO_SEND)
    parser.add_argument("--json-output-path", default=str(DEFAULT_LIVE_JSON_PATH))
    parser.add_argument("--txt-output-path", default=str(DEFAULT_LIVE_TXT_PATH))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return 0 if result.result_label == PASS_CHROME_SELF_REPORT_ATTACHED_NO_SEND else 2
