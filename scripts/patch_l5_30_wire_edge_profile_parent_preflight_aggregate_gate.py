...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_supervised_launch_profile_parent_preflight_aggregate_gate.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_supervised_launch_profile_parent_preflight_aggregate_gate.md")
TEST_PATH = Path("tests/test_l5_30_edge_supervised_launch_profile_parent_preflight_aggregate_gate_current.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L5.30 passive Microsoft Edge profile-parent preflight aggregate gate.
...
DOC_CONTENT = r'''# L5.30 Microsoft Edge supervised launch profile parent preflight aggregate gate
...
def _append_command_block() -> None:
    ...
    text = COMMANDS_PATH.read_text(encoding="utf-8")
    if "# PATCHOPS L5.30 START" in text:
        print("L5.30 command wrapper already present")
        return
    if not text.endswith("\n"):
        text += "\n"
    COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
    print("L5.30 command wrapper appended to patchops/llm_browser/commands.py")


def _append_l5_29_doc_pointer() -> None:
    l5_29_doc = Path("docs/llm_browser_live_adapter_edge_supervised_launch_profile_parent_preflight_cli_readback.md")
    if not l5_29_doc.exists():
        return
    text = l5_29_doc.read_text(encoding="utf-8")
    phrase = "L5.30 Live adapter Microsoft Edge supervised launch profile parent preflight aggregate gate"
    if phrase not in text:
        l5_29_doc.write_text(text.rstrip() + "\n\nNext accepted frontier after the profile-parent preflight CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L5.29 documentation next-frontier pointer appended")
