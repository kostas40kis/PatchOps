...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_real_probe_preflight_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_cli_readback.md")
TEST_PATH = Path("tests/test_l11_02_edge_executable_filesystem_probe_real_probe_preflight_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l11_02_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L11.2 passive Microsoft Edge executable filesystem-probe real-probe preflight CLI/readback.
...
DOC_CONTENT = r'''# L11.2 Microsoft Edge executable filesystem probe real-probe preflight CLI/readback
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_real_probe_preflight_cli_readback as readback

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-contract"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l11_02_candidate"
...
def test_l11_02_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l11_02_real_probe_preflight_readback_not_ready_without_real_probe_flag() -> None:
    payload = readback.build_edge_executable_filesystem_probe_real_probe_preflight_cli_readback(
        PROJECT_ROOT,
...
def test_l11_02_real_probe_flag_alone_is_not_ready() -> None:
    payload = readback.build_edge_executable_filesystem_probe_real_probe_preflight_cli_readback(
        PROJECT_ROOT,
...
def test_l11_02_real_probe_preflight_ready_but_execution_still_blocked() -> None:
    payload = readback.build_edge_executable_filesystem_probe_real_probe_preflight_cli_readback(
        PROJECT_ROOT,
        allow_live_start=True,
        profile_dir=DEDICATED_PROFILE,
    ...
    assert payload["real_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L11.3 Live adapter Microsoft Edge executable filesystem probe real-probe preflight fixture matrix"
    _assert_passive(payload)


def test_l11_02_default_and_missing_profile_cases_are_passive_when_real_preflight_ready() -> None:
    default_payload = readback.build_edge_executable_filesystem_probe_real_probe_preflight_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True)
    missing_payload = readback.build_edge_executable_filesystem_probe_real_probe_preflight_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l11_01_status"]["ok"] is True
    ...
    assert missing_payload["source_l11_01_status"]["ok"] is True
    assert missing_payload["real_filesystem_probe_preflight_ready"] is True
    _assert_passive(missing_payload)


def test_l11_02_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l11_02_doc_mentions_real_probe_preflight_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_cli_readback.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l11_02_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l11_01_real_probe_preflight_contract_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_real_probe_preflight_cli_readback_enforced"] is True
    assert payload["real_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_real_flag = readback.build_edge_executable_filesystem_probe_real_probe_preflight_cli_readback(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=False)
    ...
    print("PASS no_real_flag: patch=L11.2 source=L11.1 activation_ready=True real_ready=False real_exec=False fs_probe=False path_selected=False")

    real_flag_only = readback.build_edge_executable_filesystem_probe_real_probe_preflight_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=True)
    ...
    for name, profile in [("all_gates_dedicated", DEDICATED), ("all_gates_default", DEFAULT), ("all_gates_missing_profile", None)]:
        ...
        assert payload["filesystem_probe_activation_ready"] is True
        assert payload["real_filesystem_probe_preflight_ready"] is True
        print("PASS {0}: patch=L11.2 real_ready=True real_exec=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; real-probe preflight CLI/readback stayed passive")
    print("PASS L11.2 brief validation: real-probe preflight CLI/readback accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L11.2 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L11.2 command wrapper appended")
    else:
        print("L11.2 command wrapper already present")


def _append_l11_01_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_contract.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L11.2 Live adapter Microsoft Edge executable filesystem probe real-probe preflight CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the real-probe preflight contract:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L11.1 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l11_01_doc_pointer()
    print("L11.2 files written; validation is intentionally brief and compact")
    return 0
