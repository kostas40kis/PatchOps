...
SENTINEL_START = "# PATCHOPS L5.13 START"
...
def main() -> int:
    ...
    if SENTINEL_START in text:
        print("L5.13 Edge readiness readback command wrapper already present")
        return 0
    if COMMAND in text:
        print("L5.13 command text already present; sentinel missing, appending guarded wrapper anyway")
    if not text.endswith("\n"):
        text += "\n"
    COMMANDS_PATH.write_text(text + "\n" + PATCH_BLOCK.strip() + "\n", encoding="utf-8")
    print("L5.13 Edge readiness readback command wrapper appended to patchops/llm_browser/commands.py")
    return 0
