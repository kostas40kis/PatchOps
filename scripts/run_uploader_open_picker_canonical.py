...
from patchops.chatgpt_uploader.canonical_picker_trigger import (
    ...
    run_canonical_picker_trigger,
)
from patchops.chatgpt_uploader.config import resolve_config_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Canonical ChatGPT picker opener. Safe-click slash Enter once; no file select and no send.")
...
def _print_safety(run) -> None:
    print(f"CANONICAL_PICKER_TRIGGER: {CANONICAL_PICKER_TRIGGER_NAME}")
    print(f"CANONICAL_SEQUENCE: {','.join(CANONICAL_PICKER_TRIGGER_SEQUENCE)}")
    print("CANONICAL_ATTEMPTS: 1")
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence_dir = Path(args.evidence_dir).expanduser().resolve() if args.evidence_dir else repo_root / "data" / "runtime" / "chatgpt_uploader" / "canonical_picker_trigger"
    run, json_path, txt_path = run_canonical_picker_trigger(
    ...
    print(f"PATCHOPS_UPLOADER_CANONICAL_PICKER_STATUS: {run.status}")
