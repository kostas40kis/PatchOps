...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_execution_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_cli_readback.md")
TEST_PATH = Path("tests/test_l13_02_edge_executable_filesystem_probe_execution_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l13_02_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L13.2 Microsoft Edge executable filesystem-probe execution CLI/readback.
...
DOC_CONTENT = r'''# L13.2 Microsoft Edge executable filesystem probe execution CLI/readback
...
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-contract"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l13_02_candidate"
EXISTING_FIXTURE = PROJECT_ROOT / "data" / "runtime" / "edge_probe_fixture" / "msedge_l13_02.exe"
MISSING_FIXTURE = PROJECT_ROOT / "data" / "runtime" / "edge_probe_fixture" / "missing-msedge-l13-02.exe"
...
def _assert_truthful_selection(payload: dict) -> None:
    ...
    if payload["edge_executable_path_selected"]:
        ...
        assert match.get("exists") is True
        assert match.get("is_file") is True
    else:
        assert selected is None


def test_l13_02_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l13_02_readback_does_not_probe_when_l12_execution_preflight_is_not_ready() -> None:
    payload = readback.build_edge_executable_filesystem_probe_execution_cli_readback(
        PROJECT_ROOT,
...
def test_l13_02_readback_probe_runs_when_l12_execution_preflight_is_ready_and_fixture_is_observed() -> None:
    ...
    assert payload["next_patch"] == "L13.3 Live adapter Microsoft Edge executable filesystem probe execution fixture matrix"
    _assert_no_launch_side_effects(payload)


def test_l13_02_patchops_cli_json_readback_is_parseable_compact_and_no_launch_side_effects() -> None:
    EXISTING_FIXTURE.parent.mkdir(parents=True, exist_ok=True)
    EXISTING_FIXTURE.write_text("fixture executable placeholder", encoding="utf-8")
...
def test_l13_02_doc_mentions_execution_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_execution_cli_readback.md"
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L13.2 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L13.2 command wrapper appended")
    else:
        print("L13.2 command wrapper already present")


def _append_l13_01_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_contract.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L13.2 Live adapter Microsoft Edge executable filesystem probe execution CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the execution contract:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L13.1 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l13_01_doc_pointer()
    print("L13.2 files written; validation is intentionally brief and compact")
    return 0
