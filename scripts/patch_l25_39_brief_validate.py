...
from patchops.llm_browser import live_adapter_edge_real_download_artifact_manifest_validation_broad_checkpoint as broad
...
def _source_l25_38_payload() -> dict[str, Any]:
    return {
        "ok": True,
        "patch": "L25.38",
        ...
        "malformed_manifest_source_rejected": True,
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    _assert(payload.get("patch") == "L25.39", "wrong patch marker", payload)
    _assert(payload.get("manifest_validation_broad_checkpoint") is True, "broad checkpoint marker missing", payload)
    _assert(payload.get("manifest_validation_broad_checkpoint_from_source_payload_only") is True, "source-payload-only marker missing", payload)
    _assert(payload.get("manifest_validation_broad_checkpoint_allowed") is True, "broad checkpoint not allowed", payload)
    ...
    _assert(payload.get("artifact_path_opened_by_l25_39") is False, "L25.39 must not open artifact path", payload)
    _assert(payload.get("artifact_bytes_read_by_l25_39") is False, "L25.39 must not read artifact bytes", payload)
    _assert(payload.get("zip_member_opened_by_l25_39") is False, "L25.39 must not open zip members", payload)
...
def build_validation_payload(repo_root: str | Path = REPO_ROOT) -> dict[str, Any]:
    ...
    source = _source_l25_38_payload()

    default_payload = broad.build_real_download_artifact_manifest_validation_broad_checkpoint(root, source_payload=source)
    _assert(default_payload.get("ok") is False, "default should not pass without token", default_payload)
    _assert(default_payload.get("manifest_validation_broad_checkpoint_allowed") is False, "default must not allow broad checkpoint", default_payload)

    authorized_payload = broad.build_real_download_artifact_manifest_validation_broad_checkpoint(
        root,
        source_payload=source,
        allow_broad_checkpoint=True,
        authorization_token=broad.REQUIRED_BROAD_CHECKPOINT_TOKEN,
    )
    _validate_payload(authorized_payload)

    malformed_source = dict(source)
    malformed_source["malformed_manifest_source_rejected"] = False
    malformed_payload = broad.build_real_download_artifact_manifest_validation_broad_checkpoint(
        root,
        source_payload=malformed_source,
        allow_broad_checkpoint=True,
        authorization_token=broad.REQUIRED_BROAD_CHECKPOINT_TOKEN,
    )
    _assert(malformed_payload.get("ok") is False, "broad checkpoint must reject source with missing malformed-source rejection", malformed_payload)

    module_path = root / "patchops" / "llm_browser" / "live_adapter_edge_real_download_artifact_manifest_validation_broad_checkpoint.py"
    doc_path = root / "docs" / "llm_browser_live_adapter_edge_real_download_artifact_manifest_validation_broad_checkpoint.md"
    module_text = module_path.read_text(encoding="utf-8")
    doc_text = doc_path.read_text(encoding="utf-8")
    _assert(not _forbidden_import_hits(module_text), "forbidden imports in L25.39 module", authorized_payload)
    _assert(not _forbidden_call_hits(module_text), "forbidden artifact/zip/execution calls in L25.39 module", authorized_payload)
    _assert("This checkpoint does not open an artifact path" in doc_text, "doc missing no artifact-open boundary", authorized_payload)
    _assert("This checkpoint does not read artifact bytes" in doc_text, "doc missing no artifact-read boundary", authorized_payload)
    _assert("This checkpoint does not open ZIP members" in doc_text, "doc missing no zip-member boundary", authorized_payload)
    _assert("No PatchOps run-package against the artifact" in doc_text, "doc missing no run-package boundary", authorized_payload)

    return {
        "ok": True,
        "patch": "L25.39",
        "manifest_validation_broad_checkpoint": authorized_payload.get("manifest_validation_broad_checkpoint"),
        "manifest_validation_broad_checkpoint_from_source_payload_only": authorized_payload.get("manifest_validation_broad_checkpoint_from_source_payload_only"),
        "manifest_validation_broad_checkpoint_allowed": authorized_payload.get("manifest_validation_broad_checkpoint_allowed"),
        ...
        "broad_checkpoint_rejects_missing_malformed_source_rejection": malformed_payload.get("ok") is False,
        "artifact_path_opened_by_l25_39": authorized_payload.get("artifact_path_opened_by_l25_39"),
        "artifact_bytes_read_by_l25_39": authorized_payload.get("artifact_bytes_read_by_l25_39"),
        "zip_member_opened_by_l25_39": authorized_payload.get("zip_member_opened_by_l25_39"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="L25.39 brief validator")
