...
from patchops.chatgpt_uploader.canonical_upload_path_stage import run_canonical_open_write_path_no_open, write_upload_path_stage_evidence
from patchops.chatgpt_uploader.config import resolve_config_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Canonical picker opener + exact path writer. Does not press Open or send.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence_dir = Path(args.evidence_dir).expanduser().resolve() if args.evidence_dir else repo_root / "data" / "runtime" / "chatgpt_uploader" / "u2_05_open_write_path_no_open"
    result = run_canonical_open_write_path_no_open(
    ...
    json_path, txt_path = write_upload_path_stage_evidence(result, evidence_dir)
    print(f"PATCHOPS_UPLOADER_OPEN_WRITE_PATH_STATUS: {result.status}")
    print(f"REPORT_PATH: {result.report_path or ''}")
    print(f"TRIGGER_STATUS: {result.trigger_status}")
    print(f"PATH_WRITE_STATUS: {result.path_write_status}")
    print(f"PATH_WRITTEN: {str(result.path_written).lower()}")
    ...
    return 0 if result.status in {"PASS_PATH_WRITTEN_NO_OPEN", "PASS_DRY_RUN_NO_PICKER_OPEN"} else 2
