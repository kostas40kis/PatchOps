...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_discovery_aggregate_gate_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_discovery_aggregate_gate_cli_readback.md")
TEST_PATH = Path("tests/test_l6_06_edge_executable_discovery_aggregate_gate_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l6_06_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L6.6 passive Microsoft Edge executable-discovery aggregate gate CLI/readback.

This wraps the accepted L6.5/L6.5a aggregate gate in a CLI/readback checkpoint.
It keeps compact JSON readback enabled and intentionally returns only compact
source-chain status, not full nested summaries.
...
from patchops.llm_browser import live_adapter_edge_executable_discovery_aggregate_gate as l6_05

PATCH = "L6.6"
PHASE = "L6"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L6.6 Microsoft Edge Executable Discovery Aggregate Gate CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-discovery-aggregate-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-discovery-aggregate-gate"
NEXT_PATCH = "L6.7 Live adapter Microsoft Edge executable discovery broad validation checkpoint"
SIDE_EFFECT_BOUNDARY = "edge-executable-discovery-aggregate-gate-cli-readback-only"
...
def _source_l6_05_safe(summary: Mapping[str, Any]) -> bool:
    return (
        summary.get("patch") == "L6.5"
        ...
        and summary.get("edge_executable_discovery_aggregate_gate_enforced") is True
        ...
        and "source_l6_04_summary" not in summary
    )
...
def build_edge_executable_discovery_aggregate_gate_cli_readback(
    ...
    *,
    ...
    source = l6_05.build_edge_executable_discovery_aggregate_gate(
        root,
    ...
    checks = [
        _check("l6_05_executable_discovery_aggregate_gate_still_passes", _source_l6_05_safe(source), _compact_source_status(source)),
        _check("l6_05_executable_discovery_aggregate_gate_remains_passive", _source_l6_05_safe(source), {"exe_probe": source.get("edge_executable_filesystem_probe_performed"), "browser_started": source.get("browser_started")}),
        _check("l6_06_aggregate_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l6_05_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l6_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l6_docs_contain_aggregate_readback_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        ...
        _check("executable_discovery_aggregate_readback_is_enforced", source.get("edge_executable_discovery_aggregate_gate_enforced") is True, {}),
        ...
        _check("no_browser_profile_or_adapter_side_effects", _source_l6_05_safe(source), {}),
    ]
    ...
    return {
        ...
        "l6_05_executable_discovery_aggregate_gate_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l6_05_status": _compact_source_status(source),
        "source_chain_status": source.get("source_chain_status", {}),
        "edge_executable_discovery_aggregate_readback_enforced": True,
        "edge_executable_discovery_aggregate_gate_enforced": source.get("edge_executable_discovery_aggregate_gate_enforced"),
        "edge_executable_discovery_aggregate_status": source.get("edge_executable_discovery_aggregate_status"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_discovery_aggregate_gate_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L6.6 Microsoft Edge executable discovery aggregate gate CLI/readback
...
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-discovery-aggregate-gate"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l6_06_candidate"
...
def test_l6_06_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l6_06_aggregate_gate_readback_dedicated_profile_passes_passively_and_compactly() -> None:
    ...
    assert payload["next_patch"] == "L6.7 Live adapter Microsoft Edge executable discovery broad validation checkpoint"
    assert payload["source_l6_05_status"]["patch"] == "L6.5"
    assert payload["source_l6_05_status"]["ok"] is True
    assert "source_chain_status" in payload
    _assert_passive(payload)


def test_l6_06_default_and_missing_profile_cases_are_passive() -> None:
    default_payload = readback.build_edge_executable_discovery_aggregate_gate_cli_readback(
        PROJECT_ROOT,
        allow_live_start=True,
        profile_dir=DEFAULT_PROFILE,
    )
    missing_payload = readback.build_edge_executable_discovery_aggregate_gate_cli_readback(
        PROJECT_ROOT,
        allow_live_start=True,
        profile_dir=None,
    )
    assert default_payload["ok"] is True
    assert default_payload["source_l6_05_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l6_05_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l6_06_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l6_06_doc_mentions_aggregate_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_discovery_aggregate_gate_cli_readback.md"
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L6.6 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L6.6 command wrapper appended")
    else:
        print("L6.6 command wrapper already present")


def _append_l6_05_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_discovery_aggregate_gate.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L6.6 Live adapter Microsoft Edge executable discovery aggregate gate CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable discovery aggregate gate:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L6.5 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l6_05_doc_pointer()
    print("L6.6 files written; validation is intentionally brief and compact")
    return 0
