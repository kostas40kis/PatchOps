param(
    ...
    [string]$PointerPath = '',
    [switch]$PlanOnly,
    [switch]$SkipBroadValidation,
    [switch]$SkipFullPytest,
    [int]$TimeoutShortSeconds = 120,
    [int]$TimeoutBroadValidationSeconds = 2400
)
...
# Docs-contract compatibility literal: llm-browser_broad_validation.ps1
# Actual operator script path remains scripts\llm_browser_broad_validation.ps1.
...
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    ...
    $ReportPath = Join-Path $ReportFolder ("patchops_llm_browser_closeout_checkpoint_{0}.txt" -f $stamp)
}

if ([string]::IsNullOrWhiteSpace($PointerPath)) {
    $PointerPath = Join-Path $desktop 'patchops_latest_llm_browser_closeout_checkpoint.txt'
}
...
Add-Section 'PATCHOPS LLM-BROWSER CLOSEOUT VALIDATION AND PUSH CHECKPOINT'
...
Add-Line ("SkipBroadValidation : {0}" -f ([bool]$SkipBroadValidation))
Add-Line ("SkipFullPytest      : {0}" -f ([bool]$SkipFullPytest))
Add-Line ''
Add-Line 'Safety: this checkpoint does not run git commit or git push. It only prints commands for the operator after validation evidence is collected.'

Add-Section 'PLAN'
Add-Line '1. Optionally run the broad-validation script.'
Add-Line '2. Parse the latest broad-validation report.'
Add-Line '3. Run release-gate and checkpoint JSON smokes.'
Add-Line '4. Capture git status.'
Add-Line '5. Print commit and push commands for the operator.'
Add-Line ''
Add-Line 'Default report locations:'
Add-Line ("- Broad validation pointer : {0}" -f $broadPointerPath)
Add-Line ("- Closeout pointer         : {0}" -f $PointerPath)

if (-not $PlanOnly) {
    if (-not $SkipBroadValidation) {
        $broadArgs = @(
            '-NoProfile',
            '-ExecutionPolicy',
            'Bypass',
            '-File',
            (Join-Path $resolvedRepoRoot 'scripts\llm_browser_broad_validation.ps1'),
            '-RepoRoot',
            $resolvedRepoRoot
        )
        if ($SkipFullPytest) {
            $broadArgs += '-SkipFullPytest'
        }
        Invoke-NativeCapture -Label 'operator broad validation' -FilePath $psFile -Arguments $broadArgs -WorkingDirectory $resolvedRepoRoot -TimeoutSeconds $TimeoutBroadValidationSeconds | Out-Null
    } else {
        Add-Section 'COMMAND: operator broad validation'
        Add-Line 'Skipped because -SkipBroadValidation was provided.'
    }
...
Add-Section 'OPERATOR COMMIT AND PUSH COMMANDS'
Add-Line 'Run these only after this closeout checkpoint report is PASS and you have reviewed the broad-validation report.'
Add-Line ''
...
Add-Line ("PointerPath       : {0}" -f $PointerPath)
Add-Line ''
Add-Line 'Reminder: this checkpoint never commits or pushes automatically.'
...
$pointerLines = @(
    'Latest PatchOps LLM-browser closeout checkpoint report',
