...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate.md")
TEST_PATH = Path("tests/test_l11_05_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_current.py")
VALIDATE_PATH = Path("scripts/patch_l11_05_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L11.5 passive Microsoft Edge executable filesystem-probe real-probe preflight aggregate gate.

Aggregates the accepted L11.1 through L11.4 real-probe preflight stack.
Real-probe preflight readiness may be true when all required gates are present,
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_cli_readback as l11_04

PATCH = "L11.5"
PHASE = "L11"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L11.5 Microsoft Edge Executable Filesystem Probe Real-Probe Preflight Aggregate Gate"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-aggregate-gate"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-fixture-matrix-readback"
NEXT_PATCH = "L11.6 Live adapter Microsoft Edge executable filesystem probe real-probe preflight aggregate gate CLI/readback"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-real-probe-preflight-aggregate-gate-only"
...
REQUIRED_REPO_PATHS = (
    ...
    "docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate.md",
    "tests/test_l11_04_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_cli_readback_current.py",
    "tests/test_l11_05_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_current.py",
    "scripts/patch_l11_04_brief_validate.py",
    "scripts/patch_l11_05_brief_validate.py",
)
...
DOC_REQUIREMENTS = {
    ...
    "docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate.md": (
        "L11.5 Microsoft Edge executable filesystem probe real-probe preflight aggregate gate",
        COMMAND_NAME,
        SOURCE_COMMAND_NAME,
        ...
        "L11.6 Live adapter Microsoft Edge executable filesystem probe real-probe preflight aggregate gate CLI/readback",
    ),
}
...
def _source_l11_04_safe(source: Mapping[str, Any], *, expected_activation_ready: bool, expected_real_ready: bool) -> bool:
    fixture_ids = set(source.get("real_probe_preflight_fixture_ids", []))
    return (
        source.get("patch") == "L11.4"
        ...
        and "source_l11_04_summary" not in source
...
def _aggregate_state(source: Mapping[str, Any]) -> dict[str, Any]:
    chain_acceptance = {
        "l11_01": source.get("edge_executable_filesystem_probe_real_probe_preflight_contract_enforced") is True,
        "l11_02": source.get("edge_executable_filesystem_probe_real_probe_preflight_cli_readback_enforced") is True,
        "l11_03": source.get("edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_enforced") is True,
        "l11_04": source.get("ok") is True and source.get("status") == STATUS_PASS,
    }
    return {
        "edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_enforced": True,
        "real_probe_preflight_aggregate_status": "MODELED_ONLY_BLOCKED_BY_PHASE",
        "real_probe_preflight_aggregate_reason": "L11.5 aggregates L11.1-L11.4 while real filesystem probing remains reserved for a later accepted phase.",
        "l11_01_real_probe_preflight_contract_ok": chain_acceptance["l11_01"],
        "l11_02_real_probe_preflight_cli_readback_ok": chain_acceptance["l11_02"],
        "l11_03_real_probe_preflight_fixture_matrix_ok": chain_acceptance["l11_03"],
        "l11_04_real_probe_preflight_fixture_matrix_cli_readback_ok": chain_acceptance["l11_04"],
        "chain_acceptance": chain_acceptance,
        "fixture_count": source.get("real_probe_preflight_fixture_count"),
        "fixture_ids": source.get("real_probe_preflight_fixture_ids"),
        "activation_ready": source.get("filesystem_probe_activation_ready"),
        "real_probe_authorization_present": source.get("real_filesystem_probe_authorization_present"),
        "real_preflight_ready": source.get("real_filesystem_probe_preflight_ready"),
        "real_probe_execution_allowed": False,
...
def build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(
    ...
    *,
    ...
    source = l11_04.build_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_cli_readback(
        root,
    ...
    source_safe = _source_l11_04_safe(source, expected_activation_ready=expected_activation_ready, expected_real_ready=expected_real_ready)
    ...
    checks = [
        _check("l11_04_real_probe_fixture_matrix_readback_still_passes", source_safe, _compact_source_status(source)),
        _check("l11_04_real_probe_fixture_matrix_readback_remains_passive", source_safe, {"real_ready": source.get("real_filesystem_probe_preflight_ready"), "real_exec": source.get("real_filesystem_probe_execution_allowed"), "fs_probe": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l11_01_through_l11_04_chain_remains_accepted", chain_ok, aggregate["chain_acceptance"]),
        _check("l11_05_real_probe_preflight_aggregate_gate_command_registered", command_state.get("ok") is True, command_state),
        _check("l11_04_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l11_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l11_docs_contain_real_probe_preflight_aggregate_gate_boundary", doc_state.get("ok") is True, doc_state),
        _check("real_probe_preflight_aggregate_gate_is_enforced", aggregate["edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_enforced"] is True, aggregate),
        ...
        _check("real_probe_execution_remains_blocked_by_phase", source.get("real_filesystem_probe_execution_allowed") is False and aggregate["real_probe_execution_allowed"] is False, {}),
    ...
    return {
        ...
        "l11_real_probe_preflight_chain_accepted": chain_ok,
        "l11_01_real_probe_preflight_contract_remains_accepted": aggregate["l11_01_real_probe_preflight_contract_ok"],
        "l11_02_real_probe_preflight_cli_readback_remains_accepted": aggregate["l11_02_real_probe_preflight_cli_readback_ok"],
        "l11_03_real_probe_preflight_fixture_matrix_remains_accepted": aggregate["l11_03_real_probe_preflight_fixture_matrix_ok"],
        "l11_04_real_probe_preflight_fixture_matrix_cli_readback_remains_accepted": aggregate["l11_04_real_probe_preflight_fixture_matrix_cli_readback_ok"],
        "source_l11_04_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_real_probe_preflight_aggregate": aggregate,
        "edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_enforced": aggregate["edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_enforced"],
        "real_probe_preflight_aggregate_status": aggregate["real_probe_preflight_aggregate_status"],
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L11 Chain OK    : {payload.get('l11_real_probe_preflight_chain_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(
        args.repo_root,
...
DOC_CONTENT = r'''# L11.5 Microsoft Edge executable filesystem probe real-probe preflight aggregate gate

L11.5 adds a passive aggregate gate over the accepted L11 real-probe preflight stack.
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate as aggregate

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-aggregate-gate"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-fixture-matrix-readback"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l11_05_candidate"
...
def test_l11_05_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l11_05_aggregate_not_ready_without_all_required_gates() -> None:
    for kwargs in [
        ...
        payload = aggregate.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(PROJECT_ROOT, **kwargs)
        assert payload["ok"] is True
        assert payload["patch"] == "L11.5"
...
def test_l11_05_real_preflight_ready_but_execution_still_blocked() -> None:
    payload = aggregate.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(
        PROJECT_ROOT,
    ...
    assert payload["real_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L11.6 Live adapter Microsoft Edge executable filesystem probe real-probe preflight aggregate gate CLI/readback"
    _assert_passive(payload)


def test_l11_05_default_and_missing_profile_cases_are_passive_when_real_preflight_ready() -> None:
    default_payload = aggregate.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True)
    missing_payload = aggregate.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l11_04_status"]["ok"] is True
    ...
    assert missing_payload["source_l11_04_status"]["ok"] is True
    assert missing_payload["real_filesystem_probe_preflight_ready"] is True
    _assert_passive(missing_payload)


def test_l11_05_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l11_05_doc_mentions_real_probe_aggregate_gate_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-aggregate-gate"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l11_05_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l11_real_probe_preflight_chain_accepted"] is True
    assert payload["edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_enforced"] is True
    assert payload["real_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_gates = aggregate.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False)
    assert_passive(no_gates)
    assert no_gates["real_filesystem_probe_preflight_ready"] is False
    print("PASS no_gates: patch=L11.5 chain=True aggregate=True real_ready=False real_exec=False fs_probe=False path_selected=False")

    real_flag_only = aggregate.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=True)
    assert_passive(real_flag_only)
    assert real_flag_only["real_filesystem_probe_preflight_ready"] is False
    print("PASS real_flag_only: chain=True aggregate=True real_ready=False real_exec=False fs_probe=False path_selected=False")

    activation_without_real = aggregate.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=False)
    assert_passive(activation_without_real)
    assert activation_without_real["real_filesystem_probe_preflight_ready"] is False
    print("PASS activation_ready_without_real_flag: chain=True aggregate=True activation_ready=True real_ready=False real_exec=False fs_probe=False path_selected=False")
    ...
    for name, profile in [("real_preflight_ready_dedicated", DEDICATED), ("real_preflight_ready_default", DEFAULT), ("real_preflight_ready_missing_profile", None)]:
        ...
        assert payload["real_filesystem_probe_preflight_ready"] is True
        print("PASS {0}: patch=L11.5 source=L11.4 fixtures=6 real_ready=True real_exec=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; real-probe preflight aggregate gate stayed passive")
    print("PASS L11.5 brief validation: real-probe preflight aggregate gate accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L11.5 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L11.5 command wrapper appended")
    else:
        print("L11.5 command wrapper already present")


def _append_l11_04_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L11.5 Live adapter Microsoft Edge executable filesystem probe real-probe preflight aggregate gate"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the real-probe preflight fixture matrix CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L11.4 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l11_04_doc_pointer()
    print("L11.5 files written; validation is intentionally brief and compact")
    return 0
