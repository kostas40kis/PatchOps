...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_broad_validation_checkpoint.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_broad_validation_checkpoint.md")
TEST_PATH = Path("tests/test_l9_07_edge_executable_filesystem_probe_broad_validation_checkpoint_current.py")
VALIDATE_PATH = Path("scripts/patch_l9_07_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L9.7 passive Microsoft Edge executable filesystem-probe broad validation checkpoint.

Wraps the accepted L9.6 executable filesystem-probe aggregate-gate CLI/readback
and proves L9.1 through L9.6 remain accepted. The L9 stack remains
model/readback only: no authorization keeps contract_ready=false, authorized
cases can report contract_ready=true, and filesystem probing remains blocked by
phase.
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_aggregate_gate_cli_readback as l9_06

PATCH = "L9.7"
PHASE = "L9"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L9.7 Microsoft Edge Executable Filesystem Probe Broad Validation Checkpoint"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-broad-validation-checkpoint"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-aggregate-readback"
NEXT_PATCH = "L9.8 Live adapter Microsoft Edge executable filesystem probe final acceptance marker"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-broad-validation-checkpoint-only"
...
def _source_l9_06_safe(summary: Mapping[str, Any], *, expect_contract_ready: bool, expect_preflight_passed: bool) -> bool:
...
def _chain_acceptance(source: Mapping[str, Any]) -> dict[str, bool]:
    return {
        ...
        "l9_06": source.get("ok") is True and source.get("status") == STATUS_PASS,
    }


def build_edge_executable_filesystem_probe_broad_validation_checkpoint(
    ...
    *,
    ...
    source = l9_06.build_edge_executable_filesystem_probe_aggregate_gate_cli_readback(
        root,
    ...
    source_safe = _source_l9_06_safe(source, expect_contract_ready=expected_contract_ready, expect_preflight_passed=expected_preflight_passed)
    ...
    checks = [
        _check("l9_06_filesystem_probe_aggregate_readback_still_passes", source_safe, _compact_source_status(source)),
        _check("l9_06_filesystem_probe_aggregate_readback_remains_passive", source_safe, {"contract_ready": source.get("edge_executable_filesystem_probe_contract_ready"), "fs_probe": source.get("edge_executable_filesystem_probe_performed"), "path_selected": source.get("edge_executable_path_selected")}),
        _check("l9_01_through_l9_06_chain_remains_accepted", chain_ok, acceptance),
        _check("l9_07_filesystem_probe_broad_checkpoint_command_registered", command_state.get("ok") is True, command_state),
        _check("l9_06_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l9_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l9_docs_contain_filesystem_probe_broad_checkpoint_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("filesystem_probe_broad_validation_checkpoint_is_enforced", chain_ok and source.get("edge_executable_filesystem_probe_aggregate_gate_cli_readback_enforced") is True, acceptance),
    ...
    return {
        ...
        "source_l9_06_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_broad_validation_checkpoint_enforced": True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_broad_validation_checkpoint(
        args.repo_root,
...
DOC_CONTENT = r'''# L9.7 Microsoft Edge executable filesystem probe broad validation checkpoint
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-broad-validation-checkpoint"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l9_07_candidate"
...
def main() -> int:
    no_auth = checkpoint.build_edge_executable_filesystem_probe_broad_validation_checkpoint(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    ...
    print("PASS no_probe_auth: patch=L9.7 broad_ok=True chain=True preflight_passed=False contract_ready=False fs_probe=False path_selected=False")
    ...
    for name, live, profile, probe_auth in cases:
        ...
        assert payload["edge_executable_filesystem_probe_contract_ready"] is True
        print(
            "PASS {0}: patch={1} source=L9.6 chain={2} fixtures={3} preflight_passed={4} contract_ready={5} fs_probe={6} path_selected={7}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; filesystem probe broad checkpoint stayed passive")
    print("PASS L9.7 brief validation: executable filesystem probe broad checkpoint accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L9.7 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L9.7 command wrapper appended")
    else:
        print("L9.7 command wrapper already present")


def _append_l9_06_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_aggregate_gate_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L9.7 Live adapter Microsoft Edge executable filesystem probe broad validation checkpoint"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe aggregate CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L9.6 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l9_06_doc_pointer()
    print("L9.7 files written; validation is intentionally brief and compact")
    return 0
