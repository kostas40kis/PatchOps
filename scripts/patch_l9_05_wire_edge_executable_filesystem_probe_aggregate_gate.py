...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_aggregate_gate.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_aggregate_gate.md")
TEST_PATH = Path("tests/test_l9_05_edge_executable_filesystem_probe_aggregate_gate_current.py")
VALIDATE_PATH = Path("scripts/patch_l9_05_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L9.5 passive Microsoft Edge executable filesystem-probe aggregate gate.

Aggregates the accepted L9.1 through L9.4 executable filesystem-probe surfaces.
The L9 stack remains model/readback only: no authorization keeps
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_fixture_matrix_cli_readback as l9_04

PATCH = "L9.5"
PHASE = "L9"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L9.5 Microsoft Edge Executable Filesystem Probe Aggregate Gate"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-aggregate-gate"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-fixture-matrix-readback"
NEXT_PATCH = "L9.6 Live adapter Microsoft Edge executable filesystem probe aggregate gate CLI/readback"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-aggregate-gate-only"
...
REQUIRED_REPO_PATHS = (
    ...
    "docs/llm_browser_live_adapter_edge_executable_filesystem_probe_fixture_matrix_cli_readback.md",
    ...
    "tests/test_l9_04_edge_executable_filesystem_probe_fixture_matrix_cli_readback_current.py",
    ...
    "scripts/patch_l9_04_brief_validate.py",
...
def _source_l9_04_safe(summary: Mapping[str, Any], *, expect_contract_ready: bool, expect_preflight_passed: bool) -> bool:
    ...
    return (
        ...
        and "source_l9_03_summary" not in summary
    )
...
def _aggregate_state(source: Mapping[str, Any]) -> dict[str, Any]:
    # L9.4 proves L9.3, L9.3 proves L9.2, and L9.2 proves L9.1 in this additive stack.
    chain_acceptance = {
        "l9_01": source.get("edge_executable_filesystem_probe_contract_enforced") is True,
        "l9_02": source.get("edge_executable_filesystem_probe_cli_readback_enforced") is True,
        "l9_03": source.get("l9_03_executable_filesystem_probe_fixture_matrix_remains_accepted") is True,
        "l9_04": source.get("ok") is True and source.get("status") == STATUS_PASS,
    }
    return {
        "edge_executable_filesystem_probe_aggregate_gate_enforced": True,
        "edge_executable_filesystem_probe_aggregate_status": "MODELED_ONLY_BLOCKED_BY_PHASE",
        "edge_executable_filesystem_probe_aggregate_reason": "L9.5 aggregates L9.1-L9.4 while real filesystem probing remains reserved for a later accepted phase.",
        "l9_01_executable_filesystem_probe_passive_contract_ok": chain_acceptance["l9_01"],
        "l9_02_executable_filesystem_probe_cli_readback_ok": chain_acceptance["l9_02"],
        "l9_03_executable_filesystem_probe_fixture_matrix_ok": chain_acceptance["l9_03"],
        "l9_04_executable_filesystem_probe_fixture_matrix_cli_readback_ok": chain_acceptance["l9_04"],
        "chain_acceptance": chain_acceptance,
        "fixture_count": source.get("edge_executable_filesystem_probe_fixture_count"),
        "fixture_ids": source.get("edge_executable_filesystem_probe_fixture_ids"),
        "candidate_path_count": source.get("edge_executable_candidate_path_count"),
        "candidate_path_labels": source.get("edge_executable_candidate_path_labels"),
        "authorization_flag": EXECUTABLE_PROBE_AUTHORIZATION_FLAG,
        "contract_ready": source.get("edge_executable_filesystem_probe_contract_ready"),
        "preflight_passed": source.get("edge_executable_probe_safety_preflight_passed"),
        "filesystem_probe_execution_allowed": False,
...
def build_edge_executable_filesystem_probe_aggregate_gate(
    ...
    *,
    ...
    source = l9_04.build_edge_executable_filesystem_probe_fixture_matrix_cli_readback(
        root,
    ...
    source_safe = _source_l9_04_safe(source, expect_contract_ready=expected_contract_ready, expect_preflight_passed=expected_preflight_passed)
    ...
    checks = [
        _check("l9_04_filesystem_probe_fixture_matrix_readback_still_passes", source_safe, _compact_source_status(source)),
        _check("l9_04_filesystem_probe_fixture_matrix_readback_remains_passive", source_safe, {"contract_ready": source.get("edge_executable_filesystem_probe_contract_ready"), "fs_probe": source.get("edge_executable_filesystem_probe_performed"), "path_selected": source.get("edge_executable_path_selected")}),
        _check("l9_01_through_l9_04_chain_remains_accepted", chain_ok, aggregate["chain_acceptance"]),
        _check("l9_05_filesystem_probe_aggregate_gate_command_registered", command_state.get("ok") is True, command_state),
        _check("l9_04_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l9_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l9_docs_contain_filesystem_probe_aggregate_gate_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("filesystem_probe_aggregate_gate_is_enforced", aggregate["edge_executable_filesystem_probe_aggregate_gate_enforced"] is True, aggregate),
        ...
        _check("filesystem_probe_execution_remains_blocked_by_phase", source.get("edge_executable_filesystem_probe_execution_allowed") is False and aggregate["filesystem_probe_execution_allowed"] is False, {}),
    ...
    return {
        ...
        "l9_filesystem_probe_chain_accepted": chain_ok,
        "l9_01_executable_filesystem_probe_passive_contract_remains_accepted": aggregate["l9_01_executable_filesystem_probe_passive_contract_ok"],
        "l9_02_executable_filesystem_probe_cli_readback_remains_accepted": aggregate["l9_02_executable_filesystem_probe_cli_readback_ok"],
        "l9_03_executable_filesystem_probe_fixture_matrix_remains_accepted": aggregate["l9_03_executable_filesystem_probe_fixture_matrix_ok"],
        "l9_04_executable_filesystem_probe_fixture_matrix_cli_readback_remains_accepted": aggregate["l9_04_executable_filesystem_probe_fixture_matrix_cli_readback_ok"],
        "source_l9_04_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_aggregate": aggregate,
        "edge_executable_filesystem_probe_aggregate_gate_enforced": aggregate["edge_executable_filesystem_probe_aggregate_gate_enforced"],
        "edge_executable_filesystem_probe_aggregate_status": aggregate["edge_executable_filesystem_probe_aggregate_status"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_aggregate_gate(
        args.repo_root,
...
DOC_CONTENT = r'''# L9.5 Microsoft Edge executable filesystem probe aggregate gate
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_aggregate_gate as gate

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-aggregate-gate"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-fixture-matrix-readback"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l9_05_candidate"
...
def _assert_passive(payload: dict) -> None:
    ...
    assert "source_l9_04_summary" not in payload


def test_l9_05_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l9_05_no_auth_aggregate_gate_contract_not_ready_and_no_probe() -> None:
    payload = gate.build_edge_executable_filesystem_probe_aggregate_gate(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_filesystem_probe_aggregate_gate_enforced"] is True
...
def test_l9_05_authorized_aggregate_gate_contract_ready_but_filesystem_probe_stays_blocked() -> None:
    payload = gate.build_edge_executable_filesystem_probe_aggregate_gate(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L9.6 Live adapter Microsoft Edge executable filesystem probe aggregate gate CLI/readback"
    _assert_passive(payload)


def test_l9_05_default_and_missing_profile_cases_are_passive_when_authorized() -> None:
    default_payload = gate.build_edge_executable_filesystem_probe_aggregate_gate(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = gate.build_edge_executable_filesystem_probe_aggregate_gate(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l9_04_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l9_04_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l9_05_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l9_05_doc_mentions_filesystem_probe_aggregate_gate_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_aggregate_gate.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-aggregate-gate"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l9_05_candidate"
...
def main() -> int:
    no_auth = gate.build_edge_executable_filesystem_probe_aggregate_gate(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    ...
    print("PASS no_probe_auth: patch=L9.5 aggregate_ok=True chain=True preflight_passed=False contract_ready=False fs_probe=False path_selected=False")
    ...
    for name, live, profile, probe_auth in cases:
        ...
        assert payload["edge_executable_filesystem_probe_contract_ready"] is True
        print(
            "PASS {0}: patch={1} source=L9.4 chain={2} fixtures={3} preflight_passed={4} contract_ready={5} fs_probe={6} path_selected={7}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; filesystem probe aggregate gate stayed passive")
    print("PASS L9.5 brief validation: executable filesystem probe aggregate gate accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L9.5 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L9.5 command wrapper appended")
    else:
        print("L9.5 command wrapper already present")


def _append_l9_04_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_fixture_matrix_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L9.5 Live adapter Microsoft Edge executable filesystem probe aggregate gate"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe fixture matrix CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L9.4 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l9_04_doc_pointer()
    print("L9.5 files written; validation is intentionally brief and compact")
    return 0
