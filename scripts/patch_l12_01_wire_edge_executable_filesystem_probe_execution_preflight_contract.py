...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_execution_preflight_contract.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_contract.md")
TEST_PATH = Path("tests/test_l12_01_edge_executable_filesystem_probe_execution_preflight_contract_current.py")
VALIDATE_PATH = Path("scripts/patch_l12_01_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L12.1 passive Microsoft Edge executable filesystem-probe execution preflight contract.

Starts the L12 execution-preflight layer after the accepted L11 real-probe
preflight stack. This patch introduces a separate explicit execution-preflight
flag, but it still performs no filesystem probe, no path selection, and no
launch.
"""
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker as l11_08

PATCH = "L12.1"
PHASE = "L12"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L12.1 Microsoft Edge Executable Filesystem Probe Execution Preflight Contract"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-contract"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-final-acceptance-marker"
NEXT_PATCH = "L12.2 Live adapter Microsoft Edge executable filesystem probe execution preflight CLI/readback"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-execution-preflight-contract-only"
...
def _compact_source_status(source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack_accepted": source.get("l11_real_probe_preflight_stack_accepted") is True,
...
def _source_l11_08_safe(source: Mapping[str, Any], *, expected_real_ready: bool) -> bool:
    fixture_ids = set(source.get("real_probe_preflight_fixture_ids", []))
    return (
        source.get("patch") == "L11.8"
        ...
        and source.get("edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker_enforced") is True
...
def _execution_preflight_state(*, real_preflight_ready: bool, allow_executable_filesystem_probe_execution: bool) -> dict[str, Any]:
    execution_ready = bool(real_preflight_ready and allow_executable_filesystem_probe_execution)
    return {
        "edge_executable_filesystem_probe_execution_preflight_contract_enforced": True,
        "edge_executable_filesystem_probe_execution_preflight_modeled": True,
        "execution_preflight_flag": EXECUTION_PREFLIGHT_FLAG,
        "execution_preflight_authorization_present": bool(allow_executable_filesystem_probe_execution),
        "execution_preflight_ready": execution_ready,
        "execution_preflight_execution_allowed": False,
        "execution_preflight_status": "MODELED_ONLY_BLOCKED_BY_PHASE",
        "execution_preflight_blocked_reason": "L12.1 defines execution preflight only; actual filesystem probing is reserved for a later accepted patch.",
        "execution_preflight_requires_l11_stack": True,
        "execution_preflight_requires_live_start_authorization": True,
        "execution_preflight_requires_executable_probe_authorization": True,
        "execution_preflight_requires_activation_flag": True,
        "execution_preflight_requires_real_probe_flag": True,
        "execution_preflight_requires_execution_flag": True,
        "execution_preflight_does_not_probe": True,
    }


def build_edge_executable_filesystem_probe_execution_preflight_contract(
    ...
    *,
    ...
    source = l11_08.build_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker(
        root,
    ...
    source_safe = _source_l11_08_safe(source, expected_real_ready=expected_real_ready)
    execution_preflight = _execution_preflight_state(
        real_preflight_ready=bool(source.get("real_filesystem_probe_preflight_ready")),
    ...
    checks = [
        _check("l11_08_real_probe_preflight_final_marker_still_passes", source_safe, _compact_source_status(source)),
        _check("l11_08_real_probe_preflight_final_marker_remains_passive", source_safe, {"real_ready": source.get("real_filesystem_probe_preflight_ready"), "real_exec": source.get("real_filesystem_probe_execution_allowed"), "fs_probe": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l12_01_execution_preflight_command_registered", command_state.get("ok") is True, command_state),
        _check("l11_08_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l12_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l12_docs_contain_execution_preflight_boundary", doc_state.get("ok") is True, doc_state),
        _check("execution_preflight_contract_is_enforced", execution_preflight["edge_executable_filesystem_probe_execution_preflight_contract_enforced"] is True, execution_preflight),
        _check("execution_preflight_ready_requires_execution_flag", execution_preflight["execution_preflight_ready"] is bool(expected_real_ready and allow_executable_filesystem_probe_execution), execution_preflight),
        _check("execution_preflight_execution_remains_blocked_by_phase", execution_preflight["execution_preflight_execution_allowed"] is False, execution_preflight),
    ...
    return {
        ...
        "l11_08_real_probe_preflight_final_acceptance_marker_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l11_08_status": _compact_source_status(source),
        **execution_preflight,
        "edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker_enforced": source.get("edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker_enforced"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_execution_preflight_contract(
        args.repo_root,
...
DOC_CONTENT = r'''# L12.1 Microsoft Edge executable filesystem probe execution preflight contract
...
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-final-acceptance-marker"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l12_01_candidate"
...
def test_l12_01_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l12_01_execution_preflight_not_ready_without_execution_flag() -> None:
...
def test_l12_01_execution_flag_alone_is_not_ready() -> None:
    payload = preflight.build_edge_executable_filesystem_probe_execution_preflight_contract(
        PROJECT_ROOT,
...
def test_l12_01_execution_preflight_ready_but_execution_still_blocked() -> None:
    payload = preflight.build_edge_executable_filesystem_probe_execution_preflight_contract(
        PROJECT_ROOT,
        allow_live_start=True,
        profile_dir=DEDICATED_PROFILE,
    ...
    assert payload["l11_08_real_probe_preflight_final_acceptance_marker_remains_accepted"] is True
    assert payload["real_filesystem_probe_preflight_ready"] is True
    assert payload["execution_preflight_ready"] is True
    assert payload["execution_preflight_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L12.2 Live adapter Microsoft Edge executable filesystem probe execution preflight CLI/readback"
    _assert_passive(payload)


def test_l12_01_default_and_missing_profile_cases_are_passive_when_execution_preflight_ready() -> None:
    default_payload = preflight.build_edge_executable_filesystem_probe_execution_preflight_contract(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=True)
    missing_payload = preflight.build_edge_executable_filesystem_probe_execution_preflight_contract(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l11_08_status"]["ok"] is True
    assert default_payload["execution_preflight_ready"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l11_08_status"]["ok"] is True
    assert missing_payload["execution_preflight_ready"] is True
    _assert_passive(missing_payload)


def test_l12_01_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l12_01_doc_mentions_execution_preflight_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_contract.md"
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L12.1 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L12.1 command wrapper appended")
    else:
        print("L12.1 command wrapper already present")


def _append_l11_08_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L12.1 Live adapter Microsoft Edge executable filesystem probe execution preflight contract"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the real-probe preflight stack:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L11.8 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l11_08_doc_pointer()
    print("L12.1 files written; validation is intentionally brief and compact")
    return 0
