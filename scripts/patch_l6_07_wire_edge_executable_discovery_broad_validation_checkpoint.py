...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_discovery_broad_validation_checkpoint.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_discovery_broad_validation_checkpoint.md")
TEST_PATH = Path("tests/test_l6_07_edge_executable_discovery_broad_validation_checkpoint_current.py")
VALIDATE_PATH = Path("scripts/patch_l6_07_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L6.7 passive Microsoft Edge executable-discovery broad validation checkpoint.
...
DOC_CONTENT = r'''# L6.7 Microsoft Edge executable discovery broad validation checkpoint
...
COMMAND = "browser-start-supervised-launch-edge-executable-discovery-broad-validation-checkpoint"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l6_07_candidate"
...
def main() -> int:
    ...
    for name, allow, profile in cases:
        ...
        print(
            "PASS {0}: patch={1} chain=L6.1-L6.6 compact_json={2} exe_probe={3}".format(
                name,
                payload["patch"],
    ...
    print("PASS L6.7 brief validation: broad checkpoint accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L6.7 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L6.7 command wrapper appended")
    else:
        print("L6.7 command wrapper already present")


def _append_l6_06_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_discovery_aggregate_gate_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L6.7 Live adapter Microsoft Edge executable discovery broad validation checkpoint"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable discovery aggregate CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L6.6 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l6_06_doc_pointer()
    print("L6.7 files written; validation is intentionally brief and compact")
    return 0
