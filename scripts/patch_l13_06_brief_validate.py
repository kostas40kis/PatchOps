...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-aggregate-gate-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-aggregate-gate"
...
def main() -> int:
    ...
    profile = root / "data" / "runtime" / "browser_profiles" / "edge_l13_06_brief"
    existing = root / "data" / "runtime" / "edge_probe_fixture" / "msedge_l13_06_brief.exe"
    missing = root / "data" / "runtime" / "edge_probe_fixture" / "missing-msedge-l13-06-brief.exe"
    existing.parent.mkdir(parents=True, exist_ok=True)
    existing.write_text("fixture executable placeholder", encoding="utf-8")

    commands: list[dict[str, Any]] = []
    commands.append(run_cmd([sys.executable, "-m", "compileall", "patchops/llm_browser", "tests/test_l13_05_edge_executable_filesystem_probe_execution_aggregate_gate_current.py", "tests/test_l13_06_edge_executable_filesystem_probe_execution_aggregate_gate_cli_readback_current.py", "scripts/patch_l13_06_brief_validate.py"], root))
    ...
    readback_payload = parsed[1] if len(parsed) >= 2 else {}
    ok = ok and source_payload.get("patch") == "L13.5"
    ok = ok and source_payload.get("source_patch") == "L13.4"
    ...
    ok = ok and readback_payload.get("patch") == "L13.6"
    ok = ok and readback_payload.get("source_patch") == "L13.5"
    ok = ok and readback_payload.get("source_l13_05_aggregate_gate_remains_accepted") is True
    ok = ok and readback_payload.get("edge_executable_filesystem_probe_performed") is True
    ok = ok and readback_payload.get("truthful_selected_path_existing_reported_candidate") is True
    ok = ok and readback_payload.get("filesystem_probe_only_when_l12_execution_preflight_ready") is True
    ok = ok and _candidate_truthful(readback_payload)
    ok = ok and _no_launch(readback_payload)
    ...
    summary = {
        ...
        "readback_patch": readback_payload.get("patch"),
        "source_probe_performed": source_payload.get("edge_executable_filesystem_probe_performed"),
        "readback_probe_performed": readback_payload.get("edge_executable_filesystem_probe_performed"),
        "readback_truthful_selection": readback_payload.get("truthful_selected_path_existing_reported_candidate"),
        "readback_no_launch": _no_launch(readback_payload) if readback_payload else False,
