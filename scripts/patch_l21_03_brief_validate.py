...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Brief L21.3 validation")
    parser.add_argument("--repo-root", default=str(_REPO_ROOT))
    parser.add_argument("--target-url", default="https://chatgpt.com/")
    args = parser.parse_args(argv)
    payload = l21_03.build_edge_downloaded_archive_validation_passive_plan_checkpoint(Path(args.repo_root).resolve(), target_url=args.target_url)
    summary = {
        "ok": payload.get("ok"),
        "patch": payload.get("patch"),
        "source_patch": payload.get("source_patch"),
        "source_l21_2_cli_readback_checkpoint_accepted": payload.get("source_l21_2_cli_readback_checkpoint_accepted"),
        "l21_2_complete": payload.get("l21_2_complete"),
        "l21_1_passive_preflight_gate_accepted": payload.get("l21_1_passive_preflight_gate_accepted"),
        "archive_candidate_definition_blocks_l21_3_archive_execution": payload.get("archive_candidate_definition_blocks_l21_3_archive_execution"),
        "future_archive_validation_plan_is_planned_not_executed": payload.get("future_archive_validation_plan_is_planned_not_executed"),
        "future_archive_validation_plan_blocks_archive_execution": payload.get("future_archive_validation_plan_blocks_archive_execution"),
