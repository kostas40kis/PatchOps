...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Brief L19.4 validation")
    parser.add_argument("--repo-root", default=str(_REPO_ROOT))
    parser.add_argument("--target-url", default="https://chatgpt.com/")
    args = parser.parse_args(argv)

    default_payload = l19_04.build_edge_downloaded_file_validation_controlled_authorization_gate(
        Path(args.repo_root).resolve(),
        target_url=args.target_url,
    )
    authorized_payload = l19_04.build_edge_downloaded_file_validation_controlled_authorization_gate(
        Path(args.repo_root).resolve(),
        allow_downloaded_file_validation_authorization=True,
        authorization_token=l19_04.REQUIRED_FILE_VALIDATION_AUTHORIZATION_TOKEN,
        target_url=args.target_url,
    )
    ok = (
        default_payload.get("ok") is True
        and authorized_payload.get("ok") is True
        and default_payload.get("downloaded_file_validation_authorized_for_future_phase") is False
        and authorized_payload.get("downloaded_file_validation_authorized_for_future_phase") is True
        and authorized_payload.get("downloaded_file_validation_authorization_is_readback_only_in_l19_4") is True
        and authorized_payload.get("controlled_downloaded_file_validation_execution_allowed") is False
    ...
    summary = {
        ...
        "source_l19_3_passive_plan_checkpoint_accepted": authorized_payload.get("source_l19_3_passive_plan_checkpoint_accepted"),
        "l19_3_complete": authorized_payload.get("l19_3_complete"),
        "default_downloaded_file_validation_authorized_for_future_phase": default_payload.get("downloaded_file_validation_authorized_for_future_phase"),
        "authorized_downloaded_file_validation_authorized_for_future_phase": authorized_payload.get("downloaded_file_validation_authorized_for_future_phase"),
        "downloaded_file_validation_authorization_is_readback_only_in_l19_4": authorized_payload.get("downloaded_file_validation_authorization_is_readback_only_in_l19_4"),
        "controlled_downloaded_file_validation_execution_allowed": authorized_payload.get("controlled_downloaded_file_validation_execution_allowed"),
        ...
        "future_authorization_gate_plan_blocks_file_validation_file_read_archive_package_run": authorized_payload.get("future_authorization_gate_plan_blocks_file_validation_file_read_archive_package_run"),
