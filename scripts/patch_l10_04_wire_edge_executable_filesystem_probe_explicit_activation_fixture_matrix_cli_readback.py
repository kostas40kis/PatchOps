...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback.md")
TEST_PATH = Path("tests/test_l10_04_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l10_04_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L10.4 passive Microsoft Edge executable filesystem-probe activation fixture-matrix CLI/readback.

Adds a compact CLI/readback layer over the accepted L10.3 activation fixture
matrix. The fixture matrix remains modeled only: readiness can be true for all
three activation gates, but filesystem probing, path selection, and launch stay
blocked by phase.
"""
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_explicit_activation_fixture_matrix as l10_03

PATCH = "L10.4"
PHASE = "L10"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L10.4 Microsoft Edge Executable Filesystem Probe Explicit Activation Fixture Matrix CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-fixture-matrix-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-fixture-matrix"
NEXT_PATCH = "L10.5 Live adapter Microsoft Edge executable filesystem probe explicit activation aggregate gate"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-explicit-activation-fixture-matrix-cli-readback-only"
...
def _source_l10_03_safe(source: Mapping[str, Any], *, expected_ready: bool) -> bool:
    fixture_ids = set(source.get("edge_executable_filesystem_probe_activation_fixture_ids", []))
    return (
        source.get("patch") == "L10.3"
...
def build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(
    ...
    *,
    ...
    source = l10_03.build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix(
        root,
    ...
    source_safe = _source_l10_03_safe(source, expected_ready=expected_ready)
    ...
    checks = [
        _check("l10_03_activation_fixture_matrix_still_passes", source_safe, _compact_source_status(source)),
        _check("l10_03_activation_fixture_matrix_remains_passive", source_safe, {"activation_ready": source.get("filesystem_probe_activation_ready"), "activation_exec": source.get("filesystem_probe_activation_execution_allowed"), "fs_probe": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l10_04_activation_fixture_matrix_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l10_03_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l10_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l10_docs_contain_activation_fixture_matrix_readback_boundary", doc_state.get("ok") is True, doc_state),
        _check("activation_fixture_matrix_cli_readback_is_enforced", source.get("edge_executable_filesystem_probe_explicit_activation_fixture_matrix_enforced") is True, {}),
        ...
        _check("activation_execution_remains_blocked_by_phase", source.get("filesystem_probe_activation_execution_allowed") is False and source.get("fixture_activation_execution_allowed") is False, {}),
    ...
    return {
        ...
        "l10_03_explicit_activation_fixture_matrix_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l10_03_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback_enforced": True,
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L10.3 Accepted  : {payload.get('l10_03_explicit_activation_fixture_matrix_remains_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L10.4 Microsoft Edge executable filesystem probe explicit activation fixture matrix CLI/readback

L10.4 adds a passive CLI/readback layer for the accepted L10.3 Microsoft Edge executable filesystem-probe explicit activation fixture matrix.
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback as readback

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-fixture-matrix-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-fixture-matrix"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l10_04_candidate"
...
def test_l10_04_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l10_04_activation_fixture_matrix_readback_not_ready_without_all_gates() -> None:
    for kwargs in [
        ...
        payload = readback.build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(PROJECT_ROOT, **kwargs)
        assert payload["ok"] is True
        assert payload["patch"] == "L10.4"
...
def test_l10_04_all_gates_ready_but_execution_still_blocked() -> None:
    payload = readback.build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(
        PROJECT_ROOT,
    ...
    assert payload["filesystem_probe_activation_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L10.5 Live adapter Microsoft Edge executable filesystem probe explicit activation aggregate gate"
    _assert_passive(payload)


def test_l10_04_default_and_missing_profile_cases_are_passive_when_all_gates_present() -> None:
    default_payload = readback.build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True)
    missing_payload = readback.build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l10_03_status"]["ok"] is True
    ...
    assert missing_payload["source_l10_03_status"]["ok"] is True
    assert missing_payload["filesystem_probe_activation_ready"] is True
    _assert_passive(missing_payload)


def test_l10_04_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l10_04_doc_mentions_activation_fixture_matrix_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-fixture-matrix-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l10_04_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l10_03_explicit_activation_fixture_matrix_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback_enforced"] is True
    assert payload["filesystem_probe_activation_execution_allowed"] is False
    assert payload["fixture_activation_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
...
def main() -> int:
    no_activation = readback.build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False)
    assert_passive(no_activation)
    assert no_activation["filesystem_probe_activation_ready"] is False
    print("PASS no_activation: patch=L10.4 fixtures=6 activation_ready=False activation_exec=False fs_probe=False path_selected=False")

    activation_only = readback.build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=True)
    ...
    partial = readback.build_edge_executable_filesystem_probe_explicit_activation_fixture_matrix_cli_readback(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=False)
    ...
    for name, profile in [("all_gates_dedicated", DEDICATED), ("all_gates_default", DEFAULT), ("all_gates_missing_profile", None)]:
        ...
        assert payload["filesystem_probe_activation_ready"] is True
        print("PASS {0}: patch=L10.4 source=L10.3 fixtures=6 activation_ready=True activation_exec=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; activation fixture matrix CLI/readback stayed passive")
    print("PASS L10.4 brief validation: activation fixture matrix CLI/readback accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L10.4 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L10.4 command wrapper appended")
    else:
        print("L10.4 command wrapper already present")


def _append_l10_03_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_fixture_matrix.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L10.4 Live adapter Microsoft Edge executable filesystem probe explicit activation fixture matrix CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe activation fixture matrix:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L10.3 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l10_03_doc_pointer()
    print("L10.4 files written; validation is intentionally brief and compact")
    return 0
