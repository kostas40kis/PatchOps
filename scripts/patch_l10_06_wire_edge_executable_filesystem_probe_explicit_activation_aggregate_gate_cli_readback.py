...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_explicit_activation_aggregate_gate_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_aggregate_gate_cli_readback.md")
TEST_PATH = Path("tests/test_l10_06_edge_executable_filesystem_probe_explicit_activation_aggregate_gate_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l10_06_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L10.6 passive Microsoft Edge executable filesystem-probe activation aggregate-gate CLI/readback.

Adds a compact CLI/readback layer over the accepted L10.5 activation aggregate
gate. Activation readiness may be true when all three gates are present, but
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_explicit_activation_aggregate_gate as l10_05

PATCH = "L10.6"
PHASE = "L10"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L10.6 Microsoft Edge Executable Filesystem Probe Explicit Activation Aggregate Gate CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-aggregate-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-activation-aggregate-gate"
NEXT_PATCH = "L10.7 Live adapter Microsoft Edge executable filesystem probe explicit activation broad validation checkpoint"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-explicit-activation-aggregate-gate-cli-readback-only"
...
def _source_l10_05_safe(source: Mapping[str, Any], *, expected_ready: bool) -> bool:
    fixture_ids = set(source.get("edge_executable_filesystem_probe_activation_fixture_ids", []))
    aggregate = source.get("edge_executable_filesystem_probe_explicit_activation_aggregate", {})
    return (
        source.get("patch") == "L10.5"
        ...
        and aggregate.get("edge_executable_filesystem_probe_explicit_activation_aggregate_gate_enforced") is True
...
def build_edge_executable_filesystem_probe_explicit_activation_aggregate_gate_cli_readback(
    ...
    *,
    ...
    source = l10_05.build_edge_executable_filesystem_probe_explicit_activation_aggregate_gate(
        root,
    ...
    source_safe = _source_l10_05_safe(source, expected_ready=expected_ready)
    ...
    checks = [
        _check("l10_05_activation_aggregate_gate_still_passes", source_safe, _compact_source_status(source)),
        _check("l10_05_activation_aggregate_gate_remains_passive", source_safe, {"activation_ready": source.get("filesystem_probe_activation_ready"), "activation_exec": source.get("filesystem_probe_activation_execution_allowed"), "fs_probe": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l10_06_activation_aggregate_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l10_05_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l10_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l10_docs_contain_activation_aggregate_readback_boundary", doc_state.get("ok") is True, doc_state),
        _check("activation_aggregate_gate_cli_readback_is_enforced", source.get("edge_executable_filesystem_probe_explicit_activation_aggregate_gate_enforced") is True, {}),
    ...
    return {
        ...
        "l10_05_explicit_activation_aggregate_gate_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "l10_explicit_activation_chain_accepted": source.get("l10_explicit_activation_chain_accepted") is True,
        "l10_01_explicit_activation_contract_remains_accepted": source.get("l10_01_explicit_activation_contract_remains_accepted") is True,
        "l10_02_explicit_activation_cli_readback_remains_accepted": source.get("l10_02_explicit_activation_cli_readback_remains_accepted") is True,
        "l10_03_explicit_activation_fixture_matrix_remains_accepted": source.get("l10_03_explicit_activation_fixture_matrix_remains_accepted") is True,
        "l10_04_explicit_activation_fixture_matrix_cli_readback_remains_accepted": source.get("l10_04_explicit_activation_fixture_matrix_cli_readback_remains_accepted") is True,
        "source_l10_05_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_explicit_activation_aggregate_gate_cli_readback_enforced": True,
        ...
        "edge_executable_filesystem_probe_explicit_activation_aggregate_status": source.get("edge_executable_filesystem_probe_explicit_activation_aggregate_status"),
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L10.5 Accepted  : {payload.get('l10_05_explicit_activation_aggregate_gate_remains_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_explicit_activation_aggregate_gate_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L10.6 Microsoft Edge executable filesystem probe explicit activation aggregate gate CLI/readback
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L10.6 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L10.6 command wrapper appended")
    else:
        print("L10.6 command wrapper already present")


def _append_l10_05_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_explicit_activation_aggregate_gate.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L10.6 Live adapter Microsoft Edge executable filesystem probe explicit activation aggregate gate CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe activation aggregate gate:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L10.5 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l10_05_doc_pointer()
    print("L10.6 files written; validation is intentionally brief and compact")
    return 0
