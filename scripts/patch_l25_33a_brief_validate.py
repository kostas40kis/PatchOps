...
from patchops.llm_browser import live_adapter_edge_real_archive_manifest_package_run_broad_checkpoint_key_repair as broad
...
$ReportPath = Join-Path $Desktop ("synthetic_l25_33a_run_package_launcher_{0}.txt" -f $Stamp)
$Lines = @(
    'L25.33a synthetic run-package broad checkpoint launcher proof',
    'Patch Name : synthetic_l25_33a_run_package_fixture',
...
def _manifest(root: Path) -> dict[str, Any]:
    return {
        ...
        "manifest_version": "1",
        "patch_name": "synthetic_l25_33a_run_package_fixture",
        "synthetic": True,
...
def _refresh_runpkg_zip_fixture(root: Path) -> Path:
    ...
    metadata = {
        ...
        "patch_name": "synthetic_l25_33a_run_package_fixture",
        "bundle_mode": "launcher_direct_synthetic_broad_checkpoint_key_repair",
    ...
    with zipfile.ZipFile(path, "w", compression=zipfile.ZIP_STORED) as archive:
        ...
        archive.writestr(RUNPKG_README, "L25.33a controlled synthetic run-package broad checkpoint key repair. No target writes.\n")
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    _assert(payload.get("patch") == "L25.33a", "wrong patch marker", payload)
    _assert(payload.get("repairs_patch") == "L25.33", "repair marker missing", payload)
    ...
    src = payload.get("source_l25_32b_summary", {})
    _assert(src.get("ok") is True, "source ok missing", payload)
    _assert(src.get("patch") == "L25.32b", "source patch mismatch", payload)
    _assert(src.get("repairs_patches") == ["L25.32", "L25.32a"], "repair markers missing", payload)
    _assert(src.get("bundle_review_expected_shape_present") is True, "bundle shape marker missing", payload)
    _assert(src.get("runpkg_fixture_names_include_expected_bundle_shape") is True, "bundle names marker missing", payload)
    _assert(src.get("package_run_allowed") is True, "package-run allowed missing", payload)
    ...
    _assert(src.get("package_run") is True, "package_run marker missing", payload)
...
def build_validation_payload(repo_root: str | Path = REPO_ROOT) -> dict[str, Any]:
    ...
    _assert(not missing, "run-package proof zip missing expected bundle-review paths: " + ", ".join(missing))
    ...
    payload = broad.build_archive_manifest_package_run_broad_checkpoint_key_repair(root, source_payload=source_payload)
    _validate_payload(payload)

    # Also prove the key-normalizer accepts validator-summary shape.
    summary_shape = {
        "ok": True,
        "patch": "L25.32b",
        ...
        "package_run": True,
        "patchops_run_package_scope": proof.PACKAGE_RUN_SCOPE,
    }
    summary_payload = broad.build_archive_manifest_package_run_broad_checkpoint_key_repair(root, source_payload=summary_shape)
    _validate_payload(summary_payload)

    module_path = root / "patchops" / "llm_browser" / "live_adapter_edge_real_archive_manifest_package_run_broad_checkpoint_key_repair.py"
    doc_path = root / "docs" / "llm_browser_live_adapter_edge_real_archive_manifest_package_run_broad_checkpoint_key_repair.md"
    source_text = module_path.read_text(encoding="utf-8")
    doc_text = doc_path.read_text(encoding="utf-8")
    _assert(not _forbidden_import_hits(source_text), "forbidden imports in broad checkpoint repair module", payload)
    _assert(not _forbidden_call_hits(source_text), "forbidden calls in broad checkpoint repair module", payload)
    _assert("normalize both L25.32b source payload shapes" in doc_text, "doc missing normalizer repair statement", payload)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="L25.33a brief validator")
