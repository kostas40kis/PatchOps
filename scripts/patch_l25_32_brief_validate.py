...
from patchops.llm_browser import live_adapter_edge_real_archive_manifest_package_run_first_controlled_proof as proof
...
RUNPKG_LAUNCHER = "run_with_patchops.ps1"
RUNPKG_METADATA = "bundle_metadata.json"
...
RUNPKG_MANIFEST_OBJECT = _manifest("synthetic_l25_32_run_package_fixture", "patchops-run-package-only-synthetic-launcher-proof", ["synthetic", "l25_32", "run_package_only"])
...
$ReportPath = Join-Path $Desktop ("synthetic_l25_32_run_package_launcher_{0}.txt" -f $Stamp)
$Lines = @(
    'L25.32 synthetic run-package launcher proof',
    'Patch Name : synthetic_l25_32_run_package_fixture',
    ...
    'ExitCode : 0',
    'Result : PASS',
    'L25_32_RUN_PACKAGE_LAUNCHER_MARKER'
)
Set-Content -LiteralPath $ReportPath -Value $Lines -Encoding UTF8
Write-Host ('Report Path        : {0}' -f $ReportPath)
Write-Host 'L25_32_RUN_PACKAGE_LAUNCHER_MARKER'
...
def _refresh_runpkg_zip_fixture(root: Path) -> Path:
    ...
    metadata = {
        ...
        "patch_name": "synthetic_l25_32_run_package_fixture",
    ...
    with zipfile.ZipFile(path, "w", compression=zipfile.ZIP_STORED) as archive:
        ...
        archive.writestr(RUNPKG_METADATA, json.dumps(metadata, indent=2, sort_keys=True) + "\n")
        archive.writestr(MANIFEST_MEMBER, RUNPKG_MANIFEST_PAYLOAD)
        archive.writestr("README.txt", "L25.32 controlled synthetic run-package proof. No target writes.\n")
    return path
...
def build_validation_payload(repo_root: str | Path = REPO_ROOT) -> dict[str, Any]:
    ...
    default_payload = proof.build_archive_manifest_package_run_first_controlled_proof(root)
    authorized_payload = proof.build_archive_manifest_package_run_first_controlled_proof(
        root,
    ...
    module_path = root / "patchops" / "llm_browser" / "live_adapter_edge_real_archive_manifest_package_run_first_controlled_proof.py"
    doc_path = root / "docs" / "llm_browser_live_adapter_edge_real_archive_manifest_package_run_first_controlled_proof.md"
    ...
    _assert(authorized_payload.get("patch") == "L25.32", "wrong patch marker", authorized_payload)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="L25.32 brief validator")
