...
from patchops.llm_browser import live_adapter_edge_real_download_artifact_package_execution_authorization_gate as gate
...
def _source_l25_40_payload() -> dict[str, Any]:
    return {
        "ok": True,
        "patch": "L25.40",
        "manifest_validation_final_acceptance_marker": True,
        "manifest_validation_ladder_final_acceptance": True,
        "manifest_validation_final_acceptance_from_source_payload_only": True,
        "accepted_l25_35_real_artifact_authorization_gate": True,
        "accepted_l25_36_artifact_readback_proof": True,
        "accepted_l25_37_manifest_member_readback_proof": True,
        "accepted_l25_38_manifest_validation_checkpoint": True,
        "accepted_l25_39_manifest_validation_broad_checkpoint": True,
        ...
        "accepted_broad_checkpoint_rejects_missing_malformed_source_rejection": True,
        "final_marker_rejects_nonzero_manifest_validation_commands": True,
...
def _validate_payload(payload: dict[str, Any]) -> None:
    ...
    _assert(payload.get("patch") == "L25.41", "wrong patch marker", payload)
    _assert(payload.get("package_execution_authorization_gate") is True, "authorization gate marker missing", payload)
    _assert(payload.get("package_execution_authorization_from_source_payload_only") is True, "source-payload-only marker missing", payload)
    _assert(payload.get("package_execution_authorization_granted_for_future_patch") is True, "future authorization not granted", payload)
    _assert(payload.get("future_package_execution_requires_separate_l25_42_gate_and_token") is True, "separate L25.42 gate marker missing", payload)
    _assert(payload.get("accepted_manifest_validation_ladder_final_acceptance") is True, "manifest validation final acceptance missing", payload)
    ...
    _assert(payload.get("artifact_path_opened_by_l25_41") is False, "L25.41 must not open artifact path", payload)
    _assert(payload.get("artifact_bytes_read_by_l25_41") is False, "L25.41 must not read artifact bytes", payload)
    _assert(payload.get("zip_member_opened_by_l25_41") is False, "L25.41 must not open zip members", payload)
    ...
    _assert(payload.get("package_execution_performed_by_adapter") is False, "adapter package execution must remain false", payload)
...
def build_validation_payload(repo_root: str | Path = REPO_ROOT) -> dict[str, Any]:
    ...
    source = _source_l25_40_payload()

    default_payload = gate.build_real_download_artifact_package_execution_authorization_gate(root, source_payload=source)
    _assert(default_payload.get("ok") is False, "default should not pass without token", default_payload)
    _assert(default_payload.get("package_execution_authorization_granted_for_future_patch") is False, "default must not grant future package execution authorization", default_payload)

    authorized_payload = gate.build_real_download_artifact_package_execution_authorization_gate(
        root,
        source_payload=source,
        allow_package_execution_authorization=True,
        authorization_token=gate.REQUIRED_PACKAGE_EXECUTION_AUTHORIZATION_TOKEN,
    )
    _validate_payload(authorized_payload)

    malformed_source = dict(source)
    malformed_source["package_run"] = True
    malformed_payload = gate.build_real_download_artifact_package_execution_authorization_gate(
        root,
        source_payload=malformed_source,
        allow_package_execution_authorization=True,
        authorization_token=gate.REQUIRED_PACKAGE_EXECUTION_AUTHORIZATION_TOKEN,
    )
    _assert(malformed_payload.get("ok") is False, "authorization must reject source that already ran package", malformed_payload)

    module_path = root / "patchops" / "llm_browser" / "live_adapter_edge_real_download_artifact_package_execution_authorization_gate.py"
    doc_path = root / "docs" / "llm_browser_live_adapter_edge_real_download_artifact_package_execution_authorization_gate.md"
    module_text = module_path.read_text(encoding="utf-8")
    doc_text = doc_path.read_text(encoding="utf-8")
    _assert(not _forbidden_import_hits(module_text), "forbidden imports in L25.41 module", authorized_payload)
    _assert(not _forbidden_call_hits(module_text), "forbidden artifact/zip/execution calls in L25.41 module", authorized_payload)
    _assert("L25.41 does not execute any package" in doc_text, "doc missing no-execution statement", authorized_payload)
    _assert("This authorization gate does not open an artifact path" in doc_text, "doc missing no artifact-open boundary", authorized_payload)
    _assert("No PatchOps run-package against the artifact" in doc_text, "doc missing no run-package boundary", authorized_payload)

    return {
        "ok": True,
        "patch": "L25.41",
        "package_execution_authorization_gate": authorized_payload.get("package_execution_authorization_gate"),
        "package_execution_authorization_from_source_payload_only": authorized_payload.get("package_execution_authorization_from_source_payload_only"),
        "package_execution_authorization_granted_for_future_patch": authorized_payload.get("package_execution_authorization_granted_for_future_patch"),
        "future_package_execution_requires_separate_l25_42_gate_and_token": authorized_payload.get("future_package_execution_requires_separate_l25_42_gate_and_token"),
        "accepted_manifest_validation_ladder_final_acceptance": authorized_payload.get("accepted_manifest_validation_ladder_final_acceptance"),
        ...
        "authorization_rejects_source_that_already_ran_package": malformed_payload.get("ok") is False,
        "artifact_path_opened_by_l25_41": authorized_payload.get("artifact_path_opened_by_l25_41"),
        "artifact_bytes_read_by_l25_41": authorized_payload.get("artifact_bytes_read_by_l25_41"),
        "zip_member_opened_by_l25_41": authorized_payload.get("zip_member_opened_by_l25_41"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="L25.41 brief validator")
