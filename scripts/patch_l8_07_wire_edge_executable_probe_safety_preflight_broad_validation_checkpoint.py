...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_safety_preflight_broad_validation_checkpoint.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_broad_validation_checkpoint.md")
TEST_PATH = Path("tests/test_l8_07_edge_executable_probe_safety_preflight_broad_validation_checkpoint_current.py")
VALIDATE_PATH = Path("scripts/patch_l8_07_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L8.7 passive Microsoft Edge executable-probe safety-preflight broad validation checkpoint.

Wraps the accepted L8.6 safety-preflight aggregate-gate CLI/readback and proves
L8.1 through L8.6 remain accepted. The L8 safety-preflight stack remains
model/readback only: missing probe authorization remains readback_ok=true with
preflight_passed=false, authorized preflight can pass, and probe execution
...
from patchops.llm_browser import live_adapter_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback as l8_06

PATCH = "L8.7"
PHASE = "L8"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L8.7 Microsoft Edge Executable Probe Safety Preflight Broad Validation Checkpoint"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-broad-validation-checkpoint"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-readback"
NEXT_PATCH = "L8.8 Live adapter Microsoft Edge executable probe safety preflight final acceptance marker"
SIDE_EFFECT_BOUNDARY = "edge-executable-probe-safety-preflight-broad-validation-checkpoint-only"
...
def _source_l8_06_safe(summary: Mapping[str, Any], *, expect_preflight_passed: bool) -> bool:
    ids = set(summary.get("edge_executable_probe_safety_preflight_fixture_ids", []))
    return (
        summary.get("patch") == "L8.6"
...
def _chain_acceptance(source: Mapping[str, Any]) -> dict[str, bool]:
    return {
        ...
        "l8_06": source.get("ok") is True and source.get("status") == STATUS_PASS,
    }


def build_edge_executable_probe_safety_preflight_broad_validation_checkpoint(
    ...
    *,
    ...
    source = l8_06.build_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback(
        root,
    ...
    source_safe = _source_l8_06_safe(source, expect_preflight_passed=expected_preflight)
    ...
    checks = [
        _check("l8_06_aggregate_readback_still_passes", source_safe, _compact_source_status(source)),
        _check("l8_06_aggregate_readback_remains_passive", source_safe, {"preflight_passed": source.get("edge_executable_probe_safety_preflight_passed"), "probe_allowed": source.get("edge_executable_probe_preflight_allows_probe_execution"), "probe_performed": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l8_01_through_l8_06_chain_remains_accepted", chain_ok, acceptance),
        _check("l8_07_broad_checkpoint_command_registered", command_state.get("ok") is True, command_state),
        _check("l8_06_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l8_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l8_docs_contain_broad_checkpoint_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
    ...
    return {
        ...
        "source_l8_06_status": _compact_source_status(source),
        "edge_executable_probe_safety_preflight_broad_validation_checkpoint_enforced": True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_probe_safety_preflight_broad_validation_checkpoint(
        args.repo_root,
...
DOC_CONTENT = r'''# L8.7 Microsoft Edge executable probe safety preflight broad validation checkpoint
...
from patchops.llm_browser import live_adapter_edge_executable_probe_safety_preflight_broad_validation_checkpoint as checkpoint

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-broad-validation-checkpoint"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-readback"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_07_candidate"
...
def test_l8_07_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l8_07_no_auth_broad_checkpoint_readback_ok_preflight_false() -> None:
    payload = checkpoint.build_edge_executable_probe_safety_preflight_broad_validation_checkpoint(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_probe_safety_preflight_broad_validation_checkpoint_enforced"] is True
...
def test_l8_07_authorized_broad_checkpoint_passes_preflight_but_probe_stays_blocked() -> None:
    payload = checkpoint.build_edge_executable_probe_safety_preflight_broad_validation_checkpoint(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_probe_preflight_allows_probe_execution"] is False
    assert payload["next_patch"] == "L8.8 Live adapter Microsoft Edge executable probe safety preflight final acceptance marker"
    _assert_passive(payload)


def test_l8_07_default_and_missing_profile_cases_are_passive_when_authorized() -> None:
    default_payload = checkpoint.build_edge_executable_probe_safety_preflight_broad_validation_checkpoint(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = checkpoint.build_edge_executable_probe_safety_preflight_broad_validation_checkpoint(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l8_06_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l8_06_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l8_07_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l8_07_doc_mentions_safety_preflight_broad_checkpoint_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_probe_safety_preflight_broad_validation_checkpoint.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-broad-validation-checkpoint"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_07_candidate"
...
def main() -> int:
    no_auth = checkpoint.build_edge_executable_probe_safety_preflight_broad_validation_checkpoint(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    assert_passive(no_auth)
    assert no_auth["edge_executable_probe_safety_preflight_passed"] is False
    print("PASS no_probe_auth: patch=L8.7 broad_ok=True preflight_passed=False fixtures=5 probe_execution_allowed=False exe_probe=False")
    ...
    for name, live, profile, probe_auth in cases:
        ...
        assert payload["edge_executable_probe_safety_preflight_passed"] is True
        print(
            "PASS {0}: patch={1} source=L8.6 chain={2} fixtures={3} preflight_passed={4} probe_execution_allowed={5} exe_probe={6}".format(
                name,
                payload["patch"],
                payload["l8_safety_preflight_chain_accepted"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; safety preflight broad checkpoint stayed passive")
    print("PASS L8.7 brief validation: safety preflight broad checkpoint accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L8.7 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L8.7 command wrapper appended")
    else:
        print("L8.7 command wrapper already present")


def _append_l8_06_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L8.7 Live adapter Microsoft Edge executable probe safety preflight broad validation checkpoint"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe safety preflight aggregate CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L8.6 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l8_06_doc_pointer()
    print("L8.7 files written; validation is intentionally brief and compact")
    return 0
