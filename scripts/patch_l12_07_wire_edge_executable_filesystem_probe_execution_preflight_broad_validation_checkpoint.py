...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint.md")
TEST_PATH = Path("tests/test_l12_07_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint_current.py")
VALIDATE_PATH = Path("scripts/patch_l12_07_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L12.7 passive Microsoft Edge executable filesystem-probe execution preflight broad validation checkpoint.

Broadly validates the accepted L12.1 through L12.6 execution-preflight stack.
Execution preflight readiness may become true when all required gates are present,
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback as l12_06

PATCH = "L12.7"
PHASE = "L12"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L12.7 Microsoft Edge Executable Filesystem Probe Execution Preflight Broad Validation Checkpoint"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-broad-validation-checkpoint"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-aggregate-readback"
NEXT_PATCH = "L12.8 Live adapter Microsoft Edge executable filesystem probe execution preflight final acceptance marker"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-execution-preflight-broad-validation-checkpoint-only"
...
def _source_l12_06_safe(source: Mapping[str, Any], *, expected_real_ready: bool, expected_execution_ready: bool) -> bool:
    fixture_ids = set(source.get("execution_preflight_fixture_ids", []))
    return (
        source.get("patch") == "L12.6"
...
def _chain_acceptance(source: Mapping[str, Any]) -> dict[str, bool]:
    return {
        ...
        "l12_06": source.get("ok") is True and source.get("status") == STATUS_PASS,
    }


def build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(
    ...
    *,
    ...
    source = l12_06.build_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback(
        root,
    ...
    source_safe = _source_l12_06_safe(source, expected_real_ready=expected_real_ready, expected_execution_ready=expected_execution_ready)
    ...
    checks = [
        _check("l12_06_execution_preflight_aggregate_readback_still_passes", source_safe, _compact_source_status(source)),
        _check("l12_06_execution_preflight_aggregate_readback_remains_passive", source_safe, {"exec_ready": source.get("execution_preflight_ready"), "exec_allowed": source.get("execution_preflight_execution_allowed"), "fs_probe": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l12_01_through_l12_06_chain_remains_accepted", chain_ok, acceptance),
        _check("l12_07_execution_preflight_broad_checkpoint_command_registered", command_state.get("ok") is True, command_state),
        _check("l12_06_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l12_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l12_docs_contain_execution_preflight_broad_checkpoint_boundary", doc_state.get("ok") is True, doc_state),
        _check("execution_preflight_broad_validation_checkpoint_is_enforced", chain_ok and source.get("edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback_enforced") is True, acceptance),
    ...
    return {
        ...
        "l12_execution_preflight_chain_accepted": chain_ok,
        ...
        "source_l12_06_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint_enforced": True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(
        args.repo_root,
...
DOC_CONTENT = r'''# L12.7 Microsoft Edge executable filesystem probe execution preflight broad validation checkpoint

L12.7 adds a passive broad validation checkpoint over the accepted L12 execution preflight stack.
...
`browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-aggregate-readback`
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint as checkpoint

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-broad-validation-checkpoint"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-aggregate-readback"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l12_07_candidate"
...
def _assert_passive(payload: dict) -> None:
    ...
    assert "source_l12_06_summary" not in payload


def test_l12_07_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l12_07_broad_checkpoint_not_ready_without_all_required_gates() -> None:
    for kwargs in [
        ...
        payload = checkpoint.build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(PROJECT_ROOT, **kwargs)
        assert payload["ok"] is True
        assert payload["patch"] == "L12.7"
...
def test_l12_07_execution_preflight_ready_but_execution_still_blocked() -> None:
    payload = checkpoint.build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(
        PROJECT_ROOT,
    ...
    assert payload["execution_preflight_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L12.8 Live adapter Microsoft Edge executable filesystem probe execution preflight final acceptance marker"
    _assert_passive(payload)


def test_l12_07_default_and_missing_profile_cases_are_passive_when_execution_preflight_ready() -> None:
    default_payload = checkpoint.build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=True)
    missing_payload = checkpoint.build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l12_06_status"]["ok"] is True
    assert default_payload["execution_preflight_ready"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l12_06_status"]["ok"] is True
    assert missing_payload["execution_preflight_ready"] is True
    _assert_passive(missing_payload)


def test_l12_07_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l12_07_doc_mentions_execution_preflight_broad_checkpoint_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-broad-validation-checkpoint"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l12_07_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l12_execution_preflight_chain_accepted"] is True
    assert payload["edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint_enforced"] is True
    assert payload["execution_preflight_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_gates = checkpoint.build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False, allow_executable_filesystem_probe_execution=False)
    assert_passive(no_gates)
    assert no_gates["execution_preflight_ready"] is False
    print("PASS no_gates: patch=L12.7 source=L12.6 chain=True broad=True exec_ready=False exec_allowed=False fs_probe=False path_selected=False")

    execution_flag_only = checkpoint.build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False, allow_executable_filesystem_probe_execution=True)
    assert_passive(execution_flag_only)
    assert execution_flag_only["execution_preflight_ready"] is False
    print("PASS execution_flag_only: chain=True broad=True exec_ready=False exec_allowed=False fs_probe=False path_selected=False")

    real_ready_without_execution = checkpoint.build_edge_executable_filesystem_probe_execution_preflight_broad_validation_checkpoint(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=False)
    assert_passive(real_ready_without_execution)
    assert real_ready_without_execution["execution_preflight_ready"] is False
    print("PASS real_ready_without_execution_flag: chain=True broad=True real_ready=True exec_ready=False exec_allowed=False fs_probe=False path_selected=False")
    ...
    for name, profile in [("execution_preflight_ready_dedicated", DEDICATED), ("execution_preflight_ready_default", DEFAULT), ("execution_preflight_ready_missing_profile", None)]:
        ...
        assert payload["execution_preflight_ready"] is True
        print("PASS {0}: patch=L12.7 source=L12.6 fixtures=6 exec_ready=True exec_allowed=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; execution-preflight broad checkpoint stayed passive")
    print("PASS L12.7 brief validation: execution-preflight broad validation checkpoint accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L12.7 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L12.7 command wrapper appended")
    else:
        print("L12.7 command wrapper already present")


def _append_l12_06_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L12.7 Live adapter Microsoft Edge executable filesystem probe execution preflight broad validation checkpoint"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the execution preflight aggregate gate CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L12.6 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l12_06_doc_pointer()
    print("L12.7 files written; validation is intentionally brief and compact")
    return 0
