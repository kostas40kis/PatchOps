...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_safety_preflight_aggregate_gate.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_aggregate_gate.md")
TEST_PATH = Path("tests/test_l8_05_edge_executable_probe_safety_preflight_aggregate_gate_current.py")
VALIDATE_PATH = Path("scripts/patch_l8_05_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L8.5 passive Microsoft Edge executable-probe safety-preflight aggregate gate.

Aggregates the accepted L8.1 through L8.4 safety-preflight surfaces. The gate
keeps the fixture matrix and CLI/readback surfaces modeled only: missing probe
authorization remains readback_ok=true with preflight_passed=false, authorized
preflight can pass, and probe execution remains blocked by phase.
...
from patchops.llm_browser import live_adapter_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback as l8_04

PATCH = "L8.5"
PHASE = "L8"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L8.5 Microsoft Edge Executable Probe Safety Preflight Aggregate Gate"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-gate"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-fixture-matrix-readback"
NEXT_PATCH = "L8.6 Live adapter Microsoft Edge executable probe safety preflight aggregate gate CLI/readback"
SIDE_EFFECT_BOUNDARY = "edge-executable-probe-safety-preflight-aggregate-gate-only"
...
def _source_l8_04_safe(summary: Mapping[str, Any], *, expect_preflight_passed: bool) -> bool:
    ids = set(summary.get("edge_executable_probe_safety_preflight_fixture_ids", []))
    return (
        summary.get("patch") == "L8.4"
        ...
        and summary.get("edge_executable_probe_safety_preflight_fixture_matrix_cli_readback_enforced") is True
...
def _chain_acceptance(source: Mapping[str, Any]) -> dict[str, bool]:
    return {
        "l8_01": source.get("edge_executable_probe_safety_preflight_contract_enforced") is True,
        "l8_02": source.get("edge_executable_probe_safety_preflight_cli_readback_enforced") is True,
        "l8_03": source.get("l8_03_executable_probe_safety_preflight_fixture_matrix_remains_accepted") is True,
        "l8_04": source.get("ok") is True and source.get("status") == STATUS_PASS,
    }
...
def _aggregate_state(source: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "edge_executable_probe_safety_preflight_aggregate_gate_enforced": True,
        "edge_executable_probe_safety_preflight_aggregate_status": "MODELED_ONLY_BLOCKED_BY_PHASE",
        "edge_executable_probe_safety_preflight_aggregate_reason": "L8.5 aggregates L8.1-L8.4 while keeping real executable probing reserved for a later accepted phase.",
        "l8_01_executable_probe_safety_preflight_passive_contract_ok": acceptance["l8_01"],
        "l8_02_executable_probe_safety_preflight_cli_readback_ok": acceptance["l8_02"],
        "l8_03_executable_probe_safety_preflight_fixture_matrix_ok": acceptance["l8_03"],
        "l8_04_executable_probe_safety_preflight_fixture_matrix_cli_readback_ok": acceptance["l8_04"],
        "fixture_count": source.get("edge_executable_probe_safety_preflight_fixture_count"),
        "fixture_ids": source.get("edge_executable_probe_safety_preflight_fixture_ids"),
...
def build_edge_executable_probe_safety_preflight_aggregate_gate(
    ...
    *,
    ...
    source = l8_04.build_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback(
        root,
    ...
    source_safe = _source_l8_04_safe(source, expect_preflight_passed=expected_preflight)
    ...
    checks = [
        _check("l8_04_fixture_matrix_readback_still_passes", source_safe, _compact_source_status(source)),
        _check("l8_04_fixture_matrix_readback_remains_passive", source_safe, {"preflight_passed": source.get("edge_executable_probe_safety_preflight_passed"), "probe_allowed": source.get("edge_executable_probe_preflight_allows_probe_execution"), "probe_performed": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l8_01_through_l8_04_chain_remains_accepted", chain_ok, aggregate["chain_acceptance"]),
        _check("l8_05_aggregate_gate_command_registered", command_state.get("ok") is True, command_state),
        _check("l8_04_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l8_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l8_docs_contain_aggregate_gate_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("aggregate_gate_is_enforced", aggregate["edge_executable_probe_safety_preflight_aggregate_gate_enforced"] is True, aggregate),
        ...
        _check("preflight_alone_still_does_not_probe", source.get("preflight_alone_does_not_perform_probe") is True and aggregate["edge_executable_filesystem_probe_performed"] is False, {}),
        _check("probe_execution_remains_blocked_by_phase", source.get("edge_executable_probe_preflight_allows_probe_execution") is False and aggregate["probe_execution_allowed"] is False, {}),
    ...
    return {
        ...
        "l8_safety_preflight_chain_accepted": chain_ok,
        "l8_01_executable_probe_safety_preflight_passive_contract_remains_accepted": aggregate["l8_01_executable_probe_safety_preflight_passive_contract_ok"],
        "l8_02_executable_probe_safety_preflight_cli_readback_remains_accepted": aggregate["l8_02_executable_probe_safety_preflight_cli_readback_ok"],
        "l8_03_executable_probe_safety_preflight_fixture_matrix_remains_accepted": aggregate["l8_03_executable_probe_safety_preflight_fixture_matrix_ok"],
        "l8_04_executable_probe_safety_preflight_fixture_matrix_cli_readback_remains_accepted": aggregate["l8_04_executable_probe_safety_preflight_fixture_matrix_cli_readback_ok"],
        "source_l8_04_status": _compact_source_status(source),
        "edge_executable_probe_safety_preflight_aggregate": aggregate,
        "edge_executable_probe_safety_preflight_aggregate_gate_enforced": aggregate["edge_executable_probe_safety_preflight_aggregate_gate_enforced"],
        "edge_executable_probe_safety_preflight_aggregate_status": aggregate["edge_executable_probe_safety_preflight_aggregate_status"],
        "edge_executable_probe_safety_preflight_fixture_matrix_cli_readback_enforced": source.get("edge_executable_probe_safety_preflight_fixture_matrix_cli_readback_enforced"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_probe_safety_preflight_aggregate_gate(
        args.repo_root,
...
DOC_CONTENT = r'''# L8.5 Microsoft Edge executable probe safety preflight aggregate gate
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-gate"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_05_candidate"
...
def main() -> int:
    no_auth = gate.build_edge_executable_probe_safety_preflight_aggregate_gate(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    assert_passive(no_auth)
    assert no_auth["edge_executable_probe_safety_preflight_passed"] is False
    print("PASS no_probe_auth: patch=L8.5 aggregate_ok=True preflight_passed=False fixtures=5 probe_execution_allowed=False exe_probe=False")
    ...
    for name, live, profile, probe_auth in cases:
        ...
        assert payload["edge_executable_probe_safety_preflight_passed"] is True
        print(
            "PASS {0}: patch={1} source=L8.4 fixtures={2} preflight_passed={3} probe_execution_allowed={4} exe_probe={5}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; safety preflight aggregate gate stayed passive")
    print("PASS L8.5 brief validation: safety preflight aggregate gate accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L8.5 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L8.5 command wrapper appended")
    else:
        print("L8.5 command wrapper already present")


def _append_l8_04_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L8.5 Live adapter Microsoft Edge executable probe safety preflight aggregate gate"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe safety preflight fixture matrix CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L8.4 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l8_04_doc_pointer()
    print("L8.5 files written; validation is intentionally brief and compact")
    return 0
