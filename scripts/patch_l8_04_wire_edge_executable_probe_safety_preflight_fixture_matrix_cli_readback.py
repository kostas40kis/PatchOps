...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback.md")
TEST_PATH = Path("tests/test_l8_04_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l8_04_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L8.4 passive Microsoft Edge executable-probe safety-preflight fixture-matrix CLI/readback.
...
DOC_CONTENT = r'''# L8.4 Microsoft Edge executable probe safety preflight fixture matrix CLI/readback
...
from patchops.llm_browser import live_adapter_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback as readback

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-fixture-matrix-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-fixture-matrix"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_04_candidate"
...
def _assert_passive(payload: dict) -> None:
    ...
    assert "source_l8_03_summary" not in payload


def test_l8_04_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l8_04_no_auth_fixture_matrix_readback_ok_preflight_false() -> None:
    payload = readback.build_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback(
        PROJECT_ROOT,
    ...
    assert payload["patch"] == "L8.4"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["l8_03_executable_probe_safety_preflight_fixture_matrix_remains_accepted"] is True
    assert payload["edge_executable_probe_safety_preflight_fixture_matrix_cli_readback_enforced"] is True
...
def test_l8_04_authorized_fixture_matrix_readback_passes_preflight_but_probe_stays_blocked() -> None:
    payload = readback.build_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_probe_preflight_allows_probe_execution"] is False
    assert set(payload["edge_executable_probe_safety_preflight_fixture_ids"]) == EXPECTED_IDS
    assert payload["next_patch"] == "L8.5 Live adapter Microsoft Edge executable probe safety preflight aggregate gate"
    _assert_passive(payload)


def test_l8_04_default_and_missing_profile_cases_are_passive_when_authorized() -> None:
    default_payload = readback.build_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = readback.build_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l8_03_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l8_03_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l8_04_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l8_04_doc_mentions_safety_preflight_fixture_matrix_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-fixture-matrix-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_04_candidate"
...
def main() -> int:
    no_auth = readback.build_edge_executable_probe_safety_preflight_fixture_matrix_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    assert_passive(no_auth)
    assert no_auth["edge_executable_probe_safety_preflight_passed"] is False
    print("PASS no_probe_auth: patch=L8.4 readback_ok=True preflight_passed=False fixtures=5 probe_execution_allowed=False exe_probe=False")
    ...
    for name, live, profile, probe_auth in cases:
        ...
        assert payload["edge_executable_probe_safety_preflight_passed"] is True
        print(
            "PASS {0}: patch={1} source=L8.3 fixtures={2} preflight_passed={3} probe_execution_allowed={4} exe_probe={5}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; safety preflight fixture matrix CLI/readback stayed passive")
    print("PASS L8.4 brief validation: safety preflight fixture matrix CLI/readback accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L8.4 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L8.4 command wrapper appended")
    else:
        print("L8.4 command wrapper already present")


def _append_l8_03_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_fixture_matrix.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L8.4 Live adapter Microsoft Edge executable probe safety preflight fixture matrix CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe safety preflight fixture matrix:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L8.3 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l8_03_doc_pointer()
    print("L8.4 files written; validation is intentionally brief and compact")
    return 0
