...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_passive_contract.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_passive_contract.md")
TEST_PATH = Path("tests/test_l9_01_edge_executable_filesystem_probe_passive_contract_current.py")
VALIDATE_PATH = Path("scripts/patch_l9_01_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L9.1 passive Microsoft Edge executable filesystem-probe contract.

Starts the L9 executable-filesystem-probe slice after the accepted L8 safety
preflight final marker. This patch defines the passive contract and readback
shape for a later real filesystem probe, but it still performs no filesystem
probe, path selection, executable launch, Selenium import, browser start,
profile creation, click/download, paste/send, package run, commit, or push.
"""
...
from patchops.llm_browser import live_adapter_edge_executable_probe_safety_preflight_final_acceptance_marker as l8_08

PATCH = "L9.1"
PHASE = "L9"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L9.1 Microsoft Edge Executable Filesystem Probe Passive Contract"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-contract"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-final-acceptance-marker"
NEXT_PATCH = "L9.2 Live adapter Microsoft Edge executable filesystem probe CLI/readback"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-passive-contract-only"
...
REQUIRED_REPO_PATHS = (
    ...
    "docs/llm_browser_live_adapter_edge_executable_filesystem_probe_passive_contract.md",
    "tests/test_l8_08_edge_executable_probe_safety_preflight_final_acceptance_marker_current.py",
    "tests/test_l9_01_edge_executable_filesystem_probe_passive_contract_current.py",
    "scripts/patch_l8_08_brief_validate.py",
    "scripts/patch_l9_01_brief_validate.py",
)
...
def _source_l8_08_safe(summary: Mapping[str, Any], *, expect_preflight_passed: bool) -> bool:
    ids = set(summary.get("edge_executable_probe_safety_preflight_fixture_ids", []))
    return (
        summary.get("patch") == "L8.8"
        and summary.get("ok") is True
        and summary.get("status") == STATUS_PASS
        and summary.get("command_name") == SOURCE_COMMAND_NAME
        and summary.get("l8_executable_probe_safety_preflight_slice_accepted") is True
        and summary.get("edge_executable_probe_safety_preflight_final_acceptance_marker_enforced") is True
        ...
        and "source_l8_08_summary" not in summary
...
def _compact_source_status(source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "slice_accepted": source.get("l8_executable_probe_safety_preflight_slice_accepted") is True,
...
def _filesystem_probe_contract_state(source: Mapping[str, Any], allow_executable_probe: bool) -> dict[str, Any]:
    safety_preflight_passed = bool(source.get("edge_executable_probe_safety_preflight_passed"))
    contract_ready = bool(allow_executable_probe and safety_preflight_passed)
    candidates = [{"label": label, "path_probe_performed": False, "exists": None, "selected": False} for label in EDGE_EXECUTABLE_CANDIDATE_LABELS]
    return {
        "edge_executable_filesystem_probe_contract_enforced": True,
        "edge_executable_filesystem_probe_modeled": True,
        "edge_executable_filesystem_probe_status": "MODELED_ONLY_BLOCKED_BY_PHASE",
        "edge_executable_filesystem_probe_requested": bool(allow_executable_probe),
        "edge_executable_filesystem_probe_contract_ready": contract_ready,
        "edge_executable_filesystem_probe_execution_allowed": False,
        "edge_executable_filesystem_probe_performed": False,
        "edge_executable_filesystem_probe_blocked_reason": "L9.1 defines the passive filesystem-probe contract only; real probing is reserved for a later accepted phase.",
        "edge_executable_candidate_path_labels_modeled": True,
        "edge_executable_candidate_path_labels": list(EDGE_EXECUTABLE_CANDIDATE_LABELS),
        "edge_executable_candidate_path_count": len(EDGE_EXECUTABLE_CANDIDATE_LABELS),
        "edge_executable_candidate_path_results": candidates,
...
def build_edge_executable_filesystem_probe_passive_contract(
    ...
    *,
    ...
    source = l8_08.build_edge_executable_probe_safety_preflight_final_acceptance_marker(
        root,
    ...
    source_safe = _source_l8_08_safe(source, expect_preflight_passed=expected_preflight)
    contract = _filesystem_probe_contract_state(source, allow_executable_probe)
    ...
    checks = [
        _check("l8_08_safety_preflight_final_marker_still_passes", source_safe, _compact_source_status(source)),
        _check("l8_08_safety_preflight_final_marker_remains_passive", source_safe, {"preflight_passed": source.get("edge_executable_probe_safety_preflight_passed"), "probe_allowed": source.get("edge_executable_probe_preflight_allows_probe_execution"), "filesystem_probe": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l9_01_filesystem_probe_contract_command_registered", command_state.get("ok") is True, command_state),
        _check("l8_08_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l9_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l9_docs_contain_filesystem_probe_contract_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("filesystem_probe_contract_is_modeled", contract["edge_executable_filesystem_probe_contract_enforced"] is True and contract["edge_executable_filesystem_probe_modeled"] is True, contract),
        _check("candidate_path_labels_are_modeled_only", contract["edge_executable_candidate_path_labels_modeled"] is True and contract["edge_executable_candidate_path_count"] >= 3, contract),
        _check("filesystem_probe_execution_remains_blocked_by_phase", contract["edge_executable_filesystem_probe_execution_allowed"] is False, contract),
        _check("filesystem_probe_is_not_performed", contract["edge_executable_filesystem_probe_performed"] is False, contract),
        _check("edge_executable_path_stays_unselected", contract["edge_executable_path_selected"] is False and contract["edge_executable_selected_path"] is None, contract),
        _check("edge_executable_launch_stays_unattempted", contract["edge_executable_launch_attempted"] is False, contract),
        ...
        _check("authorized_preflight_can_pass_but_filesystem_probe_stays_blocked", True if not allow_executable_probe else (source.get("edge_executable_probe_safety_preflight_passed") is True and contract["edge_executable_filesystem_probe_execution_allowed"] is False), contract),
    ...
    return {
        ...
        "l8_08_executable_probe_safety_preflight_final_marker_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "l8_executable_probe_safety_preflight_slice_accepted": source.get("l8_executable_probe_safety_preflight_slice_accepted") is True,
        "source_l8_08_status": _compact_source_status(source),
        **contract,
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L8.8 Accepted   : {payload.get('l8_08_executable_probe_safety_preflight_final_marker_remains_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_passive_contract(
        args.repo_root,
...
DOC_CONTENT = r'''# L9.1 Microsoft Edge executable filesystem probe passive contract
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_passive_contract as contract

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-contract"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-final-acceptance-marker"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l9_01_candidate"
...
def test_l9_01_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l9_01_no_auth_contract_readback_ok_preflight_false_and_no_probe() -> None:
    payload = contract.build_edge_executable_filesystem_probe_passive_contract(
        PROJECT_ROOT,
    ...
    assert payload["patch"] == "L9.1"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["l8_08_executable_probe_safety_preflight_final_marker_remains_accepted"] is True
...
def test_l9_01_authorized_contract_ready_but_filesystem_probe_stays_blocked() -> None:
    payload = contract.build_edge_executable_filesystem_probe_passive_contract(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L9.2 Live adapter Microsoft Edge executable filesystem probe CLI/readback"
    _assert_passive(payload)


def test_l9_01_default_and_missing_profile_cases_are_passive_when_authorized() -> None:
    default_payload = contract.build_edge_executable_filesystem_probe_passive_contract(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = contract.build_edge_executable_filesystem_probe_passive_contract(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l8_08_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l8_08_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l9_01_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l9_01_doc_mentions_filesystem_probe_contract_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_passive_contract.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-contract"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l9_01_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["l8_08_executable_probe_safety_preflight_final_marker_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_contract_enforced"] is True
    assert payload["edge_executable_filesystem_probe_modeled"] is True
    assert payload["edge_executable_candidate_path_labels_modeled"] is True
    assert payload["edge_executable_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
...
def main() -> int:
    no_auth = contract.build_edge_executable_filesystem_probe_passive_contract(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    ...
    print("PASS no_probe_auth: patch=L9.1 readback_ok=True preflight_passed=False contract_ready=False fs_probe=False path_selected=False")
    ...
    for name, live, profile, probe_auth in cases:
        ...
        assert payload["edge_executable_filesystem_probe_contract_ready"] is True
        print(
            "PASS {0}: patch={1} source=L8.8 preflight_passed={2} contract_ready={3} fs_probe={4} path_selected={5}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; filesystem probe contract stayed passive")
    print("PASS L9.1 brief validation: executable filesystem probe contract accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L9.1 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L9.1 command wrapper appended")
    else:
        print("L9.1 command wrapper already present")


def _append_l8_08_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_final_acceptance_marker.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L9.1 Live adapter Microsoft Edge executable filesystem probe passive contract"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe safety preflight slice:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L8.8 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l8_08_doc_pointer()
    print("L9.1 files written; validation is intentionally brief and compact")
    return 0
