...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback.md")
TEST_PATH = Path("tests/test_l11_06_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l11_06_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L11.6 passive Microsoft Edge executable filesystem-probe real-probe preflight aggregate-gate CLI/readback.

Adds compact CLI/readback over the accepted L11.5 real-probe preflight aggregate gate.
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate as l11_05

PATCH = "L11.6"
PHASE = "L11"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L11.6 Microsoft Edge Executable Filesystem Probe Real-Probe Preflight Aggregate Gate CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-aggregate-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-aggregate-gate"
NEXT_PATCH = "L11.7 Live adapter Microsoft Edge executable filesystem probe real-probe preflight broad validation checkpoint"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-real-probe-preflight-aggregate-gate-cli-readback-only"
...
def _source_l11_05_safe(source: Mapping[str, Any], *, expected_activation_ready: bool, expected_real_ready: bool) -> bool:
    fixture_ids = set(source.get("real_probe_preflight_fixture_ids", []))
    aggregate = source.get("edge_executable_filesystem_probe_real_probe_preflight_aggregate", {})
    return (
        source.get("patch") == "L11.5"
        ...
        and aggregate.get("edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_enforced") is True
...
def build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback(
    ...
    *,
    ...
    source = l11_05.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate(
        root,
    ...
    source_safe = _source_l11_05_safe(source, expected_activation_ready=expected_activation_ready, expected_real_ready=expected_real_ready)
    ...
    checks = [
        _check("l11_05_real_probe_preflight_aggregate_gate_still_passes", source_safe, _compact_source_status(source)),
        _check("l11_05_real_probe_preflight_aggregate_gate_remains_passive", source_safe, {"real_ready": source.get("real_filesystem_probe_preflight_ready"), "real_exec": source.get("real_filesystem_probe_execution_allowed"), "fs_probe": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l11_06_real_probe_preflight_aggregate_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l11_05_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l11_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l11_docs_contain_real_probe_preflight_aggregate_readback_boundary", doc_state.get("ok") is True, doc_state),
        _check("real_probe_preflight_aggregate_gate_cli_readback_is_enforced", source.get("edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_enforced") is True, {}),
    ...
    return {
        ...
        "l11_05_real_probe_preflight_aggregate_gate_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "l11_real_probe_preflight_chain_accepted": source.get("l11_real_probe_preflight_chain_accepted") is True,
        "l11_01_real_probe_preflight_contract_remains_accepted": source.get("l11_01_real_probe_preflight_contract_remains_accepted") is True,
        "l11_02_real_probe_preflight_cli_readback_remains_accepted": source.get("l11_02_real_probe_preflight_cli_readback_remains_accepted") is True,
        "l11_03_real_probe_preflight_fixture_matrix_remains_accepted": source.get("l11_03_real_probe_preflight_fixture_matrix_remains_accepted") is True,
        "l11_04_real_probe_preflight_fixture_matrix_cli_readback_remains_accepted": source.get("l11_04_real_probe_preflight_fixture_matrix_cli_readback_remains_accepted") is True,
        "source_l11_05_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback_enforced": True,
        ...
        "real_probe_preflight_aggregate_status": source.get("real_probe_preflight_aggregate_status"),
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L11.5 Accepted  : {payload.get('l11_05_real_probe_preflight_aggregate_gate_remains_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L11.6 Microsoft Edge executable filesystem probe real-probe preflight aggregate gate CLI/readback
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-aggregate-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l11_06_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l11_05_real_probe_preflight_aggregate_gate_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback_enforced"] is True
    assert payload["real_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_gates = readback.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False)
    assert_passive(no_gates)
    assert no_gates["real_filesystem_probe_preflight_ready"] is False
    print("PASS no_gates: patch=L11.6 source=L11.5 chain=True aggregate_readback=True real_ready=False real_exec=False fs_probe=False path_selected=False")

    real_flag_only = readback.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=True)
    assert_passive(real_flag_only)
    assert real_flag_only["real_filesystem_probe_preflight_ready"] is False
    print("PASS real_flag_only: chain=True aggregate_readback=True real_ready=False real_exec=False fs_probe=False path_selected=False")

    activation_without_real = readback.build_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate_cli_readback(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=False)
    assert_passive(activation_without_real)
    assert activation_without_real["real_filesystem_probe_preflight_ready"] is False
    print("PASS activation_ready_without_real_flag: chain=True aggregate_readback=True activation_ready=True real_ready=False real_exec=False fs_probe=False path_selected=False")
    ...
    for name, profile in [("real_preflight_ready_dedicated", DEDICATED), ("real_preflight_ready_default", DEFAULT), ("real_preflight_ready_missing_profile", None)]:
        ...
        assert payload["real_filesystem_probe_preflight_ready"] is True
        print("PASS {0}: patch=L11.6 source=L11.5 fixtures=6 real_ready=True real_exec=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; real-probe preflight aggregate gate CLI/readback stayed passive")
    print("PASS L11.6 brief validation: real-probe preflight aggregate gate CLI/readback accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L11.6 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L11.6 command wrapper appended")
    else:
        print("L11.6 command wrapper already present")


def _append_l11_05_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_aggregate_gate.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L11.6 Live adapter Microsoft Edge executable filesystem probe real-probe preflight aggregate gate CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the real-probe preflight aggregate gate:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L11.5 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l11_05_doc_pointer()
    print("L11.6 files written; validation is intentionally brief and compact")
    return 0
