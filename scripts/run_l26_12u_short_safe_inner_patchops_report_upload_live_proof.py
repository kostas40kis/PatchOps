...
from patchops.edge_rpa.edge_l26_12u_short_safe_inner_patchops_report_upload import assert_acceptance, run_short_safe_inner_report_upload
...
def main() -> int:
    ...
    a = p.parse_args()
    result = run_short_safe_inner_report_upload(
        Path(a.output_dir),
        Path(a.inner_patchops_report_path),
        Path(a.operator_report_path),
        Path(a.short_upload_dir),
        a.allow_report_upload,
        a.allow_chatgpt_submit,
        a.observe_seconds,
    )
    print("L26_12U_SHORT_SAFE_INNER_PATCHOPS_REPORT_UPLOAD_JSON_START")
    print(json.dumps(result.to_payload(), indent=2, sort_keys=True))
    print("L26_12U_SHORT_SAFE_INNER_PATCHOPS_REPORT_UPLOAD_JSON_END")
    assert_acceptance(result)
    print("L26_12U_ACCEPTANCE: PASS")
    print("uploaded_report_role:inner_patchops_apply_report_short_safe_copy")
