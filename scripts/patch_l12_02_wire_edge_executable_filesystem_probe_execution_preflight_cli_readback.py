...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_execution_preflight_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_cli_readback.md")
TEST_PATH = Path("tests/test_l12_02_edge_executable_filesystem_probe_execution_preflight_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l12_02_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L12.2 passive Microsoft Edge executable filesystem-probe execution preflight CLI/readback.
...
DOC_CONTENT = r'''# L12.2 Microsoft Edge executable filesystem probe execution preflight CLI/readback
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l12_02_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l12_01_execution_preflight_contract_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_execution_preflight_cli_readback_enforced"] is True
    assert payload["execution_preflight_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_execution_flag = readback.build_edge_executable_filesystem_probe_execution_preflight_cli_readback(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=False)
    ...
    print("PASS no_execution_flag: patch=L12.2 source=L12.1 real_ready=True exec_ready=False exec_allowed=False fs_probe=False path_selected=False")

    execution_flag_only = readback.build_edge_executable_filesystem_probe_execution_preflight_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False, allow_executable_filesystem_probe_execution=True)
    ...
    for name, profile in [("execution_preflight_ready_dedicated", DEDICATED), ("execution_preflight_ready_default", DEFAULT), ("execution_preflight_ready_missing_profile", None)]:
        ...
        assert payload["execution_preflight_ready"] is True
        print("PASS {0}: patch=L12.2 exec_ready=True exec_allowed=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; execution-preflight CLI/readback stayed passive")
    print("PASS L12.2 brief validation: execution-preflight CLI/readback accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L12.2 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L12.2 command wrapper appended")
    else:
        print("L12.2 command wrapper already present")


def _append_l12_01_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_contract.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L12.2 Live adapter Microsoft Edge executable filesystem probe execution preflight CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the execution preflight contract:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L12.1 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l12_01_doc_pointer()
    print("L12.2 files written; validation is intentionally brief and compact")
    return 0
