...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_authorization_fixture_matrix.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_authorization_fixture_matrix.md")
TEST_PATH = Path("tests/test_l7_03_edge_executable_probe_authorization_fixture_matrix_current.py")
VALIDATE_PATH = Path("scripts/patch_l7_03_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L7.3 passive Microsoft Edge executable-probe authorization fixture matrix.
...
DOC_CONTENT = r'''# L7.3 Microsoft Edge executable probe authorization fixture matrix
...
from patchops.llm_browser import live_adapter_edge_executable_probe_authorization_fixture_matrix as matrix

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-probe-authorization-fixture-matrix"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-probe-authorization-readback"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l7_03_candidate"
...
def test_l7_03_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l7_03_fixture_matrix_dedicated_profile_passes_passively() -> None:
    payload = matrix.build_edge_executable_probe_authorization_fixture_matrix(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_probe_authorization_fixture_matrix_enforced"] is True
    assert payload["edge_executable_probe_authorization_fixture_count"] >= 5
    assert set(payload["edge_executable_probe_authorization_fixture_ids"]) == {
        "no_probe_auth_dedicated_profile",
        "live_auth_without_probe_auth",
        "probe_auth_dedicated_profile",
        "probe_auth_default_profile",
    ...
    assert payload["next_patch"] == "L7.4 Live adapter Microsoft Edge executable probe authorization fixture matrix CLI/readback"
    _assert_passive(payload)


def test_l7_03_no_auth_default_and_missing_profile_cases_are_passive() -> None:
    no_auth = matrix.build_edge_executable_probe_authorization_fixture_matrix(PROJECT_ROOT, allow_live_start=False, profile_dir=DEDICATED_PROFILE, allow_executable_probe=False)
    default_payload = matrix.build_edge_executable_probe_authorization_fixture_matrix(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = matrix.build_edge_executable_probe_authorization_fixture_matrix(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert no_auth["ok"] is True
    assert no_auth["edge_executable_probe_authorized"] is False
    _assert_passive(no_auth)
    assert default_payload["ok"] is True
    assert default_payload["source_l7_02_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l7_02_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l7_03_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l7_03_doc_mentions_fixture_matrix_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_probe_authorization_fixture_matrix.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-authorization-fixture-matrix"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l7_03_candidate"
...
def main() -> int:
    ...
    for name, live, profile, probe_auth in cases:
        ...
        print(
            "PASS {0}: patch={1} fixtures={2} probe_authorized={3} probe_permitted={4} exe_probe={5}".format(
                name,
                payload["patch"],
                payload["edge_executable_probe_authorization_fixture_count"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; authorization fixture matrix stayed passive")
    print("PASS L7.3 brief validation: authorization fixture matrix accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L7.3 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L7.3 command wrapper appended")
    else:
        print("L7.3 command wrapper already present")


def _append_l7_02_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_authorization_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L7.3 Live adapter Microsoft Edge executable probe authorization fixture matrix"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe authorization CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L7.2 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l7_02_doc_pointer()
    print("L7.3 files written; validation is intentionally brief and compact")
    return 0
