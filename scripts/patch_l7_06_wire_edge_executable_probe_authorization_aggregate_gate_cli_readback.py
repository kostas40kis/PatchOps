...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_authorization_aggregate_gate_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_authorization_aggregate_gate_cli_readback.md")
TEST_PATH = Path("tests/test_l7_06_edge_executable_probe_authorization_aggregate_gate_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l7_06_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L7.6 passive Edge executable-probe authorization aggregate-gate CLI/readback.
...
DOC_CONTENT = r'''# L7.6 Microsoft Edge executable probe authorization aggregate gate CLI/readback
...
from patchops.llm_browser import live_adapter_edge_executable_probe_authorization_aggregate_gate_cli_readback as readback

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-probe-authorization-aggregate-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-probe-authorization-aggregate-gate"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l7_06_candidate"
...
def test_l7_06_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l7_06_aggregate_readback_dedicated_profile_passes_passively() -> None:
    payload = readback.build_edge_executable_probe_authorization_aggregate_gate_cli_readback(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_probe_authorization_aggregate_gate_cli_readback_enforced"] is True
    assert payload["edge_executable_probe_authorized"] is True
    assert payload["edge_executable_probe_permitted_by_phase"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L7.7 Live adapter Microsoft Edge executable probe authorization broad validation checkpoint"
    _assert_passive(payload)


def test_l7_06_no_auth_default_and_missing_profile_cases_are_passive() -> None:
    no_auth = readback.build_edge_executable_probe_authorization_aggregate_gate_cli_readback(PROJECT_ROOT, allow_live_start=False, profile_dir=DEDICATED_PROFILE, allow_executable_probe=False)
    default_payload = readback.build_edge_executable_probe_authorization_aggregate_gate_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = readback.build_edge_executable_probe_authorization_aggregate_gate_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert no_auth["ok"] is True
    assert no_auth["edge_executable_probe_authorized"] is False
    _assert_passive(no_auth)
    assert default_payload["ok"] is True
    assert default_payload["source_l7_05_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l7_05_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l7_06_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l7_06_doc_mentions_aggregate_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_probe_authorization_aggregate_gate_cli_readback.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-authorization-aggregate-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l7_06_candidate"
...
def main() -> int:
    ...
    for name, live, profile, probe_auth in cases:
        ...
        print(
            "PASS {0}: patch={1} source=L7.5 probe_authorized={2} probe_permitted={3} exe_probe={4}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; aggregate gate CLI/readback stayed passive")
    print("PASS L7.6 brief validation: aggregate gate CLI/readback accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L7.6 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L7.6 command wrapper appended")
    else:
        print("L7.6 command wrapper already present")


def _append_l7_05_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_authorization_aggregate_gate.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L7.6 Live adapter Microsoft Edge executable probe authorization aggregate gate CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe authorization aggregate gate:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L7.5 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l7_05_doc_pointer()
    print("L7.6 files written; validation is intentionally brief and compact")
    return 0
