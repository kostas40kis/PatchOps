...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-fixture-matrix-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-fixture-matrix"
...
def _no_launch(payload: dict[str, Any]) -> bool:
    ...
    list_fields = ["filesystem_writes_performed", "adapter_filesystem_writes_performed", "side_effects_performed"]
...
def main() -> int:
    ...
    for item in commands[1:]:
        if item["exit_code"] == 0:
            ...
            except Exception as exc:  # noqa: BLE001 - this is a validator diagnostic
    ...
    l13_04_payload = parsed[1] if len(parsed) >= 2 else {}
    ok = ok and source_payload.get("patch") == "L13.3"
    ...
    ok = ok and l13_04_payload.get("patch") == "L13.4"
    ok = ok and l13_04_payload.get("source_patch") == "L13.3"
    ok = ok and l13_04_payload.get("edge_executable_filesystem_probe_performed") is True
    ok = ok and l13_04_payload.get("truthful_selected_path_existing_reported_candidate") is True
    ok = ok and _candidate_truthful(l13_04_payload)
    ok = ok and _no_launch(l13_04_payload)
    ...
    summary = {
        ...
        "readback_patch": l13_04_payload.get("patch"),
        "source_probe_performed": source_payload.get("edge_executable_filesystem_probe_performed"),
        "readback_probe_performed": l13_04_payload.get("edge_executable_filesystem_probe_performed"),
        "readback_truthful_selection": l13_04_payload.get("truthful_selected_path_existing_reported_candidate"),
        "readback_no_launch": _no_launch(l13_04_payload) if l13_04_payload else False,
    }
