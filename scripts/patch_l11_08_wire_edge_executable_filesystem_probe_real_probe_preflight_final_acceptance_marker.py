...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker.md")
TEST_PATH = Path("tests/test_l11_08_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker_current.py")
VALIDATE_PATH = Path("scripts/patch_l11_08_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L11.8 passive Microsoft Edge executable filesystem-probe real-probe preflight final acceptance marker.
...
DOC_CONTENT = r'''# L11.8 Microsoft Edge executable filesystem probe real-probe preflight final acceptance marker
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker as marker

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-final-acceptance-marker"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-broad-validation-checkpoint"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l11_08_candidate"
...
def test_l11_08_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l11_08_final_marker_not_ready_without_all_required_gates() -> None:
    for kwargs in [
        ...
        payload = marker.build_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker(PROJECT_ROOT, **kwargs)
        assert payload["ok"] is True
        assert payload["patch"] == "L11.8"
        assert payload["l11_real_probe_preflight_stack_accepted"] is True
        assert payload["l11_07_real_probe_preflight_broad_validation_checkpoint_remains_accepted"] is True
...
def test_l11_08_real_preflight_ready_but_execution_still_blocked() -> None:
    payload = marker.build_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker(
        PROJECT_ROOT,
    ...
    assert payload["real_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L12.1 Live adapter Microsoft Edge executable filesystem probe execution preflight contract"
    _assert_passive(payload)


def test_l11_08_default_and_missing_profile_cases_are_passive_when_real_preflight_ready() -> None:
    default_payload = marker.build_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True)
    missing_payload = marker.build_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l11_07_status"]["ok"] is True
    ...
    assert missing_payload["source_l11_07_status"]["ok"] is True
    assert missing_payload["real_filesystem_probe_preflight_ready"] is True
    _assert_passive(missing_payload)


def test_l11_08_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l11_08_doc_mentions_real_probe_final_marker_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-final-acceptance-marker"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l11_08_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l11_real_probe_preflight_stack_accepted"] is True
    assert payload["edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker_enforced"] is True
    assert payload["real_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_gates = marker.build_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False)
    assert_passive(no_gates)
    assert no_gates["real_filesystem_probe_preflight_ready"] is False
    print("PASS no_gates: patch=L11.8 source=L11.7 stack=True final=True real_ready=False real_exec=False fs_probe=False path_selected=False")

    real_flag_only = marker.build_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=True)
    assert_passive(real_flag_only)
    assert real_flag_only["real_filesystem_probe_preflight_ready"] is False
    print("PASS real_flag_only: stack=True final=True real_ready=False real_exec=False fs_probe=False path_selected=False")

    activation_without_real = marker.build_edge_executable_filesystem_probe_real_probe_preflight_final_acceptance_marker(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=False)
    assert_passive(activation_without_real)
    assert activation_without_real["real_filesystem_probe_preflight_ready"] is False
    print("PASS activation_ready_without_real_flag: stack=True final=True activation_ready=True real_ready=False real_exec=False fs_probe=False path_selected=False")
    ...
    for name, profile in [("real_preflight_ready_dedicated", DEDICATED), ("real_preflight_ready_default", DEFAULT), ("real_preflight_ready_missing_profile", None)]:
        ...
        assert payload["real_filesystem_probe_preflight_ready"] is True
        print("PASS {0}: patch=L11.8 source=L11.7 fixtures=6 real_ready=True real_exec=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; real-probe preflight final marker stayed passive")
    print("PASS L11.8 brief validation: real-probe preflight final acceptance marker accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L11.8 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L11.8 command wrapper appended")
    else:
        print("L11.8 command wrapper already present")


def _append_l11_07_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_broad_validation_checkpoint.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L11.8 Live adapter Microsoft Edge executable filesystem probe real-probe preflight final acceptance marker"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the real-probe preflight broad validation checkpoint:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L11.7 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l11_07_doc_pointer()
    print("L11.8 files written; validation is intentionally brief and compact")
    return 0
