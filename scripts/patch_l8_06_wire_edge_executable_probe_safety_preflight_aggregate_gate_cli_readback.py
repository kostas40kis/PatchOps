...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback.md")
TEST_PATH = Path("tests/test_l8_06_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l8_06_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L8.6 passive Microsoft Edge executable-probe safety-preflight aggregate-gate CLI/readback.

Wraps the accepted L8.5 safety-preflight aggregate gate in a compact
CLI/readback layer. The L8 safety-preflight chain remains model/readback only:
missing probe authorization remains readback_ok=true with preflight_passed=false,
authorized preflight can pass, and probe execution remains blocked by phase.
...
from patchops.llm_browser import live_adapter_edge_executable_probe_safety_preflight_aggregate_gate as l8_05

PATCH = "L8.6"
PHASE = "L8"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L8.6 Microsoft Edge Executable Probe Safety Preflight Aggregate Gate CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-gate"
NEXT_PATCH = "L8.7 Live adapter Microsoft Edge executable probe safety preflight broad validation checkpoint"
SIDE_EFFECT_BOUNDARY = "edge-executable-probe-safety-preflight-aggregate-gate-cli-readback-only"
...
REQUIRED_REPO_PATHS = (
    ...
    "docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_aggregate_gate.md",
    "docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback.md",
    "tests/test_l8_05_edge_executable_probe_safety_preflight_aggregate_gate_current.py",
    "tests/test_l8_06_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback_current.py",
    "scripts/patch_l8_05_brief_validate.py",
    "scripts/patch_l8_06_brief_validate.py",
)
...
DOC_REQUIREMENTS = {
    ...
    "docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback.md": (
        "L8.6 Microsoft Edge executable probe safety preflight aggregate gate CLI/readback",
        COMMAND_NAME,
        SOURCE_COMMAND_NAME,
        ...
        "safety preflight aggregate gate CLI/readback enforced",
        ...
        "L8.7 Live adapter Microsoft Edge executable probe safety preflight broad validation checkpoint",
    ),
}
...
def _source_l8_05_safe(summary: Mapping[str, Any], *, expect_preflight_passed: bool) -> bool:
    ids = set(summary.get("edge_executable_probe_safety_preflight_fixture_ids", []))
    return (
        summary.get("patch") == "L8.5"
        ...
        and "source_l8_05_summary" not in summary
        and "source_l8_04_summary" not in summary
    )
...
def build_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback(
    ...
    *,
    ...
    source = l8_05.build_edge_executable_probe_safety_preflight_aggregate_gate(
        root,
    ...
    source_safe = _source_l8_05_safe(source, expect_preflight_passed=expected_preflight)
    ...
    checks = [
        _check("l8_05_aggregate_gate_still_passes", source_safe, _compact_source_status(source)),
        _check("l8_05_aggregate_gate_remains_passive", source_safe, {"preflight_passed": source.get("edge_executable_probe_safety_preflight_passed"), "probe_allowed": source.get("edge_executable_probe_preflight_allows_probe_execution"), "probe_performed": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l8_06_aggregate_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l8_05_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l8_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l8_docs_contain_aggregate_readback_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("aggregate_gate_cli_readback_is_enforced", source.get("edge_executable_probe_safety_preflight_aggregate_gate_enforced") is True, {}),
        _check("l8_chain_remains_accepted", source.get("l8_safety_preflight_chain_accepted") is True, _compact_source_status(source)),
    ...
    return {
        ...
        "l8_05_executable_probe_safety_preflight_aggregate_gate_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "l8_safety_preflight_chain_accepted": source.get("l8_safety_preflight_chain_accepted") is True,
        "l8_01_executable_probe_safety_preflight_passive_contract_remains_accepted": source.get("l8_01_executable_probe_safety_preflight_passive_contract_remains_accepted") is True,
        "l8_02_executable_probe_safety_preflight_cli_readback_remains_accepted": source.get("l8_02_executable_probe_safety_preflight_cli_readback_remains_accepted") is True,
        "l8_03_executable_probe_safety_preflight_fixture_matrix_remains_accepted": source.get("l8_03_executable_probe_safety_preflight_fixture_matrix_remains_accepted") is True,
        "l8_04_executable_probe_safety_preflight_fixture_matrix_cli_readback_remains_accepted": source.get("l8_04_executable_probe_safety_preflight_fixture_matrix_cli_readback_remains_accepted") is True,
        "source_l8_05_status": _compact_source_status(source),
        "edge_executable_probe_safety_preflight_aggregate_gate_cli_readback_enforced": True,
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L8.5 Accepted   : {payload.get('l8_05_executable_probe_safety_preflight_aggregate_gate_remains_accepted')}",
        f"L8 Chain OK     : {payload.get('l8_safety_preflight_chain_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L8.6 Microsoft Edge executable probe safety preflight aggregate gate CLI/readback

L8.6 adds a passive CLI/readback layer for the accepted L8.5 Microsoft Edge executable-probe safety preflight aggregate gate.
...
from patchops.llm_browser import live_adapter_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback as readback

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-gate"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_06_candidate"
...
def _assert_passive(payload: dict) -> None:
    ...
    assert "source_l8_06_summary" not in payload
    assert "source_l8_05_summary" not in payload


def test_l8_06_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l8_06_no_auth_aggregate_readback_ok_preflight_false() -> None:
    payload = readback.build_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_probe_safety_preflight_aggregate_gate_cli_readback_enforced"] is True
...
def test_l8_06_authorized_aggregate_readback_passes_preflight_but_probe_stays_blocked() -> None:
    payload = readback.build_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_probe_preflight_allows_probe_execution"] is False
    assert payload["next_patch"] == "L8.7 Live adapter Microsoft Edge executable probe safety preflight broad validation checkpoint"
    _assert_passive(payload)


def test_l8_06_default_and_missing_profile_cases_are_passive_when_authorized() -> None:
    default_payload = readback.build_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = readback.build_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l8_05_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l8_05_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l8_06_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l8_06_doc_mentions_safety_preflight_aggregate_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-aggregate-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_06_candidate"
...
def main() -> int:
    no_auth = readback.build_edge_executable_probe_safety_preflight_aggregate_gate_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False)
    assert_passive(no_auth)
    assert no_auth["edge_executable_probe_safety_preflight_passed"] is False
    print("PASS no_probe_auth: patch=L8.6 aggregate_readback_ok=True preflight_passed=False fixtures=5 probe_execution_allowed=False exe_probe=False")
    ...
    for name, live, profile, probe_auth in cases:
        ...
        assert payload["edge_executable_probe_safety_preflight_passed"] is True
        print(
            "PASS {0}: patch={1} source=L8.5 fixtures={2} preflight_passed={3} probe_execution_allowed={4} exe_probe={5}".format(
                name,
                payload["patch"],
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; safety preflight aggregate gate CLI/readback stayed passive")
    print("PASS L8.6 brief validation: safety preflight aggregate gate CLI/readback accepted with no executable probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L8.6 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L8.6 command wrapper appended")
    else:
        print("L8.6 command wrapper already present")


def _append_l8_05_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_aggregate_gate.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L8.6 Live adapter Microsoft Edge executable probe safety preflight aggregate gate CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe safety preflight aggregate gate:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L8.5 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l8_05_doc_pointer()
    print("L8.6 files written; validation is intentionally brief and compact")
    return 0
