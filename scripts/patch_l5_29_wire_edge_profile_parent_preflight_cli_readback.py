...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_supervised_launch_profile_parent_preflight_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_supervised_launch_profile_parent_preflight_cli_readback.md")
TEST_PATH = Path("tests/test_l5_29_edge_supervised_launch_profile_parent_preflight_cli_readback_current.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L5.29 passive Microsoft Edge profile-parent preflight CLI/readback.

This module wraps the accepted L5.28 profile-parent preflight contract in a
CLI/readback checkpoint. It remains passive: no Selenium import, browser start,
Edge process, driver/session creation, profile directory creation, profile
parent filesystem probe, click/download, paste/send, package-run, commit, or
push.
"""
...
from patchops.llm_browser import live_adapter_edge_supervised_launch_profile_parent_preflight_contract as l5_28

PATCH = "L5.29"
PHASE = "L5"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L5.29 Microsoft Edge Supervised Launch Profile Parent Preflight CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-live-start-profile-parent-preflight-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-live-start-profile-parent-preflight-contract"
NEXT_PATCH = "L5.30 Live adapter Microsoft Edge supervised launch profile parent preflight aggregate gate"
SIDE_EFFECT_BOUNDARY = "edge-supervised-launch-profile-parent-preflight-cli-readback-only"
...
COMMAND_PLAN: tuple[str, ...] = (
    "python -m compileall patchops/llm_browser tests scripts/patch_l5_29_wire_edge_profile_parent_preflight_cli_readback.py",
    "python -m pytest -q tests/test_l5_28_edge_supervised_launch_profile_parent_preflight_contract_current.py tests/test_l5_29_edge_supervised_launch_profile_parent_preflight_cli_readback_current.py",
    "python -m patchops.llm_browser.live_adapter_edge_supervised_launch_profile_parent_preflight_cli_readback --repo-root C:\\dev\\patchops --allow-live-start --profile-dir C:\\dev\\patchops\\data\\runtime\\browser_profiles\\edge_l5_29_candidate --json --compact",
    "python -m patchops.llm_browser.live_adapter_edge_supervised_launch_profile_parent_preflight_cli_readback --repo-root C:\\dev\\patchops --allow-live-start --profile-dir C:\\Users\\kostas\\AppData\\Local\\Microsoft\\Edge\\User Data\\Default --json --compact",
    "py -m patchops.cli llm-browser browser-start-supervised-launch-edge-live-start-profile-parent-preflight-readback --repo-root C:\\dev\\patchops --allow-live-start --profile-dir C:\\dev\\patchops\\data\\runtime\\browser_profiles\\edge_l5_29_candidate --json --compact",
...
def _source_l5_28_is_safe(summary: Mapping[str, Any]) -> bool:
    return (
        summary.get("patch") == "L5.28"
...
def build_edge_supervised_launch_profile_parent_preflight_cli_readback(
    ...
    *,
    ...
    source_summary = l5_28.build_edge_supervised_launch_profile_parent_preflight_contract(
        root,
    ...
    checks = [
        _check("l5_28_profile_parent_preflight_contract_still_passes", _source_l5_28_is_safe(source_summary), source_summary),
        _check("l5_28_profile_parent_preflight_contract_remains_passive", _source_l5_28_is_safe(source_summary), source_summary),
        _check("l5_28_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("l5_29_profile_parent_preflight_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("edge_l5_required_source_docs_tests_present", not missing_repo_paths, {"missing": missing_repo_paths}),
        _check("edge_l5_docs_contain_profile_parent_preflight_cli_readback_boundary", doc_state.get("ok") is True, doc_state),
        _check("l5_29_command_plan_is_readback_only", command_plan_state.get("ok") is True, command_plan_state),
        _check("adapter_logic_executes_no_validation_commands", command_plan_state.get("executed_by_adapter_logic") == [], command_plan_state),
        _check("profile_parent_preflight_cli_readback_is_passive", _source_l5_28_is_safe(source_summary), source_summary),
        _check("profile_parent_path_is_derived_by_source_contract", (not source_summary.get("profile_dir_argument_present")) or source_summary.get("profile_parent_path_derived") is True, source_summary),
        _check("profile_parent_filesystem_probe_stays_false", source_summary.get("profile_parent_filesystem_probe_performed") is False, source_summary),
        _check("profile_parent_directory_stays_uncreated", source_summary.get("profile_parent_directory_created") is False, source_summary),
        ...
        _check("selenium_not_imported_by_profile_parent_preflight_readback", not newly_loaded_forbidden and "selenium" not in loaded_forbidden, {"newly_loaded_forbidden": newly_loaded_forbidden, "loaded_forbidden": loaded_forbidden}),
        _check(
    ...
    payload: dict[str, Any] = {
        ...
        "live_start_performed": False,
        "source_l5_28_summary": dict(source_summary),
        "l5_28_summary": dict(source_summary),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_supervised_launch_profile_parent_preflight_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L5.29 Microsoft Edge supervised launch profile parent preflight CLI/readback
...
def _append_command_block() -> None:
    ...
    text = COMMANDS_PATH.read_text(encoding="utf-8")
    if "# PATCHOPS L5.29 START" in text:
        print("L5.29 command wrapper already present")
        return
    if not text.endswith("\n"):
        text += "\n"
    COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
    print("L5.29 command wrapper appended to patchops/llm_browser/commands.py")


def _append_l5_28_doc_pointer() -> None:
    l5_28_doc = Path("docs/llm_browser_live_adapter_edge_supervised_launch_profile_parent_preflight_contract.md")
    if not l5_28_doc.exists():
        return
    text = l5_28_doc.read_text(encoding="utf-8")
    phrase = "L5.29 Live adapter Microsoft Edge supervised launch profile parent preflight CLI/readback"
    if phrase not in text:
        l5_28_doc.write_text(text.rstrip() + "\n\nNext accepted frontier after the profile-parent preflight contract:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L5.28 documentation next-frontier pointer appended")
