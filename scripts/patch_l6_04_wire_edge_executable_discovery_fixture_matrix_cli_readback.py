...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_discovery_fixture_matrix_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_discovery_fixture_matrix_cli_readback.md")
TEST_PATH = Path("tests/test_l6_04_edge_executable_discovery_fixture_matrix_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l6_04_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L6.4 passive Microsoft Edge executable-discovery fixture matrix CLI/readback.

This wraps the accepted L6.3 fixture matrix in a CLI/readback checkpoint.
...
from patchops.llm_browser import live_adapter_edge_executable_discovery_fixture_matrix as l6_03

PATCH = "L6.4"
PHASE = "L6"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L6.4 Microsoft Edge Executable Discovery Fixture Matrix CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-discovery-fixture-matrix-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-discovery-fixture-matrix"
NEXT_PATCH = "L6.5 Live adapter Microsoft Edge executable discovery aggregate gate"
SIDE_EFFECT_BOUNDARY = "edge-executable-discovery-fixture-matrix-cli-readback-only"
...
def _source_l6_03_safe(summary: Mapping[str, Any]) -> bool:
    return (
        summary.get("patch") == "L6.3"
        ...
        and set(summary.get("edge_executable_fixture_ids", [])) == {"win_program_files_edge", "win_program_files_x86_edge", "win_localappdata_edge", "future_override_edge_path"}
...
def build_edge_executable_discovery_fixture_matrix_cli_readback(
    ...
    *,
    ...
    source = l6_03.build_edge_executable_discovery_fixture_matrix(
        root,
    ...
    checks = [
        _check("l6_03_executable_discovery_fixture_matrix_still_passes", _source_l6_03_safe(source), {"patch": source.get("patch"), "ok": source.get("ok"), "status": source.get("status")}),
        _check("l6_03_executable_discovery_fixture_matrix_remains_passive", _source_l6_03_safe(source), {"exe_probe": source.get("edge_executable_fixture_filesystem_probe_performed"), "browser_started": source.get("browser_started")}),
        _check("l6_04_fixture_matrix_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l6_03_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l6_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l6_docs_contain_fixture_matrix_readback_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("fixture_matrix_cli_readback_is_enforced", source.get("edge_executable_fixture_matrix_enforced") is True, {}),
        _check("fixture_ids_remain_stable", set(source.get("edge_executable_fixture_ids", [])) == {"win_program_files_edge", "win_program_files_x86_edge", "win_localappdata_edge", "future_override_edge_path"}, {"fixture_ids": source.get("edge_executable_fixture_ids")}),
        _check("edge_executable_filesystem_probe_stays_false", source.get("edge_executable_filesystem_probe_performed") is False and source.get("edge_executable_fixture_filesystem_probe_performed") is False, {}),
        _check("edge_executable_path_stays_unselected", source.get("edge_executable_path_selected") is False and source.get("edge_executable_fixture_path_selected") is False, {}),
        _check("edge_executable_launch_stays_unattempted", source.get("edge_executable_launch_attempted") is False and source.get("edge_executable_fixture_launch_attempted") is False, {}),
        ...
        _check("no_browser_profile_or_adapter_side_effects", _source_l6_03_safe(source), {}),
    ]
    ...
    return {
        ...
        "l6_03_executable_discovery_fixture_matrix_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l6_03_summary": source,
        "source_l6_02_summary": source.get("source_l6_02_summary", {}),
        "source_l6_01_summary": source.get("source_l6_01_summary", {}),
        "edge_executable_fixture_matrix_cli_readback_enforced": True,
        ...
        "edge_executable_fixture_matrix": source.get("edge_executable_fixture_matrix"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_discovery_fixture_matrix_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L6.4 Microsoft Edge executable discovery fixture matrix CLI/readback
...
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-discovery-fixture-matrix"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l6_04_candidate"
...
def test_l6_04_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l6_04_fixture_matrix_readback_dedicated_profile_passes_passively() -> None:
    ...
    assert payload["next_patch"] == "L6.5 Live adapter Microsoft Edge executable discovery aggregate gate"
    _assert_passive(payload)


def test_l6_04_default_and_missing_profile_cases_are_passive() -> None:
    default_payload = readback.build_edge_executable_discovery_fixture_matrix_cli_readback(
        PROJECT_ROOT,
        allow_live_start=True,
        profile_dir=DEFAULT_PROFILE,
    )
    missing_payload = readback.build_edge_executable_discovery_fixture_matrix_cli_readback(
        PROJECT_ROOT,
        allow_live_start=True,
        profile_dir=None,
    )
    assert default_payload["ok"] is True
    assert default_payload["source_l6_03_summary"]["source_l6_02_summary"]["source_l6_01_summary"]["source_l5_33_summary"]["source_l5_32_summary"]["source_l5_31_summary"]["source_l5_30_summary"]["default_profile_candidate_detected"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l6_03_summary"]["source_l6_02_summary"]["source_l6_01_summary"]["source_l5_33_summary"]["source_l5_32_summary"]["source_l5_31_summary"]["source_l5_30_summary"]["profile_parent_path_derived"] is False
    _assert_passive(missing_payload)


def test_l6_04_patchops_cli_json_readback_is_parseable_and_passive() -> None:
...
def test_l6_04_doc_mentions_fixture_matrix_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_discovery_fixture_matrix_cli_readback.md"
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L6.4 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L6.4 command wrapper appended")
    else:
        print("L6.4 command wrapper already present")


def _append_l6_03_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_discovery_fixture_matrix.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L6.4 Live adapter Microsoft Edge executable discovery fixture matrix CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable discovery fixture matrix:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L6.3 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l6_03_doc_pointer()
    print("L6.4 files written; validation is intentionally brief")
    return 0
