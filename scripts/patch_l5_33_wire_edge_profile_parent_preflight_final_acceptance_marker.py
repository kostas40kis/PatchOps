...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_supervised_launch_profile_parent_preflight_final_acceptance_marker.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_supervised_launch_profile_parent_preflight_final_acceptance_marker.md")
TEST_PATH = Path("tests/test_l5_33_edge_supervised_launch_profile_parent_preflight_final_acceptance_marker_current.py")
VALIDATE_PATH = Path("scripts/patch_l5_33_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L5.33 final acceptance marker for Edge profile-parent preflight.
...
DOC_CONTENT = r'''# L5.33 Microsoft Edge supervised launch profile parent preflight final acceptance marker
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L5.33 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L5.33 command wrapper appended")
    else:
        print("L5.33 command wrapper already present")


def _append_l5_32_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_supervised_launch_profile_parent_preflight_broad_validation_checkpoint.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L5.33 Live adapter Microsoft Edge supervised launch profile parent preflight final acceptance marker"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the broad validation checkpoint:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L5.32 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l5_32_doc_pointer()
    print("L5.33 files written; validation is intentionally brief")
    return 0
