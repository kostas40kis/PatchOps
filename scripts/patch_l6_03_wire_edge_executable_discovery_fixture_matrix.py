...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_discovery_fixture_matrix.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_discovery_fixture_matrix.md")
TEST_PATH = Path("tests/test_l6_03_edge_executable_discovery_fixture_matrix_current.py")
VALIDATE_PATH = Path("scripts/patch_l6_03_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L6.3 passive Microsoft Edge executable-discovery fixture matrix.

This module adds fixture-matrix readback over the accepted L6.2 executable
CLI/readback surface. It models candidate-path categories and expected passive
outcomes without probing the filesystem, selecting an executable, importing
Selenium, starting Edge, creating profiles, clicking/downloading, pasting/sending,
running packages, committing, or pushing.
"""
...
from patchops.llm_browser import live_adapter_edge_executable_discovery_cli_readback as l6_02

PATCH = "L6.3"
PHASE = "L6"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L6.3 Microsoft Edge Executable Discovery Fixture Matrix"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-discovery-fixture-matrix"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-discovery-readback"
NEXT_PATCH = "L6.4 Live adapter Microsoft Edge executable discovery fixture matrix CLI/readback"
SIDE_EFFECT_BOUNDARY = "edge-executable-discovery-fixture-matrix-only"
...
FIXTURE_MATRIX = (
    {
        "fixture_id": "win_program_files_edge",
        "category": "system_install_64bit",
        "candidate_path": r"C:\Program Files\Microsoft\Edge\Application\msedge.exe",
        "expected_probe_performed": False,
        "expected_selected": False,
        "expected_launch_attempted": False,
    },
    {
        "fixture_id": "win_program_files_x86_edge",
        "category": "system_install_32bit_compat",
        "candidate_path": r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe",
        "expected_probe_performed": False,
        "expected_selected": False,
        "expected_launch_attempted": False,
    },
    {
        "fixture_id": "win_localappdata_edge",
        "category": "per_user_install",
        "candidate_path": r"%LOCALAPPDATA%\Microsoft\Edge\Application\msedge.exe",
        "expected_probe_performed": False,
        "expected_selected": False,
        "expected_launch_attempted": False,
    },
    {
        "fixture_id": "future_override_edge_path",
        "category": "future_operator_override",
        "candidate_path": r"D:\PortableApps\Microsoft\Edge\Application\msedge.exe",
        "expected_probe_performed": False,
        "expected_selected": False,
        "expected_launch_attempted": False,
    },
...
REQUIRED_REPO_PATHS = (
    ...
    "scripts/patch_l6_02_brief_validate.py",
    "scripts/patch_l6_03_brief_validate.py",
)
...
def _source_l6_02_safe(summary: Mapping[str, Any]) -> bool:
    return (
        summary.get("patch") == "L6.2"
...
def _fixture_matrix_state() -> dict[str, Any]:
    ...
    return {
        "edge_executable_fixture_matrix_enforced": True,
        "edge_executable_fixture_matrix_modeled": True,
        "edge_executable_fixture_count": len(fixtures),
        "edge_executable_fixture_ids": [fixture["fixture_id"] for fixture in fixtures],
        "edge_executable_fixture_matrix": fixtures,
        ...
        "phase_allows_executable_selection": False,
        "phase_allows_live_start": False,
    }


def build_edge_executable_discovery_fixture_matrix(
    ...
    *,
    ...
    source = l6_02.build_edge_executable_discovery_cli_readback(
        root,
    ...
    checks = [
        _check("l6_02_executable_discovery_cli_readback_still_passes", _source_l6_02_safe(source), {"patch": source.get("patch"), "ok": source.get("ok"), "status": source.get("status")}),
        _check("l6_02_executable_discovery_cli_readback_remains_passive", _source_l6_02_safe(source), {"exe_probe": source.get("edge_executable_filesystem_probe_performed"), "browser_started": source.get("browser_started")}),
        _check("l6_03_executable_discovery_fixture_matrix_command_registered", command_state.get("ok") is True, command_state),
        _check("l6_02_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l6_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l6_docs_contain_executable_fixture_matrix_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("edge_executable_fixture_matrix_is_modeled", matrix["edge_executable_fixture_matrix_modeled"] is True and matrix["edge_executable_fixture_count"] >= 4, {"fixture_count": matrix["edge_executable_fixture_count"]}),
        _check("edge_executable_fixture_ids_are_stable", set(matrix["edge_executable_fixture_ids"]) == {"win_program_files_edge", "win_program_files_x86_edge", "win_localappdata_edge", "future_override_edge_path"}, {"fixture_ids": matrix["edge_executable_fixture_ids"]}),
        _check("edge_executable_fixture_paths_reference_msedge", all("msedge.exe" in fixture["candidate_path"].lower() for fixture in matrix["edge_executable_fixture_matrix"]), {}),
        _check("edge_executable_fixture_filesystem_probe_stays_false", matrix["edge_executable_fixture_filesystem_probe_performed"] is False and source.get("edge_executable_filesystem_probe_performed") is False, {}),
        _check("edge_executable_fixture_path_stays_unselected", matrix["edge_executable_fixture_path_selected"] is False and source.get("edge_executable_path_selected") is False, {}),
        _check("edge_executable_fixture_launch_stays_unattempted", matrix["edge_executable_fixture_launch_attempted"] is False and source.get("edge_executable_launch_attempted") is False, {}),
        ...
        _check("no_browser_profile_or_adapter_side_effects", _source_l6_02_safe(source), {}),
    ]
    ...
    return {
        ...
        "l6_02_executable_discovery_cli_readback_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l6_02_summary": source,
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"Fixture Count   : {payload.get('edge_executable_fixture_count')}",
        f"Exe Probe       : {payload.get('edge_executable_fixture_filesystem_probe_performed')}",
        f"Exe Selected    : {payload.get('edge_executable_fixture_path_selected')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_discovery_fixture_matrix(
        args.repo_root,
...
DOC_CONTENT = r'''# L6.3 Microsoft Edge executable discovery fixture matrix
...
COMMAND = "browser-start-supervised-launch-edge-executable-discovery-fixture-matrix"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l6_03_candidate"
...
def main() -> int:
    ...
    for name, allow, profile in cases:
        ...
        print(
            "PASS {0}: patch={1} source=L6.2 fixtures={2} exe_probe={3}".format(
                name,
                payload["patch"],
    ...
    print("PASS L6.3 brief validation: fixture matrix modeled with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L6.3 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L6.3 command wrapper appended")
    else:
        print("L6.3 command wrapper already present")


def _append_l6_02_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_discovery_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L6.3 Live adapter Microsoft Edge executable discovery fixture matrix"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable discovery CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L6.2 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l6_02_doc_pointer()
    print("L6.3 files written; validation is intentionally brief")
    return 0
