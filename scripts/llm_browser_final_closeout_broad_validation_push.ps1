param(
    ...
    [string]$PointerPath = '',
    [switch]$PlanOnly,
    [switch]$SkipFinalOperatorCheckpoint,
    [switch]$SkipFullPytest,
    [int]$TimeoutShortSeconds = 120,
    [int]$TimeoutFinalCheckpointSeconds = 3000
)
...
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    ...
    $ReportPath = Join-Path $ReportFolder ("patchops_llm_browser_final_closeout_broad_validation_push_{0}.txt" -f $stamp)
}

if ([string]::IsNullOrWhiteSpace($PointerPath)) {
    $PointerPath = Join-Path $desktop 'patchops_latest_llm_browser_final_closeout_broad_validation_push.txt'
}
...
$finalOperatorPointerPath = Join-Path $desktop 'patchops_latest_llm_browser_final_operator_commit_checkpoint.txt'
...
Add-Section 'PATCHOPS LLM-BROWSER FINAL CLOSEOUT BROAD VALIDATION AND PUSH'
...
Add-Line ("SkipFinalOperatorCheckpoint : {0}" -f ([bool]$SkipFinalOperatorCheckpoint))
Add-Line ("SkipFullPytest              : {0}" -f ([bool]$SkipFullPytest))
Add-Line ''
Add-Line 'Safety: this final closeout helper does not run git commit or git push. It validates evidence and prints manual operator commands.'

Add-Section 'PLAN'
Add-Line '1. Optionally run the final operator validation and commit checkpoint script.'
Add-Line '2. Read final operator, closeout, and broad-validation pointers.'
Add-Line '3. Parse latest broad-validation report with --strict.'
...
Add-Line ("- Final operator pointer   : {0}" -f $finalOperatorPointerPath)
Add-Line ("- Closeout pointer         : {0}" -f $closeoutPointerPath)
Add-Line ("- Broad validation pointer : {0}" -f $broadPointerPath)
Add-Line ("- Final closeout pointer   : {0}" -f $PointerPath)

if (-not $PlanOnly) {
    if (-not $SkipFinalOperatorCheckpoint) {
        $finalArgs = @(
            '-NoProfile',
            '-ExecutionPolicy',
            'Bypass',
            '-File',
            (Join-Path $resolvedRepoRoot 'scripts\llm_browser_final_operator_validation_commit_checkpoint.ps1'),
            '-RepoRoot',
            $resolvedRepoRoot
        )
        if ($SkipFullPytest) {
            $finalArgs += '-SkipFullPytest'
        }
        Invoke-NativeCapture -Label 'final operator validation and commit checkpoint' -FilePath $psFile -Arguments $finalArgs -WorkingDirectory $resolvedRepoRoot -TimeoutSeconds $TimeoutFinalCheckpointSeconds | Out-Null
    } else {
        Add-Section 'COMMAND: final operator validation and commit checkpoint'
        Add-Line 'Skipped because -SkipFinalOperatorCheckpoint was provided.'
    }

    $finalOperatorReportPath = Get-ReportPathFromPointer -Path $finalOperatorPointerPath
    if ([string]::IsNullOrWhiteSpace($finalOperatorReportPath)) {
        [void]$script:Failures.Add("latest final operator checkpoint pointer was missing or did not contain ReportPath: $finalOperatorPointerPath")
    } elseif (-not (Test-Path -LiteralPath $finalOperatorReportPath)) {
        [void]$script:Failures.Add("latest final operator checkpoint report path from pointer does not exist: $finalOperatorReportPath")
    } else {
        Add-Section 'FINAL OPERATOR REPORT POINTER CHECK'
        Add-Line ("FinalOperatorReportPath : {0}" -f $finalOperatorReportPath)
    }
...
Add-Section 'FINAL MANUAL COMMIT AND PUSH COMMANDS'
Add-Line 'Run these only after this final closeout report is PASS and you have reviewed final operator, closeout, and broad-validation reports.'
Add-Line ''
...
Add-Line ('notepad "{0}"' -f $finalOperatorPointerPath)
Add-Line ('notepad "{0}"' -f $closeoutPointerPath)
Add-Line ('notepad "{0}"' -f $broadPointerPath)
if (-not [string]::IsNullOrWhiteSpace($finalOperatorReportPath)) {
    Add-Line ('notepad "{0}"' -f $finalOperatorReportPath)
}
...
Add-Line ("PointerPath              : {0}" -f $PointerPath)
Add-Line ''
Add-Line 'Reminder: this final closeout helper never commits or pushes automatically.'
