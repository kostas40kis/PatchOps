...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix.md")
TEST_PATH = Path("tests/test_l11_03_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_current.py")
VALIDATE_PATH = Path("scripts/patch_l11_03_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L11.3 passive Microsoft Edge executable filesystem-probe real-probe preflight fixture matrix.

Builds a modeled fixture matrix over the accepted L11.2 real-probe preflight
CLI/readback. Real-probe preflight readiness may be true when all required
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_real_probe_preflight_cli_readback as l11_02

PATCH = "L11.3"
PHASE = "L11"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L11.3 Microsoft Edge Executable Filesystem Probe Real-Probe Preflight Fixture Matrix"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-fixture-matrix"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-readback"
NEXT_PATCH = "L11.4 Live adapter Microsoft Edge executable filesystem probe real-probe preflight fixture matrix CLI/readback"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-real-probe-preflight-fixture-matrix-only"
...
FIXTURE_MATRIX = (
    {"fixture_id": "no_gates", "allow_live_start": False, "allow_executable_probe": False, "activate_executable_filesystem_probe": False, "allow_real_filesystem_probe": False, "profile_kind": "dedicated", "expected_activation_ready": False, "expected_real_ready": False, "expected_real_exec": False, "expected_fs_probe": False, "expected_path_selected": False},
    {"fixture_id": "real_flag_only", "allow_live_start": False, "allow_executable_probe": False, "activate_executable_filesystem_probe": False, "allow_real_filesystem_probe": True, "profile_kind": "dedicated", "expected_activation_ready": False, "expected_real_ready": False, "expected_real_exec": False, "expected_fs_probe": False, "expected_path_selected": False},
    {"fixture_id": "activation_ready_without_real_flag", "allow_live_start": True, "allow_executable_probe": True, "activate_executable_filesystem_probe": True, "allow_real_filesystem_probe": False, "profile_kind": "dedicated", "expected_activation_ready": True, "expected_real_ready": False, "expected_real_exec": False, "expected_fs_probe": False, "expected_path_selected": False},
    {"fixture_id": "real_preflight_ready_dedicated_profile", "allow_live_start": True, "allow_executable_probe": True, "activate_executable_filesystem_probe": True, "allow_real_filesystem_probe": True, "profile_kind": "dedicated", "expected_activation_ready": True, "expected_real_ready": True, "expected_real_exec": False, "expected_fs_probe": False, "expected_path_selected": False},
    {"fixture_id": "real_preflight_ready_default_profile", "allow_live_start": True, "allow_executable_probe": True, "activate_executable_filesystem_probe": True, "allow_real_filesystem_probe": True, "profile_kind": "default", "expected_activation_ready": True, "expected_real_ready": True, "expected_real_exec": False, "expected_fs_probe": False, "expected_path_selected": False},
    {"fixture_id": "real_preflight_ready_missing_profile", "allow_live_start": True, "allow_executable_probe": True, "activate_executable_filesystem_probe": True, "allow_real_filesystem_probe": True, "profile_kind": "missing", "expected_activation_ready": True, "expected_real_ready": True, "expected_real_exec": False, "expected_fs_probe": False, "expected_path_selected": False},
)
...
def _source_l11_02_safe(source: Mapping[str, Any], *, expected_activation_ready: bool, expected_real_ready: bool) -> bool:
    return (
        source.get("patch") == "L11.2"
...
def _fixture_state() -> dict[str, Any]:
    ...
    return {
        "edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_enforced": True,
        "edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_modeled": True,
        "real_probe_preflight_fixture_count": len(fixtures),
        "real_probe_preflight_fixture_ids": [fixture["fixture_id"] for fixture in fixtures],
        "real_probe_preflight_fixture_matrix": fixtures,
...
def build_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix(
    ...
    *,
    ...
    source = l11_02.build_edge_executable_filesystem_probe_real_probe_preflight_cli_readback(
        root,
    ...
    source_safe = _source_l11_02_safe(source, expected_activation_ready=expected_activation_ready, expected_real_ready=expected_real_ready)
    ...
    checks = [
        _check("l11_02_real_probe_preflight_readback_still_passes", source_safe, _compact_source_status(source)),
        _check("l11_02_real_probe_preflight_readback_remains_passive", source_safe, {"real_ready": source.get("real_filesystem_probe_preflight_ready"), "real_exec": source.get("real_filesystem_probe_execution_allowed"), "fs_probe": source.get("edge_executable_filesystem_probe_performed")}),
        _check("l11_03_real_probe_preflight_fixture_matrix_command_registered", command_state.get("ok") is True, command_state),
        _check("l11_02_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l11_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l11_docs_contain_real_probe_preflight_fixture_matrix_boundary", doc_state.get("ok") is True, doc_state),
        _check("real_probe_preflight_fixture_matrix_is_enforced", fixtures["edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_enforced"] is True, fixtures),
        _check("real_probe_fixture_ids_are_stable", set(fixtures["real_probe_preflight_fixture_ids"]) == EXPECTED_FIXTURE_IDS, fixtures),
        ...
        _check("real_probe_execution_remains_blocked_by_phase", source.get("real_filesystem_probe_execution_allowed") is False and fixtures["fixture_real_probe_execution_allowed"] is False, {}),
    ...
    return {
        ...
        "l11_02_real_probe_preflight_cli_readback_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l11_02_status": _compact_source_status(source),
        **fixtures,
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L11.2 Accepted  : {payload.get('l11_02_real_probe_preflight_cli_readback_remains_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix(
        args.repo_root,
...
DOC_CONTENT = r'''# L11.3 Microsoft Edge executable filesystem probe real-probe preflight fixture matrix

L11.3 adds a passive fixture matrix over the accepted L11.2 real-probe preflight CLI/readback.
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-real-probe-preflight-fixture-matrix"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l11_03_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l11_02_real_probe_preflight_cli_readback_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix_enforced"] is True
    assert payload["real_filesystem_probe_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_gates = matrix.build_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False)
    assert_passive(no_gates)
    assert no_gates["real_filesystem_probe_preflight_ready"] is False
    print("PASS no_gates: patch=L11.3 fixtures=6 real_ready=False real_exec=False fs_probe=False path_selected=False")

    real_flag_only = matrix.build_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=True)
    ...
    activation_without_real = matrix.build_edge_executable_filesystem_probe_real_probe_preflight_fixture_matrix(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=False)
    ...
    for name, profile in [("real_preflight_ready_dedicated", DEDICATED), ("real_preflight_ready_default", DEFAULT), ("real_preflight_ready_missing_profile", None)]:
        ...
        assert payload["real_filesystem_probe_preflight_ready"] is True
        print("PASS {0}: patch=L11.3 source=L11.2 fixtures=6 real_ready=True real_exec=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; real-probe preflight fixture matrix stayed passive")
    print("PASS L11.3 brief validation: real-probe preflight fixture matrix accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L11.3 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L11.3 command wrapper appended")
    else:
        print("L11.3 command wrapper already present")


def _append_l11_02_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_real_probe_preflight_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L11.3 Live adapter Microsoft Edge executable filesystem probe real-probe preflight fixture matrix"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the real-probe preflight CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L11.2 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l11_02_doc_pointer()
    print("L11.3 files written; validation is intentionally brief and compact")
    return 0
