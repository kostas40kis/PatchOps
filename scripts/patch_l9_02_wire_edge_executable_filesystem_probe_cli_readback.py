...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_cli_readback.md")
TEST_PATH = Path("tests/test_l9_02_edge_executable_filesystem_probe_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l9_02_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L9.2 passive Microsoft Edge executable filesystem-probe CLI/readback.

Wraps the accepted L9.1 passive filesystem-probe contract in a compact
CLI/readback layer. The readback models contract readiness and candidate path
labels, but still performs no filesystem probe, no path selection, no launch,
no Selenium import, no browser start, no profile creation, no click/download,
no paste/send, no package run, no commit, and no push.
"""
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_passive_contract as l9_01

PATCH = "L9.2"
PHASE = "L9"
STATUS_PASS = "PASS"
STATUS_FAIL = "FAIL"
NAME = "L9.2 Microsoft Edge Executable Filesystem Probe CLI Readback"
COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-readback"
SOURCE_COMMAND_NAME = "browser-start-supervised-launch-edge-executable-filesystem-probe-contract"
NEXT_PATCH = "L9.3 Live adapter Microsoft Edge executable filesystem probe fixture matrix"
SIDE_EFFECT_BOUNDARY = "edge-executable-filesystem-probe-cli-readback-only"
...
def _source_l9_01_safe(summary: Mapping[str, Any], *, expect_contract_ready: bool, expect_preflight_passed: bool) -> bool:
    return (
        summary.get("patch") == "L9.1"
        and summary.get("ok") is True
        and summary.get("status") == STATUS_PASS
        and summary.get("command_name") == SOURCE_COMMAND_NAME
        and summary.get("l8_08_executable_probe_safety_preflight_final_marker_remains_accepted") is True
...
def build_edge_executable_filesystem_probe_cli_readback(
    ...
    *,
    ...
    source = l9_01.build_edge_executable_filesystem_probe_passive_contract(
        root,
    ...
    source_safe = _source_l9_01_safe(source, expect_contract_ready=expected_contract_ready, expect_preflight_passed=expected_preflight_passed)
    ...
    checks = [
        _check("l9_01_filesystem_probe_contract_still_passes", source_safe, _compact_source_status(source)),
        _check("l9_01_filesystem_probe_contract_remains_passive", source_safe, {"contract_ready": source.get("edge_executable_filesystem_probe_contract_ready"), "fs_probe": source.get("edge_executable_filesystem_probe_performed"), "path_selected": source.get("edge_executable_path_selected")}),
        _check("l9_02_filesystem_probe_readback_command_registered", command_state.get("ok") is True, command_state),
        _check("l9_01_source_command_still_registered", source_command_state.get("ok") is True, source_command_state),
        _check("edge_l9_required_source_docs_tests_present", not missing_paths, {"missing": missing_paths}),
        _check("edge_l9_docs_contain_filesystem_probe_readback_boundary", doc_state.get("ok") is True, {"missing_docs": doc_state.get("missing_docs"), "missing_phrases": doc_state.get("missing_phrases")}),
        _check("filesystem_probe_cli_readback_is_enforced", source.get("edge_executable_filesystem_probe_contract_enforced") is True, {}),
    ...
    return {
        ...
        "l9_01_executable_filesystem_probe_passive_contract_remains_accepted": source.get("ok") is True and source.get("status") == STATUS_PASS,
        "source_l9_01_status": _compact_source_status(source),
        "edge_executable_filesystem_probe_cli_readback_enforced": True,
...
def render_text(payload: Mapping[str, Any]) -> str:
    lines = [
        ...
        f"L9.1 Accepted   : {payload.get('l9_01_executable_filesystem_probe_passive_contract_remains_accepted')}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = build_edge_executable_filesystem_probe_cli_readback(
        args.repo_root,
...
DOC_CONTENT = r'''# L9.2 Microsoft Edge executable filesystem probe CLI/readback
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L9.2 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L9.2 command wrapper appended")
    else:
        print("L9.2 command wrapper already present")


def _append_l9_01_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_passive_contract.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L9.2 Live adapter Microsoft Edge executable filesystem probe CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable filesystem probe contract:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L9.1 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l9_01_doc_pointer()
    print("L9.2 files written; validation is intentionally brief and compact")
    return 0
