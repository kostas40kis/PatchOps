...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback.md")
TEST_PATH = Path("tests/test_l12_06_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback_current.py")
VALIDATE_PATH = Path("scripts/patch_l12_06_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L12.6 passive Microsoft Edge executable filesystem-probe execution preflight aggregate-gate CLI/readback.
...
DOC_CONTENT = r'''# L12.6 Microsoft Edge executable filesystem probe execution preflight aggregate gate CLI/readback

L12.6 adds a passive CLI/readback layer over the accepted L12.5 execution preflight aggregate gate.
...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback as readback

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-aggregate-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-aggregate-gate"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l12_06_candidate"
...
def test_l12_06_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l12_06_aggregate_readback_not_ready_without_all_required_gates() -> None:
    for kwargs in [
        ...
        payload = readback.build_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback(PROJECT_ROOT, **kwargs)
        assert payload["ok"] is True
        assert payload["patch"] == "L12.6"
...
def test_l12_06_execution_preflight_ready_but_execution_still_blocked() -> None:
    payload = readback.build_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback(
        PROJECT_ROOT,
    ...
    assert payload["execution_preflight_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["next_patch"] == "L12.7 Live adapter Microsoft Edge executable filesystem probe execution preflight broad validation checkpoint"
    _assert_passive(payload)


def test_l12_06_default_and_missing_profile_cases_are_passive_when_execution_preflight_ready() -> None:
    default_payload = readback.build_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=True)
    missing_payload = readback.build_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l12_05_status"]["ok"] is True
    assert default_payload["execution_preflight_ready"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l12_05_status"]["ok"] is True
    assert missing_payload["execution_preflight_ready"] is True
    _assert_passive(missing_payload)


def test_l12_06_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l12_06_doc_mentions_execution_preflight_aggregate_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback.md"
...
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-preflight-aggregate-readback"
DEDICATED = ROOT / "data" / "runtime" / "browser_profiles" / "edge_l12_06_candidate"
...
def assert_passive(payload: dict) -> None:
    ...
    assert payload["status"] == "PASS"
    assert payload["l12_05_execution_preflight_aggregate_gate_remains_accepted"] is True
    assert payload["edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback_enforced"] is True
    assert payload["execution_preflight_execution_allowed"] is False
    assert payload["edge_executable_filesystem_probe_performed"] is False
    assert payload["edge_executable_path_selected"] is False
    assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    no_gates = readback.build_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False, allow_executable_filesystem_probe_execution=False)
    assert_passive(no_gates)
    assert no_gates["execution_preflight_ready"] is False
    print("PASS no_gates: patch=L12.6 source=L12.5 chain=True aggregate_readback=True exec_ready=False exec_allowed=False fs_probe=False path_selected=False")

    execution_flag_only = readback.build_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback(ROOT, allow_live_start=False, profile_dir=DEDICATED, allow_executable_probe=False, activate_executable_filesystem_probe=False, allow_real_filesystem_probe=False, allow_executable_filesystem_probe_execution=True)
    assert_passive(execution_flag_only)
    assert execution_flag_only["execution_preflight_ready"] is False
    print("PASS execution_flag_only: chain=True aggregate_readback=True exec_ready=False exec_allowed=False fs_probe=False path_selected=False")

    real_ready_without_execution = readback.build_edge_executable_filesystem_probe_execution_preflight_aggregate_gate_cli_readback(ROOT, allow_live_start=True, profile_dir=DEDICATED, allow_executable_probe=True, activate_executable_filesystem_probe=True, allow_real_filesystem_probe=True, allow_executable_filesystem_probe_execution=False)
    ...
    print("PASS real_ready_without_execution_flag: chain=True aggregate_readback=True real_ready=True exec_ready=False exec_allowed=False fs_probe=False path_selected=False")
    ...
    for name, profile in [("execution_preflight_ready_dedicated", DEDICATED), ("execution_preflight_ready_default", DEFAULT), ("execution_preflight_ready_missing_profile", None)]:
        ...
        assert payload["execution_preflight_ready"] is True
        print("PASS {0}: patch=L12.6 source=L12.5 fixtures=6 exec_ready=True exec_allowed=False fs_probe=False path_selected=False".format(name))
    ...
    assert len(completed.stdout) < 80000
    assert_passive(payload)
    print("PASS main_cli: compact JSON parsed quickly; execution-preflight aggregate gate CLI/readback stayed passive")
    print("PASS L12.6 brief validation: execution-preflight aggregate gate CLI/readback accepted with no filesystem probe or browser side effects")
    return 0
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L12.6 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L12.6 command wrapper appended")
    else:
        print("L12.6 command wrapper already present")


def _append_l12_05_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_preflight_aggregate_gate.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L12.6 Live adapter Microsoft Edge executable filesystem probe execution preflight aggregate gate CLI/readback"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the execution preflight aggregate gate:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L12.5 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l12_05_doc_pointer()
    print("L12.6 files written; validation is intentionally brief and compact")
    return 0
