...
REPAIR_NOTE_PATH = Path("docs/llm_browser_live_adapter_edge_executable_filesystem_probe_execution_contract_repair_note.md")
...
def selected_candidate(payload: dict) -> dict | None:
    selected = payload.get("edge_executable_selected_path")
    if not selected:
        return None
    for item in payload.get("edge_executable_probe_candidates", []) or []:
...
def fixture_candidate(payload: dict) -> dict | None:
    expected = str(EXISTING_FIXTURE)
    for item in payload.get("edge_executable_probe_candidates", []) or []:
        if item.get("path") == expected:
            return item
    return None
...
def assert_truthful_selection(payload: dict) -> None:
    if payload["edge_executable_path_selected"]:
        ...
        assert selected.get("exists") is True
        assert selected.get("is_file") is True
        assert selected.get("path") == payload["edge_executable_selected_path"]
    else:
        assert payload["edge_executable_selected_path"] is None
...
def main() -> int:
    ...
    observed_fixture = fixture_candidate(ready)
    ...
    print("PASS ready_probe: patch=L13.1 source=L12.8 exec_ready=True probe_allowed=True probe_performed=True fixture_observed=True selected_path={0} launch=False".format(ready.get("edge_executable_selected_path") or "(none)"))
    ...
    observed_fixture = fixture_candidate(payload)
    assert observed_fixture is not None
    assert observed_fixture.get("exists") is True
    assert observed_fixture.get("is_file") is True
    assert_truthful_selection(payload)
    print("PASS main_cli: compact JSON parsed quickly; read-only filesystem probe was truthful and launched nothing")
    print("PASS L13.1a repair validation: environment-dependent selected path assertion repaired")
    return 0
...
REPAIR_NOTE = r'''# L13.1a repair note
...
def _patch_doc() -> None:
    ...
    phrase = "the selected path, if any, is the first existing file from the reported candidate order"
...
def main() -> int:
    ...
    _write(REPAIR_NOTE_PATH, REPAIR_NOTE)
    _patch_doc()
    print("L13.1a repair applied: selected-path assertion is now environment-independent")
    return 0
