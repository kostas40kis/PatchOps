...
MODULE_PATH = Path("patchops/llm_browser/live_adapter_edge_executable_probe_safety_preflight_fixture_matrix.py")
DOC_PATH = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_fixture_matrix.md")
TEST_PATH = Path("tests/test_l8_03_edge_executable_probe_safety_preflight_fixture_matrix_current.py")
VALIDATE_PATH = Path("scripts/patch_l8_03_brief_validate.py")
COMMANDS_PATH = Path("patchops/llm_browser/commands.py")

MODULE_CONTENT = r'''"""L8.3 passive Microsoft Edge executable-probe safety-preflight fixture matrix.
...
DOC_CONTENT = r'''# L8.3 Microsoft Edge executable probe safety preflight fixture matrix
...
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-probe-safety-preflight-readback"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l8_03_candidate"
...
def test_l8_03_command_is_registered() -> None:
    from patchops.llm_browser import commands
...
def test_l8_03_no_auth_fixture_matrix_readback_ok_preflight_false() -> None:
    ...
    assert all(f["expected_readback_ok"] is True for f in payload["edge_executable_probe_safety_preflight_fixture_matrix"])
    assert any(f["expected_preflight_passed"] is False for f in payload["edge_executable_probe_safety_preflight_fixture_matrix"])
    _assert_passive(payload)


def test_l8_03_authorized_fixture_matrix_passes_preflight_but_probe_stays_blocked() -> None:
    payload = matrix.build_edge_executable_probe_safety_preflight_fixture_matrix(
        PROJECT_ROOT,
    ...
    assert payload["edge_executable_probe_preflight_allows_probe_execution"] is False
    assert set(payload["edge_executable_probe_safety_preflight_fixture_ids"]) == EXPECTED_IDS
    assert all(f["expected_probe_execution_allowed"] is False for f in payload["edge_executable_probe_safety_preflight_fixture_matrix"])
    assert payload["next_patch"] == "L8.4 Live adapter Microsoft Edge executable probe safety preflight fixture matrix CLI/readback"
    _assert_passive(payload)


def test_l8_03_default_and_missing_profile_cases_are_passive_when_authorized() -> None:
    default_payload = matrix.build_edge_executable_probe_safety_preflight_fixture_matrix(PROJECT_ROOT, allow_live_start=True, profile_dir=DEFAULT_PROFILE, allow_executable_probe=True)
    missing_payload = matrix.build_edge_executable_probe_safety_preflight_fixture_matrix(PROJECT_ROOT, allow_live_start=True, profile_dir=None, allow_executable_probe=True)
    assert default_payload["ok"] is True
    assert default_payload["source_l8_02_status"]["ok"] is True
    _assert_passive(default_payload)
    assert missing_payload["ok"] is True
    assert missing_payload["source_l8_02_status"]["ok"] is True
    _assert_passive(missing_payload)


def test_l8_03_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
...
def test_l8_03_doc_mentions_safety_preflight_fixture_matrix_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_probe_safety_preflight_fixture_matrix.md"
...
def _append_command_block() -> None:
    ...
    if "# PATCHOPS L8.3 START" not in text:
        if not text.endswith("\n"):
            text += "\n"
        COMMANDS_PATH.write_text(text + "\n" + COMMAND_BLOCK.strip() + "\n", encoding="utf-8")
        print("L8.3 command wrapper appended")
    else:
        print("L8.3 command wrapper already present")


def _append_l8_02_doc_pointer() -> None:
    path = Path("docs/llm_browser_live_adapter_edge_executable_probe_safety_preflight_cli_readback.md")
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    phrase = "L8.3 Live adapter Microsoft Edge executable probe safety preflight fixture matrix"
    if phrase not in text:
        path.write_text(text.rstrip() + "\n\nNext accepted frontier after the executable probe safety preflight CLI/readback:\n\n`" + phrase + "`\n", encoding="utf-8")
        print("L8.2 doc next-frontier pointer appended")
...
def main() -> int:
    ...
    _append_l8_02_doc_pointer()
    print("L8.3 files written; validation is intentionally brief and compact")
    return 0
