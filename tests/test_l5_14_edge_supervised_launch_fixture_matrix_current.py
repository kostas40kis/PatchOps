...
from patchops.llm_browser import live_adapter_edge_supervised_launch_fixture_matrix as edge_matrix

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-fixtures"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-readiness-readback"
...
def test_l5_14_edge_fixture_matrix_payload_is_passive_and_edge_first() -> None:
    payload = edge_matrix.build_edge_supervised_launch_fixture_matrix(PROJECT_ROOT)
    checks = {check["name"]: check for check in payload["checks"]}

    assert payload["ok"] is True
    assert payload["status"] == "PASS"
    assert payload["patch"] == "L5.14"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["edge_first"] is True
    assert payload["browser_priority"] == ["edge", "opera"]
    assert payload["next_patch"] == "L5.15 Live adapter Microsoft Edge supervised launch fixture matrix CLI/readback"
    ...
    required_checks = {
        "l5_13_edge_readiness_cli_readback_still_passes",
        "l5_13_source_command_still_registered",
        "l5_14_edge_fixture_matrix_command_registered",
        "edge_fixture_matrix_has_expected_cases",
        "edge_fixture_matrix_blocks_live_side_effects",
        "edge_remains_first_supported_live_browser",
        "l5_14_artifacts_present",
...
def test_l5_14_fixture_matrix_contains_expected_edge_cases() -> None:
    payload = edge_matrix.build_edge_supervised_launch_fixture_matrix(PROJECT_ROOT)
    ...
    assert payload["edge_fixture_matrix_summary"]["case_count"] == 6
...
def test_l5_14_preserves_l5_13_readback_and_l5_12_contract() -> None:
    payload = edge_matrix.build_edge_supervised_launch_fixture_matrix(PROJECT_ROOT)
    l5_13 = payload["l5_13_edge_readiness_cli_readback"]
    l5_12 = l5_13["l5_12_edge_readiness_contract"]
    contract = l5_12["edge_readiness_contract"]

    assert l5_13["patch"] == "L5.13"
    assert l5_13["ok"] is True
    assert l5_12["patch"] == "L5.12"
    assert l5_12["ok"] is True
...
def test_l5_14_command_name_is_registered_in_llm_browser_cli() -> None:
    commands = _commands()
...
def test_l5_14_main_cli_readback_is_passive_json() -> None:
...
def test_l5_14_module_cli_text_is_operator_safe() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "patchops.llm_browser.live_adapter_edge_supervised_launch_fixture_matrix",
            "--repo-root",
            str(PROJECT_ROOT),
        ],
        cwd=PROJECT_ROOT,
    ...
    assert "PatchOps L5.14 Microsoft Edge supervised-launch fixture matrix" in stdout
    assert "Status          : PASS" in stdout
    assert "Command         : browser-start-supervised-launch-edge-fixtures" in stdout
    assert "Source Command  : browser-start-supervised-launch-edge-readiness-readback" in stdout
    ...
    assert "Next Patch      : L5.15 Live adapter Microsoft Edge supervised launch fixture matrix CLI/readback" in stdout


def test_l5_14_module_readback_does_not_import_selenium() -> None:
...
def test_l5_14_docs_are_present_and_passive() -> None:
    canonical = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_supervised_launch_fixture_matrix.md"
