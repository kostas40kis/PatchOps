...
from patchops.llm_browser import live_adapter_edge_supervised_launch_l5_broad_validation_checkpoint_cli_readback as readback

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-l5-broad-validation-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-l5-broad-validation"
...
def test_l5_22_broad_validation_readback_payload_is_passive_and_edge_first() -> None:
    payload = readback.build_edge_supervised_launch_l5_broad_validation_checkpoint_cli_readback(PROJECT_ROOT)
    checks = {check["name"]: check for check in payload["checks"]}

    assert payload["ok"] is True
    assert payload["status"] == "PASS"
    assert payload["patch"] == "L5.22"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["edge_first"] is True
    assert payload["browser_priority"] == ["edge", "opera"]
    assert payload["next_patch"] == "L5.23 Live adapter Microsoft Edge supervised launch live-start preflight contract"
    ...
    required_checks = {
        "l5_21_edge_l5_broad_validation_checkpoint_still_passes",
        "l5_21_edge_l5_broad_validation_checkpoint_remains_passive",
        "l5_21_source_command_still_registered",
        "l5_22_broad_validation_readback_command_registered",
        "edge_l5_required_command_set_registered",
        "edge_l5_required_source_docs_tests_present",
        "edge_l5_docs_contain_safety_boundary",
        "l5_22_command_plan_is_readback_only",
...
def test_l5_22_wraps_accepted_l5_21_broad_validation_checkpoint() -> None:
    payload = readback.build_edge_supervised_launch_l5_broad_validation_checkpoint_cli_readback(PROJECT_ROOT)
    summary = payload["l5_21_summary"]
    source = payload["l5_21_edge_l5_broad_validation_checkpoint"]

    assert summary["ok"] is True
    assert summary["status"] == "PASS"
    assert summary["patch"] == "L5.21"
    ...
    assert summary["command_plan_ok"] is True
    assert summary["executed_validation_commands"] == []
    ...
    assert source["patch"] == "L5.21"


def test_l5_22_required_command_set_is_registered_in_llm_browser_cli() -> None:
    commands = _commands()
...
def test_l5_22_main_cli_readback_is_passive_json() -> None:
    ...
    assert payload["l5_21_summary"]["status"] == "PASS"
    assert payload["doc_state"]["ok"] is True


def test_l5_22_module_cli_text_is_operator_safe() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "patchops.llm_browser.live_adapter_edge_supervised_launch_l5_broad_validation_checkpoint_cli_readback",
            "--repo-root",
            str(PROJECT_ROOT),
        ],
        cwd=PROJECT_ROOT,
    ...
    assert "PatchOps L5.22 Microsoft Edge supervised-launch L5 broad validation checkpoint CLI/readback" in stdout
    assert "Status          : PASS" in stdout
    assert "Command         : browser-start-supervised-launch-edge-l5-broad-validation-readback" in stdout
    assert "Source Command  : browser-start-supervised-launch-edge-l5-broad-validation" in stdout
    ...
    assert "Next Patch      : L5.23 Live adapter Microsoft Edge supervised launch live-start preflight contract" in stdout


def test_l5_22_module_readback_does_not_import_selenium() -> None:
...
def test_l5_22_docs_are_present_and_passive() -> None:
    payload = readback.build_edge_supervised_launch_l5_broad_validation_checkpoint_cli_readback(PROJECT_ROOT)
    ...
    canonical = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_supervised_launch_l5_broad_validation_checkpoint_cli_readback.md"
