...
def test_l14_08_command_is_registered() -> None:
...
def test_l14_08_profile_broad_checkpoint_is_truthful_passive_and_allowed() -> None:
    payload = l14_08.build_edge_dedicated_profile_lifecycle_broad_validation_checkpoint(PROJECT_ROOT)
    assert payload["ok"] is True
    assert payload["status"] == "PASS"
    assert payload["patch"] == "L14.8"
    assert payload["phase"] == "L14"
    assert payload["source_patch"] == "L14.7"
    ...
    assert payload["dedicated_profile_lifecycle_broad_validation_checkpoint_enforced"] is True
    ...
    assert payload["profile_broad_checkpoint_truthful"] is True
    assert payload["profile_broad_checkpoint_is_passive"] is True
    assert payload["planned_broad_validation_commands_are_passive"] is True
    assert payload["broad_validation_commands_executed_by_checkpoint"] is False
    assert any("pytest" in item and "test_l14_08" in item for item in payload["planned_broad_validation_commands"])
    ...
    assert payload["source_l14_07_status"]["profile_created"] is False
    _assert_passive(payload)
    assert payload["next_patch"] == "L14.9 Microsoft Edge dedicated profile lifecycle final acceptance marker, still no launch"


def test_l14_08_rejects_disallowed_profile_path_and_stays_passive() -> None:
    payload = l14_08.build_edge_dedicated_profile_lifecycle_broad_validation_checkpoint(PROJECT_ROOT, profile_relative_path="../outside_profile")
    assert payload["ok"] is False
    assert payload["profile_candidate_under_allowed_runtime_root"] is False
    assert payload["profile_broad_checkpoint_truthful"] is False
    assert payload["profile_directory_created"] is False
    assert payload["profile_directory_mutated"] is False
    _assert_passive(payload)


def test_l14_08_cli_compact_json_is_parseable_and_passive() -> None:
...
def test_l14_08_doc_mentions_required_boundaries() -> None:
    text = (PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_dedicated_profile_lifecycle_broad_validation_checkpoint.md").read_text(encoding="utf-8")
    for phrase in [
