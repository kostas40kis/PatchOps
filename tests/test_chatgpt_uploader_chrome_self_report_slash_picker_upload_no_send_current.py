...
from patchops.chatgpt_uploader.chrome_self_report_slash_picker_upload_no_send import (
    BLOCKED_CHROME_SLASH_ATTACHMENT_NOT_VERIFIED,
    BLOCKED_CHROME_SLASH_CONFIRMATION_MISMATCH,
    BLOCKED_CHROME_SLASH_CONFIRMATION_REQUIRED,
    BLOCKED_CHROME_SLASH_LIVE_BROWSER_REQUIRED,
    BLOCKED_CHROME_SLASH_OPEN_CLICK_FAILED,
    BLOCKED_CHROME_SLASH_PATH_WRITE_FAILED,
    BLOCKED_CHROME_SLASH_PICKER_AMBIGUOUS,
    BLOCKED_CHROME_SLASH_PICKER_NOT_OPENED,
    BLOCKED_CHROME_SLASH_REPORT_HASH_MISMATCH,
    BLOCKED_CHROME_SLASH_TARGET_NOT_READY,
    CONFIRM_CHROME_SLASH_ENTER_ENTER_SELF_REPORT_UPLOAD_NO_SEND,
    PASS_CHROME_SELF_REPORT_SLASH_ENTER_ENTER_ATTACHED_NO_SEND,
    run_slash_picker_upload_no_send,
    sha256_file,
    sha256_text,
    write_evidence,
)
...
def make_report(tmp_path: Path) -> Path:
    report = tmp_path / "pseudo_self_report_slash_picker_upload_no_send_report.txt"
    report.write_text("pseudo patch slash picker self report\n", encoding="utf-8")
    return report
...
def run_ready(report: Path, **overrides: object):
    values = {
        ...
        "provider": "fake-ready",
        "live_browser": True,
        "confirmation_text": CONFIRM_CHROME_SLASH_ENTER_ENTER_SELF_REPORT_UPLOAD_NO_SEND,
        "self_report_path": str(report),
        "expected_self_report_sha256": sha256_file(report),
    }
    values.update(overrides)
    return run_slash_picker_upload_no_send(**values)  # type: ignore[arg-type]


def test_fake_ready_slash_picker_attach_no_send(tmp_path: Path) -> None:
    report = make_report(tmp_path)
    result = run_ready(report)

    assert result.ok is True
    assert result.result_label == PASS_CHROME_SELF_REPORT_SLASH_ENTER_ENTER_ATTACHED_NO_SEND
    assert result.selected_action == "upload_self_report_slash_enter_enter_no_send"
...
def test_confirmation_required(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_CONFIRMATION_REQUIRED
...
def test_confirmation_must_match(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_CONFIRMATION_MISMATCH
    assert result.browser_action_performed is False
...
def test_live_browser_required(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_LIVE_BROWSER_REQUIRED
    assert result.browser_action_performed is False
...
def test_report_hash_must_match(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_REPORT_HASH_MISMATCH
    assert result.browser_action_performed is False
...
def test_fake_target_missing(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_TARGET_NOT_READY
...
def test_fake_no_picker(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_PICKER_NOT_OPENED
    assert result.slash_key_pressed is True
    assert result.upload_command_enter_pressed is True
    assert result.file_upload_attempted is False
...
def test_fake_ambiguous_picker(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_PICKER_AMBIGUOUS
...
def test_fake_path_failed(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_PATH_WRITE_FAILED
...
def test_fake_open_failed(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_OPEN_CLICK_FAILED
    assert result.path_written is True
    assert result.open_button_clicked is False
...
def test_fake_not_verified(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_CHROME_SLASH_ATTACHMENT_NOT_VERIFIED
    assert result.path_written is True
    assert result.open_button_clicked is True
...
def test_write_slash_picker_evidence(tmp_path: Path) -> None:
    report = make_report(tmp_path)
    result = run_ready(report)
    json_path = tmp_path / "slash_picker.json"
    txt_path = tmp_path / "slash_picker.txt"

    write_evidence(result, json_output_path=json_path, txt_output_path=txt_path)

    payload = json.loads(json_path.read_text(encoding="utf-8"))
    assert payload["result_label"] == PASS_CHROME_SELF_REPORT_SLASH_ENTER_ENTER_ATTACHED_NO_SEND
    ...
    assert "result_label: PASS_CHROME_SELF_REPORT_SLASH_ENTER_ENTER_ATTACHED_NO_SEND" in text
    assert "slash_key_pressed: true" in text
    assert "send_button_pressed: false" in text

def test_slash_enter_enter_trigger_contract() -> None:
    source = Path("patchops/chatgpt_uploader/chrome_self_report_slash_picker_upload_no_send.py").read_text(encoding="utf-8")

    assert 'keyboard.send_keys("/", pause=0.05)' in source
    assert source.count('keyboard.send_keys("{ENTER}", pause=0.05)') >= 2
    assert "second Enter picker trigger" in source
    assert "native file picker did not open after slash + Enter + Enter-if-needed" in source
    assert "{TAB}" not in source

def test_repair_09a_no_collapsed_picker_wait_if_line() -> None:
    source = Path("patchops/chatgpt_uploader/chrome_self_report_slash_picker_upload_no_send.py").read_text(encoding="utf-8")

    assert "pickers = self._wait_for_picker()        if len(pickers) == 0:" not in source
    assert "pickers = self._wait_for_picker()\n        if len(pickers) == 0:" in source or "pickers = self._wait_for_picker()\r\n        if len(pickers) == 0:" in source
    assert 'keyboard.send_keys("/", pause=0.05)' in source
    assert source.count('keyboard.send_keys("{ENTER}", pause=0.05)') >= 2
    assert "{TAB}" not in source

def test_repair_09d_detector_is_same_line_only() -> None:
    source = Path("patchops/chatgpt_uploader/chrome_self_report_slash_picker_upload_no_send.py").read_text(encoding="utf-8")
    test_source = Path("tests/test_chatgpt_uploader_chrome_self_report_slash_picker_upload_no_send_current.py").read_text(encoding="utf-8")

    assert "pickers = self._wait_for_picker()        if len(pickers) == 0:" not in source
    assert 'keyboard.send_keys("/", pause=0.05)' in source
    assert source.count('keyboard.send_keys("{ENTER}", pause=0.05)') >= 2
    assert "{TAB}" not in source
    assert "PASS_CHROME_SELF_REPORT_SLASH_ENTER_ENTER_ATTACHED_NO_SEND" in test_source
    assert "upload_self_report_slash_enter_enter_no_send" in test_source