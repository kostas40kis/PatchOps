...
COMMAND = "browser-start-supervised-launch-edge-downloaded-file-validation-controlled-authorization-gate"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-downloaded-file-validation-passive-plan-checkpoint"
TOKEN = "PATCHOPS_L19_EDGE_DOWNLOADED_FILE_VALIDATION_CONTROLLED_AUTHORIZED_READBACK_ONLY"
...
def test_l19_04_builds_default_authorization_gate() -> None:
    payload = l19_04.build_edge_downloaded_file_validation_controlled_authorization_gate(PROJECT_ROOT)

    assert payload["ok"] is True
    assert payload["patch"] == "L19.4"
    assert payload["source_patch"] == "L19.3"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["source_l19_3_passive_plan_checkpoint_accepted"] is True
    assert payload["l19_3_complete"] is True
    assert payload["downloaded_file_validation_authorization_requested"] is False
    assert payload["downloaded_file_validation_authorization_token_present"] is False
    assert payload["downloaded_file_validation_authorized_for_future_phase"] is False
    assert payload["downloaded_file_validation_authorization_is_readback_only_in_l19_4"] is True
    assert payload["future_authorization_gate_plan_is_planned_not_executed"] is True
    assert payload["future_authorization_gate_plan_blocks_file_validation_file_read_archive_package_run"] is True
    assert payload["missing_commands"] == []
    assert payload["missing_doc_phrases"] == []
    assert payload["required_repo_paths"]["ok"] is True
    assert payload["next_patch"] == "L19.5 Microsoft Edge first controlled downloaded-file metadata validation proof"
    _assert_passive(payload)


def test_l19_04_authorized_readback_still_blocks_validation_execution() -> None:
    payload = l19_04.build_edge_downloaded_file_validation_controlled_authorization_gate(
        PROJECT_ROOT,
    ...
    assert payload["ok"] is True
    assert payload["downloaded_file_validation_authorization_requested"] is True
    assert payload["downloaded_file_validation_authorization_token_present"] is True
    assert payload["downloaded_file_validation_authorized_for_future_phase"] is True
    assert payload["downloaded_file_validation_authorization_is_readback_only_in_l19_4"] is True
    assert payload["controlled_downloaded_file_validation_execution_allowed"] is False
...
def test_l19_04_wrong_token_does_not_authorize_future_phase() -> None:
    payload = l19_04.build_edge_downloaded_file_validation_controlled_authorization_gate(
        PROJECT_ROOT,
        allow_downloaded_file_validation_authorization=True,
...
def test_l19_04_plan_blocks_real_file_checks_archive_and_package_run() -> None:
    plan = l19_04.future_downloaded_file_validation_authorization_gate_plan()
    assert all(item["status"] == "planned_not_executed" for item in plan)
    assert plan[0]["name"] == "confirm_l19_3_passive_plan_checkpoint_accepted"
    assert plan[1]["name"] == "require_explicit_downloaded_file_validation_authorization_flag"
    assert plan[2]["required_authorization_token"] == TOKEN
    assert plan[3]["allowed_only_in_future_l19_5_or_later"] is True
    assert plan[3]["read_file_bytes"] is False
    assert plan[3]["archive_open"] is False
    assert plan[3]["package_run"] is False
    assert plan[4]["file_existence_check_in_l19_4"] is False
    assert plan[4]["file_stat_in_l19_4"] is False
    assert plan[4]["file_hash_in_l19_4"] is False
    assert plan[4]["file_byte_read_in_l19_4"] is False
    assert plan[-1]["downloaded_file_validation_execution_allowed"] is False
    assert plan[-1]["downloaded_file_exists_check_performed"] is False
...
def test_l19_04_cli_default_and_authorized_compact_json_smokes() -> None:
    ...
    assert default_payload["patch"] == "L19.4"
    assert default_payload["downloaded_file_validation_authorized_for_future_phase"] is False
    ...
    authorized_completed = subprocess.run(
        [
            ...
            "--target-url",
            "https://chatgpt.com/",
            "--allow-downloaded-file-validation-authorization",
            "--authorization-token",
            TOKEN,
            "--json",
            "--compact",
    ...
    assert authorized_payload["patch"] == "L19.4"
    assert authorized_payload["downloaded_file_validation_authorized_for_future_phase"] is True
    assert authorized_payload["downloaded_file_validation_execution_allowed"] is False
    assert authorized_payload["downloaded_file_validation_performed"] is False
...
def test_l19_04_rejects_disallowed_target_url_without_side_effects() -> None:
    payload = l19_04.build_edge_downloaded_file_validation_controlled_authorization_gate(
        PROJECT_ROOT,
    ...
    assert payload["source_l19_3_passive_plan_checkpoint_accepted"] is False
    assert payload["downloaded_file_validation_authorized_for_future_phase"] is True
    _assert_passive(payload)


def test_l19_04_command_registered() -> None:
...
def test_l19_04_doc_mentions_safety_contract() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_downloaded_file_validation_controlled_authorization_gate.md"
