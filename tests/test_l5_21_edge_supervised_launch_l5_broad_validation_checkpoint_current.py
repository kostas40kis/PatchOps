...
from patchops.llm_browser import live_adapter_edge_supervised_launch_l5_broad_validation_checkpoint as checkpoint

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-l5-broad-validation"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-l5-documentation-checkpoint"
...
def test_l5_21_broad_validation_checkpoint_payload_is_passive_and_edge_first() -> None:
    payload = checkpoint.build_edge_supervised_launch_l5_broad_validation_checkpoint(PROJECT_ROOT)
    checks = {check["name"]: check for check in payload["checks"]}

    assert payload["ok"] is True
    assert payload["status"] == "PASS"
    assert payload["patch"] == "L5.21"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["edge_first"] is True
    assert payload["browser_priority"] == ["edge", "opera"]
    assert payload["next_patch"] == "L5.22 Live adapter Microsoft Edge supervised launch L5 broad validation checkpoint CLI/readback"
    ...
    required_checks = {
        "l5_20_edge_l5_documentation_checkpoint_still_passes",
        "l5_20_edge_l5_documentation_checkpoint_remains_passive",
        "l5_20_source_command_still_registered",
        "l5_21_broad_validation_command_registered",
        "edge_l5_required_command_set_registered",
        "edge_l5_required_source_docs_tests_present",
        "edge_l5_docs_contain_safety_boundary",
        "broad_validation_command_plan_is_readback_only",
...
def test_l5_21_wraps_accepted_l5_20_documentation_checkpoint() -> None:
    payload = checkpoint.build_edge_supervised_launch_l5_broad_validation_checkpoint(PROJECT_ROOT)
    summary = payload["l5_20_summary"]
    source = payload["l5_20_edge_l5_documentation_checkpoint"]

    assert summary["ok"] is True
    assert summary["status"] == "PASS"
    assert summary["patch"] == "L5.20"
    assert summary["command_name"] == SOURCE_COMMAND
    assert summary["l5_19_patch"] == "L5.19"
    assert summary["l5_19_status"] == "PASS"
    ...
    assert source["patch"] == "L5.20"


def test_l5_21_required_command_set_is_registered_in_llm_browser_cli() -> None:
    commands = _commands()
...
def test_l5_21_broad_validation_command_plan_is_readback_only() -> None:
    payload = checkpoint.build_edge_supervised_launch_l5_broad_validation_checkpoint(PROJECT_ROOT)
    state = payload["command_plan_state"]
    plan = payload["broad_validation_command_plan"]
    ...
    assert any("test_l5_21_edge_supervised_launch_l5_broad_validation_checkpoint_current.py" in command for command in plan)
    assert any("live_adapter_edge_supervised_launch_l5_broad_validation_checkpoint" in command for command in plan)
    assert any(COMMAND in command for command in plan)
    assert any("git status --short --branch" in command for command in plan)


def test_l5_21_main_cli_readback_is_passive_json() -> None:
    ...
    assert payload["selenium_imported_by_readback"] is False
    assert payload["executed_validation_commands"] == []
    assert payload["l5_20_summary"]["status"] == "PASS"
    assert payload["doc_state"]["ok"] is True


def test_l5_21_module_cli_text_is_operator_safe() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "patchops.llm_browser.live_adapter_edge_supervised_launch_l5_broad_validation_checkpoint",
            "--repo-root",
            str(PROJECT_ROOT),
        ],
        cwd=PROJECT_ROOT,
    ...
    assert "PatchOps L5.21 Microsoft Edge supervised-launch L5 broad validation checkpoint" in stdout
    assert "Status          : PASS" in stdout
    assert "Command         : browser-start-supervised-launch-edge-l5-broad-validation" in stdout
    assert "Source Command  : browser-start-supervised-launch-edge-l5-documentation-checkpoint" in stdout
    ...
    assert "Planned broad-validation commands:" in stdout
    assert "test_l5_21_edge_supervised_launch_l5_broad_validation_checkpoint_current.py" in stdout
    assert "Next Patch      : L5.22 Live adapter Microsoft Edge supervised launch L5 broad validation checkpoint CLI/readback" in stdout


def test_l5_21_module_readback_does_not_import_selenium() -> None:
...
def test_l5_21_docs_are_present_and_passive() -> None:
    payload = checkpoint.build_edge_supervised_launch_l5_broad_validation_checkpoint(PROJECT_ROOT)
    ...
    canonical = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_supervised_launch_l5_broad_validation_checkpoint.md"
