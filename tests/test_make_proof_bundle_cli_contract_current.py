...
def test_make_proof_bundle_cli_generates_launcher_risk_bundle_that_review_rejects(tmp_path: Path) -> None:
    ...
    output_zip = tmp_path / "proof_launcher_risk_bundle.zip"
    build_result = build_bundle_zip(bundle_root, output_zip)
    assert build_result.ok is True

    inspect_result = _run_cli("inspect-bundle", str(output_zip))
    plan_result = _run_cli("plan-bundle", str(output_zip))

    assert inspect_result.returncode == 1, inspect_result.stderr
    assert plan_result.returncode == 1, plan_result.stderr

    inspect_payload = json.loads(inspect_result.stdout)
    plan_payload = json.loads(plan_result.stdout)

    assert inspect_payload["ok"] is False
    assert plan_payload["ok"] is False
    assert inspect_payload["launcher_status"] == "reject"
    assert plan_payload["launcher_status"] == "reject"
    assert "launcher_risk_detected" in inspect_payload["launcher_issue_codes"]
    assert "launcher_risk_detected" in plan_payload["launcher_issue_codes"]