...
from patchops.chatgpt_uploader.edge_report_uploader import (
    ...
    BLOCKED_SEND_RISK,
    CONFIRM_EDGE_FAIL_REPORT_UPLOAD_NO_SEND,
    PASS_EDGE_FAIL_REPORT_ATTACHED_NO_SEND,
    read_report_values_from_gate,
    run_edge_fail_report_upload_no_send_probe,
    sha256_file,
...
def ready_values(report: Path) -> dict[str, object]:
    return {
        ...
        "live_browser": True,
        "confirmation_text": CONFIRM_EDGE_FAIL_REPORT_UPLOAD_NO_SEND,
...
def run_ready(report: Path, **overrides: object):
    ...
    return run_edge_fail_report_upload_no_send_probe(**values)  # type: ignore[arg-type]


def test_fake_ready_attach_no_send_passes_and_never_submits(tmp_path: Path) -> None:
    report = make_report(tmp_path)
    result = run_ready(report)

    assert result.ok is True
    assert result.result_label == PASS_EDGE_FAIL_REPORT_ATTACHED_NO_SEND
...
def test_confirmation_is_required_before_live_action(tmp_path: Path) -> None:
    ...
    assert result.result_label == BLOCKED_EDGE_UPLOAD_CONFIRMATION_REQUIRED
...
def test_confirmation_mismatch_blocks_before_live_action(tmp_path: Path) -> None:
    ...
    assert result.ok is False
    assert result.result_label == BLOCKED_EDGE_UPLOAD_CONFIRMATION_MISMATCH
...
def test_selected_action_must_be_upload_operator_report(tmp_path: Path) -> None:
    ...
    assert result.ok is False
    assert result.result_label == BLOCKED_EDGE_UPLOAD_ROUTE_MISMATCH
    assert result.upload_trigger_attempted is False


def test_browser_lane_must_be_edge(tmp_path: Path) -> None:
    report = make_report(tmp_path)
    result = run_ready(report, browser_lane="chrome")
...
def test_target_not_ready_blocks(tmp_path: Path) -> None:
    ...
    assert result.ok is False
    assert result.result_label == BLOCKED_EDGE_TARGET_NOT_READY
    assert result.upload_trigger_attempted is False
    assert result.operator_report_uploaded is False
...
def test_no_attach_button_blocks_as_target_not_ready(tmp_path: Path) -> None:
    report = make_report(tmp_path)
    result = run_ready(report, provider="fake-no-attach-button")
...
def test_picker_failure_blocks_attachment_verified(tmp_path: Path) -> None:
    report = make_report(tmp_path)
...
def test_attachment_not_verified_blocks_outer_pass(tmp_path: Path) -> None:
    report = make_report(tmp_path)
...
def test_write_upload_evidence_redacts_title_and_records_no_send(tmp_path: Path) -> None:
    ...
    assert payload["result_label"] == PASS_EDGE_FAIL_REPORT_ATTACHED_NO_SEND
    ...
    assert "result_label: PASS_EDGE_FAIL_REPORT_ATTACHED_NO_SEND" in text
