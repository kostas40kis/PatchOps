...
COMMAND = "browser-start-supervised-launch-edge-downloaded-file-validation-cli-readback-checkpoint"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-downloaded-file-validation-passive-preflight-gate"
...
def test_l19_02_builds_passive_cli_readback_checkpoint() -> None:
    payload = l19_02.build_edge_downloaded_file_validation_cli_readback_checkpoint(PROJECT_ROOT)

    assert payload["ok"] is True
    assert payload["status"] == "PASS"
    assert payload["patch"] == "L19.2"
    assert payload["source_patch"] == "L19.1"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["l19_1_passive_preflight_gate_accepted"] is True
    assert payload["l19_1_default_compact_readback_ok"] is True
    assert payload["l19_1_authorized_compact_readback_ok"] is True
    assert payload["default_downloaded_file_validation_preflight_authorized"] is False
    assert payload["authorized_downloaded_file_validation_preflight_authorized"] is True
    assert payload["downloaded_file_validation_preflight_authorization_remains_readback_only"] is True
    ...
    assert payload["next_patch"] == "L19.3 Microsoft Edge downloaded-file validation passive plan checkpoint"
    _assert_passive(payload)


def test_l19_02_source_readbacks_are_compact_and_passive() -> None:
    payload = l19_02.build_edge_downloaded_file_validation_cli_readback_checkpoint(PROJECT_ROOT)
    default = payload["source_default_summary"]
    authorized = payload["source_authorized_summary"]

    assert default["ok"] is True
    assert default["patch"] == "L19.1"
    assert default["downloaded_file_validation_preflight_authorized"] is False
    assert default["downloaded_file_validation_execution_allowed"] is False
    assert default["downloaded_file_validation_active"] is False
    assert default["downloaded_file_validation_performed"] is False
    ...
    assert authorized["patch"] == "L19.1"
    assert authorized["downloaded_file_validation_preflight_authorized"] is True
    assert authorized["downloaded_file_validation_execution_allowed"] is False
    assert authorized["downloaded_file_validation_active"] is False
    assert authorized["downloaded_file_validation_performed"] is False
    assert authorized["downloaded_file_exists_check_performed"] is False
...
def test_l19_02_cli_compact_json_smoke_is_shallow() -> None:
...
def test_l19_02_rejects_disallowed_target_url_without_side_effects() -> None:
    ...
    assert payload["l19_1_authorized_compact_readback_ok"] is False
    _assert_passive(payload)


def test_l19_02_command_registered() -> None:
...
def test_l19_02_doc_mentions_safety_contract() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_downloaded_file_validation_cli_readback_checkpoint.md"
