...
SCRIPT = Path("scripts/run_pseudo_self_report_win32_dialog_filename_submit_no_send_apply_smoke.py")


def test_source_contract_uses_win32_dialog_filename_submit_no_clipboard_no_clicks() -> None:
    ...
    assert "FILE_NAME_CONTROL_ID = 1148" in source
    assert "GetDlgItem" in source
    assert "SetWindowTextW" in source
    assert "WM_SETTEXT" in source
    assert "candidate_filename_controls" in source
    assert "submit_dialog" in source
    assert "IDOK" in source
...
def test_fake_ready_writes_expected_evidence(tmp_path: Path) -> None:
    module = SourceFileLoader("win32_dialog_smoke", str(SCRIPT)).load_module()
    desktop = tmp_path / "Desktop"
    desktop.mkdir()
    report = desktop / "pseudo_self_report_upload_no_send_repair_16_win32_dialog_filename_submit_self_report.txt"
    ...
    assert payload["dialog_filename_control_attempted"] is True
    assert payload["dialog_filename_candidate_count"] >= 1
    assert payload["full_path_written_to_picker"] is True
    assert payload["dialog_submit_attempted"] is True
    ...
    assert "dialog_submit_attempted: true" in text
