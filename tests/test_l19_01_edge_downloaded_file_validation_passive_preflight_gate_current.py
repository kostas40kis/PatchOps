...
COMMAND = "browser-start-supervised-launch-edge-downloaded-file-validation-passive-preflight-gate"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-download-workflow-final-acceptance-marker"
TOKEN = "PATCHOPS_L19_EDGE_DOWNLOADED_FILE_VALIDATION_PREFLIGHT_AUTHORIZED_READBACK_ONLY"
...
def test_l19_01_default_readback_is_passive() -> None:
    payload = l19_01.build_edge_downloaded_file_validation_passive_preflight_gate(PROJECT_ROOT)

    assert payload["ok"] is True
    assert payload["patch"] == "L19.1"
    assert payload["source_patch"] == "L18.7"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["source_l18_7_download_workflow_final_marker_accepted"] is True
    assert payload["l18_download_workflow_stream_complete"] is True
    assert payload["downloaded_file_validation_preflight_requested"] is False
    assert payload["downloaded_file_validation_preflight_authorized"] is False
    assert payload["downloaded_file_validation_preflight_authorization_is_readback_only_in_l19_1"] is True
    assert payload["missing_commands"] == []
    assert payload["missing_doc_phrases"] == []
    assert payload["required_repo_paths"]["ok"] is True
    assert payload["next_patch"] == "L19.2 Microsoft Edge downloaded-file validation CLI/readback checkpoint"
    _assert_passive(payload)


def test_l19_01_authorized_readback_still_blocks_file_validation() -> None:
    payload = l19_01.build_edge_downloaded_file_validation_passive_preflight_gate(
        PROJECT_ROOT,
    ...
    assert payload["ok"] is True
    assert payload["downloaded_file_validation_preflight_requested"] is True
    assert payload["downloaded_file_validation_preflight_authorized"] is True
    assert payload["downloaded_file_validation_preflight_readback"]["downloaded_file_validation_preflight_authorization_token_present"] is True
...
def test_l19_01_wrong_token_does_not_authorize() -> None:
    payload = l19_01.build_edge_downloaded_file_validation_passive_preflight_gate(
        PROJECT_ROOT,
        allow_file_validation_preflight=True,
...
def test_l19_01_cli_default_and_authorized_compact_json_smokes() -> None:
    ...
    assert default_payload["downloaded_file_validation_preflight_authorized"] is False
    ...
    assert authorized_payload["downloaded_file_validation_preflight_authorized"] is True
...
def test_l19_01_rejects_disallowed_target_url_without_side_effects() -> None:
    payload = l19_01.build_edge_downloaded_file_validation_passive_preflight_gate(
        PROJECT_ROOT,
    ...
    assert payload["source_l18_7_download_workflow_final_marker_accepted"] is False
    _assert_passive(payload)


def test_l19_01_command_registered() -> None:
...
def test_l19_01_doc_mentions_safety_contract() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_downloaded_file_validation_passive_preflight_gate.md"
