...
COMMAND = "browser-start-supervised-launch-handoff-fixtures"
...
def test_l5_04_fixture_matrix_cli_command_is_registered() -> None:
    commands = _commands()
    assert COMMAND in commands.llm_browser_command_names()
    assert COMMAND in _subcommands(commands.build_parser())


def test_l5_04_fixture_matrix_cli_json_readback_is_passive() -> None:
...
def test_l5_04_fixture_matrix_cli_text_readback_is_operator_safe() -> None:
    completed = subprocess.run(
...
def test_l5_04_fixture_matrix_cli_matches_module_json() -> None:
...
def test_l5_04_fixture_matrix_cli_does_not_import_selenium_or_browser_optional_dependencies() -> None:
...
def test_l5_04_command_plan_stays_readback_only() -> None:
    completed = subprocess.run(
        [sys.executable, "-m", "patchops.cli", "llm-browser", COMMAND, "--repo-root", str(PROJECT_ROOT), "--json", "--compact"],
        cwd=PROJECT_ROOT,
        text=True,
    ...
    assert "live_adapter_browser_start_supervised_launch_handoff_fixtures" in command_text
...
def test_l5_04_docs_are_present_and_passive() -> None:
    canonical = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_browser_start_supervised_launch_handoff_fixture_matrix_cli_readback.md"
