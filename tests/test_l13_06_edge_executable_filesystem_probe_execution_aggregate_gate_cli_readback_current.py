...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_aggregate_gate_cli_readback as readback
...
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l13_06_candidate"
EXISTING_FIXTURE = PROJECT_ROOT / "data" / "runtime" / "edge_probe_fixture" / "msedge_l13_06.exe"
MISSING_FIXTURE = PROJECT_ROOT / "data" / "runtime" / "edge_probe_fixture" / "missing-msedge-l13-06.exe"
...
def _assert_no_launch_side_effects(payload: dict) -> None:
    ...
    assert payload["no_executable_launch_attempted_by_aggregate_readback"] is True
...
def _payload(**kwargs: object) -> dict:
    return readback.build_edge_executable_filesystem_probe_execution_aggregate_gate_readback(PROJECT_ROOT, **kwargs)


def test_l13_06_command_is_registered() -> None:
...
def test_l13_06_not_ready_cases_do_not_probe_and_keep_l13_05_accepted() -> None:
    cases = [
    ...
    for kwargs in cases:
        ...
        assert payload["patch"] == "L13.6"
        assert payload["source_patch"] == "L13.5"
        assert payload["source_l13_05_aggregate_gate_remains_accepted"] is True
        ...
        assert payload["execution_aggregate_gate_cli_readback_enforced"] is True
...
def test_l13_06_ready_case_probes_observes_fixture_and_keeps_truthful_selection() -> None:
    ...
    assert payload["patch"] == "L13.6"
    assert payload["source_patch"] == "L13.5"
    assert payload["source_l13_05_status"]["patch"] == "L13.5"
    assert payload["source_l13_05_aggregate_gate_remains_accepted"] is True
    assert payload["l13_04_fixture_matrix_readback_remains_accepted"] is True
    assert payload["l13_03_execution_fixture_matrix_remains_accepted"] is True
    assert payload["execution_aggregate_gate_cli_readback_enforced"] is True
    assert payload["execution_aggregate_gate_enforced"] is True
    ...
    _assert_truthful_selection(payload)
    _assert_no_launch_side_effects(payload)
    assert payload["next_patch"] == "L13.7 Live adapter Microsoft Edge executable filesystem probe execution broad validation checkpoint"


def test_l13_06_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
    EXISTING_FIXTURE.parent.mkdir(parents=True, exist_ok=True)
    EXISTING_FIXTURE.write_text("fixture executable placeholder", encoding="utf-8")
    ...
    assert len(completed.stdout) < 125000
...
def test_l13_06_doc_mentions_aggregate_readback_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_execution_aggregate_gate_cli_readback.md"
