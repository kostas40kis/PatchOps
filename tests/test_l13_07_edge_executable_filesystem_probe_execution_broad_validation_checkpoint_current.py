...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_broad_validation_checkpoint as broad

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-broad-validation-checkpoint"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-executable-filesystem-probe-execution-aggregate-gate-readback"
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l13_07_candidate"
EXISTING_FIXTURE = PROJECT_ROOT / "data" / "runtime" / "edge_probe_fixture" / "msedge_l13_07.exe"
MISSING_FIXTURE = PROJECT_ROOT / "data" / "runtime" / "edge_probe_fixture" / "missing-msedge-l13-07.exe"
...
def _assert_no_launch_side_effects(payload: dict) -> None:
    ...
    assert payload["no_executable_launch_attempted_by_broad_checkpoint"] is True
...
def _payload(**kwargs: object) -> dict:
    return broad.build_edge_executable_filesystem_probe_execution_broad_validation_checkpoint(PROJECT_ROOT, **kwargs)


def test_l13_07_command_is_registered() -> None:
...
def test_l13_07_not_ready_cases_do_not_probe_and_keep_l13_06_accepted() -> None:
    cases = [
    ...
    for kwargs in cases:
        ...
        assert payload["patch"] == "L13.7"
        assert payload["source_patch"] == "L13.6"
        assert payload["source_l13_06_aggregate_gate_readback_remains_accepted"] is True
        ...
        assert payload["broad_validation_checkpoint_enforced"] is True
        assert payload["planned_broad_validation_commands_are_passive"] is True
...
def test_l13_07_ready_case_probes_observes_fixture_and_keeps_truthful_selection() -> None:
    ...
    assert payload["patch"] == "L13.7"
    assert payload["source_patch"] == "L13.6"
    assert payload["source_l13_06_status"]["patch"] == "L13.6"
    assert payload["source_l13_06_aggregate_gate_readback_remains_accepted"] is True
    assert payload["l13_05_aggregate_gate_remains_accepted"] is True
    assert payload["l13_04_fixture_matrix_readback_remains_accepted"] is True
    assert payload["l13_03_execution_fixture_matrix_remains_accepted"] is True
    ...
    assert payload["planned_broad_validation_commands_are_passive"] is True
    assert payload["broad_validation_commands_executed_by_checkpoint"] is False
    assert any("pytest" in item and "test_l13_07" in item for item in payload["planned_broad_validation_commands"])
    ...
    _assert_truthful_selection(payload)
    _assert_no_launch_side_effects(payload)
    assert payload["next_patch"] == "L13.8 Live adapter Microsoft Edge executable filesystem probe execution final acceptance marker"


def test_l13_07_patchops_cli_json_readback_is_parseable_compact_and_passive() -> None:
    EXISTING_FIXTURE.parent.mkdir(parents=True, exist_ok=True)
    EXISTING_FIXTURE.write_text("fixture executable placeholder", encoding="utf-8")
    ...
    assert len(completed.stdout) < 150000
...
def test_l13_07_doc_mentions_broad_checkpoint_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_execution_broad_validation_checkpoint.md"
