...
COMMAND = "browser-start-supervised-launch-edge-dedicated-profile-lifecycle-aggregate-gate-cli-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-dedicated-profile-lifecycle-aggregate-gate"
...
def test_l14_07_command_is_registered() -> None:
...
def test_l14_07_profile_aggregate_cli_readback_is_truthful_passive_and_allowed() -> None:
    payload = l14_07.build_edge_dedicated_profile_lifecycle_aggregate_gate_cli_readback(PROJECT_ROOT)
    assert payload["ok"] is True
    assert payload["status"] == "PASS"
    assert payload["patch"] == "L14.7"
    assert payload["phase"] == "L14"
    assert payload["source_patch"] == "L14.6"
    assert payload["source_l13_complete"] is True
    assert payload["dedicated_profile_lifecycle_aggregate_gate_enforced"] is True
    assert payload["dedicated_profile_lifecycle_aggregate_gate_cli_readback_enforced"] is True
    assert payload["profile_aggregate_truthful"] is True
    assert payload["profile_aggregate_gate_is_passive"] is True
    assert payload["profile_aggregate_cli_readback_truthful"] is True
    assert payload["profile_aggregate_cli_readback_is_passive"] is True
    ...
    assert payload["source_l14_06_status"]["profile_created"] is False
    _assert_passive(payload)
    assert payload["next_patch"] == "L14.8 Microsoft Edge dedicated profile lifecycle broad validation checkpoint, still no launch"


def test_l14_07_rejects_disallowed_profile_path_and_stays_passive() -> None:
    payload = l14_07.build_edge_dedicated_profile_lifecycle_aggregate_gate_cli_readback(PROJECT_ROOT, profile_relative_path="../outside_profile")
    assert payload["ok"] is False
    assert payload["profile_candidate_under_allowed_runtime_root"] is False
    assert payload["profile_aggregate_cli_readback_truthful"] is False
    assert payload["profile_directory_created"] is False
    assert payload["profile_directory_mutated"] is False
    _assert_passive(payload)


def test_l14_07_cli_compact_json_is_parseable_and_passive() -> None:
...
def test_l14_07_doc_mentions_required_boundaries() -> None:
    text = (PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_dedicated_profile_lifecycle_aggregate_gate_cli_readback.md").read_text(encoding="utf-8")
    for phrase in [
