...
from patchops.llm_browser import live_adapter_edge_supervised_launch_readiness_cli_readback as edge_readback

PROJECT_ROOT = Path(__file__).resolve().parents[1]
COMMAND = "browser-start-supervised-launch-edge-readiness-readback"
SOURCE_COMMAND = "browser-start-supervised-launch-edge-readiness"
...
def test_l5_13_edge_readiness_cli_readback_payload_is_passive_and_edge_first() -> None:
    payload = edge_readback.build_edge_supervised_launch_readiness_cli_readback(PROJECT_ROOT)
    checks = {check["name"]: check for check in payload["checks"]}

    assert payload["ok"] is True
    assert payload["status"] == "PASS"
    assert payload["patch"] == "L5.13"
    assert payload["command_name"] == COMMAND
    assert payload["source_command_name"] == SOURCE_COMMAND
    assert payload["edge_first"] is True
    assert payload["browser_priority"] == ["edge", "opera"]
    assert payload["next_patch"] == "L5.14 Live adapter Microsoft Edge supervised launch fixture matrix"
    ...
    required_checks = {
        "l5_12_edge_readiness_contract_still_passes",
        "l5_12_source_command_still_registered",
        "l5_13_edge_readiness_readback_command_registered",
        "edge_readiness_contract_still_blocks_live_startup",
        "edge_remains_first_supported_live_browser",
        "l5_13_artifacts_present",
...
def test_l5_13_preserves_l5_12_edge_contract_and_review_boundary() -> None:
    payload = edge_readback.build_edge_supervised_launch_readiness_cli_readback(PROJECT_ROOT)
    l5_12 = payload["l5_12_edge_readiness_contract"]
    contract = l5_12["edge_readiness_contract"]

    assert l5_12["patch"] == "L5.12"
    assert l5_12["ok"] is True
    assert l5_12["status"] == "PASS"
...
def test_l5_13_command_name_is_registered_in_llm_browser_cli() -> None:
    commands = _commands()
...
def test_l5_13_main_cli_readback_is_passive_json() -> None:
    ...
    assert payload["ok"] is True
    assert payload["patch"] == "L5.13"
    assert payload["l5_12_edge_readiness_contract"]["patch"] == "L5.12"
    assert payload["browser_started"] is False
    assert payload["edge_process_started"] is False
    assert payload["browser_session_created"] is False
    assert payload["profile_directory_created"] is False
...
def test_l5_13_module_cli_text_is_operator_safe() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "patchops.llm_browser.live_adapter_edge_supervised_launch_readiness_cli_readback",
            "--repo-root",
            str(PROJECT_ROOT),
        ],
        cwd=PROJECT_ROOT,
    ...
    assert "PatchOps L5.13 Microsoft Edge supervised-launch readiness CLI/readback" in stdout
    assert "Status          : PASS" in stdout
    assert "Command         : browser-start-supervised-launch-edge-readiness-readback" in stdout
    assert "Source Command  : browser-start-supervised-launch-edge-readiness" in stdout
    ...
    assert "Next Patch      : L5.14 Live adapter Microsoft Edge supervised launch fixture matrix" in stdout


def test_l5_13_module_readback_does_not_import_selenium() -> None:
...
def test_l5_13_docs_are_present_and_passive() -> None:
    canonical = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_supervised_launch_readiness_cli_readback.md"
