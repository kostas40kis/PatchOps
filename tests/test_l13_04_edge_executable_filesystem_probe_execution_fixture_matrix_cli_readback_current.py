...
from patchops.llm_browser import live_adapter_edge_executable_filesystem_probe_execution_fixture_matrix_cli_readback as readback
...
DEDICATED_PROFILE = PROJECT_ROOT / "data" / "runtime" / "browser_profiles" / "edge_l13_04_candidate"
EXISTING_FIXTURE = PROJECT_ROOT / "data" / "runtime" / "edge_probe_fixture" / "msedge_l13_04.exe"
MISSING_FIXTURE = PROJECT_ROOT / "data" / "runtime" / "edge_probe_fixture" / "missing-msedge-l13-04.exe"
...
def _payload(**kwargs: object) -> dict:
    return readback.build_edge_executable_filesystem_probe_execution_fixture_matrix_readback(PROJECT_ROOT, **kwargs)


def test_l13_04_command_is_registered() -> None:
...
def test_l13_04_readback_not_ready_cases_do_not_probe() -> None:
    cases = [
    ...
    for kwargs in cases:
        ...
        assert payload["patch"] == "L13.4"
        assert payload["source_patch"] == "L13.3"
...
def test_l13_04_ready_case_probes_observes_fixture_and_never_launches() -> None:
    ...
    _assert_truthful_selection(payload)
    _assert_no_launch_side_effects(payload)
    assert payload["next_patch"] == "L13.5 Live adapter Microsoft Edge executable filesystem probe execution aggregate gate"


def test_l13_04_cli_compact_json_is_parseable_and_no_launch_side_effects() -> None:
    EXISTING_FIXTURE.parent.mkdir(parents=True, exist_ok=True)
    EXISTING_FIXTURE.write_text("fixture executable placeholder", encoding="utf-8")
...
def test_l13_04_doc_mentions_required_boundaries() -> None:
    doc = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_edge_executable_filesystem_probe_execution_fixture_matrix_cli_readback.md"
