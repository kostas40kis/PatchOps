...
def test_final_operator_checkpoint_script_exists_and_uses_desktop_report_contract() -> None:
    text = _read("scripts/llm_browser_final_operator_validation_commit_checkpoint.ps1")

    required = [
        "PATCHOPS LLM-BROWSER FINAL OPERATOR VALIDATION AND COMMIT CHECKPOINT",
        "patchops_reports",
        "patchops_llm_browser_final_operator_commit_checkpoint_",
...
def test_final_operator_checkpoint_script_runs_validation_but_not_git_mutation() -> None:
    text = _read("scripts/llm_browser_final_operator_validation_commit_checkpoint.ps1")

    required = [
        "closeout validation and push checkpoint",
        "llm_browser_closeout_validation_push_checkpoint.ps1",
        "broad-report",
        "--strict",
        ...
        "this final operator checkpoint never commits or pushes automatically",
    ]
...
def test_final_operator_checkpoint_script_has_plan_skip_and_timeout_controls() -> None:
    text = _read("scripts/llm_browser_final_operator_validation_commit_checkpoint.ps1")

    required = [
        "PlanOnly",
        "SkipCloseoutValidation",
        "SkipFullPytest",
        "TimeoutCloseoutSeconds",
        "TimeoutShortSeconds",
...
def test_final_operator_checkpoint_docs_include_command_outputs_and_safety() -> None:
    text = _read("docs/llm_browser_final_operator_validation_commit_checkpoint.md")
...
def test_final_operator_checkpoint_docs_include_clipboard_and_pass_requirements() -> None:
    text = _read("docs/llm_browser_final_operator_validation_commit_checkpoint.md")
...
def test_llm_browser_runner_doc_links_d0_38_final_operator_checkpoint() -> None:
    ...
    required = [
        ...
        "runs the closeout validation and push checkpoint",
