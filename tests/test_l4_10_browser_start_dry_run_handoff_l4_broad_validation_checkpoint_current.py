...
def test_l4_10_broad_validation_checkpoint_payload_is_passive_and_green() -> None:
    from patchops.llm_browser.live_adapter_browser_start_dry_run_handoff_l4_broad_validation_checkpoint import (
        build_l4_browser_start_dry_run_handoff_broad_validation_checkpoint,
    )

    payload = build_l4_browser_start_dry_run_handoff_broad_validation_checkpoint(PROJECT_ROOT)
    ...
    assert payload["missing_previous_paths"] == []
    assert payload["missing_l4_paths"] == []
    assert payload["missing_commands"] == []
    assert payload["missing_safety_phrases"] == []
    assert payload["missing_progression_phrases"] == []
    assert payload["forbidden_command_fragments"] == []
    assert payload["aggregate_gate"]["ok"] is True
    assert payload["documentation_checkpoint"]["ok"] is True
    assert all(check["ok"] is True for check in payload["checks"])


def test_l4_10_broad_validation_checkpoint_json_smoke() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "patchops.llm_browser.live_adapter_browser_start_dry_run_handoff_l4_broad_validation_checkpoint",
            "--repo-root",
            str(PROJECT_ROOT),
            "--json",
            "--compact",
...
def test_l4_10_broad_validation_checkpoint_text_readback_is_operator_safe() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_l4_10_broad_validation_checkpoint_does_not_import_selenium_or_browser_optional_dependencies() -> None:
...
def test_l4_10_broad_validation_checkpoint_docs_are_present_and_passive() -> None:
    canonical = PROJECT_ROOT / "docs" / "llm_browser_live_adapter_browser_start_dry_run_handoff_l4_broad_validation_checkpoint.md"
