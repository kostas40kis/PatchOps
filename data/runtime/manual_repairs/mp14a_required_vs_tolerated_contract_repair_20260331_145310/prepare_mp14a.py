...
def main() -> int:
    ...
    decision = "mp14a_test_contract_repair"
    rationale_bits = []
...
"""
    ...
    if str(target_test_rel).replace("\\\\", "/") not in validation_targets:
        validation_targets.append(str(target_test_rel).replace("\\\\", "/"))
    ...
    manifest_data = {
        ...
        "backup_files": [str(target_test_rel).replace("\\\\", "/")],
        "files_to_write": [
            {
                "path": str(target_test_rel).replace("\\\\", "/"),
    ...
    verify_helper_probe = {
        "command_sections_exists": command_sections_exists,
        "sections_mentions_name_lines": "NAME    :" in sections_text,
        "short_circuit_evidence": short_circuit_evidence,
    }

    prepare_result = {
        "decision": decision,
        "rationale": rationale_bits,
        "manifest_path": str(manifest_path),
        "target_test_path": str(target_test_path),
        "validation_targets": validation_targets,
        "failed_report_exists": failed_report_exists,
        "failed_report_path": str(failed_report_path),
        "failed_report_shows_required": report_shows_required,
        "failed_report_hides_later_success": report_hides_later,
        "mp15_probe": verify_helper_probe,
    }
    write_text(working_root / "prepare_result.json", json.dumps(prepare_result, indent=2) + "\\n")
