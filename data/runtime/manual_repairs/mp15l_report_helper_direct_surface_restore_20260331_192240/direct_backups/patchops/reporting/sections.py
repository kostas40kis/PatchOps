...
from patchops.reporting.command_sections import (
    ...
    render_report_command_output_section as _shared_render_report_command_output_section,
)
...
def wrapper_only_retry_section(result: WorkflowResult) -> str:
    ...
    lines = [_rule("WRAPPER ONLY RETRY")] + list(render_wrapper_only_retry_report_lines(state))
    return "\n".join(lines)
...
def render_report_command_output_section(section) -> str:
...
def full_output_section(results: list[CommandResult], title: str) -> str:
    section = build_report_command_section(title, results, _rule)
