...
def main() -> int:
    if len(sys.argv) != 3:
        print("usage: prepare_mp15s.py <wrapper_root> <working_root>", file=sys.stderr)
        return 2
    ...
    write_text(content_root / command_sections_rel, command_sections_text)
    write_text(content_root / sections_rel, sections_text)
    ...
    manifest_data = {
        "manifest_version": "1",
        "patch_name": "mp15s_report_helper_alias_and_contract_restore",
        ...
        "validation_commands": [
            {
                "name": "report_helper_alias_restore_pytest",
                "program": "python",
        ...
        "report_preferences": {
            ...
            "report_name_prefix": "mp15s_report_helper_alias_and_contract_restore",
            "write_to_desktop": False,
        },
        "tags": ["maintenance", "pythonization", "mp15", "report_helper", "contract_restore"],
        "notes": "Direct MP15 repair. Restore render_command_output_section plus the command-section/output helper contracts and wrapper-only retry call shape without another fragile inline smoke layer.",
    }
    ...
    prepare_lines = [
        "decision=mp15s_report_helper_alias_and_contract_restore",
        f"manifest_path={manifest_path}",
        "copied_files=patchops/reporting/command_sections.py;patchops/reporting/sections.py",
        f"validation_targets={';'.join(validation_targets)}",
        "rationale=Restore render_command_output_section and the remaining command-section/output-helper contracts while keeping the wrapper-only retry call on the known baseline signature.",
    ]
