...
def main() -> int:
    ...
    first_top_level_def_present = "\ndef " in ("\n" + source)
    failure_assignment_present = "failure=failure," in source
    ...
    if helper_import_present and helper_def_present and helper_call_present and current_test_exists:
        write_state(
            prep_state_path,
            {
                ...
                "first_top_level_def_present": "yes" if first_top_level_def_present else "no",
                "failure_assignment_present": "yes" if failure_assignment_present else "no",
                "inner_report_root": str(inner_report_root),
                "manifest_path": str(manifest_path),
            },
        )
    ...
    if not first_top_level_def_present or not failure_assignment_present:
        write_state(
            prep_state_path,
            {
                "decision": "stop_unexpected_apply_shape",
                "decision_reason": "apply_patch.py no longer contains a usable top-level function anchor or failure assignment anchor.",
                "apply_path_exists": "yes",
                "categories_exists": "yes",
                "rules_test_exists": "yes",
                "legacy_test_exists": "yes" if legacy_test_exists else "no",
    ...
    if normalize_import not in patched_source:
        import_lines = [m.end() for m in __import__('re').finditer(r"^(from\s+.+\s+import\s+.+|import\s+.+)$", patched_source, __import__('re').MULTILINE)]
        if import_lines:
            insert_at = import_lines[-1]
            patched_source = patched_source[:insert_at] + "\n" + normalize_import.rstrip("\n") + patched_source[insert_at:]
        else:
            patched_source = normalize_import + patched_source
...
'''
    if "def _normalize_failure_info(" not in patched_source:
        import re
        first_def_match = re.search(r"(?m)^def\s+[A-Za-z_][A-Za-z0-9_]*\s*\(", patched_source)
        if first_def_match is None:
            raise SystemExit("No top-level function definition found for helper insertion.")
        patched_source = patched_source[:first_def_match.start()] + helper_block + patched_source[first_def_match.start():]
