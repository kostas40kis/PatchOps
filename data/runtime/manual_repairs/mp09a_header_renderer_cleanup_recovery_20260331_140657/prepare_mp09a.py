...
"""
    ...
    helper_marker = "def test_header_helper_preserves_current_header_shape"
    if helper_marker in tests_text:
        tests_text = tests_text.split(helper_marker, 1)[0].rstrip() + "\n"

    old_import_block = """from patchops.reporting.metadata import (
    build_report_header_metadata,
    render_report_header,
)
from patchops.reporting.renderer import render_workflow_report
"""
    plain_import_marker = "from patchops.reporting.renderer import render_workflow_report\n"
    tests_text = tests_text.replace(old_import_block, plain_import_marker)
...
from patchops.reporting.renderer import render_workflow_report
"""
    if plain_import_marker not in tests_text:
        raise RuntimeError("Could not locate render_workflow_report import in tests/test_reporting.py.")
    tests_text = tests_text.replace(plain_import_marker, import_block, 1)
