...
This note records the Patch 134D follow-on hardening after the Patch 134C derivation repair.

## Purpose

Patch 134C repaired the rendered inner report summary so required validation and smoke failures could not render `PASS`.

Patch 134D hardens the workflow-facing surfaces that still depended on raw `WorkflowResult.exit_code` and `WorkflowResult.result_label` values.

## Narrow rule added in Patch 134D
