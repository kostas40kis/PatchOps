...
PATCH_NAME = "patch_134h_apply_report_dir_creation_repair"
BLOCK = """
...
def append_block(path: Path) -> str:
    ...
    if "PATCHOPS_PATCH134H_REPORT_DIR_REPAIR" in text:
        return text
...
def main() -> None:
    ...
    writes = {
        ...
        patch_root / "validate_patch_134h.py": VALIDATE,
    }
    ...
    manifest = {
        ...
        "validation_commands": [
            {
                "name": "patch_134h_validation",
                "program": "py",
                "args": ["validate_patch_134h.py", str(project_root)],
                "working_directory": str(patch_root),
                "use_profile_runtime": False,
                "allowed_exit_codes": [0],
            },
            {
                "name": "report_dir_current",
                "program": "py",
        ...
        "tags": ["summary_integrity", "self_hosted", "report_dir_repair"],
        "notes": "Patch 134H repairs apply-flow report directory creation so a custom report_dir works even when the directory does not already exist.",
    }
