...
PATCH_NAME = "mp21d2_remaining_baseline_contract_repair_manifest_safe"
...
def main() -> int:
    ...
    if "resume" not in handoff_data:
        handoff_data["resume"] = {
            "mode": handoff_data.get("resume_mode", "maintenance"),
            "next_action": handoff_data.get("next_action"),
            "must_read_files": handoff_data.get("must_read_files", []),
        }
    ...
    manifest = {
        ...
        "report_preferences": {
            ...
            "report_name_prefix": "mp21d2",
            "write_to_desktop": False,
        },
        "tags": ["pythonization", "baseline_repair", "mp21d2", "self_hosted"],
        "notes": "Repair the remaining baseline contracts after MP21C boot restore using content_path staging only.",
    }

    manifest_path = working_root / "patch_manifest.json"
    write_text(manifest_path, json.dumps(manifest, indent=2) + "\n")

    # Self-check: prove the manifest is valid JSON before handing off to PatchOps.
    ...
    audit = {
        ...
        "files_to_write_count": len(loaded.get("files_to_write", [])),
        "current_handoff_has_resume_before": "resume" in handoff_data,
        "next_prompt_has_content_path_marker_before": "[content_path repair stream]" in next_prompt_path.read_text(encoding="utf-8"),
