...
if "def _write_workflow_report(report_path: Path, report_text: str) -> None:" not in apply_patch_text:
    import_anchor = "from patchops.workflows.common import build_report_path, default_report_directory, infer_workspace_root\n\n\n"
    if import_anchor not in apply_patch_text:
        raise SystemExit("Could not find apply_patch import anchor for helper insertion.")
    apply_patch_text = apply_patch_text.replace(import_anchor, import_anchor + helper_block, 1)
...
if new_tail not in apply_patch_text:
    if old_tail_current in apply_patch_text:
        apply_patch_text = apply_patch_text.replace(old_tail_current, new_tail, 1)
    elif old_tail_previous in apply_patch_text:
        apply_patch_text = apply_patch_text.replace(old_tail_previous, new_tail, 1)
    else:
        raise SystemExit("Could not find apply_patch report-write tail to replace.")
...
manifest = {
    "manifest_version": "1",
    "patch_name": "patch_134m_apply_report_write_hardening",
    "active_profile": "generic_python",
    ...
    "validation_commands": [
        {
            ...
            "args": [
                ...
                "tests/test_generic_python_powershell_apply_flow.py",
    ...
    "report_preferences": {
        ...
        "report_name_prefix": "patch_134m_apply_report_write_hardening",
        "write_to_desktop": False,
    },
    "tags": ["self_hosted", "summary_integrity", "apply_flow", "report_write_hardening"],
    "notes": "Patch 134M hardens the final apply report write and adds an exact self-hosted docs-shape regression test.",
}
