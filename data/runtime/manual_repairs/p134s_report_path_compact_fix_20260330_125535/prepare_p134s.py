...
if "from hashlib import sha1" not in common_text:
    if old_import not in common_text:
        raise SystemExit("Could not find common.py import block to extend.")
    common_text = common_text.replace(old_import, new_import, 1)
...
new_block = """def build_report_path(prefix: str, patch_name: str, report_dir: Path) -> Path:
    ...
    if len(str(compact)) <= 220:
        return compact

    return report_dir / f"r_{digest}_{timestamp}.txt"
"""

if new_block not in common_text:
    if old_block not in common_text:
        raise SystemExit("Could not find current collapsed build_report_path block to replace.")
...
def test_apply_manifest_uses_compact_report_name_for_long_self_hosted_shape(tmp_path: Path) -> None:
    ...
    manifest_data = {
        ...
        "notes": "Temporary manifest used by test_apply_manifest_uses_compact_report_name_for_long_self_hosted_shape.",
    }
...
manifest = {
    "manifest_version": "1",
    "patch_name": "p134s_report_path_compact_fix",
    "active_profile": "generic_python",
    ...
    "validation_commands": [
        {
            "name": "report_path_compact_pytest",
            "program": "py",
            "args": [
                "-m",
                "pytest",
    ...
    "report_preferences": {
        ...
        "report_name_prefix": "p134s",
        "write_to_desktop": False,
    },
    "tags": ["self_hosted", "summary_integrity", "report_path", "compact_fix"],
    "notes": "Patch 134S compacts long generated report paths for Windows-hostile self-hosted shapes.",
}
