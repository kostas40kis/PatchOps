...
def main() -> int:
    ...
    helper_name = "should_use_wrapper_only_retry"
    helper_exists = helper_name in source
    normalize_present = "normalize_failure_category" in source
    verify_helper_present = "should_use_verify_only" in source
    ...
    if decision == "author_mp38":
        if not normalize_present or not verify_helper_present:
            raise SystemExit("MP37 prerequisite shape missing from patchops/rerun_decisions.py.")

        new_function = '''

def should_use_wrapper_only_retry(*, failure_class: str | None, writes_applied: bool, target_content_state: str | None) -> bool:
    """Return True when wrapper-only repair or retry is the narrow correct path."""
    normalized = normalize_failure_category(failure_class)
    if normalized != WRAPPER_FAILURE:
        return False
    if writes_applied:
        return False
    state = (target_content_state or "").strip().lower()
    return state in {"unknown", "unchanged", "already_correct", "already_green"}
'''
...
from patchops.rerun_decisions import should_use_verify_only, should_use_wrapper_only_retry


def test_wrapper_retry_helper_accepts_wrapper_failure_when_target_unchanged():
    assert should_use_wrapper_only_retry(
        failure_class=WRAPPER_FAILURE,
        writes_applied=False,
        target_content_state="unchanged",
    ) is True


def test_wrapper_retry_helper_accepts_wrapper_failure_when_target_already_green():
    assert should_use_wrapper_only_retry(
        failure_class="wrapper_failure",
        writes_applied=False,
        target_content_state="already_green",
    ) is True


def test_wrapper_retry_helper_rejects_when_writes_were_applied():
    assert should_use_wrapper_only_retry(
        failure_class=WRAPPER_FAILURE,
        writes_applied=True,
        target_content_state="unchanged",
...
def test_wrapper_retry_and_verify_only_helpers_remain_separate():
    assert should_use_verify_only(failure_class=TARGET_PROJECT_FAILURE, writes_applied=True) is False
    assert should_use_wrapper_only_retry(
        failure_class=WRAPPER_FAILURE,
        writes_applied=False,
        target_content_state="already_correct",
    ) is True
'''
