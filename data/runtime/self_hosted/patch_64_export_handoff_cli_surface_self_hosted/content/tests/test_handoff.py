...
def test_recommendation_for_green_result_is_new_patch(tmp_path: Path) -> None:
    ...
    assert recommendation["next_action"] == "Continue with Patch 65."
...
def test_write_current_handoff_json_writes_recommendation_payload(tmp_path: Path) -> None:
    ...
    assert payload["recommendation"]["next_action"] == "Continue with Patch 65."
...
def test_export_handoff_bundle_from_green_report_creates_bundle(tmp_path: Path) -> None:
...
def test_export_handoff_bundle_from_target_failure_report_recommends_repair(tmp_path: Path) -> None:
...
def test_cli_export_handoff_command_generates_bundle(tmp_path: Path) -> None:
    report_path = _write_apply_report(tmp_path, result_label="PASS")
