...
<!-- PATCHOPS_PATCH56B_READINESS_REPAIR:START -->
## Stage 2 note: Patch 56B

Patch 56B repairs the release-readiness launcher edge.

It keeps the launcher thin, makes the `"release-readiness"` command token explicit in the PowerShell file text, and aligns the launcher execution test with the actual release-readiness surface by seeding the required release launchers as part of the green fixture.
<!-- PATCHOPS_PATCH56B_READINESS_REPAIR:END -->

<!-- PATCHOPS_PATCH57_RELEASE_EVIDENCE:START -->
## Stage 2 note: Patch 57

Patch 57 adds the release-readiness evidence surface.

The `release-readiness` command can now emit deterministic report lines in JSON output and write a stable UTF-8 text artifact with `--report-path`. The matching PowerShell launcher can forward `-ReportPath` without moving release logic into PowerShell.
<!-- PATCHOPS_PATCH57_RELEASE_EVIDENCE:END -->

<!-- PATCHOPS_PATCH57B_RELEASE_CLI_REPAIR:START -->
## Stage 2 note: Patch 57B

Patch 57B repairs the Patch 57 CLI surface after a bad parser insertion caused an `IndentationError` during test collection.

This repair only normalizes the `release-readiness` parser block and handler block in `patchops/cli.py`. It does not widen the readiness rules or change the report contract.
<!-- PATCHOPS_PATCH57B_RELEASE_CLI_REPAIR:END -->

<!-- PATCHOPS_PATCH57C_RELEASE_CLI_FULL_REWRITE:START -->
## Stage 2 note: Patch 57C

Patch 57C repairs the release-readiness CLI by rewriting `patchops/cli.py` from a known-good command surface baseline instead of trying another surgical parser edit on already broken syntax.

This preserves:
- apply / verify / wrapper-retry / inspect / check / plan
- release-readiness with `--report-path`
- profiles / template / doctor / examples / schema

The repair is intentionally limited to `patchops/cli.py` plus ledger/status notes.
<!-- PATCHOPS_PATCH57C_RELEASE_CLI_FULL_REWRITE:END -->

<!-- PATCHOPS_PATCH57D_READINESS_REPORTPATH_LITERAL:START -->
## Stage 2 note: Patch 57D

Patch 57D is a tiny launcher text-contract repair.

It does not change release-readiness behavior. It only makes the literal `-ReportPath` usage visible inside `powershell/Invoke-PatchReadiness.ps1` so the launcher contract test can pass without widening the launcher logic.
<!-- PATCHOPS_PATCH57D_READINESS_REPORTPATH_LITERAL:END -->

<!-- PATCHOPS_PATCH58_RELEASE_TESTS:START -->
## Stage 2 note: Patch 58

Patch 58 hardens the release-readiness surface with explicit required test-file checks.

The readiness snapshot now treats docs, examples, workflows, launchers, tests, and profiles as explicit maintained surfaces. The command and launcher tests were expanded so missing release-test files fail clearly instead of silently drifting behind the readiness output.
<!-- PATCHOPS_PATCH58_RELEASE_TESTS:END -->

<!-- PATCHOPS_PATCH58B_READINESS_COMPAT_REPAIR:START -->
## Stage 2 note: Patch 58B

Patch 58B repairs backward compatibility in `patchops/readiness.py`.

Patch 58 added the release-test surface checks correctly, but it replaced the earlier initial-milestone readiness exports that `patchops.initial_milestone_gate` and older readiness tests still import. This repair restores the initial-milestone readiness surface and keeps the newer release-readiness surface in the same module.
<!-- PATCHOPS_PATCH58B_READINESS_COMPAT_REPAIR:END -->

<!-- PATCHOPS_PATCH59_MANIFEST_VERSION_POLICY:START -->
## Stage 2 note: Patch 59

Patch 59 adds an explicit manifest version policy surface.

The repo now states clearly that:
- manifest authoring currently targets version `1`,
- version `1` is the only supported manifest version,
- unsupported versions fail closed with readable errors,
- future manifest versions require explicit validator, schema, doc, and test updates.

This keeps current v1 behavior stable while making future manifest evolution easier to reason about.
<!-- PATCHOPS_PATCH59_MANIFEST_VERSION_POLICY:END -->
