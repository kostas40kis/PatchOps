...
def header_section(result: WorkflowResult) -> str:
    return "\n".join(
        [
            ...
            f"Workspace Root       : {display_path(result.workspace_root) if result.workspace_root else '(unknown)'}",
            ...
            f"Runtime Path         : {display_path(result.runtime_path) if result.runtime_path else '(none)'}",
            f"Backup Root          : {display_path(result.backup_root) if result.backup_root else '(not used)'}",
...
def target_files_section(target_paths: list[Path]) -> str:
    ...
    if not target_paths:
        lines.append("(none)")
        return "\n".join(lines)
    lines.extend(display_path(path) for path in target_paths)
    return "\n".join(lines)
