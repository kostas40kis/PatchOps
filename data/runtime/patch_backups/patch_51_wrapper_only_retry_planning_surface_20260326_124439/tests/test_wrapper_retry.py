...
def test_wrapper_only_retry_needs_escalation_when_expected_files_are_missing(tmp_path: Path) -> None:
    manifest = {
...
def test_wrapper_only_retry_stays_narrow_when_expected_files_exist(tmp_path: Path) -> None:
    existing_file = tmp_path / "docs" / "present.md"
    existing_file.parent.mkdir(parents=True, exist_ok=True)
    existing_file.write_text("ok", encoding="utf-8")
    ...
    assert wrapper_only_retry_needs_escalation(state) is False
    assert wrapper_only_retry_allows_writes(state) is False
