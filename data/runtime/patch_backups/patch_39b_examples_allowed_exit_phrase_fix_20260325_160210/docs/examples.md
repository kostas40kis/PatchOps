...
- `examples/generic_allowed_exit_patch.json` demonstrates the allowed-exit-codes pattern for cases where a command may legitimately return a non-zero code that is still considered acceptable by the manifest contract.
- `examples/generic_content_path_patch.json` demonstrates the content-path pattern for cases where file content is sourced from a separate path instead of being embedded directly inside the manifest.
