...
Patch 23 adds an apply-flow test proving that a manifest with `backup_files` can overwrite an existing target file, produce a PASS result, and emit backup evidence in the canonical report.


## Patch 24 addition

Patch 24 adds an apply-flow test proving that the `generic_python_powershell` profile can run a PowerShell validation command, a Python smoke command, write a target file, and produce a clean PASS report.
