...
## Schema before template generation

When an LLM is new to PatchOps, the safest discovery order is:
1. `patchops profiles`
...
The `schema` command is intentionally stable and read-only. It gives the model a compact manifest contract without requiring it to read every documentation file first.
