...
## Patch 14 - schema command and launcher

Patch 14 adds a read-only `schema` command and `Invoke-PatchSchema.ps1` so operators and future LLMs can retrieve a stable manifest field reference before authoring or editing manifests.

## Patch 15

Added report-preference example manifests and tests so future authors can see concrete uses of `report_name_prefix` and `report_dir` before touching a real target repository.
