...
## Stage 1 operator documentation

For day-to-day use, read these first:

- `docs/operator_quickstart.md`
- `docs/command_matrix.md`
- `docs/repair_rerun_matrix.md`

These docs turn the current PatchOps command surface into a consistent operator flow for humans and future LLMs.

## Generic Python + PowerShell profile examples

Stage 1 now includes dedicated example manifests for the `generic_python_powershell` profile.
Use them when you want a project-agnostic patch flow that is still allowed to model PowerShell-native smoke commands without switching into trader-specific behavior.

Recommended first-use order:

1. inspect a generic Python example
2. inspect a generic Python backup example
3. inspect a generic Python + PowerShell example
4. run `doctor`, `check`, and `plan`
5. only then move toward trader manifests

## Schema command

Use `patchops schema` when you want a stable JSON field reference for manifest authoring.

It is meant for humans and future LLMs that need a compact reminder of:
- required top-level fields
- optional top-level fields
- command-group purpose
- report-preference fields
- starter authoring guidance

## Report preference examples

PatchOps now ships example manifests that demonstrate deterministic report naming via `report_name_prefix` and explicit report placement via `report_dir`. Use them as references before writing manifests that need a predictable evidence location.
