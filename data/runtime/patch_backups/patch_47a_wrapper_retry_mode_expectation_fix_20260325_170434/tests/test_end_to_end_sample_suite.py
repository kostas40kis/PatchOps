...
from patchops.workflows.wrapper_retry import build_wrapper_only_retry_state
...
def test_wrapper_retry_surface_is_visible_for_sample_manifest() -> None:
    ...
    assert state.mode == "wrapper_only_retry"
    assert state.writes_skipped is True
    assert state.retry_reason == "reporting failed after likely successful validation"
