...
Start with `docs/examples.md` when you want concrete manifest walkthroughs rather than only raw example files.
...
`docs/operational_patch_types.md` explains how to choose between code, documentation, validation, cleanup, archive, and verify-only workflows.
It is the single map for deciding which patch class fits a real task.
